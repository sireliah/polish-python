# This file jest used to verify that 2to3 falls back to a slower, iterative pattern matching
# scheme w the event that the faster recursive system fails due to infinite recursion.
z ctypes zaimportuj *
STRING = c_char_p
...
# values dla unnamed enumeration
klasa aes_key_st(Structure):
    dalej
aes_key_st._fields_ = [
    ('rd_key', c_ulong * 60),
    ('rounds', c_int),
...
klasa asn1_object_st(Structure):
    dalej
...
klasa asn1_string_st(Structure):
    dalej
...
klasa ASN1_ENCODING_st(Structure):
    dalej
...
klasa asn1_string_table_st(Structure):
    dalej
...
klasa asn1_header_st(Structure):
    dalej
...
klasa BIT_STRING_BITNAME_st(Structure):
    dalej
...
klasa bio_st(Structure):
    dalej
BIO = bio_st
bio_info_cb = CFUNCTYPE(Nic, POINTER(bio_st), c_int, STRING, c_int, c_long, c_long)
klasa bio_method_st(Structure):
    dalej
...
klasa crypto_ex_data_st(Structure):
    dalej
klasa stack_st(Structure):
    dalej
STACK = stack_st
...
klasa bio_f_buffer_ctx_struct(Structure):
    dalej
...
klasa hostent(Structure):
    dalej
hostent._fields_ = [
]
klasa bf_key_st(Structure):
    dalej
bf_key_st._fields_ = [
    ('P', c_uint * 18),
    ('S', c_uint * 1024),
...
BIGNUM = bignum_st
klasa bignum_ctx(Structure):
    dalej
bignum_ctx._fields_ = [
]
BN_CTX = bignum_ctx
klasa bn_blinding_st(Structure):
    dalej
...
klasa bn_mont_ctx_st(Structure):
    dalej
...
klasa bn_recp_ctx_st(Structure):
    dalej
...
klasa comp_method_st(Structure):
    dalej
comp_method_st._fields_ = [
    ('type', c_int),
    ('name', STRING),
    ('init', CFUNCTYPE(c_int)),
    ('finish', CFUNCTYPE(Nic)),
...
klasa CRYPTO_dynlock_value(Structure):
    dalej
CRYPTO_dynlock_value._fields_ = [
]
klasa CRYPTO_dynlock(Structure):
    dalej
...
CRYPTO_EX_free = CFUNCTYPE(Nic, c_void_p, c_void_p, POINTER(CRYPTO_EX_DATA), c_int, c_long, c_void_p)
CRYPTO_EX_dup = CFUNCTYPE(c_int, POINTER(CRYPTO_EX_DATA), POINTER(CRYPTO_EX_DATA), c_void_p, c_int, c_long, c_void_p)
klasa crypto_ex_data_func_st(Structure):
    dalej
...
klasa st_CRYPTO_EX_DATA_IMPL(Structure):
    dalej
...
klasa DES_ks(Structure):
    dalej
klasa N6DES_ks3DOLLAR_9E(Union):
    dalej
...
klasa _ossl_old_des_ks_struct(Structure):
    dalej
klasa N23_ossl_old_des_ks_struct4DOLLAR_10E(Union):
    dalej
...
klasa dh_st(Structure):
    dalej
DH = dh_st
klasa dh_method(Structure):
    dalej
dh_method._fields_ = [
    ('name', STRING),
...
klasa dsa_st(Structure):
    dalej
DSA = dsa_st
klasa DSA_SIG_st(Structure):
    dalej
DSA_SIG_st._fields_ = [
    ('r', POINTER(BIGNUM)),
    ('s', POINTER(BIGNUM)),
...
klasa evp_pkey_st(Structure):
    dalej
klasa N11evp_pkey_st4DOLLAR_12E(Union):
    dalej
klasa rsa_st(Structure):
    dalej
...
klasa env_md_st(Structure):
    dalej
klasa env_md_ctx_st(Structure):
    dalej
...
klasa evp_cipher_st(Structure):
    dalej
klasa evp_cipher_ctx_st(Structure):
    dalej
...
klasa evp_cipher_info_st(Structure):
    dalej
...
klasa evp_Encode_Ctx_st(Structure):
    dalej
...
LHASH_DOALL_FN_TYPE = CFUNCTYPE(Nic, c_void_p)
LHASH_DOALL_ARG_FN_TYPE = CFUNCTYPE(Nic, c_void_p, c_void_p)
klasa lhash_st(Structure):
    dalej
lhash_st._fields_ = [
...
klasa x509_st(Structure):
    dalej
X509 = x509_st
klasa X509_algor_st(Structure):
    dalej
X509_ALGOR = X509_algor_st
klasa X509_crl_st(Structure):
    dalej
X509_CRL = X509_crl_st
klasa X509_name_st(Structure):
    dalej
X509_NAME = X509_name_st
klasa x509_store_st(Structure):
    dalej
X509_STORE = x509_store_st
klasa x509_store_ctx_st(Structure):
    dalej
X509_STORE_CTX = x509_store_ctx_st
engine_st._fields_ = [
]
klasa PEM_Encode_Seal_st(Structure):
    dalej
...
klasa pem_ctx_st(Structure):
    dalej
klasa N10pem_ctx_st4DOLLAR_16E(Structure):
    dalej
...
klasa N10pem_ctx_st4DOLLAR_17E(Structure):
    dalej
...
klasa pkcs7_issuer_and_serial_st(Structure):
    dalej
...
klasa pkcs7_signer_info_st(Structure):
    dalej
...
klasa pkcs7_recip_info_st(Structure):
    dalej
...
klasa pkcs7_signed_st(Structure):
    dalej
klasa pkcs7_st(Structure):
    dalej
...
klasa pkcs7_enc_content_st(Structure):
    dalej
...
klasa pkcs7_enveloped_st(Structure):
    dalej
...
klasa pkcs7_signedandenveloped_st(Structure):
    dalej
...
klasa pkcs7_digest_st(Structure):
    dalej
...
klasa pkcs7_encrypted_st(Structure):
    dalej
...
klasa N8pkcs7_st4DOLLAR_15E(Union):
    dalej
...
klasa RIPEMD160state_st(Structure):
    dalej
...
openssl_fptr = CFUNCTYPE(Nic)
klasa SHAstate_st(Structure):
    dalej
...
SHA_CTX = SHAstate_st
klasa ssl_st(Structure):
    dalej
ssl_crock_st = POINTER(ssl_st)
klasa ssl_cipher_st(Structure):
    dalej
...
SSL = ssl_st
klasa ssl_ctx_st(Structure):
    dalej
SSL_CTX = ssl_ctx_st
klasa ssl_method_st(Structure):
    dalej
klasa ssl3_enc_method(Structure):
    dalej
ssl_method_st._fields_ = [
    ('version', c_int),
    ('ssl_new', CFUNCTYPE(c_int, POINTER(SSL))),
    ('ssl_clear', CFUNCTYPE(Nic, POINTER(SSL))),
    ('ssl_free', CFUNCTYPE(Nic, POINTER(SSL))),
    ...
    ('ssl_callback_ctrl', CFUNCTYPE(c_long, POINTER(SSL), c_int, CFUNCTYPE(Nic))),
    ('ssl_ctx_callback_ctrl', CFUNCTYPE(c_long, POINTER(SSL_CTX), c_int, CFUNCTYPE(Nic))),
]
...
klasa ssl_session_st(Structure):
    dalej
klasa sess_cert_st(Structure):
    dalej
...
klasa N10ssl_ctx_st4DOLLAR_18E(Structure):
    dalej
...
ssl_ctx_st._fields_ = [
    ...
    ('remove_session_cb', CFUNCTYPE(Nic, POINTER(ssl_ctx_st), POINTER(SSL_SESSION))),
    ...
    ('info_callback', CFUNCTYPE(Nic, POINTER(SSL), c_int, c_int)),
    ...
    ('read_ahead', c_int),
    ('msg_callback', CFUNCTYPE(Nic, c_int, c_int, c_int, c_void_p, c_ulong, POINTER(SSL), c_void_p)),
...
klasa ssl2_state_st(Structure):
    dalej
klasa ssl3_state_st(Structure):
    dalej
ssl_st._fields_ = [
    ('version', c_int),
    ('type', c_int),
...
klasa N13ssl2_state_st4DOLLAR_19E(Structure):
    dalej
...
klasa ssl3_record_st(Structure):
    dalej
...
klasa ssl3_buffer_st(Structure):
    dalej
...
klasa N13ssl3_state_st4DOLLAR_20E(Structure):
    dalej
...
klasa ui_st(Structure):
    dalej
ui_st._fields_ = [
]
UI = ui_st
klasa ui_method_st(Structure):
    dalej
ui_method_st._fields_ = [
]
UI_METHOD = ui_method_st
klasa ui_string_st(Structure):
    dalej
ui_string_st._fields_ = [
]
UI_STRING = ui_string_st

# values dla enumeration 'UI_string_types'
UI_string_types = c_int # enum
klasa X509_objects_st(Structure):
    dalej
...
klasa X509_pubkey_st(Structure):
    dalej
...
klasa X509_name_entry_st(Structure):
    dalej
...
klasa X509_extension_st(Structure):
    dalej
...
klasa x509_attributes_st(Structure):
    dalej
klasa N18x509_attributes_st4DOLLAR_13E(Union):
    dalej
...
klasa X509_req_info_st(Structure):
    dalej
...
klasa x509_cert_aux_st(Structure):
    dalej
...
klasa AUTHORITY_KEYID_st(Structure):
    dalej
x509_st._fields_ = [
...
klasa X509_revoked_st(Structure):
    dalej
...
klasa X509_crl_info_st(Structure):
    dalej
...
klasa private_key_st(Structure):
    dalej
...
klasa Netscape_spkac_st(Structure):
    dalej
...
klasa Netscape_spki_st(Structure):
    dalej
...
klasa Netscape_certificate_sequence(Structure):
    dalej
...
klasa PBKDF2PARAM_st(Structure):
    dalej
...
klasa pkcs8_priv_key_info_st(Structure):
    dalej
...
klasa x509_hash_dir_st(Structure):
    dalej
...
klasa x509_object_st(Structure):
    dalej
klasa N14x509_object_st4DOLLAR_14E(Union):
    dalej
...
klasa x509_lookup_st(Structure):
    dalej
X509_LOOKUP = x509_lookup_st
klasa x509_lookup_method_st(Structure):
    dalej
x509_lookup_method_st._fields_ = [
    ('name', STRING),
    ('new_item', CFUNCTYPE(c_int, POINTER(X509_LOOKUP))),
    ('free', CFUNCTYPE(Nic, POINTER(X509_LOOKUP))),
...
klasa __sFILEX(Structure):
    dalej
__sFILEX._fields_ = [
]
klasa __sFILE(Structure):
    dalej
__sFILE._pack_ = 4
__sFILE._fields_ = [
    ('_p', POINTER(c_ubyte)),
...
klasa mcontext(Structure):
    dalej
mcontext._fields_ = [
]
klasa mcontext64(Structure):
    dalej
mcontext64._fields_ = [
]
klasa __darwin_pthread_handler_rec(Structure):
    dalej
__darwin_pthread_handler_rec._fields_ = [
    ('__routine', CFUNCTYPE(Nic, c_void_p)),
    ('__arg', c_void_p),
...
klasa _opaque_pthread_attr_t(Structure):
    dalej
...
klasa _opaque_pthread_cond_t(Structure):
    dalej
...
klasa _opaque_pthread_condattr_t(Structure):
    dalej
...
klasa _opaque_pthread_mutex_t(Structure):
    dalej
...
klasa _opaque_pthread_mutexattr_t(Structure):
    dalej
...
klasa _opaque_pthread_once_t(Structure):
    dalej
...
klasa _opaque_pthread_rwlock_t(Structure):
    dalej
...
klasa _opaque_pthread_rwlockattr_t(Structure):
    dalej
...
klasa _opaque_pthread_t(Structure):
    dalej
...
klasa sigevent(Structure):
    dalej
sigevent._fields_ = [
    ('sigev_notify', c_int),
    ('sigev_signo', c_int),
    ('sigev_value', sigval),
    ('sigev_notify_function', CFUNCTYPE(Nic, sigval)),
...
# values dla enumeration 'idtype_t'
idtype_t = c_int # enum
id_t = __darwin_id_t
klasa wait(Union):
    dalej
klasa N4wait3DOLLAR_3E(Structure):
    dalej
...
klasa N4wait3DOLLAR_4E(Structure):
    dalej
