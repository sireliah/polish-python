# Generated by h2py z /usr/include/sys/stropts.h

# Included z sys/feature_tests.h

# Included z sys/isa_defs.h
_CHAR_ALIGNMENT = 1
_SHORT_ALIGNMENT = 2
_INT_ALIGNMENT = 4
_LONG_ALIGNMENT = 8
...
# Included z sys/conf.h

# Included z sys/t_lock.h

# Included z sys/machlock.h
z TYPES zaimportuj *
LOCK_HELD_VALUE = 0xff
def SPIN_LOCK(pl): zwróć ((pl) > ipltospl(LOCK_LEVEL))

def LOCK_SAMPLE_INTERVAL(i): zwróć (((i) & 0xff) == 0)
...
def btodb(bytes): zwróć   \

def dbtob(db): zwróć   \

def lbtodb(bytes): zwróć   \
...
FSHIFT = 8
FSCALE = (1<<FSHIFT)
def DELAY(n): zwróć drv_usecwait(n)

def mmu_ptob(x): zwróć ((x) << MMU_PAGESHIFT)

def mmu_btop(x): zwróć (((x)) >> MMU_PAGESHIFT)

def mmu_btopr(x): zwróć ((((x) + MMU_PAGEOFFSET) >> MMU_PAGESHIFT))

def mmu_ptod(x): zwróć ((x) << (MMU_PAGESHIFT - DEV_BSHIFT))

def ptod(x): zwróć ((x) << (PAGESHIFT - DEV_BSHIFT))

def ptob(x): zwróć ((x) << PAGESHIFT)

def btop(x): zwróć (((x) >> PAGESHIFT))

def btopr(x): zwróć ((((x) + PAGEOFFSET) >> PAGESHIFT))

def dtop(DD): zwróć (((DD) + NDPP - 1) >> (PAGESHIFT - DEV_BSHIFT))

def dtopt(DD): zwróć ((DD) >> (PAGESHIFT - DEV_BSHIFT))
...
# Included z sys/mutex.h
z TYPES zaimportuj *
def MUTEX_HELD(x): zwróć (mutex_owned(x))


# Included z sys/rwlock.h
z TYPES zaimportuj *
def RW_READ_HELD(x): zwróć (rw_read_held((x)))

def RW_WRITE_HELD(x): zwróć (rw_write_held((x)))

def RW_LOCK_HELD(x): zwróć (rw_lock_held((x)))

def RW_ISWRITER(x): zwróć (rw_iswriter(x))


# Included z sys/semaphore.h

# Included z sys/thread.h
z TYPES zaimportuj *

# Included z sys/klwp.h
z TYPES zaimportuj *

# Included z sys/condvar.h
z TYPES zaimportuj *

# Included z sys/time.h

# Included z sys/types32.h

# Included z sys/int_types.h
TIME32_MAX = INT32_MAX
TIME32_MIN = INT32_MIN
def TIMEVAL_OVERFLOW(tv): zwróć \
...
ITIMER_PROF = 2
ITIMER_REALPROF = 3
def ITIMERVAL_OVERFLOW(itv): zwróć \
...
# Included z sys/time_impl.h
def TIMESPEC_OVERFLOW(ts): zwróć \

def ITIMERSPEC_OVERFLOW(it): zwróć \
...
TIMER_RELTIME = 0x0
TIMER_ABSTIME = 0x1
def TICK_TO_SEC(tick): zwróć ((tick) / hz)

def SEC_TO_TICK(sec): zwróć ((sec) * hz)

def TICK_TO_MSEC(tick): zwróć \

def MSEC_TO_TICK(msec): zwróć \

def MSEC_TO_TICK_ROUNDUP(msec): zwróć \

def TICK_TO_USEC(tick): zwróć ((tick) * usec_per_tick)

def USEC_TO_TICK(usec): zwróć ((usec) / usec_per_tick)

def USEC_TO_TICK_ROUNDUP(usec): zwróć \

def TICK_TO_NSEC(tick): zwróć ((tick) * nsec_per_tick)

def NSEC_TO_TICK(nsec): zwróć ((nsec) / nsec_per_tick)

def NSEC_TO_TICK_ROUNDUP(nsec): zwróć \

def TIMEVAL_TO_TICK(tvp): zwróć \

def TIMESTRUC_TO_TICK(tsp): zwróć \


# Included z time.h
z TYPES zaimportuj *

# Included z iso/time_iso.h
NULL = 0
NULL = 0
CLOCKS_PER_SEC = 1000000

# Included z sys/select.h
FD_SETSIZE = 65536
FD_SETSIZE = 1024
_NBBY = 8
NBBY = _NBBY
def FD_ZERO(p): zwróć bzero((p), sizeof (*(p)))


# Included z sys/signal.h

# Included z sys/iso/signal_iso.h
SIGHUP = 1
SIGINT = 2
SIGQUIT = 3
SIGILL = 4
...
# Included z sys/time_std_impl.h
z TYPES zaimportuj *
SI32_MAXSZ = 128
def SI_CANQUEUE(c): zwróć ((c) <= SI_QUEUE)
...
# Included z sys/ucontext.h
z TYPES zaimportuj *

# Included z sys/regset.h
REG_CCR = (0)
REG_PSR = (0)
REG_PSR = (0)
REG_PC = (1)
...
# Included z v7/sys/privregs.h

# Included z v7/sys/psr.h
PSR_CWP = 0x0000001F
PSR_ET = 0x00000020
PSR_PS = 0x00000040
PSR_S = 0x00000080
...
def USERMODE(ps): zwróć (((ps) & PSR_PS) == 0)


# Included z sys/fsr.h
FSR_CEXC = 0x0000001f
FSR_AEXC = 0x000003e0
FSR_FCC = 0x00000c00
FSR_PR = 0x00001000
...
FPRS_FEF = 0x4
PIL_MAX = 0xf
def SAVE_GLOBALS(RP): zwróć \

def RESTORE_GLOBALS(RP): zwróć \

def SAVE_OUTS(RP): zwróć \

def RESTORE_OUTS(RP): zwróć \

def SAVE_WINDOW(SBP): zwróć \

def RESTORE_WINDOW(SBP): zwróć \

def STORE_FPREGS(FP): zwróć \

def LOAD_FPREGS(FP): zwróć \
...
# Included z sys/lwp.h

# Included z sys/synch.h
z TYPES zaimportuj *
USYNC_THREAD = 0x00
USYNC_PROCESS = 0x01
LOCK_NORMAL = 0x00
...
def CPR_VSTOPPED(t): zwróć \

def THREAD_TRANSITION(tp): zwróć thread_transition(tp);

def THREAD_STOP(tp): zwróć \

def THREAD_ZOMB(tp): zwróć THREAD_SET_STATE(tp, TS_ZOMB, NULL)

def SEMA_HELD(x): zwróć (sema_held((x)))

NO_LOCKS_HELD = 1
NO_COMPETING_THREADS = 1
FMNAMESZ = 8

# Included z sys/systm.h
z TYPES zaimportuj *

# Included z sys/proc.h

# Included z sys/cred.h

# Included z sys/user.h
z TYPES zaimportuj *

# Included z sys/resource.h
z TYPES zaimportuj *
PRIO_PROCESS = 0
PRIO_PGRP = 1
PRIO_USER = 2
...
# Included z sys/model.h

# Included z sys/debug.h
def ASSERT64(x): zwróć ASSERT(x)

def ASSERT32(x): zwróć ASSERT(x)
...
def STRUCT_SIZE(handle): zwróć \

def STRUCT_BUF(handle): zwróć ((handle).ptr.m64)

def SIZEOF_PTR(umodel): zwróć \

def STRUCT_SIZE(handle): zwróć (sizeof (*(handle).ptr))

def STRUCT_BUF(handle): zwróć ((handle).ptr)

def SIZEOF_PTR(umodel): zwróć sizeof (caddr_t)

def lwp_getdatamodel(t): zwróć DATAMODEL_ILP32

RUSAGE_SELF = 0
RUSAGE_CHILDREN = -1

# Included z sys/auxv.h
AT_NULL = 0
AT_IGNORE = 1
AT_EXECFD = 2
AT_PHDR = 3
...
# Included z sys/watchpoint.h
z TYPES zaimportuj *

# Included z vm/seg_enum.h

# Included z sys/copyops.h
z TYPES zaimportuj *

# Included z sys/buf.h

# Included z sys/kstat.h
z TYPES zaimportuj *
KSTAT_STRLEN = 31
def KSTAT_ENTER(k): zwróć \

def KSTAT_EXIT(k): zwróć \
...
B_SHADOW = 0x200000
B_RETRYWRI = 0x400000
def notavail(bp): zwróć \

def BWRITE(bp): zwróć \

def BWRITE2(bp): zwróć \


# Included z sys/aio_req.h

# Included z sys/uio.h
z TYPES zaimportuj *
WP_NOWATCH = 0x01
WP_SETPROT = 0x02

# Included z sys/timer.h
z TYPES zaimportuj *
_TIMER_MAX = 32
ITLK_LOCKED = 0x01
ITLK_WANTED = 0x02
...
# Included z sys/refstr.h

# Included z sys/task.h
z TYPES zaimportuj *
TASK_NORMAL = 0x0
TASK_FINAL = 0x1
TASK_FINALITY = 0x1

# Included z sys/id_space.h
z TYPES zaimportuj *

# Included z sys/vmem.h
z TYPES zaimportuj *
VM_SLEEP = 0x00000000
...
# Included z sys/exacct_impl.h
z TYPES zaimportuj *

# Included z sys/kmem.h
z TYPES zaimportuj *
KM_SLEEP = 0x0000
KM_NOSLEEP = 0x0001
KM_PANIC = 0x0002
...
# Included z sys/varargs.h

# Included z sys/va_list.h
VA_ALIGN = 8
def _ARGSIZEOF(t): zwróć ((sizeof (t) + VA_ALIGN - 1) & ~(VA_ALIGN - 1))
...
# Included z sys/devops.h
z TYPES zaimportuj *

# Included z sys/poll.h
POLLIN = 0x0001
POLLPRI = 0x0002
POLLOUT = 0x0004
POLLRDNORM = 0x0040
...
MODEMASK = 0o7777
PERMMASK = 0o0777
def MANDMODE(mode): zwróć (((mode) & (VSGID|(VEXEC>>3))) == VSGID)
...
FC_NOMAP = 0x5
FC_NOSUPPORT = 0x6
def FC_MAKE_ERR(e): zwróć (((e) << 8) | FC_OBJERR)

def FC_CODE(fc): zwróć ((fc) & 0xff)

def FC_ERRNO(fc): zwróć ((unsigned)(fc) >> 8)


# Included z vm/hat.h
z TYPES zaimportuj *

# Included z vm/page.h
PAGE_HASHAVELEN = 4
PAGE_HASHVPSHIFT = 6
PG_EXCL = 0x0001
PG_WAIT = 0x0002
...
PG_LIST_TAIL = 0
PG_LIST_HEAD = 1
def page_next_raw(PP): zwróć page_nextn_raw((PP), 1)
...
P_FREE = 0x80
P_NORELOC = 0x40
def PP_SETAGED(pp): zwróć ASSERT(PP_ISAGED(pp))
...
P_REF = 0x2
P_RO = 0x4
def hat_ismod(pp): zwróć (hat_page_getattr(pp, P_MOD))

def hat_isref(pp): zwróć (hat_page_getattr(pp, P_REF))

def hat_isro(pp): zwróć (hat_page_getattr(pp, P_RO))

def hat_setmod(pp): zwróć (hat_page_setattr(pp, P_MOD))

def hat_setref(pp): zwróć (hat_page_setattr(pp, P_REF))

def hat_setrefmod(pp): zwróć (hat_page_setattr(pp, P_REF|P_MOD))

def hat_clrmod(pp): zwróć (hat_page_clrattr(pp, P_MOD))

def hat_clrref(pp): zwróć (hat_page_clrattr(pp, P_REF))

def hat_clrrefmod(pp): zwróć (hat_page_clrattr(pp, P_REF|P_MOD))

def hat_page_is_mapped(pp): zwróć (hat_page_getshare(pp))
...
SEGP_SUCCESS = 0
SEGP_FAIL = 1
def seg_pages(seg): zwróć \

IE_NOMEM = -1
AS_PAGLCK = 0x80
AS_CLAIMGAP = 0x40
AS_UNMAPWAIT = 0x20
def AS_TYPE_64BIT(as_): zwróć \
...
# Included z sys/ddimapreq.h

# Included z sys/mman.h
PROT_READ = 0x1
PROT_WRITE = 0x2
PROT_EXEC = 0x4
PROT_USER = 0x8
...
# Included z sys/ddipropdefs.h
def CELLS_1275_TO_BYTES(n): zwróć ((n) * PROP_1275_CELL_SIZE)

def BYTES_TO_1275_CELLS(n): zwróć ((n) / PROP_1275_CELL_SIZE)
...
# Included z sys/nexusdefs.h

# Included z sys/nexusintr.h
BUSO_REV = 4
BUSO_REV_3 = 3
BUSO_REV_4 = 4
DEVO_REV = 3
