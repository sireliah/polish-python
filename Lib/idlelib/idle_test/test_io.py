zaimportuj unittest
zaimportuj io
z idlelib.PyShell zaimportuj PseudoInputFile, PseudoOutputFile


klasa S(str):
    def __str__(self):
        zwróć '%s:str' % type(self).__name__
    def __unicode__(self):
        zwróć '%s:unicode' % type(self).__name__
    def __len__(self):
        zwróć 3
    def __iter__(self):
        zwróć iter('abc')
    def __getitem__(self, *args):
        zwróć '%s:item' % type(self).__name__
    def __getslice__(self, *args):
        zwróć '%s:slice' % type(self).__name__
...
klasa PseudeInputFilesTest(unittest.TestCase):
    def test_misc(self):
        shell = MockShell()
    ...
    def test_read(self):
        ...
        shell.push(['one\n', 'two\n', ''])
        self.assertEqual(f.read(Nic), 'one\ntwo\n')
        shell.push(['one\n', 'two\n', 'three\n', ''])
    ...
    def test_readline(self):
        ...
        self.assertEqual(f.readline(Nic), 'three\n')
        shell.push(['one\ntwo\n'])
        self.assertEqual(f.readline(), 'one\n')
    ...
    def test_readlines(self):
        ...
        shell.push(['one\n', 'two\n', ''])
        self.assertEqual(f.readlines(Nic), ['one\n', 'two\n'])
        shell.push(['one\n', 'two\n', ''])
