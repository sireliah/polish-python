...
Customize font faces, sizes, oraz colorization attributes.  Set indentation
defaults.  Customize keybindings.  Colorization oraz keybindings can be
saved jako user defined sets.  Select startup options including shell/editor
and default window size.  Define additional help sources.

Note that tab width w IDLE jest currently fixed at eight due to Tk issues.
Refer to comments w EditorWindow autoindent code dla details.

"""
z tkinter zaimportuj *
zaimportuj tkinter.messagebox jako tkMessageBox
zaimportuj tkinter.colorchooser jako tkColorChooser
zaimportuj tkinter.font jako tkFont

z idlelib.configHandler zaimportuj idleConf
z idlelib.dynOptionMenuWidget zaimportuj DynOptionMenu
z idlelib.keybindingDialog zaimportuj GetKeysDialog
z idlelib.configSectionNameDialog zaimportuj GetCfgSectionNameDialog
z idlelib.configHelpSourceEdit zaimportuj GetHelpSourceDialog
z idlelib.tabbedpages zaimportuj TabbedPageSet
z idlelib zaimportuj macosxSupport
klasa ConfigDialog(Toplevel):

    def __init__(self, parent, title='', _htest=Nieprawda, _utest=Nieprawda):
        """
        ...
        self.title(title albo 'IDLE Preferences')
        self.geometry(
                "+%d+%d" % (parent.winfo_rootx() + 20,
                parent.winfo_rooty() + (30 jeżeli nie _htest inaczej 150)))
        #Theme Elements. Each theme element key jest its display name.
        #The first value of the tuple jest the sample area tag name.
        #The second value jest the display name list sort index.
        self.themeElements={
        ...
        self.ResetChangedItems() #load initial values w changed items dict
        self.CreateWidgets()
    ...
    def create_action_buttons(self):
        jeżeli macosxSupport.isAquaTk():
            # Changing the default padding on OSX results w unreadable
            # text w the buttons
            paddingArgs = {}
        inaczej:
...
# Comment out Help button creation oraz packing until implement self.Help
...
##        self.buttonHelp.pack(side=RIGHT, padx=5)
    ...
    def CreatePageFontTab(self):
        ...
        self.optMenuFontSize = DynOptionMenu(
                frameFontParam, self.fontSize, Nic, command=self.SetFontSample)
    ...
    def CreatePageHighlight(self):
        ...
        textAndTags=(
            ...
            ('stderr', 'stderr'), ('\n', 'normal'))
        dla txTa w textAndTags:
            text.insert(END, txTa[0], txTa[1])
        dla element w self.themeElements:
        ...
        buttonSetColour = Button(
                self.frameColourSet, text='Choose Colour dla :',
                command=self.GetColour, highlightthickness=0)
        self.optMenuHighlightTarget = DynOptionMenu(
                self.frameColourSet, self.highlightTarget, Nic,
        ...
        buttonSaveCustomTheme = Button(
                frameCustom, text='Save jako New Custom Theme',
        ...
        self.optMenuThemeBuiltin = DynOptionMenu(
                frameTheme, self.builtinTheme, Nic, command=Nic)
        self.optMenuThemeCustom=DynOptionMenu(
                frameTheme, self.customTheme, Nic, command=Nic)
    ...
    def CreatePageKeys(self):
        ...
        self.buttonNewKeys = Button(
                frameCustom, text='Get New Keys dla Selection',
        ...
        self.buttonDeleteCustomKeys.pack(side=LEFT, fill=X, expand=Prawda, padx=2)
        buttonSaveCustomKeys.pack(side=LEFT, fill=X, expand=Prawda, padx=2)
        frames[0].pack(side=TOP, fill=BOTH, expand=Prawda)
        frames[1].pack(side=TOP, fill=X, expand=Prawda, pady=2)
        zwróć frame
    ...
    def VarChanged_font(self, *params):
        '''When one font attribute changes, save them all, jako they are
        nie independent z each other. In particular, when we are
    ...
    def VarChanged_keyBinding(self, *params):
        ...
        jeżeli idleConf.IsCoreBinding(event):
            #this jest a core keybinding
            self.AddChangedItem('keys', keySet, event, value)
        inaczej: #this jest an extension key binding
    ...
    def ResetChangedItems(self):
        #When any config item jest changed w this dialog, an entry
        #should be made w the relevant section (config type) of this
        #dictionary. The key should be the config file section name oraz the
    ...
    def SetThemeType(self):
        jeżeli self.themeIsBuiltin.get():
    ...
    def SetKeysType(self):
        jeżeli self.keysAreBuiltin.get():
    ...
    def GetNewKeys(self):
        ...
        jeżeli currentKeySetName w self.changedItems['keys']: #unsaved changes
            keySetChanges = self.changedItems['keys'][currentKeySetName]
            dla event w keySetChanges:
                currentBindings[event] = keySetChanges[event].split()
        currentKeySequences = list(currentBindings.values())
        newKeys = GetKeysDialog(self, 'Get New Keys', bindName,
                currentKeySequences).result
        jeżeli newKeys: #new keys were specified
            jeżeli self.keysAreBuiltin.get(): #current key set jest a built-in
                message = ('Your changes will be saved jako a new Custom Key Set.'
                           ' Enter a name dla your new Custom Key Set below.')
                newKeySet = self.GetNewKeysName(message)
                jeżeli nie newKeySet: #user cancelled custom key set creation
                    self.listBindings.select_set(listIndex)
                    self.listBindings.select_anchor(listIndex)
                    zwróć
                inaczej: #create new custom key set based on previously active key set
    ...
    def CreateNewKeySet(self, newKeySetName):
        ...
        newKeys = {}
        dla event w prevKeys: #add key set to changed items
            eventName = event[2:-2] #trim off the angle brackets
            binding = ' '.join(prevKeys[event])
            newKeys[eventName] = binding
        #handle any unsaved changes to prev key set
        jeżeli prevKeySetName w self.changedItems['keys']:
    ...
    def LoadKeysList(self, keySetName):
        reselect = 0
        newKeySet = 0
        jeżeli self.listBindings.curselection():
            reselect = 1
            listIndex = self.listBindings.index(ANCHOR)
        ...
        dla bindName w bindNames:
            key = ' '.join(keySet[bindName]) #make key(s) into a string
            bindName = bindName[2:-2] #trim off the angle brackets
            jeżeli keySetName w self.changedItems['keys']:
                #handle any unsaved changes to this key set
                jeżeli bindName w self.changedItems['keys'][keySetName]:
    ...
    def DeleteCustomKeys(self):
        ...
        jeżeli nie tkMessageBox.askyesno(
                'Delete Key Set',  delmsg % keySetName, parent=self):
            zwróć
        #remove key set z config
    ...
    def DeleteCustomTheme(self):
        ...
        jeżeli nie tkMessageBox.askyesno(
            ...
            zwróć
        #remove theme z config
        idleConf.userCfg['highlight'].remove_section(themeName)
        jeżeli themeName w self.changedItems['highlight']:
    ...
    def GetColour(self):
        ...
        rgbTuplet, colourString = tkColorChooser.askcolor(
                parent=self, title='Pick new colour dla : '+target,
                initialcolor=prevColour)
        jeżeli colourString oraz (colourString != prevColour):
            #user didn't cancel, oraz they chose a new colour
            jeżeli self.themeIsBuiltin.get():  #current theme jest a built-in
                message = ('Your changes will be saved jako a new Custom Theme. '
                           'Enter a name dla your new Custom Theme below.')
                newTheme = self.GetNewThemeName(message)
                jeżeli nie newTheme:  #user cancelled custom theme creation
                    zwróć
                inaczej:  #create new custom theme based on previously active theme
                    self.CreateNewTheme(newTheme)
                    self.colour.set(colourString)
            inaczej:  #current theme jest user defined
                self.colour.set(colourString)

    def OnNewColourSet(self):
        newColour=self.colour.get()
        self.frameColourSet.config(bg=newColour)  #set sample
        plane ='foreground' jeżeli self.fgHilite.get() inaczej 'background'
    ...
    def SetFontSample(self, event=Nic):
        fontName = self.fontName.get()
        fontWeight = tkFont.BOLD jeżeli self.fontBold.get() inaczej tkFont.NORMAL
        newFont = (fontName, self.fontSize.get(), fontWeight)
        self.labelFontSample.config(font=newFont)
        self.textHighlightSample.configure(font=newFont)

    def SetHighlightTarget(self):
        jeżeli self.highlightTarget.get() == 'Cursor':  #bg nie possible
            self.radioFg.config(state=DISABLED)
            self.radioBg.config(state=DISABLED)
            self.fgHilite.set(1)
        inaczej:  #both fg oraz bg can be set
    ...
    def PaintThemeSample(self):
        jeżeli self.themeIsBuiltin.get():  #a default theme
            theme = self.builtinTheme.get()
        inaczej:  #a user theme
            theme = self.customTheme.get()
        dla elementTitle w self.themeElements:
            element = self.themeElements[elementTitle][0]
            colours = idleConf.GetHighlight(theme, element)
            jeżeli element == 'cursor': #cursor sample needs special painting
                colours['background'] = idleConf.GetHighlight(
                        theme, 'normal', fgBg='bg')
            #handle any unsaved changes to this theme
            jeżeli theme w self.changedItems['highlight']:
                themeDict = self.changedItems['highlight'][theme]
                jeżeli element + '-foreground' w themeDict:
                    colours['foreground'] = themeDict[element + '-foreground']
                jeżeli element + '-background' w themeDict:
    ...
    def SetHelpListButtonStates(self):
        jeżeli self.listHelp.size() < 1:  #no entries w list
            self.buttonHelpListEdit.config(state=DISABLED)
            self.buttonHelpListRemove.config(state=DISABLED)
        inaczej: #there are some entries
            jeżeli self.listHelp.curselection():  #there currently jest a selection
                self.buttonHelpListEdit.config(state=NORMAL)
                self.buttonHelpListRemove.config(state=NORMAL)
            inaczej:  #there currently jest nie a selection
                self.buttonHelpListEdit.config(state=DISABLED)
                self.buttonHelpListRemove.config(state=DISABLED)
    ...
    def HelpListItemEdit(self):
        ...
        jeżeli (nie newHelpSource) albo (newHelpSource == helpSource):
            zwróć #no changes
    ...
    def UpdateUserHelpChangedItems(self):
        "Clear oraz rebuild the HelpFiles section w self.changedItems"
        self.changedItems['main']['HelpFiles'] = {}
        dla num w range(1, len(self.userHelpList) + 1):
            self.AddChangedItem(
    ...
    def LoadFontCfg(self):
        ...
        lc_fonts = [s.lower() dla s w fonts]
        spróbuj:
    ...
    def LoadThemeCfg(self):
        ...
        jeżeli self.themeIsBuiltin.get(): #default theme selected
    ...
    def LoadKeyCfg(self):
        ...
        jeżeli self.keysAreBuiltin.get(): #default theme selected
    ...
    def LoadGeneralCfg(self):
        ...
        dla helpItem w self.userHelpList:
            self.listHelp.insert(END, helpItem[0])
        self.SetHelpListButtonStates()

    def LoadConfigs(self):
        """
        load configuration z default oraz user config files oraz populate
    ...
    def SaveNewKeySet(self, keySetName, keySet):
        ...
        """
        jeżeli nie idleConf.userCfg['keys'].has_section(keySetName):
    ...
    def SaveNewTheme(self, themeName, theme):
        ...
        """
        jeżeli nie idleConf.userCfg['highlight'].has_section(themeName):
    ...
    def SetUserValue(self, configType, section, item, value):
        jeżeli idleConf.defaultCfg[configType].has_option(section, item):
            jeżeli idleConf.defaultCfg[configType].Get(section, item) == value:
                #the setting equals a default setting, remove it z user cfg
                zwróć idleConf.userCfg[configType].RemoveOption(section, item)
        #jeżeli we got here set the option
        zwróć idleConf.userCfg[configType].SetOption(section, item, value)

    def SaveAllChangedConfigs(self):
        "Save configuration changes to the user config file."
        idleConf.userCfg['main'].Save()
        dla configType w self.changedItems:
            cfgTypeHasChanges = Nieprawda
            dla section w self.changedItems[configType]:
                jeżeli section == 'HelpFiles':
                    #this section gets completely replaced
                    idleConf.userCfg['main'].remove_section('HelpFiles')
                    cfgTypeHasChanges = Prawda
                dla item w self.changedItems[configType][section]:
                    value = self.changedItems[configType][section][item]
                    jeżeli self.SetUserValue(configType, section, item, value):
                        cfgTypeHasChanges = Prawda
            jeżeli cfgTypeHasChanges:
                idleConf.userCfg[configType].Save()
        dla configType w ['keys', 'highlight']:
            # save these even jeżeli unchanged!
            idleConf.userCfg[configType].Save()
        self.ResetChangedItems() #clear the changed items dict

    def DeactivateCurrentConfig(self):
        #Before a config jest saved, some cleanup of current
        #config must be done - remove the previous keybindings
        winInstances = self.parent.instance_dict.keys()
        dla instance w winInstances:
...
klasa VerticalScrolledFrame(Frame):
    """A pure Tkinter vertically scrollable frame.

    * Use the 'interior' attribute to place widgets inside the scrollable frame
    * Construct oraz pack/place/grid normally
    * This frame only allows vertical scrolling
    """
    def __init__(self, parent, *args, **kw):
        Frame.__init__(self, parent, *args, **kw)

        # create a canvas object oraz a vertical scrollbar dla scrolling it
        ...
        # create a frame inside the canvas which will be scrolled przy it
        self.interior = interior = Frame(canvas)
        interior_id = canvas.create_window(0, 0, window=interior, anchor=NW)

        # track changes to the canvas oraz frame width oraz sync them,
        # also updating the scrollbar
        def _configure_interior(event):
            # update the scrollbars to match the size of the inner frame
            size = (interior.winfo_reqwidth(), interior.winfo_reqheight())
            canvas.config(scrollregion="0 0 %s %s" % size)
            jeżeli interior.winfo_reqwidth() != canvas.winfo_width():
...
def is_int(s):
    "Return 's jest blank albo represents an int'"
    jeżeli nie s:
        zwróć Prawda
    spróbuj:
        int(s)
...
# TODO:
# * Revert to default(s)? Per option albo per extension?
# * List options w their original order (possible??)
klasa ConfigExtensionsDialog(Toplevel):
    """A dialog dla configuring IDLE extensions.

    This dialog jest generic - it works dla any oraz all IDLE extensions.

    IDLE extensions save their configuration options using idleConf.
    ConfigExtensionsDialog reads the current configuration using idleConf,
    supplies a GUI interface to change the configuration values, oraz saves the
    changes using idleConf.

    Not all changes take effect immediately - some may require restarting IDLE.
    This depends on each extension's implementation.

    All values are treated jako text, oraz it jest up to the user to supply
    reasonable values. The only exception to this are the 'enable*' options,
    which are boolean, oraz can be toggled przy an Prawda/Nieprawda button.
    """
    def __init__(self, parent, title=Nic, _htest=Nieprawda):
        Toplevel.__init__(self, parent)
        self.wm_withdraw()
        ...
        self.geometry(
                "+%d+%d" % (parent.winfo_rootx() + 20,
                parent.winfo_rooty() + (30 jeżeli nie _htest inaczej 150)))
        self.wm_title(title albo 'IDLE Extensions Configuration')
        ...
        # wait dla window to be generated
        self.update()
        # set current width jako the minimum width
    ...
    def load_extensions(self):
        "Fill self.extensions przy data z the default oraz user configs."
        self.extensions = {}
        dla ext_name w idleConf.GetExtensions(active_only=Nieprawda):
            self.extensions[ext_name] = []

        dla ext_name w self.extensions:
            opt_list = sorted(self.defaultCfg.GetOptionList(ext_name))

            # bring 'enable' options to the beginning of the list
            enables = [opt_name dla opt_name w opt_list
                       jeżeli opt_name.startswith('enable')]
            dla opt_name w enables:
                opt_list.remove(opt_name)
            opt_list = enables + opt_list

            dla opt_name w opt_list:
                def_str = self.defaultCfg.Get(
                        ext_name, opt_name, raw=Prawda)
                spróbuj:
                    def_obj = {'Prawda':Prawda, 'Nieprawda':Nieprawda}[def_str]
                    opt_type = 'bool'
                wyjąwszy KeyError:
                    spróbuj:
                        def_obj = int(def_str)
                ...
                spróbuj:
                    value = self.userCfg.Get(
                            ext_name, opt_name, type=opt_type, raw=Prawda,
                            default=def_obj)
                wyjąwszy ValueError:  # Need this until .Get fixed
    ...
    def create_tab_page(self, ext_name):
        ...
        # create an entry dla each configuration option
        dla row, opt w enumerate(self.extensions[ext_name]):
            # create a row przy a label oraz entry/checkbutton
            label = Label(entry_area, text=opt['name'])
            label.grid(row=row, column=0, sticky=NW)
            var = opt['var']
            jeżeli opt['type'] == 'bool':
                Checkbutton(entry_area, textvariable=var, variable=var,
                            onvalue='Prawda', offvalue='Nieprawda',
                            indicatoron=FALSE, selectcolor='', width=8
                    ).grid(row=row, column=1, sticky=W, padx=7)
            albo_inaczej opt['type'] == 'int':
    ...
    def set_user_value(self, section, opt):
        name = opt['name']
        default = opt['default']
        value = opt['var'].get().strip() albo default
        opt['var'].set(value)
        # jeżeli self.defaultCfg.has_section(section):
        # Currently, always true; jeżeli not, indent to zwróć
        jeżeli (value == default):
            zwróć self.userCfg.RemoveOption(section, name)
        # set the option
        zwróć self.userCfg.SetOption(section, name, value)

    def save_all_changed_configs(self):
        """Save configuration changes to the user config file."""
        has_changes = Nieprawda
        dla ext_name w self.extensions:
            options = self.extensions[ext_name]
            dla opt w options:
                jeżeli self.set_user_value(ext_name, opt):
                    has_changes = Prawda
        jeżeli has_changes:
            self.userCfg.Save()
