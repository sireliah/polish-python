# A lot of failures w these tests on Mac OS X.
# Byte order related?
...
klasa CFunctions(unittest.TestCase):
    _dll = CDLL(_ctypes_test.__file__)

    def S(self):
        zwróć c_longlong.in_dll(self._dll, "last_tf_arg_s").value
    def U(self):
        zwróć c_ulonglong.in_dll(self._dll, "last_tf_arg_u").value
    ...
    def test_void(self):
        self._dll.tv_i.restype = Nic
        self._dll.tv_i.argtypes = (c_int,)
        self.assertEqual(self._dll.tv_i(42), Nic)
        self.assertEqual(self.S(), 42)
        self.assertEqual(self._dll.tv_i(-42), Nic)
        self.assertEqual(self.S(), -42)

# The following repeats the above tests przy stdcall functions (where
# they are available)
spróbuj:
    WinDLL
wyjąwszy NameError:
    def stdcall_dll(*_): dalej
inaczej:
    klasa stdcall_dll(WinDLL):
        def __getattr__(self, name):
            jeżeli name[:2] == '__' oraz name[-2:] == '__':
...
@need_symbol('WinDLL')
klasa stdcallCFunctions(CFunctions):
