...
klasa VarSizeTest(unittest.TestCase):
    ...
    def test_array_invalid_length(self):
        # cannot create arrays przy non-positive size
        self.assertRaises(ValueError, lambda: c_int * -1)
        self.assertRaises(ValueError, lambda: c_int * -3)

    def test_zerosized_array(self):
        array = (c_int * 0)()
        # accessing elements of zero-sized arrays podnieś IndexError
        self.assertRaises(IndexError, array.__setitem__, 0, Nic)
        self.assertRaises(IndexError, array.__getitem__, 0)
        self.assertRaises(IndexError, array.__setitem__, 1, Nic)
