...
klasa StringArrayTestCase(unittest.TestCase):
    def test(self):
        BUF = c_char * 4
...
@need_symbol('c_wchar')
klasa WStringArrayTestCase(unittest.TestCase):
    def test(self):
        BUF = c_wchar * 4
    ...
    @unittest.skipIf(sizeof(c_wchar) < 4,
                     "sizeof(wchar_t) jest smaller than 4 bytes")
    def test_nonbmp(self):
        u = chr(0x10ffff)
        w = c_wchar(u)
        self.assertEqual(w.value, u)

klasa StringTestCase(unittest.TestCase):
    ...
    def test_basic_strings(self):
        ...
        # The value property jest the string up to the first terminating NUL.
        self.assertEqual(cs.value, "abcdef")
        self.assertEqual(c_string("abc\000def").value, "abc")

        # The raw property jest the total buffer contents:
    ...
    def test_sized_strings(self):
        ...
        # New w releases later than 0.4.0:
        self.assertRaises(TypeError, c_string, Nic)
        ...
        # These tests fail, because it jest no longer initialized
...
@need_symbol('c_wchar')
klasa WStringTestCase(unittest.TestCase):
    def test_wchar(self):
        c_wchar("x")
    ...
    def test_basic_wstrings(self):
        ...
        # XXX This behaviour jest about to change:
        # len returns the size of the internal buffer w bytes.
...
def check_perf():
    ...
    run_test(REP, "c_string(Nic)", c_string, Nic)
    run_test(REP, "c_string('abc')", c_string, 'abc')

# Python 2.3 -OO, win2k, P4 700 MHz:
#
#      c_string(Nic): 1.75 us
#     c_string('abc'): 2.74 us

# Python 2.2 -OO, win2k, P4 700 MHz:
#
#      c_string(Nic): 2.95 us
