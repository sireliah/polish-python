...
##dla n w "ABCDEFGHIMNOPQRS":
##    print n, hex(getattr(BITS, n).size), getattr(BITS, n).offset

klasa C_Test(unittest.TestCase):

    def test_ints(self):
        dla i w range(512):
            dla name w "ABCDEFGHI":
                b = BITS()
                setattr(b, name, i)
...
klasa BitFieldTest(unittest.TestCase):
    ...
    def test_unsigned(self):
        dla c_typ w unsigned_int_types:
            klasa X(Structure):
                _fields_ = [("a", c_typ, 3),
                            ("b", c_typ, 3),
                            ("c", c_typ, 1)]
    ...
    def fail_fields(self, *fields):
        zwróć self.get_except(type(Structure), "X", (),
                               {"_fields_": fields})

    def test_nonint_types(self):
        # bit fields are nie allowed on non-integer types.
        result = self.fail_fields(("a", c_char_p, 1))
        self.assertEqual(result, (TypeError, 'bit fields nie allowed dla type c_char_p'))

        result = self.fail_fields(("a", c_void_p, 1))
        self.assertEqual(result, (TypeError, 'bit fields nie allowed dla type c_void_p'))

        jeżeli c_int != c_long:
            result = self.fail_fields(("a", POINTER(c_int), 1))
            self.assertEqual(result, (TypeError, 'bit fields nie allowed dla type LP_c_int'))

        result = self.fail_fields(("a", c_char, 1))
        self.assertEqual(result, (TypeError, 'bit fields nie allowed dla type c_char'))

        klasa Dummy(Structure):
            _fields_ = []

        result = self.fail_fields(("a", Dummy, 1))
        self.assertEqual(result, (TypeError, 'bit fields nie allowed dla type Dummy'))

    @need_symbol('c_wchar')
    def test_c_wchar(self):
        result = self.fail_fields(("a", c_wchar, 1))
        self.assertEqual(result,
                (TypeError, 'bit fields nie allowed dla type c_wchar'))

    def test_single_bitfield_size(self):
        dla c_typ w int_types:
            result = self.fail_fields(("a", c_typ, -1))
            self.assertEqual(result, (ValueError, 'number of bits invalid dla bit field'))
    ...
    def test_mixed_4(self):
        klasa X(Structure):
            _fields_ = [("a", c_short, 4),
                        ...
                        ("e", c_short, 4),
                        ("f", c_int, 24)]
        # MSVC does NOT combine c_short oraz c_int into one field, GCC
        # does (unless GCC jest run przy '-mms-bitfields' which
        # produces code compatible przy MSVC).
        jeżeli os.name w ("nt", "ce"):
    ...
    def test_uint32_swap_little_endian(self):
        # Issue #23319
        klasa Little(LittleEndianStructure):
