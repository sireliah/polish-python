...
klasa Test(unittest.TestCase):
    ...
    def test_array2pointer(self):
        ...
        self.assertEqual([ptr[i] dla i w range(3)], [42, 17, 2])

        jeżeli 2*sizeof(c_short) == sizeof(c_int):
            ptr = cast(array, POINTER(c_short))
            jeżeli sys.byteorder == "little":
                self.assertEqual([ptr[i] dla i w range(6)],
                                     [42, 0, 17, 0, 2, 0])
            inaczej:
                self.assertEqual([ptr[i] dla i w range(6)],
                                     [0, 42, 0, 17, 0, 2])
    ...
    def test_p2a_objects(self):
        array = (c_char_p * 5)()
        self.assertEqual(array._objects, Nic)
        array[0] = b"foo bar"
        self.assertEqual(array._objects, {'0': b"foo bar"})

        p = cast(array, POINTER(c_char_p))
        # array oraz p share a common _objects attribute
