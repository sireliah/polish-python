...
klasa VARIANT_BOOL(ctypes._SimpleCData):
    _type_ = "v"
    def __repr__(self):
        zwróć "%s(%r)" % (self.__class__.__name__, self.value)
...
# w the windows header files, these are structures.
...
# WPARAM jest defined jako UINT_PTR (unsigned type)
# LPARAM jest defined jako LONG_PTR (signed type)
jeżeli ctypes.sizeof(ctypes.c_long) == ctypes.sizeof(ctypes.c_void_p):
    WPARAM = ctypes.c_ulong
    LPARAM = ctypes.c_long
albo_inaczej ctypes.sizeof(ctypes.c_longlong) == ctypes.sizeof(ctypes.c_void_p):
...
################################################################
# HANDLE types
HANDLE = ctypes.c_void_p # w the header files: void *
...
klasa _SMALL_RECT(ctypes.Structure):
    _fields_ = [('Left', SHORT),
                ('Top', SHORT),
                ('Right', SHORT),
                ('Bottom', SHORT)]
...
klasa _COORD(ctypes.Structure):
    _fields_ = [('X', SHORT),
                ('Y', SHORT)]

klasa POINT(ctypes.Structure):
    _fields_ = [("x", LONG),
                ("y", LONG)]
tagPOINT = _POINTL = POINTL = POINT

klasa SIZE(ctypes.Structure):
    _fields_ = [("cx", LONG),
                ("cy", LONG)]
tagSIZE = SIZEL = SIZE

def RGB(red, green, blue):
    zwróć red + (green << 8) + (blue << 16)

klasa FILETIME(ctypes.Structure):
    _fields_ = [("dwLowDateTime", DWORD),
                ("dwHighDateTime", DWORD)]
_FILETIME = FILETIME

klasa MSG(ctypes.Structure):
    _fields_ = [("hWnd", HWND),
                ("message", UINT),
                ("wParam", WPARAM),
                ("lParam", LPARAM),
...
klasa WIN32_FIND_DATAA(ctypes.Structure):
...
klasa WIN32_FIND_DATAW(ctypes.Structure):
    _fields_ = [("dwFileAttributes", DWORD),
                ("ftCreationTime", FILETIME),
                ("ftLastAccessTime", FILETIME),
                ("ftLastWriteTime", FILETIME),
