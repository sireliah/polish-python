...
def decode(input, errors='strict'):
    zwróć codecs.utf_32_decode(input, errors, Prawda)
...
klasa IncrementalDecoder(codecs.BufferedIncrementalDecoder):
    ...
    def _buffer_decode(self, input, errors, final):
        jeżeli self.decoder jest Nic:
            ...
            albo_inaczej consumed >= 4:
                podnieś UnicodeError("UTF-32 stream does nie start przy BOM")
    ...
    def getstate(self):
        ...
        addstate = int((sys.byteorder == "big") !=
                       (self.decoder jest codecs.utf_32_be_decode))
    ...
    def setstate(self, state):
        ...
        jeżeli state == 0:
            self.decoder = (codecs.utf_32_be_decode
                            ...
                            inaczej codecs.utf_32_le_decode)
        albo_inaczej state == 1:
            self.decoder = (codecs.utf_32_le_decode
                            jeżeli sys.byteorder == "big"
                            inaczej codecs.utf_32_be_decode)
        inaczej:
            self.decoder = Nic
...
klasa StreamReader(codecs.StreamReader):
    ...
    def decode(self, input, errors='strict'):
        (object, consumed, byteorder) = \
            codecs.utf_32_ex_decode(input, errors, 0, Nieprawda)
        jeżeli byteorder == -1:
