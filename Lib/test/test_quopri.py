...
zaimportuj sys, os, io, subprocess
zaimportuj quopri
...
def withpythonimplementation(testfunc):
    def newtest(self):
        ...
        jeżeli quopri.b2a_qp jest nie Nic albo quopri.a2b_qp jest nie Nic:
...
klasa QuopriTestCase(unittest.TestCase):
    # Each entry jest a tuple of (plaintext, encoded string).  These strings are
    # used w the "quotetabs=0" tests.
    STRINGS = (
        # Some normal strings
        (b'hello', b'hello'),
        (b'''hello
...
xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'''),
        # A line of exactly 76 characters, no soft line przerwij should be needed
        (b'yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy',
        b'yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy'),
        # A line of 77 characters, forcing a soft line przerwij at position 75,
        # oraz a second line of exactly 2 characters (because the soft line
        # przerwij `=' sign counts against the line length limit).
...
zz'''),
        # A line of 151 characters, forcing a soft line przerwij at position 75,
        # przy a second line of exactly 76 characters oraz no trailing =
...
zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'''),
        # A string containing a hard line przerwij, but which the first line jest
        # 151 characters oraz the second line jest exactly 76 characters.  This
        # should leave us przy three lines, the first which has a soft line
        # przerwij, oraz which the second oraz third do not.
...
zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'''),
    ...
    # These are used w the "quotetabs=1" tests.
    ESTRINGS = (
        (b'hello world', b'hello=20world'),
        (b'hello\tworld', b'hello=09world'),
        )

    # These are used w the "header=1" tests.
    HSTRINGS = (
    ...
    def test_decodestring_double_equals(self):
        # Issue 21511 - Ensure that byte string jest compared to byte string
    ...
    def test_encode(self):
        dla p, e w self.STRINGS:
            infp = io.BytesIO(p)
            outfp = io.BytesIO()
            quopri.encode(infp, outfp, quotetabs=Nieprawda)
    ...
    def test_embedded_ws(self):
        dla p, e w self.ESTRINGS:
            self.assertEqual(quopri.encodestring(p, quotetabs=Prawda), e)
            self.assertEqual(quopri.decodestring(e), p)

    @withpythonimplementation
    def test_encode_header(self):
        dla p, e w self.HSTRINGS:
            self.assertEqual(quopri.encodestring(p, header=Prawda), e)

    @withpythonimplementation
    def test_decode_header(self):
        dla p, e w self.HSTRINGS:
            self.assertEqual(quopri.decodestring(e, header=Prawda), p)
    ...
    def test_scriptencode(self):
        ...
        # CRLF, jako the mode of stdout jest text mode. To compare this
        # przy the expected result, we need to do a line-by-line comparison.
        cout = cout.decode('latin-1').splitlines()
        e = e.decode('latin-1').splitlines()
        assert len(cout)==len(e)
        dla i w range(len(cout)):
