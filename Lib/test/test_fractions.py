"""Tests dla Lib/fractions.py."""

z decimal zaimportuj Decimal
z test.support zaimportuj requires_IEEE_754
zaimportuj math
zaimportuj numbers
zaimportuj operator
...
zaimportuj unittest
zaimportuj warnings
z copy zaimportuj copy, deepcopy
z pickle zaimportuj dumps, loads
F = fractions.Fraction
gcd = fractions.gcd

klasa DummyFloat(object):
    """Dummy float klasa dla testing comparisons przy Fractions"""

    def __init__(self, value):
        jeżeli nie isinstance(value, float):
            podnieś TypeError("DummyFloat can only be initialized z float")
        self.value = value

    def _richcmp(self, other, op):
        jeżeli isinstance(other, numbers.Rational):
            zwróć op(F.from_float(self.value), other)
        albo_inaczej isinstance(other, DummyFloat):
            zwróć op(self.value, other.value)
        inaczej:
            zwróć NotImplemented

    def __eq__(self, other): zwróć self._richcmp(other, operator.eq)
    def __le__(self, other): zwróć self._richcmp(other, operator.le)
    def __lt__(self, other): zwróć self._richcmp(other, operator.lt)
    def __ge__(self, other): zwróć self._richcmp(other, operator.ge)
    def __gt__(self, other): zwróć self._richcmp(other, operator.gt)

    # shouldn't be calling __float__ at all when doing comparisons
    def __float__(self):
        assert Nieprawda, "__float__ should nie be invoked dla comparisons"

    # same goes dla subtraction
    def __sub__(self, other):
        assert Nieprawda, "__sub__ should nie be invoked dla comparisons"
    __rsub__ = __sub__


klasa DummyRational(object):
    """Test comparison of Fraction przy a naive rational implementation."""
    ...
    def __eq__(self, other):
        jeżeli isinstance(other, fractions.Fraction):
            zwróć (self.num == other._numerator oraz
    ...
    # this klasa jest dla testing comparisons; conversion to float
    # should never be used dla a comparison, since it loses accuracy
    def __float__(self):
        assert Nieprawda, "__float__ should nie be invoked"

klasa DummyFraction(fractions.Fraction):
    """Dummy Fraction subclass dla copy oraz deepcopy testing."""

klasa GcdTest(unittest.TestCase):

    def testMisc(self):
        # fractions.gcd() jest deprecated
        przy self.assertWarnsRegex(DeprecationWarning, r'fractions\.gcd'):
            gcd(1, 1)
...
def _components(r):
    zwróć (r.numerator, r.denominator)


klasa FractionTest(unittest.TestCase):

    def assertTypedEquals(self, expected, actual):
        """Asserts that both the types oraz values are the same."""
        self.assertEqual(type(expected), type(actual))
        self.assertEqual(expected, actual)

    def assertRaisesMessage(self, exc_type, message,
                            callable, *args, **kwargs):
        """Asserts that callable(*args, **kwargs) podnieśs exc_type(message)."""
        spróbuj:
            callable(*args, **kwargs)
        wyjąwszy exc_type jako e:
            self.assertEqual(message, str(e))
        inaczej:
            self.fail("%s nie podnieśd" % exc_type.__name__)
    ...
    def testInitFromFloat(self):
        ...
        self.assertEqual((3602879701896397, 36028797018963968),
                         _components(F(0.1)))
        # bug 16469: error types should be consistent przy float -> int
    ...
    def testInitFromDecimal(self):
        ...
        # bug 16469: error types should be consistent przy decimal -> int
    ...
    def testFromString(self):
        ...
        self.assertRaisesMessage(
            ...
            F, "3/0")
        self.assertRaisesMessage(
            ValueError, "Invalid literal dla Fraction: '3/'",
            F, "3/")
        self.assertRaisesMessage(
            ValueError, "Invalid literal dla Fraction: '/2'",
            F, "/2")
        self.assertRaisesMessage(
            ValueError, "Invalid literal dla Fraction: '3 /2'",
            F, "3 /2")
        self.assertRaisesMessage(
        ...
        self.assertRaisesMessage(
            ...
            F, "+ 3/2")
        self.assertRaisesMessage(
            # Avoid treating '.' jako a regex special character.
            ValueError, "Invalid literal dla Fraction: '3a2'",
            F, "3a2")
        self.assertRaisesMessage(
            # Don't accept combinations of decimals oraz rationals.
            ValueError, "Invalid literal dla Fraction: '3/7.2'",
            F, "3/7.2")
        self.assertRaisesMessage(
            # Don't accept combinations of decimals oraz rationals.
            ValueError, "Invalid literal dla Fraction: '3.2/7'",
            F, "3.2/7")
        self.assertRaisesMessage(
            # Allow 3. oraz .3, but nie .
            ValueError, "Invalid literal dla Fraction: '.'",
            F, ".")
    ...
    def testImmutable(self):
        ...
        # But jeżeli you _really_ need to:
        r._numerator = 4
        r._denominator = 2
        self.assertEqual((4, 2), _components(r))
        # Which przerwijs some important operations:
    ...
    def testConversions(self):
        ...
        self.assertEqual(Nieprawda, bool(F(0, 1)))
        self.assertEqual(Prawda, bool(F(3, 2)))
        self.assertTypedEquals(0.1, float(F(1, 10)))

        # Check that __float__ isn't implemented by converting the
        # numerator oraz denominator to float before dividing.
    ...
    def testMixedArithmetic(self):
        ...
        # No need dla divmod since we don't override it.
    ...
    def testMixingWithDecimal(self):
        # Decimal refuses mixed arithmetic (but nie mixed comparisons)
    ...
    def testComparisons(self):
        self.assertPrawda(F(1, 2) < F(2, 3))
        self.assertNieprawda(F(1, 2) < F(1, 2))
        self.assertPrawda(F(1, 2) <= F(2, 3))
        self.assertPrawda(F(1, 2) <= F(1, 2))
        self.assertNieprawda(F(2, 3) <= F(1, 2))
        self.assertPrawda(F(1, 2) == F(1, 2))
        self.assertNieprawda(F(1, 2) == F(1, 3))
        self.assertNieprawda(F(1, 2) != F(1, 2))
        self.assertPrawda(F(1, 2) != F(1, 3))

    def testComparisonsDummyRational(self):
        self.assertPrawda(F(1, 2) == DummyRational(1, 2))
        self.assertPrawda(DummyRational(1, 2) == F(1, 2))
        self.assertNieprawda(F(1, 2) == DummyRational(3, 4))
        self.assertNieprawda(DummyRational(3, 4) == F(1, 2))

        self.assertPrawda(F(1, 2) < DummyRational(3, 4))
        self.assertNieprawda(F(1, 2) < DummyRational(1, 2))
        self.assertNieprawda(F(1, 2) < DummyRational(1, 7))
        self.assertNieprawda(F(1, 2) > DummyRational(3, 4))
        self.assertNieprawda(F(1, 2) > DummyRational(1, 2))
        self.assertPrawda(F(1, 2) > DummyRational(1, 7))
        self.assertPrawda(F(1, 2) <= DummyRational(3, 4))
        self.assertPrawda(F(1, 2) <= DummyRational(1, 2))
        self.assertNieprawda(F(1, 2) <= DummyRational(1, 7))
        self.assertNieprawda(F(1, 2) >= DummyRational(3, 4))
        ...
        self.assertPrawda(DummyRational(1, 2) < F(3, 4))
        self.assertNieprawda(DummyRational(1, 2) < F(1, 2))
        self.assertNieprawda(DummyRational(1, 2) < F(1, 7))
        self.assertNieprawda(DummyRational(1, 2) > F(3, 4))
        self.assertNieprawda(DummyRational(1, 2) > F(1, 2))
        self.assertPrawda(DummyRational(1, 2) > F(1, 7))
        self.assertPrawda(DummyRational(1, 2) <= F(3, 4))
        self.assertPrawda(DummyRational(1, 2) <= F(1, 2))
        self.assertNieprawda(DummyRational(1, 2) <= F(1, 7))
        self.assertNieprawda(DummyRational(1, 2) >= F(3, 4))
    ...
    def testComparisonsDummyFloat(self):
        x = DummyFloat(1./3.)
        y = F(1, 3)
        self.assertPrawda(x != y)
        self.assertPrawda(x < y albo x > y)
        self.assertNieprawda(x == y)
        self.assertNieprawda(x <= y oraz x >= y)
        self.assertPrawda(y != x)
        self.assertPrawda(y < x albo y > x)
        self.assertNieprawda(y == x)
        self.assertNieprawda(y <= x oraz y >= x)

    def testMixedLess(self):
        self.assertPrawda(2 < F(5, 2))
        self.assertNieprawda(2 < F(4, 2))
        self.assertPrawda(F(5, 2) < 3)
        self.assertNieprawda(F(4, 2) < 2)

        self.assertPrawda(F(1, 2) < 0.6)
        self.assertNieprawda(F(1, 2) < 0.4)
        self.assertPrawda(0.4 < F(1, 2))
        self.assertNieprawda(0.5 < F(1, 2))

        self.assertNieprawda(float('inf') < F(1, 2))
        self.assertPrawda(float('-inf') < F(0, 10))
        self.assertNieprawda(float('nan') < F(-3, 7))
        self.assertPrawda(F(1, 2) < float('inf'))
        self.assertNieprawda(F(17, 12) < float('-inf'))
        self.assertNieprawda(F(144, -89) < float('nan'))

    def testMixedLessEqual(self):
        self.assertPrawda(0.5 <= F(1, 2))
        self.assertNieprawda(0.6 <= F(1, 2))
        self.assertPrawda(F(1, 2) <= 0.5)
        self.assertNieprawda(F(1, 2) <= 0.4)
        self.assertPrawda(2 <= F(4, 2))
        self.assertNieprawda(2 <= F(3, 2))
        self.assertPrawda(F(4, 2) <= 2)
        self.assertNieprawda(F(5, 2) <= 2)
    ...
    def testBigFloatComparisons(self):
        # Because 10**23 can't be represented exactly jako a float:
        self.assertNieprawda(F(10**23) == float(10**23))
        # The first test demonstrates why these are important.
        self.assertNieprawda(1e23 < float(F(math.trunc(1e23) + 1)))
        self.assertPrawda(1e23 < F(math.trunc(1e23) + 1))
        self.assertNieprawda(1e23 <= F(math.trunc(1e23) - 1))
        self.assertPrawda(1e23 > F(math.trunc(1e23) - 1))
        self.assertNieprawda(1e23 >= F(math.trunc(1e23) + 1))

    def testBigComplexComparisons(self):
        self.assertNieprawda(F(10**23) == complex(10**23))
        ...
        z = complex(0.375, 0.0)
        w = complex(0.375, 0.2)
        self.assertPrawda(x == z)
        self.assertNieprawda(x != z)
        self.assertNieprawda(x == w)
        self.assertPrawda(x != w)
        dla op w operator.lt, operator.le, operator.gt, operator.ge:
    ...
    def testMixedEqual(self):
        ...
        self.assertPrawda(F(4, 2) == 2)
        self.assertNieprawda(F(5, 2) == 2)
        self.assertNieprawda(F(5, 2) == float('nan'))
        self.assertNieprawda(float('nan') == F(3, 7))
        self.assertNieprawda(F(5, 2) == float('inf'))
        self.assertNieprawda(float('-inf') == F(2, 5))
    ...
    def testHash(self):
        ...
        # Check that __hash__ produces the same value jako hash(), for
        # consistency przy int oraz Decimal.  (See issue #10356.)
