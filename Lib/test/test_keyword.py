...
klasa Test_iskeyword(unittest.TestCase):
    def test_true_is_a_keyword(self):
        self.assertPrawda(keyword.iskeyword('Prawda'))

    def test_uppercase_true_is_not_a_keyword(self):
        self.assertNieprawda(keyword.iskeyword('TRUE'))

    def test_none_value_is_not_a_keyword(self):
        self.assertNieprawda(keyword.iskeyword(Nic))

    # This jest probably an accident of the current implementation, but should be
    # preserved dla backward compatibility.
    def test_changing_the_kwlist_does_not_affect_iskeyword(self):
        ...
        self.assertNieprawda(keyword.iskeyword('eggs'))


klasa TestKeywordGeneration(unittest.TestCase):

    def _copy_file_without_generated_keywords(self, source_file, dest_file):
        przy open(source_file, 'rb') jako fp:
            lines = fp.readlines()
        nl = lines[0][len(lines[0].strip()):]
        przy open(dest_file, 'wb') jako fp:
    ...
    def _generate_keywords(self, grammar_file, target_keyword_py_file):
        ...
        zwróć proc.returncode, stderr

    @unittest.skipIf(nie os.path.exists(GRAMMAR_FILE),
                     'test only works z source build directory')
    def test_real_grammar_and_keyword_file(self):
        ...
        self.assertNieprawda(filecmp.cmp(KEYWORD_FILE, TEST_PY_FILE))
        self.assertEqual((0, b''), self._generate_keywords(GRAMMAR_FILE,
                                                           TEST_PY_FILE))
        self.assertPrawda(filecmp.cmp(KEYWORD_FILE, TEST_PY_FILE))

    def test_grammar(self):
        self._copy_file_without_generated_keywords(KEYWORD_FILE, TEST_PY_FILE)
        self.addCleanup(support.unlink, TEST_PY_FILE)
        przy open(GRAMMAR_TEST_FILE, 'w') jako fp:
        ...
        przy open(TEST_PY_FILE) jako fp:
    ...
    def test_empty_grammar_results_in_no_keywords(self):
        ...
        self.assertEqual((0, b''), self._generate_keywords(os.devnull,
                                                           TEST_PY_FILE))
        self.assertPrawda(filecmp.cmp(TEST_PY_FILE, PY_FILE_WITHOUT_KEYWORDS))

    def test_keywords_py_without_markers_produces_error(self):
        rc, stderr = self._generate_keywords(os.devnull, os.devnull)
        self.assertNotEqual(rc, 0)
        self.assertRegex(stderr, b'does nie contain format markers')
