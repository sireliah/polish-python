...
# Re test suite oraz benchmark suite v1.5

# The 3 possible outcomes dla each pattern
[SUCCEED, FAIL, SYNTAX_ERROR] = range(3)

# Benchmark suite (needs expansion)
#
# The benchmark suite does nie test correctness, just speed.  The
# first element of each tuple jest the regex pattern; the second jest a
...
benchmarks = [
    ...
    ('.*(Python)', 'Python'),           # Bad text literal przy grouping

]

# Test suite (dla verifying correctness)
#
# The test suite jest a list of 5- albo 3-tuples.  The 5 parts of a
# complete tuple are:
# element 0: a string containing the pattern
#         1: the string to match against the pattern
#         2: the expected result (SUCCEED, FAIL, SYNTAX_ERROR)
#         3: a string that will be eval()'ed to produce a test string.
#            This jest an arbitrary Python expression; the available
#            variables are "found" (the whole match), oraz "g1", "g2", ...
#            up to "g99" contain the contents of each group, albo the
#            string 'Nic' jeżeli the group wasn't given a value, albo the
#            string 'Error' jeżeli the group index was out of range;
#            also "groups", the zwróć value of m.group() (a tuple).
#         4: The expected result of evaluating the expression.
#            If the two don't match, an error jest reported.
#
# If the regex isn't expected to work, the latter two elements can be omitted.

tests = [
    # Test ?P< oraz ?P= extensions
    ('(?P<foo_123', '', SYNTAX_ERROR),      # Unterminated group identifier
    ('(?P<1>a)', '', SYNTAX_ERROR),         # Begins przy a digit
    ('(?P<!>a)', '', SYNTAX_ERROR),         # Begins przy an illegal char
    ('(?P<foo!>a)', '', SYNTAX_ERROR),      # Begins przy an illegal char

    # Same tests, dla the ?P= form
    ...
    # Test \0 jest handled everywhere
    ...
    (r'[\a][\b][\f][\n][\r][\t][\v]', '\a\b\f\n\r\t\v', SUCCEED, 'found', '\a\b\f\n\r\t\v'),
    # NOTE: nie an error under PCRE/PRE:
    ...
    ('^a(bc+|b[eh])g|.h$', 'abh', SUCCEED, 'found+"-"+g1', 'bh-Nic'),
    ('(bc+d$|ef*g.|h?i(j|k))', 'effgz', SUCCEED, 'found+"-"+g1+"-"+g2', 'effgz-effgz-Nic'),
    ('(bc+d$|ef*g.|h?i(j|k))', 'ij', SUCCEED, 'found+"-"+g1+"-"+g2', 'ij-ij-j'),
    ('(bc+d$|ef*g.|h?i(j|k))', 'effg', FAIL),
    ('(bc+d$|ef*g.|h?i(j|k))', 'bcdd', FAIL),
    ('(bc+d$|ef*g.|h?i(j|k))', 'reffgz', SUCCEED, 'found+"-"+g1+"-"+g2', 'effgz-effgz-Nic'),
    ('(((((((((a)))))))))', 'a', SUCCEED, 'found', 'a'),
    ...
    ('([abc]*)\\1', 'abcabc', SUCCEED, 'found+"-"+g1', 'abcabc-abc'),
    ('(a)(b)c|ab', 'ab', SUCCEED, 'found+"-"+g1+"-"+g2', 'ab-Nic-Nic'),
    ...
    ('(a)+b|aac', 'aac', SUCCEED, 'found+"-"+g1', 'aac-Nic'),
...
# Python does nie have the same rules dla \\41 so this jest a syntax error
#    ('((((((((((a))))))))))\\41', 'aa', FAIL),
#    ('((((((((((a))))))))))\\41', 'a!', SUCCEED, 'found', 'a!'),
    ...
    ('(?i)(a+|b){0,1}?', 'AB', SUCCEED, 'found+"-"+g1', '-Nic'),
    ('(?i))(', '-', SYNTAX_ERROR),
    ...
    ('(?i)^a(bc+|b[eh])g|.h$', 'ABH', SUCCEED, 'found+"-"+g1', 'BH-Nic'),
    ('(?i)(bc+d$|ef*g.|h?i(j|k))', 'EFFGZ', SUCCEED, 'found+"-"+g1+"-"+g2', 'EFFGZ-EFFGZ-Nic'),
    ('(?i)(bc+d$|ef*g.|h?i(j|k))', 'IJ', SUCCEED, 'found+"-"+g1+"-"+g2', 'IJ-IJ-J'),
    ('(?i)(bc+d$|ef*g.|h?i(j|k))', 'EFFG', FAIL),
    ('(?i)(bc+d$|ef*g.|h?i(j|k))', 'BCDD', FAIL),
    ('(?i)(bc+d$|ef*g.|h?i(j|k))', 'REFFGZ', SUCCEED, 'found+"-"+g1+"-"+g2', 'EFFGZ-EFFGZ-Nic'),
    ('(?i)((((((((((a))))))))))', 'A', SUCCEED, 'g10', 'A'),
...
#    ('(?i)a(bc)d', 'ABCD', SUCCEED, 'g1+"-"+\\g1+"-"+\\\\g1', 'BC-$1-\\BC'),
    ...
    # lookbehind: split by : but nie jeżeli it jest escaped by -.
    ('(?<!-):(.*?)(?<!-):', 'a:bc-:de:f', SUCCEED, 'g1', 'bc-:de' ),
    # escaping przy \ jako we know it
    ('(?<!\\\):(.*?)(?<!\\\):', 'a:bc\\:de:f', SUCCEED, 'g1', 'bc\\:de' ),
    # terminating przy ' oraz escaping przy ? jako w edifact
    ("(?<!\\?)'(.*?)(?<!\\?)'", "a'bc?'de'f", SUCCEED, 'g1', "bc?'de" ),
...
123""", SUCCEED, 'found', 'abc'),
    ...
    # test \w, etc. both inside oraz outside character classes
    ...
    # xmllib problem
    (r'(([a-z]+):)?([a-z]+)$', 'smil', SUCCEED, 'g1+"-"+g2+"-"+g3', 'Nic-Nic-smil'),
    ...
    # bug 115040: rescan jeżeli flags are modified inside pattern
    (r' (?x)foo ', 'foo', SUCCEED, 'found', 'foo'),
    # bug 115618: negative lookahead
    (r'(?<!abc)(d.f)', 'abcdefdof', SUCCEED, 'found', 'dof'),
    # bug 116251: character klasa bug
    ...
    (r'(?i)[M]+', 'MMM', SUCCEED, 'found', 'MMM'),
    (r'(?i)[m]+', 'MMM', SUCCEED, 'found', 'MMM'),
    # bug 130748: ^* should be an error (niehing to repeat)
    (r'^*', '', SYNTAX_ERROR),
    ...
    (r'^((a)c)?(ab)$', 'ab', SUCCEED, 'g1+"-"+g2+"-"+g3', 'Nic-Nic-ab'),
    # another minimizing repeat problem (capturing groups w assertions)
    ('^([ab]*?)(?=(b)?)c', 'abc', SUCCEED, 'g1+"-"+g2', 'ab-Nic'),
    ('^([ab]*?)(?!(b))c', 'abc', SUCCEED, 'g1+"-"+g2', 'ab-Nic'),
    ('^([ab]*?)(?<!(a))c', 'abc', SUCCEED, 'g1+"-"+g2', 'ab-Nic'),
]
