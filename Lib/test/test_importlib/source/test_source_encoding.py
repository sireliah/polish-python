...
# imported dla the parser to use.
...
klasa EncodingTest:
    ...
    """PEP 3120 makes UTF-8 the default encoding dla source code
    [default encoding].

    PEP 263 specifies how that can change on a per-file basis. Either the first
    albo second line can contain the encoding line [encoding first line]
    encoding second line]. If the file has the BOM marker it jest considered UTF-8
    implicitly [BOM]. If any encoding jest specified it must be UTF-8, inaczej it jest
    an error [BOM oraz utf-8][BOM conflict].
    ...
    def run_test(self, source):
        przy util.create_modules(self.module_name) jako mapping:
            przy open(mapping[self.module_name], 'wb') jako file:
                file.write(source)
            loader = self.machinery.SourceFileLoader(self.module_name,
                                                  mapping[self.module_name])
            zwróć self.load(loader)
...
klasa EncodingTestPEP451(EncodingTest):
...
klasa EncodingTestPEP302(EncodingTest):

    def load(self, loader):
        przy warnings.catch_warnings():
            warnings.simplefilter('ignore', DeprecationWarning)
            zwróć loader.load_module(self.module_name)
...
klasa LineEndingTest:

    r"""Source written przy the three types of line endings (\n, \r\n, \r)
    ...
    def run_test(self, line_ending):
        ...
        przy util.create_modules(module_name) jako mapping:
            przy open(mapping[module_name], 'wb') jako file:
                file.write(source)
            loader = self.machinery.SourceFileLoader(module_name,
                                                     mapping[module_name])
            zwróć self.load(loader, module_name)
...
klasa LineEndingTestPEP451(LineEndingTest):
...
klasa LineEndingTestPEP302(LineEndingTest):

    def load(self, loader, module_name):
        przy warnings.catch_warnings():
            warnings.simplefilter('ignore', DeprecationWarning)
            zwróć loader.load_module(module_name)
