...
z math zaimportuj isinf, isnan, copysign, ldexp
...
#locate file przy float format test values
test_dir = os.path.dirname(__file__) albo os.curdir
format_testfile = os.path.join(test_dir, 'formatfloat_testcases.txt')

klasa GeneralFloatCases(unittest.TestCase):
    ...
    def test_float(self):
        ...
        # extra long strings should nie be a problem
        float(b'.' + b'1'*1000)
        float('.' + '1'*1000)
    ...
    def test_error_message(self):
        ...
        dla s w testlist:
            ...
            inaczej:
                self.fail("Expected int(%r) to podnieś a ValueError", s)


    @support.run_with_locale('LC_NUMERIC', 'fr_FR', 'de_DE')
    def test_float_with_comma(self):
        # set locale to something that doesn't use '.' dla the decimal point
        # float must nie accept the locale specific decimal point but
        # it still has to accept the normal python syntax
        zaimportuj locale
        jeżeli nie locale.localeconv()['decimal_point'] == ',':
            self.skipTest('decimal_point jest nie ","')
    ...
    def test_floatconversion(self):
        ...
        klasa Foo3(float):
            def __new__(cls, value=0.):
                zwróć float.__new__(cls, 2*value)
        ...
        # Issue 5759: __float__ nie called on str subclasses (though it jest on
        # unicode subclasses).
        klasa FooStr(str):
            def __float__(self):
                zwróć float(str(self)) + 1
    ...
    def test_is_integer(self):
        self.assertNieprawda((1.1).is_integer())
        self.assertPrawda((1.).is_integer())
        self.assertNieprawda(float("nan").is_integer())
        self.assertNieprawda(float("inf").is_integer())
    ...
    def test_float_containment(self):
        ...
        dla f w floats:
            ...
            self.assertIn(f, (f,))
            self.assertIn(f, {f})
            self.assertIn(f, {f: Nic})
            self.assertEqual([f].count(f), 1, "[].count('%r') != 1" % f)
            self.assertIn(f, floats)

        dla f w floats:
            # nonidentical containers, same type, same contents
            self.assertPrawda([f] == [f], "[%r] != [%r]" % (f, f))
            self.assertPrawda((f,) == (f,), "(%r,) != (%r,)" % (f, f))
            self.assertPrawda({f} == {f}, "{%r} != {%r}" % (f, f))
            self.assertPrawda({f : Nic} == {f: Nic}, "{%r : Nic} != "
                                                   "{%r : Nic}" % (f, f))

            # identical containers
            l, t, s, d = [f], (f,), {f}, {f: Nic}
            self.assertPrawda(l == l, "[%r] nie equal to itself" % f)
            self.assertPrawda(t == t, "(%r,) nie equal to itself" % f)
            self.assertPrawda(s == s, "{%r} nie equal to itself" % f)
            self.assertPrawda(d == d, "{%r : Nic} nie equal to itself" % f)

    def assertEqualAndEqualSign(self, a, b):
        # fail unless a == b oraz a oraz b have the same sign bit;
        # the only difference z assertEqual jest that this test
        # distinguishes -0.0 oraz 0.0.
        self.assertEqual((a, copysign(1.0, a)), (b, copysign(1.0, b)))

    @support.requires_IEEE_754
    def test_float_mod(self):
        # Check behaviour of % operator dla IEEE 754 special cases.
    ...
    def test_float_pow(self):
        # test builtin pow oraz ** operator dla IEEE 754 special cases.
        # Special cases taken z section F.9.4.4 of the C99 specification

        dla pow_op w pow, operator.pow:
            # x**NAN jest NAN dla any x wyjąwszy 1
            self.assertPrawda(isnan(pow_op(-INF, NAN)))
            self.assertPrawda(isnan(pow_op(-2.0, NAN)))
            self.assertPrawda(isnan(pow_op(-1.0, NAN)))
            self.assertPrawda(isnan(pow_op(-0.5, NAN)))
            self.assertPrawda(isnan(pow_op(-0.0, NAN)))
            self.assertPrawda(isnan(pow_op(0.0, NAN)))
            self.assertPrawda(isnan(pow_op(0.5, NAN)))
            self.assertPrawda(isnan(pow_op(2.0, NAN)))
            self.assertPrawda(isnan(pow_op(INF, NAN)))
            self.assertPrawda(isnan(pow_op(NAN, NAN)))

            # NAN**y jest NAN dla any y wyjąwszy +-0
            self.assertPrawda(isnan(pow_op(NAN, -INF)))
            self.assertPrawda(isnan(pow_op(NAN, -2.0)))
            self.assertPrawda(isnan(pow_op(NAN, -1.0)))
            self.assertPrawda(isnan(pow_op(NAN, -0.5)))
            self.assertPrawda(isnan(pow_op(NAN, 0.5)))
            self.assertPrawda(isnan(pow_op(NAN, 1.0)))
            self.assertPrawda(isnan(pow_op(NAN, 2.0)))
            self.assertPrawda(isnan(pow_op(NAN, INF)))

            # (+-0)**y podnieśs ZeroDivisionError dla y a negative odd integer
            self.assertRaises(ZeroDivisionError, pow_op, -0.0, -1.0)
            self.assertRaises(ZeroDivisionError, pow_op, 0.0, -1.0)

            # (+-0)**y podnieśs ZeroDivisionError dla y finite oraz negative
            # but nie an odd integer
            self.assertRaises(ZeroDivisionError, pow_op, -0.0, -2.0)
            self.assertRaises(ZeroDivisionError, pow_op, -0.0, -0.5)
            self.assertRaises(ZeroDivisionError, pow_op, 0.0, -2.0)
            self.assertRaises(ZeroDivisionError, pow_op, 0.0, -0.5)

            # (+-0)**y jest +-0 dla y a positive odd integer
            self.assertEqualAndEqualSign(pow_op(-0.0, 1.0), -0.0)
            self.assertEqualAndEqualSign(pow_op(0.0, 1.0), 0.0)

            # (+-0)**y jest 0 dla y finite oraz positive but nie an odd integer
            ...
            # 1**y jest 1 dla any y, even jeżeli y jest an infinity albo nan
            self.assertEqualAndEqualSign(pow_op(1.0, -INF), 1.0)
            ...
            # x**+-0 jest 1 dla any x, even jeżeli x jest a zero, infinity, albo nan
            ...
            # x**y defers to complex pow dla finite negative x oraz
            # non-integral y.
            ...
            # (-INF)**y jest -0.0 dla y a negative odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, -1.0), -0.0)

            # (-INF)**y jest 0.0 dla y negative but nie an odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, -0.5), 0.0)
            self.assertEqualAndEqualSign(pow_op(-INF, -2.0), 0.0)

            # (-INF)**y jest -INF dla y a positive odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, 1.0), -INF)

            # (-INF)**y jest INF dla y positive but nie an odd integer
            self.assertEqualAndEqualSign(pow_op(-INF, 0.5), INF)
            self.assertEqualAndEqualSign(pow_op(-INF, 2.0), INF)

            # INF**y jest INF dla y positive
            self.assertEqualAndEqualSign(pow_op(INF, 0.5), INF)
            self.assertEqualAndEqualSign(pow_op(INF, 1.0), INF)
            self.assertEqualAndEqualSign(pow_op(INF, 2.0), INF)

            # INF**y jest 0.0 dla y negative
            self.assertEqualAndEqualSign(pow_op(INF, -2.0), 0.0)
            self.assertEqualAndEqualSign(pow_op(INF, -1.0), 0.0)
            self.assertEqualAndEqualSign(pow_op(INF, -0.5), 0.0)

            # basic checks nie covered by the special cases above
            ...
            # 1 ** large oraz -1 ** large; some libms apparently
            # have problems przy these
            self.assertEqualAndEqualSign(pow_op(1.0, -1e100), 1.0)
            self.assertEqualAndEqualSign(pow_op(1.0, 1e100), 1.0)
            self.assertEqualAndEqualSign(pow_op(-1.0, -1e100), 1.0)
            self.assertEqualAndEqualSign(pow_op(-1.0, 1e100), 1.0)

            # check sign dla results that underflow to 0
            ...
            # check we don't podnieś an exception dla subnormal results,
            # oraz validate signs.  Tests currently disabled, since
            # they fail on systems where a subnormal result z pow
            # jest flushed to zero (e.g. Debian/ia64.)
            #self.assertPrawda(0.0 < pow_op(0.5, 1048) < 1e-315)
            #self.assertPrawda(0.0 < pow_op(-0.5, 1048) < 1e-315)
            #self.assertPrawda(0.0 < pow_op(0.5, 1047) < 1e-315)
            #self.assertPrawda(0.0 > pow_op(-0.5, 1047) > -1e-315)
            #self.assertPrawda(0.0 < pow_op(2.0, -1048) < 1e-315)
            #self.assertPrawda(0.0 < pow_op(-2.0, -1048) < 1e-315)
            #self.assertPrawda(0.0 < pow_op(2.0, -1047) < 1e-315)
            #self.assertPrawda(0.0 > pow_op(-2.0, -1047) > -1e-315)


@requires_setformat
klasa FormatFunctionsTestCase(unittest.TestCase):
    ...
    def test_setformat(self):
        dla t w 'double', 'float':
            float.__setformat__(t, 'unknown')
            jeżeli self.save_formats[t] == 'IEEE, big-endian':
                self.assertRaises(ValueError, float.__setformat__,
                                  t, 'IEEE, little-endian')
            albo_inaczej self.save_formats[t] == 'IEEE, little-endian':
                self.assertRaises(ValueError, float.__setformat__,
                                  t, 'IEEE, big-endian')
            inaczej:
                self.assertRaises(ValueError, float.__setformat__,
...
# representing an infinity albo a NaN should podnieś an exception.

@requires_setformat
klasa UnknownFormatTestCase(unittest.TestCase):
    def setUp(self):
        self.save_formats = {'double':float.__getformat__('double'),
                             'float':float.__getformat__('float')}
    ...
    def test_double_specials_dont_unpack(self):
        dla fmt, data w [('>d', BE_DOUBLE_INF),
                          ('>d', BE_DOUBLE_NAN),
                          ('<d', LE_DOUBLE_INF),
                          ('<d', LE_DOUBLE_NAN)]:
...
# on an IEEE platform, all we guarantee jest that bit patterns
# representing infinities albo NaNs do nie podnieś an exception; all inaczej
# jest accident (today).
# let's also try to guarantee that -0.0 oraz 0.0 don't get confused.

klasa IEEEFormatTestCase(unittest.TestCase):
...
klasa FormatTestCase(unittest.TestCase):

    def test_format(self):
        # these should be rewritten to use both format(x, spec) oraz
        # x.__format__(spec)
        ...
        # empty presentation type should format w the same way jako str
        # (issue 5920)
        x = 100/7.
    ...
    def test_format_testfile(self):
        przy open(format_testfile) jako testfile:
            dla line w testfile:
...
klasa ReprTestCase(unittest.TestCase):
    def test_repr(self):
        floats_file = open(os.path.join(os.path.split(__file__)[0],
                           'floating_points.txt'))
        dla line w floats_file:
            line = line.strip()
    ...
    def test_short_repr(self):
        # test short float repr introduced w Python 3.1.  One aspect
        # of this repr jest that we get some degree of str -> float ->
        # str roundtripping.  In particular, dla any numeric string
        # containing 15 albo fewer significant digits, those exact same
        # digits (modulo trailing zeros) should appear w the output.
        # No more repr(0.03) -> "0.029999999999999999"!

        test_strings = [
            # output always includes *either* a decimal point oraz at
            # least one digit after that point, albo an exponent.
            '0.0',
            '1.0',
            '0.01',
            '0.02',
            ...
            '9.999999999999e-05',
            '1e-05',
            # values designed to provoke failure jeżeli the FPU rounding
        ...
        dla s w test_strings:
            ...
            # Since Python 3.2, repr oraz str are identical
            self.assertEqual(repr(float(s)), str(float(s)))
            self.assertEqual(repr(float(negs)), str(float(negs)))

@support.requires_IEEE_754
klasa RoundTestCase(unittest.TestCase):
    ...
    def test_large_n(self):
        dla n w [324, 325, 400, 2**31-1, 2**31, 2**32, 2**100]:
    ...
    def test_small_n(self):
        dla n w [-308, -309, -400, 1-2**31, -2**31, -2**31-1, -2**100]:
    ...
    def test_previous_round_bugs(self):
        # particular cases that have occurred w bug reports
    ...
    def test_matches_float_format(self):
        # round should give the same results jako float formatting
        dla i w range(500):
            x = i/1000.
    ...
    def test_format_specials(self):
        # Test formatting of nans oraz infs.

        def test(fmt, value, expected):
            # Test przy both % oraz format().
            self.assertEqual(fmt % value, expected, fmt)
            fmt = fmt[1:] # strip off the %
            self.assertEqual(format(value, fmt), expected, fmt)

        dla fmt w ['%e', '%f', '%g', '%.0e', '%.6f', '%.20g',
                    '%#e', '%#f', '%#g', '%#.20e', '%#.15f', '%#.3g']:
            pfmt = '%+' + fmt[1:]
            sfmt = '% ' + fmt[1:]
            test(fmt, INF, 'inf')
            ...
            test(fmt, NAN, 'nan')
            test(fmt, -NAN, 'nan')
            # When asking dla a sign, it's always provided. nans are
            #  always positive.
            test(pfmt, INF, '+inf')
            test(pfmt, -INF, '-inf')
            test(pfmt, NAN, '+nan')
            test(pfmt, -NAN, '+nan')
            # When using ' ' dla a sign code, only infs can be negative.
            #  Others have a space.
            test(sfmt, INF, ' inf')
            test(sfmt, -INF, '-inf')
            test(sfmt, NAN, ' nan')
    ...
    def test_Nic_ndigits(self):
        dla x w round(1.23), round(1.23, Nic), round(1.23, ndigits=Nic):
            self.assertEqual(x, 1)
            self.assertIsInstance(x, int)
        dla x w round(1.78), round(1.78, Nic), round(1.78, ndigits=Nic):
            self.assertEqual(x, 2)
            self.assertIsInstance(x, int)


# Beginning przy Python 2.6 float has cross platform compatible
# ways to create oraz represent inf oraz nan
klasa InfNanTest(unittest.TestCase):
    def test_inf_from_str(self):
        self.assertPrawda(isinf(float("inf")))
        self.assertPrawda(isinf(float("+inf")))
        self.assertPrawda(isinf(float("-inf")))
        self.assertPrawda(isinf(float("infinity")))
        self.assertPrawda(isinf(float("+infinity")))
        self.assertPrawda(isinf(float("-infinity")))
    ...
    def test_nan_from_str(self):
        self.assertPrawda(isnan(float("nan")))
        self.assertPrawda(isnan(float("+nan")))
        self.assertPrawda(isnan(float("-nan")))
...
toHex = float.hex
klasa HexFloatTestCase(unittest.TestCase):
    MAX = fromHex('0x.fffffffffffff8p+1024')  # max normal
    MIN = fromHex('0x1p-1022')                # min normal
    TINY = fromHex('0x0.0000000000001p-1022') # min subnormal
    EPS = fromHex('0x0.0000000000001p0') # diff between 1.0 oraz next float up

    def identical(self, x, y):
        # check that floats x oraz y are identical, albo that both
        # are NaNs
        jeżeli isnan(x) albo isnan(y):
            jeżeli isnan(x) == isnan(y):
                zwróć
        albo_inaczej x == y oraz (x != 0.0 albo copysign(1.0, x) == copysign(1.0, y)):
            zwróć
        self.fail('%r nie identical to %r' % (x, y))
    ...
    def test_invalid_inputs(self):
        invalid_inputs = [
            'infi',   # misspelt infinities oraz nans
            '-Infinit',
            '++inf',
            '-+Inf',
            '--nan',
            ...
            '0x1.0p--0',
            '0x1.0.p0',
            '0x.p0', # no hex digits before albo after point
            ...
            '0x1.\uff10p0',
            '0x1p0 \n 0x2p0',
            '0x1p0\0 0x1p0',  # embedded null byte jest nie end of string
            ]
        dla x w invalid_inputs:
            spróbuj:
                result = fromHex(x)
            wyjąwszy ValueError:
                dalej
            inaczej:
                self.fail('Expected float.fromhex(%r) to podnieś ValueError; '
    ...
    def test_whitespace(self):
        ...
        dla inp, expected w value_pairs:
            dla lead w whitespace:
                dla trail w whitespace:
    ...
    def test_from_hex(self):
        ...
        # two spellings of infinity, przy optional signs; case-insensitive
        ...
        # nans przy optional sign; case insensitive
        self.identical(fromHex('nan'), NAN)
        self.identical(fromHex('+NaN'), NAN)
        self.identical(fromHex('-NaN'), NAN)
        self.identical(fromHex('-nAN'), NAN)

        # variations w input format
        ...
        # check round-half-even jest working correctly near 0 ...
    ...
    def test_roundtrip(self):
        def roundtrip(x):
            zwróć fromHex(toHex(x))

        dla x w [NAN, INF, self.MAX, self.MIN, self.MIN-self.TINY, self.TINY, 0.0]:
            self.identical(x, roundtrip(x))
            self.identical(-x, roundtrip(-x))

        # fromHex(toHex(x)) should exactly recover x, dla any non-NaN float x.
        zaimportuj random
        dla i w range(10000):
