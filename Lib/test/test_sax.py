# regression test dla SAX 2.0
# $Id$

z xml.sax zaimportuj make_parser, ContentHandler, \
                    SAXException, SAXReaderNotAvailable, SAXParseException
zaimportuj unittest
spróbuj:
    make_parser()
wyjąwszy SAXReaderNotAvailable:
    # don't try to test this module jeżeli we cannot create a parser
    podnieś unittest.SkipTest("no XML parsers available")
z xml.sax.saxutils zaimportuj XMLGenerator, escape, unescape, quoteattr, \
                             XMLFilterBase, prepare_input_source
z xml.sax.expatreader zaimportuj create_parser
z xml.sax.handler zaimportuj feature_namespaces
z xml.sax.xmlreader zaimportuj InputSource, AttributesImpl, AttributesNSImpl
...
z test.support zaimportuj findfile, run_unittest, TESTFN
...
supports_nonascii_filenames = Prawda
jeżeli nie os.path.supports_unicode_filenames:
    ...
    wyjąwszy (UnicodeError, TypeError):
        ...
        supports_nonascii_filenames = Nieprawda
...
klasa XmlTestBase(unittest.TestCase):
    def verify_empty_attrs(self, attrs):
        ...
        self.assertEqual(attrs.get("attrs"), Nic)
    ...
    def verify_empty_nsattrs(self, attrs):
        ...
        self.assertEqual(list(attrs.keys()), [])
        self.assertEqual(attrs.get((ns_uri, "attr")), Nic)
...
def xml_str(doc, encoding=Nic):
    jeżeli encoding jest Nic:
        zwróć doc
    zwróć '<?xml version="1.0" encoding="%s"?>\n%s' % (encoding, doc)

def xml_bytes(doc, encoding, decl_encoding=...):
    jeżeli decl_encoding jest ...:
        decl_encoding = encoding
    zwróć xml_str(doc, decl_encoding).encode(encoding, 'xmlcharrefreplace')

def make_xml_file(doc, encoding, decl_encoding=...):
    jeżeli decl_encoding jest ...:
        decl_encoding = encoding
    przy open(TESTFN, 'w', encoding=encoding, errors='xmlcharrefreplace') jako f:
        f.write(xml_str(doc, decl_encoding))


klasa ParseTest(unittest.TestCase):
    ...
    def test_parse_text(self):
        ...
        dla encoding w encodings:
            ...
            przy open(TESTFN, 'r', encoding=encoding) jako f:
                self.check_parse(f)
            self.check_parse(StringIO(self.data))
            make_xml_file(self.data, encoding, Nic)
            przy open(TESTFN, 'r', encoding=encoding) jako f:
                self.check_parse(f)

    def test_parse_bytes(self):
        # UTF-8 jest default encoding, US-ASCII jest compatible przy UTF-8,
        # UTF-16 jest autodetected
        ...
        dla encoding w encodings:
            ...
            przy open(TESTFN, 'rb') jako f:
                self.check_parse(f)
            self.check_parse(BytesIO(xml_bytes(self.data, encoding, Nic)))
            make_xml_file(self.data, encoding, Nic)
            self.check_parse(TESTFN)
            przy open(TESTFN, 'rb') jako f:
                self.check_parse(f)
        # accept UTF-8 przy BOM
        self.check_parse(BytesIO(xml_bytes(self.data, 'utf-8-sig', 'utf-8')))
        make_xml_file(self.data, 'utf-8-sig', 'utf-8')
        self.check_parse(TESTFN)
        przy open(TESTFN, 'rb') jako f:
            self.check_parse(f)
        self.check_parse(BytesIO(xml_bytes(self.data, 'utf-8-sig', Nic)))
        make_xml_file(self.data, 'utf-8-sig', Nic)
        self.check_parse(TESTFN)
        przy open(TESTFN, 'rb') jako f:
            self.check_parse(f)
        # accept data przy declared encoding
        ...
        przy self.assertRaises(SAXException):
            self.check_parse(BytesIO(xml_bytes(self.data, 'iso-8859-1', Nic)))
        make_xml_file(self.data, 'iso-8859-1', Nic)
        przy support.check_warnings(('unclosed file', ResourceWarning)):
    ...
    def test_parse_InputSource(self):
        # accept data without declared but przy explicitly specified encoding
        make_xml_file(self.data, 'iso-8859-1', Nic)
        przy open(TESTFN, 'rb') jako f:
            input = InputSource()
    ...
    def check_parseString(self, s):
        z xml.sax zaimportuj parseString
        result = StringIO()
    ...
    def test_parseString_bytes(self):
        ...
        dla encoding w encodings:
            ...
            self.check_parseString(xml_bytes(self.data, encoding, Nic))
        # accept UTF-8 przy BOM
        self.check_parseString(xml_bytes(self.data, 'utf-8-sig', 'utf-8'))
        self.check_parseString(xml_bytes(self.data, 'utf-8-sig', Nic))
        # accept data przy declared encoding
        self.check_parseString(xml_bytes(self.data, 'iso-8859-1'))
        # fail on non-UTF-8 incompatible data without declared encoding
        przy self.assertRaises(SAXException):
            self.check_parseString(xml_bytes(self.data, 'iso-8859-1', Nic))

klasa MakeParserTest(unittest.TestCase):
    def test_make_parser2(self):
        # Creating parsers several times w a row should succeed.
        # Testing this because there have been failures of this kind
        # before.
        z xml.sax zaimportuj make_parser
        p = make_parser()
        z xml.sax zaimportuj make_parser
        p = make_parser()
        z xml.sax zaimportuj make_parser
...
klasa SaxutilsTest(unittest.TestCase):
    # ===== escape
    ...
    def test_single_double_quoteattr(self):
        self.assertEqual(quoteattr("Includes 'single' oraz \"double\" quotes"),
                         "\"Includes 'single' oraz &quot;double&quot; quotes\"")
...
klasa PrepareInputSourceTest(unittest.TestCase):

    def setUp(self):
        self.file = support.TESTFN
        przy open(self.file, "w") jako tmp:
            tmp.write("This was read z a file.")

    def tearDown(self):
        support.unlink(self.file)

    def make_byte_stream(self):
        zwróć BytesIO(b"This jest a byte stream.")

    def make_character_stream(self):
        zwróć StringIO("This jest a character stream.")

    def checkContent(self, stream, content):
        self.assertIsNotNic(stream)
        self.assertEqual(stream.read(), content)
        stream.close()


    def test_character_stream(self):
        # If the source jest an InputSource przy a character stream, use it.
        src = InputSource(self.file)
        src.setCharacterStream(self.make_character_stream())
        prep = prepare_input_source(src)
        self.assertIsNic(prep.getByteStream())
        self.checkContent(prep.getCharacterStream(),
                          "This jest a character stream.")

    def test_byte_stream(self):
        # If the source jest an InputSource that does nie have a character
        # stream but does have a byte stream, use the byte stream.
        src = InputSource(self.file)
        src.setByteStream(self.make_byte_stream())
        prep = prepare_input_source(src)
        self.assertIsNic(prep.getCharacterStream())
        self.checkContent(prep.getByteStream(),
                          b"This jest a byte stream.")

    def test_system_id(self):
        # If the source jest an InputSource that has neither a character
        # stream nor a byte stream, open the system ID.
        src = InputSource(self.file)
        prep = prepare_input_source(src)
        self.assertIsNic(prep.getCharacterStream())
        self.checkContent(prep.getByteStream(),
                          b"This was read z a file.")

    def test_string(self):
        # If the source jest a string, use it jako a system ID oraz open it.
    ...
    def test_binary_file(self):
        # If the source jest a binary file-like object, use it jako a byte
        # stream.
    ...
    def test_text_file(self):
        # If the source jest a text file-like object, use it jako a character
        # stream.
...
klasa XmlgenTest:
    ...
    def test_xmlgen_basic_empty(self):
        result = self.ioclass()
        gen = XMLGenerator(result, short_empty_elements=Prawda)
        gen.startDocument()
        gen.startElement("doc", {})
        gen.endElement("doc")
        gen.endDocument()
    ...
    def test_xmlgen_ns(self):
        ...
        gen.startElementNS((Nic, "udoc"), Nic, {})
        gen.endElementNS((Nic, "udoc"), Nic)
    ...
    def test_1463026_1(self):
        ...
        gen.startDocument()
        gen.startElementNS((Nic, 'a'), 'a', {(Nic, 'b'):'c'})
        gen.endElementNS((Nic, 'a'), 'a')
        gen.endDocument()
    ...
    def test_1463026_2(self):
        ...
        gen.startDocument()
        gen.startPrefixMapping(Nic, 'qux')
        gen.startElementNS(('qux', 'a'), 'a', {})
        gen.endElementNS(('qux', 'a'), 'a')
        gen.endPrefixMapping(Nic)
        gen.endDocument()
    ...
    def test_1463026_3(self):
        ...
        gen.startElementNS(('qux', 'a'), 'a', {(Nic, 'b'):'c'})
        gen.endElementNS(('qux', 'a'), 'a')
    ...
    def test_5027_1(self):
        # The xml prefix (as w xml:lang below) jest reserved oraz bound by
        # definition to http://www.w3.org/XML/1998/namespace.  XMLGenerator had
        # a bug whereby a KeyError jest podnieśd because this namespace jest missing
        # z a dictionary.
        #
        ...
        parser = make_parser()
        parser.setFeature(feature_namespaces, Prawda)
        result = self.ioclass()
        gen = XMLGenerator(result)
    ...
    def test_no_close_file(self):
        ...
        def func(out):
            ...
            gen.startElement("doc", {})
        func(result)
        self.assertNieprawda(result.closed)
...
klasa StringXmlgenTest(XmlgenTest, unittest.TestCase):
    ioclass = StringIO

    def xml(self, doc, encoding='iso-8859-1'):
        zwróć '<?xml version="1.0" encoding="%s"?>\n%s' % (encoding, doc)

    test_xmlgen_unencodable = Nic

klasa BytesXmlgenTest(XmlgenTest, unittest.TestCase):
    ioclass = BytesIO
...
klasa WriterXmlgenTest(BytesXmlgenTest):
    klasa ioclass(list):
        write = list.append
        closed = Nieprawda
        ...
        def tell(self):
            # zwróć 0 at start oraz nie 0 after start
            zwróć len(self)

        def getvalue(self):
            zwróć b''.join(self)

klasa StreamWriterXmlgenTest(XmlgenTest, unittest.TestCase):
    def ioclass(self):
        raw = BytesIO()
...
klasa StreamReaderWriterXmlgenTest(XmlgenTest, unittest.TestCase):
    ...
    def ioclass(self):
        ...
        def getvalue():
            # Windows will nie let use reopen without first closing
            writer.close()
            przy open(writer.name, 'rb') jako f:
                zwróć f.read()
...
klasa XMLFilterBaseTest(unittest.TestCase):
...
przy open(TEST_XMLFILE_OUT, 'rb') jako f:
    xml_test_out = f.read()

klasa ExpatReaderTest(XmlTestBase):
    ...
    def test_expat_binary_file(self):
        ...
        przy open(TEST_XMLFILE, 'rb') jako f:
            parser.parse(f)
    ...
    def test_expat_text_file(self):
        ...
        parser.setContentHandler(xmlgen)
        przy open(TEST_XMLFILE, 'rt', encoding='iso-8859-1') jako f:
            parser.parse(f)
    ...
    def test_expat_binary_file_int_name(self):
        ...
        parser.setContentHandler(xmlgen)
        przy open(TEST_XMLFILE, 'rb') jako f:
            przy open(f.fileno(), 'rb', closefd=Nieprawda) jako f2:
                parser.parse(f2)
    ...
    def test_expat_dtdhandler(self):
        ...
        self.assertEqual(handler._notations,
            [("GIF", "-//CompuServe//NOTATION Graphics Interchange Format 89a//EN", Nic)])
        self.assertEqual(handler._entities, [("img", Nic, "expat.gif", "GIF")])

    # ===== EntityResolver support

    klasa TestEntityResolver:
    ...
    klasa AttrGatherer(ContentHandler):
    ...
    def test_expat_nsattrs_wattr(self):
        ...
        self.assertPrawda((attrs.getQNames() == [] albo
    ...
    def test_expat_locator_noinfo(self):
        ...
        self.assertEqual(parser.getSystemId(), Nic)
        self.assertEqual(parser.getPublicId(), Nic)
...
klasa ErrorReportingTest(unittest.TestCase):
    ...
    def test_sax_parse_exception_str(self):
        # dalej various values z a locator to the SAXParseException to
        # make sure that the __str__() doesn't fall apart when Nic jest
        # dalejed instead of an integer line oraz column number
        #
        # use "normal" values dla the locator:
        str(SAXParseException("message", Nic,
                              self.DummyLocator(1, 1)))
        # use Nic dla the line number:
        str(SAXParseException("message", Nic,
                              self.DummyLocator(Nic, 1)))
        # use Nic dla the column number:
        str(SAXParseException("message", Nic,
                              self.DummyLocator(1, Nic)))
        # use Nic dla both:
        str(SAXParseException("message", Nic,
                              self.DummyLocator(Nic, Nic)))
...
klasa XmlReaderTest(XmlTestBase):
