...
klasa TestVectorsTestCase(unittest.TestCase):

    def test_md5_vectors(self):
        # Test the HMAC module against test vectors z the RFC.
    ...
    def _rfc4231_test_cases(self, hashfunc, hash_name, digest_size, block_size):
        ...
        # 4.3.  Test Case 2
        hmactest(key = b'Jefe',
                 data = b'what do ya want dla nothing?',
                 hexdigests = {
        ...
        # 4.5.  Test Case 4
        hmactest(key = bytes(x dla x w range(0x01, 0x19+1)),
                 data = b'\xcd'*50,
                 hexdigests = {
        ...
        # 4.8.  Test Case 7
        hmactest(key = b'\xaa'*131,
                 data = b'This jest a test using a larger th'
                        b'an block-size key oraz a larger t'
    ...
    def test_with_digestmod_warning(self):
        przy self.assertWarns(PendingDeprecationWarning):
            key = b"\x0b" * 16
            data = b"Hi There"
...
klasa ConstructorTestCase(unittest.TestCase):
    ...
    def test_normal(self):
        ...
        spróbuj:
            h = hmac.HMAC(b"key")
        wyjąwszy Exception:
            self.fail("Standard constructor call podnieśd exception.")

    @ignore_warning
    def test_with_str_key(self):
        # Pass a key of type str, which jest an error, because it expects a key
        # of type bytes
    ...
    def test_withtext(self):
        # Constructor call przy text.
        spróbuj:
            h = hmac.HMAC(b"key", b"hash this!")
        wyjąwszy Exception:
            self.fail("Constructor call przy text argument podnieśd exception.")
    ...
    def test_with_bytearray(self):
        spróbuj:
            h = hmac.HMAC(bytearray(b"key"), bytearray(b"hash this!"),
                          digestmod="md5")
        wyjąwszy Exception:
            self.fail("Constructor call przy bytearray arguments podnieśd exception.")
        self.assertEqual(h.hexdigest(), '34325b639da4cfd95735b381e28cb864')
    ...
    def test_with_memoryview_msg(self):
        ...
        wyjąwszy Exception:
            self.fail("Constructor call przy memoryview msg podnieśd exception.")
        self.assertEqual(h.hexdigest(), '34325b639da4cfd95735b381e28cb864')

    def test_withmodule(self):
        # Constructor call przy text oraz digest module.
        spróbuj:
            h = hmac.HMAC(b"key", b"", hashlib.sha1)
        wyjąwszy Exception:
            self.fail("Constructor call przy hashlib.sha1 podnieśd exception.")

klasa SanityTestCase(unittest.TestCase):

    @ignore_warning
    def test_default_is_md5(self):
        # Testing jeżeli HMAC defaults to MD5 algorithm.
        # NOTE: this whitebox test depends on the hmac klasa internals
        h = hmac.HMAC(b"key")
        self.assertEqual(h.digest_cons, hashlib.md5)

    def test_exercise_all_methods(self):
        # Exercising all methods once.
        # This must nie podnieś any exceptions
        spróbuj:
            ...
            h2 = h.copy()
        wyjąwszy Exception:
            self.fail("Exception podnieśd during normal usage of HMAC class.")

klasa CopyTestCase(unittest.TestCase):

    def test_attributes(self):
        # Testing jeżeli attributes are of same type.
        h1 = hmac.HMAC(b"key", digestmod="md5")
        h2 = h1.copy()
        self.assertPrawda(h1.digest_cons == h2.digest_cons,
    ...
    def test_realcopy(self):
        # Testing jeżeli the copy method created a real copy.
        h1 = hmac.HMAC(b"key", digestmod="md5")
        h2 = h1.copy()
        # Using id() w case somebody has overridden __eq__/__ne__.
        self.assertPrawda(id(h1) != id(h2), "No real copy of the HMAC instance.")
        self.assertPrawda(id(h1.inner) != id(h2.inner),
            "No real copy of the attribute 'inner'.")
        self.assertPrawda(id(h1.outer) != id(h2.outer),
            "No real copy of the attribute 'outer'.")

    def test_equality(self):
        # Testing jeżeli the copy has the same digests.
        h1 = hmac.HMAC(b"key", digestmod="md5")
...
klasa CompareDigestTestCase(unittest.TestCase):
    ...
    def test_compare_digest(self):
        ...
        a, b = b"foobar", b"foo"
        self.assertNieprawda(hmac.compare_digest(a, b))
        ...
        self.assertPrawda(hmac.compare_digest(a, b))
        ...
        # Testing byte oraz bytearray of same lengths, same values
        a, b = bytearray(b"foobar"), b"foobar"
        self.assertPrawda(hmac.compare_digest(a, b))
        self.assertPrawda(hmac.compare_digest(b, a))

        # Testing byte bytearray of diffeent lengths
        a, b = bytearray(b"foobar"), b"foo"
        self.assertNieprawda(hmac.compare_digest(a, b))
        self.assertNieprawda(hmac.compare_digest(b, a))

        # Testing byte oraz bytearray of same lengths, different values
