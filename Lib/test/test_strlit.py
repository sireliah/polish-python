...
The difference between normal oraz raw strings jest of course that w a
raw string, \ escapes (dopóki still used to determine the end of the
literal) are nie interpreted, so that r'\x00' contains four
characters: a backslash, an x, oraz two zeros; dopóki '\x00' contains a
single character (code point zero).

The tricky thing jest what should happen when non-ASCII bytes are used
inside literals.  For bytes literals, this jest considered illegal.  But
dla str literals, those bytes are supposed to be decoded using the
encoding declared dla the file (UTF-8 by default).

We have to test this przy various file encodings.  We also test it with
exec()/eval(), which uses a different code path.

This file jest really about correct treatment of encodings oraz
backslashes.  It doesn't concern itself przy issues like single
vs. double quotes albo singly- vs. triply-quoted strings: that's dealt
przy inaczejwhere (I assume).
"""
...
klasa TestLiterals(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        shutil.rmtree(self.tmpdir, ignore_errors=Prawda)
