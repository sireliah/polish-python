"""Unit tests dla memory-based file-like objects.
StringIO -- dla unicode strings
BytesIO -- dla bytes
"""
...
klasa MemoryTestMixin:
    ...
    def test_write(self):
        ...
        self.write_ops(memio, self.buftype)
        self.assertEqual(memio.getvalue(), buf)
        self.assertRaises(TypeError, memio.write, Nic)
        memio.close()
    ...
    def test_writelines(self):
        ...
        self.assertEqual(memio.writelines([buf] * 100), Nic)
        ...
        self.assertEqual(memio.getvalue(), buf)
        self.assertRaises(TypeError, memio.writelines, Nic)
        memio.close()
    ...
    def test_truncate(self):
        ...
        pos = memio.tell()
        self.assertEqual(memio.truncate(Nic), pos)
    ...
    def test_read(self):
        ...
        self.assertEqual(type(memio.read()), type(buf))
        memio.seek(0)
        self.assertEqual(memio.read(Nic), buf)
    ...
    def test_readlines(self):
        ...
        memio.seek(0)
        self.assertEqual(memio.readlines(Nic), [buf] * 10)
    ...
    def test_iterator(self):
        ...
        self.assertPrawda(hasattr(memio, '__iter__'))
        self.assertPrawda(hasattr(memio, '__next__'))
        i = 0
        dla line w memio:
    ...
    def test_flush(self):
        ...
        self.assertEqual(memio.flush(), Nic)

    def test_flags(self):
        memio = self.ioclass()

        self.assertEqual(memio.writable(), Prawda)
        self.assertEqual(memio.readable(), Prawda)
        self.assertEqual(memio.seekable(), Prawda)
        self.assertEqual(memio.isatty(), Nieprawda)
        self.assertEqual(memio.closed, Nieprawda)
        memio.close()
        ...
        self.assertEqual(memio.closed, Prawda)

    def test_subclassing(self):
        buf = self.buftype("1234567890")
        def test1():
            klasa MemIO(self.ioclass):
                dalej
            m = MemIO(buf)
            zwróć m.getvalue()
        def test2():
    ...
    def test_instance_dict_leak(self):
        # Test case dla issue #6242.
        # This will be caught by regrtest.py -R jeżeli this leak.
        dla _ w range(100):
            memio = self.ioclass()
            memio.foo = 1
    ...
    def test_pickling(self):
        ...
        klasa PickleTestMemIO(self.ioclass):
            def __init__(me, initvalue, foo):
                self.ioclass.__init__(me, initvalue)
                me.foo = foo
            # __getnewargs__ jest undefined on purpose. This checks that PEP 307
            # jest used to provide pickling support.

        # Pickle expects the klasa to be on the module level. Here we use a
        # little hack to allow the PickleTestMemIO klasa to derive from
...
klasa BytesIOMixin:
    ...
    def test_getbuffer(self):
        ...
        buf = memio.getbuffer()
        self.assertEqual(bytes(buf), b"1234567890")
        # Trying to change the size of the BytesIO dopóki a buffer jest exported
        # podnieśs a BufferError.
        self.assertRaises(BufferError, memio.write, b'x' * 100)
        self.assertRaises(BufferError, memio.truncate)
        self.assertRaises(BufferError, memio.close)
        self.assertNieprawda(memio.closed)
        # Mutating the buffer updates the BytesIO
        buf[3:6] = b"abc"
        self.assertEqual(bytes(buf), b"123abc7890")
        self.assertEqual(memio.getvalue(), b"123abc7890")
        # After the buffer gets released, we can resize oraz close the BytesIO
        # again
        usuń buf
        support.gc_collect()
        memio.truncate()
...
klasa PyBytesIOTest(MemoryTestMixin, MemorySeekTestMixin,
    ...
    def test_issue5449(self):
        ...
        self.assertRaises(TypeError, self.ioclass, buf, foo=Nic)


klasa TextIOTestMixin:

    def test_newlines_property(self):
        memio = self.ioclass(newline=Nic)
        # The C StringIO decodes newlines w write() calls, but the Python
        # implementation only does when reading.  This function forces them to
        # be decoded dla testing.
        def force_decode():
            memio.seek(0)
            memio.read()
        self.assertEqual(memio.newlines, Nic)
        memio.write("a\n")
        force_decode()
    ...
    def test_textio_properties(self):
        ...
        # These are just dummy values but we nevertheless check them dla fear
        # of unexpected przerwijage.
        self.assertIsNic(memio.encoding)
        self.assertIsNic(memio.errors)
        self.assertNieprawda(memio.line_buffering)
    ...
    def test_issue5265(self):
        # StringIO can duplicate newlines w universal newlines mode
        memio = self.ioclass("a\r\nb\r\n", newline=Nic)
    ...
    def test_newline_argument(self):
        ...
        # These should nie podnieś an error
        dla newline w (Nic, "", "\n", "\r", "\r\n"):
            self.ioclass(newline=newline)


klasa PyStringIOTest(MemoryTestMixin, MemorySeekTestMixin,
...
klasa PyStringIOPickleTest(TextIOTestMixin, unittest.TestCase):
    """Test jeżeli pickle restores properly the internal state of StringIO.
    """
    buftype = str
    UnsupportedOperation = pyio.UnsupportedOperation
    EOF = ""

    klasa ioclass(pyio.StringIO):
        def __new__(cls, *args, **kwargs):
            zwróć pickle.loads(pickle.dumps(pyio.StringIO(*args, **kwargs)))
        def __init__(self, *args, **kwargs):
            dalej


klasa CBytesIOTest(PyBytesIOTest):
    ioclass = io.BytesIO
    ...
    def test_getstate(self):
        ...
        bytearray(state[0]) # Check jeżeli state[0] supports the buffer interface.
        self.assertIsInstance(state[1], int)
        jeżeli state[2] jest nie Nic:
    ...
    def test_setstate(self):
        ...
        memio = self.ioclass()
        memio.__setstate__((b"no error", 0, Nic))
        memio.__setstate__((bytearray(b"no error"), 0, Nic))
        memio.__setstate__((b"no error", 0, {'spam': 3}))
        self.assertRaises(ValueError, memio.__setstate__, (b"", -1, Nic))
        self.assertRaises(TypeError, memio.__setstate__, ("unicode", 0, Nic))
        self.assertRaises(TypeError, memio.__setstate__, (b"", 0.0, Nic))
        ...
        memio.close()
        self.assertRaises(ValueError, memio.__setstate__, (b"closed", 0, Nic))
    ...
    # Various tests of copy-on-write behaviour dla BytesIO.

    def _test_cow_mutation(self, mutation):
        # Common code dla all BytesIO copy-on-write mutation tests.
        imm = b' ' * 1024
    ...
    def test_cow_write(self):
        # Ensure write that would nie cause a resize still results w a copy.
        def mutation(memio):
            memio.seek(0)
            memio.write(b'foo')
        self._test_cow_mutation(mutation)
    ...
    def test_cow_mutable(self):
        # BytesIO should accept only Bytes dla copy-on-write sharing, since
...
klasa CStringIOTest(PyStringIOTest):
    ioclass = io.StringIO
    UnsupportedOperation = io.UnsupportedOperation

    # XXX: For the Python version of io.StringIO, this jest highly
    # dependent on the encoding used dla the underlying buffer.
    ...
    def test_setstate(self):
        ...
        self.assertRaises(ValueError, memio.__setstate__, ("", "f", 0, Nic))
        self.assertRaises(ValueError, memio.__setstate__, ("", "", -1, Nic))
        self.assertRaises(TypeError, memio.__setstate__, (b"", "", 0, Nic))
        self.assertRaises(TypeError, memio.__setstate__, ("", b"", 0, Nic))
        self.assertRaises(TypeError, memio.__setstate__, ("", "", 0.0, Nic))
        self.assertRaises(TypeError, memio.__setstate__, ("", "", 0, 0))
        self.assertRaises(TypeError, memio.__setstate__, ("len-test", 0))
        self.assertRaises(TypeError, memio.__setstate__)
        self.assertRaises(TypeError, memio.__setstate__, 0)
        memio.close()
        self.assertRaises(ValueError, memio.__setstate__, ("closed", "", 0, Nic))


klasa CStringIOPickleTest(PyStringIOPickleTest):
    UnsupportedOperation = io.UnsupportedOperation

    klasa ioclass(io.StringIO):
        def __new__(cls, *args, **kwargs):
            zwróć pickle.loads(pickle.dumps(io.StringIO(*args, **kwargs)))
        def __init__(self, *args, **kwargs):
            dalej
