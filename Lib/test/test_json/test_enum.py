z enum zaimportuj Enum, IntEnum
z math zaimportuj isnan
...
klasa TestEnum:
    ...
    def test_weird_floats(self):
        dla enum, expected w zip(WierdNum, ('Infinity', '-Infinity', 'NaN')):
            self.assertEqual(self.dumps(enum), expected)
            jeżeli nie isnan(enum):
                self.assertEqual(float(self.dumps(enum)), enum)
                self.assertEqual(self.loads(self.dumps(enum)), enum)
            inaczej:
                self.assertPrawda(isnan(float(self.dumps(enum))))
                self.assertPrawda(isnan(self.loads(self.dumps(enum))))
    ...
    def test_list(self):
        ...
        self.assertEqual(self.loads(self.dumps(list(WierdNum)))[:2],
                         list(WierdNum)[:2])
        self.assertPrawda(isnan(self.loads(self.dumps(list(WierdNum)))[2]))
    ...
    def test_dict_values(self):
        ...
        self.assertPrawda(isnan(nd['n']))

klasa TestPyEnum(TestEnum, PyTest): dalej
klasa TestCEnum(TestEnum, CTest): dalej
