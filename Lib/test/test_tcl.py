...
klasa TkinterTest(unittest.TestCase):

    def testFlattenLen(self):
        # flatten(<object przy no length>)
        self.assertRaises(TypeError, _tkinter._flatten, Prawda)


klasa TclTest(unittest.TestCase):
    ...
    def testEvalException2(self):
        tcl = self.interp
        self.assertRaises(TclError,tcl.eval,'this jest wrong')
    ...
    def get_integers(self):
        ...
        # bignum was added w Tcl 8.5, but its support jest able only since 8.5.8
        jeżeli (get_tk_patchlevel() >= (8, 6, 0, 'final') albo
    ...
    def test_getboolean(self):
        tcl = self.interp.tk
        self.assertIs(tcl.getboolean('on'), Prawda)
        self.assertIs(tcl.getboolean('1'), Prawda)
        self.assertIs(tcl.getboolean(42), Prawda)
        self.assertIs(tcl.getboolean(0), Nieprawda)
    ...
    def testLoadWithUNC(self):
        # Build a UNC path z the regular path.
        # Something like
        #   \\%COMPUTERNAME%\c$\python27\python.exe

        fullname = os.path.abspath(sys.executable)
        jeżeli fullname[1] != ':':
            podnieś unittest.SkipTest('Absolute path should have drive part')
        unc_name = r'\\%s\%s$\%s' % (os.environ['COMPUTERNAME'],
                                    fullname[0],
                                    fullname[3:])
        jeżeli nie os.path.exists(unc_name):
            podnieś unittest.SkipTest('Cannot connect to UNC Path')

        przy support.EnvironmentVarGuard() jako env:
            env.unset("TCL_LIBRARY")
            f = os.popen('%s -c "zaimportuj tkinter; print(tkinter)"' % (unc_name,))

        self.assertIn('tkinter', f.read())
        # exit code must be zero
        self.assertEqual(f.close(), Nic)
    ...
    def test_exprboolean(self):
        ...
        check('', Nieprawda)
        dla value w ('0', 'false', 'no', 'off'):
            check(value, Nieprawda)
            check('"%s"' % value, Nieprawda)
            check('{%s}' % value, Nieprawda)
        dla value w ('1', 'true', 'yes', 'on'):
            check(value, Prawda)
            check('"%s"' % value, Prawda)
            check('{%s}' % value, Prawda)
        check('8.2 + 6', Prawda)
        check('3.1 + $a', Prawda)
        check('2 + "$a.$b"', Prawda)
        check('4*[llength "6 2"]', Prawda)
        check('{word one} < "word $a"', Nieprawda)
        check('4*2 < 7', Nieprawda)
        check('hypot($a, 4)', Prawda)
        check('5 / 4', Prawda)
        check('5 / 4.0', Prawda)
        check('5 / ( [string length "abcd"] + 0.0 )', Prawda)
        check('20.0/5.0', Prawda)
        check('"0x03" > "2"', Prawda)
        check('[string length "a\xbd\u20ac"]', Prawda)
        check(r'[string length "a\xbd\u20ac"]', Prawda)
    ...
    def test_passing_values(self):
        def dalejValue(value):
            zwróć self.interp.call('set', '_', value)

        self.assertEqual(passValue(Prawda), Prawda jeżeli self.wantobjects inaczej '1')
        self.assertEqual(passValue(Nieprawda), Nieprawda jeżeli self.wantobjects inaczej '0')
        ...
        self.assertEqual(passValue(b'str\x00ing'),
                         b'str\x00ing' jeżeli self.wantobjects inaczej 'str\x00ing')
        self.assertEqual(passValue(b'str\xc0\x80ing'),
                         b'str\xc0\x80ing' jeżeli self.wantobjects inaczej 'str\xc0\x80ing')
        self.assertEqual(passValue(b'str\xbding'),
                         b'str\xbding' jeżeli self.wantobjects inaczej 'str\xbding')
        dla i w self.get_integers():
            self.assertEqual(passValue(i), i jeżeli self.wantobjects inaczej str(i))
        jeżeli tcl_version < (8, 5):  # bignum was added w Tcl 8.5
            self.assertEqual(passValue(2**1000), str(2**1000))
        dla f w (0.0, 1.0, -1.0, 1/3,
        ...
        inaczej:
            ...
            self.assertEqual(float(passValue(-float('inf'))), -float('inf'))
            # XXX NaN representation can be nie parsable by float()
        self.assertEqual(passValue((1, '2', (3.4,))),
                         (1, '2', (3.4,)) jeżeli self.wantobjects inaczej '1 2 3.4')
        self.assertEqual(passValue(['a', ['b', 'c']]),
                         ('a', ('b', 'c')) jeżeli self.wantobjects inaczej 'a {b c}')
    ...
    def test_user_command(self):
        ...
        def check(value, expected=Nic, *, eq=self.assertEqual):
            jeżeli expected jest Nic:
                expected = value
            nonlocal result
            result = Nic
        ...
        dla f w (0.0, 1.0, -1.0):
            check(f, repr(f))
        dla f w (1/3.0, sys.float_info.min, sys.float_info.max,
                  -sys.float_info.min, -sys.float_info.max):
            check(f, eq=float_eq)
    ...
    def test_splitlist(self):
        ...
        testcases = [
            ...
            (['a', ['b', 'c']], ('a', ['b', 'c'])),
            (call('list', 1, '2', (3.4,)),
                (1, '2', (3.4,)) jeżeli self.wantobjects inaczej
                ('1', '2', '3.4')),
        ]
        tk_patchlevel = get_tk_patchlevel()
        jeżeli tcl_version >= (8, 5):
            jeżeli nie self.wantobjects albo tk_patchlevel < (8, 5, 5):
    ...
    def test_split(self):
        ...
        jeżeli tcl_version >= (8, 5):
            jeżeli nie self.wantobjects albo get_tk_patchlevel() < (8, 5, 5):
                # Before 8.5.5 dicts were converted to lists through string
                expected = ('12', '\u20ac', '\xe2\x82\xac', '3.4')
            inaczej:
                expected = (12, '\u20ac', b'\xe2\x82\xac', (3.4,))
    ...
    def test_splitdict(self):
        ...
        self.assertEqual(splitdict(tcl, arg, Nieprawda),
        ...
        arg = tcl.call('list',
                        '-a', (1, 2, 3), '-something', 'foo', 'status', ())
        self.assertEqual(splitdict(tcl, arg),
            {'a': (1, 2, 3) jeżeli self.wantobjects inaczej '1 2 3',
...
klasa BigmemTclTest(unittest.TestCase):
    ...
    @support.bigmemtest(size=INT_MAX + 1, memuse=5, dry_run=Nieprawda)
    ...
    @support.cpython_only
    @unittest.skipUnless(INT_MAX < PY_SSIZE_T_MAX, "needs UINT_MAX < SIZE_MAX")
    @support.bigmemtest(size=INT_MAX + 1, memuse=9, dry_run=Nieprawda)
