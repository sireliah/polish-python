""" Test script dla the Unicode implementation.
...
def search_function(encoding):
    def decode1(input, errors="strict"):
        zwróć 42 # nie a tuple
    def encode1(input, errors="strict"):
        zwróć 42 # nie a tuple
    def encode2(input, errors="strict"):
        zwróć (42, 42) # no unicode
    def decode2(input, errors="strict"):
        zwróć (42, 42) # no unicode
    jeżeli encoding=="test.unicode1":
        zwróć (encode1, decode1, Nic, Nic)
    albo_inaczej encoding=="test.unicode2":
        zwróć (encode2, decode2, Nic, Nic)
    inaczej:
        zwróć Nic
...
def duplicate_string(text):
    ...
    new object przy a reference count of 1.

    This jest a best-effort: latin1 single letters oraz the empty
    string ('') are singletons oraz cannot be cloned.
    """
    zwróć text.encode().decode()

klasa UnicodeTest(string_tests.CommonTest,
    ...
    def checkequalnofix(self, result, object, methodname, *args):
        ...
        self.assertPrawda(type(realresult) jest type(result))

        # jeżeli the original jest returned make sure that
        # this doesn't happen przy subclasses
        jeżeli realresult jest object:
            klasa usub(str):
                def __repr__(self):
                    zwróć 'usub(%r)' % str.__repr__(self)
            object = usub(object)
            method = getattr(object, methodname)
            realresult = method(*args)
            self.assertEqual(realresult, result)
            self.assertPrawda(object jest nie realresult)
    ...
    def test_literals(self):
        ...
        # raw strings should nie have unicode escapes
    ...
    def test_maketrans_translate(self):
        # these work przy plain translate()
        self.checkequalnofix('bbbc', 'abababc', 'translate',
                             {ord('a'): Nic})
        self.checkequalnofix('iiic', 'abababc', 'translate',
                             {ord('a'): Nic, ord('b'): ord('i')})
        self.checkequalnofix('iiix', 'abababc', 'translate',
                             {ord('a'): Nic, ord('b'): ord('i'), ord('c'): 'x'})
        ...
        self.checkequalnofix('abababc', 'abababc', 'translate',
                             {'b': '<i>'})
        tbl = self.type2test.maketrans({'a': Nic, 'b': '<i>'})
        self.checkequalnofix('<i><i><i>c', 'abababc', 'translate', tbl)
        # test alternative way of calling maketrans()
        tbl = self.type2test.maketrans('abc', 'xyz', 'd')
        self.checkequalnofix('xyzzy', 'abdcdcbdddd', 'translate', tbl)

        # various tests switching z ASCII to latin1 albo the opposite;
        # same length, remove a letter, albo replace przy a longer string.
        self.assertEqual("[a]".translate(str.maketrans('a', 'X')),
                         "[X]")
        self.assertEqual("[a]".translate(str.maketrans({'a': 'X'})),
                         "[X]")
        self.assertEqual("[a]".translate(str.maketrans({'a': Nic})),
                         "[]")
        ...
        self.assertEqual("[\xe9]".translate(str.maketrans({'\xe9': 'a'})),
                         "[a]")
        self.assertEqual("[\xe9]".translate(str.maketrans({'\xe9': Nic})),
                         "[]")

        # invalid Unicode characters
        invalid_char = 0x10ffff+1
        dla before w "a\xe9\u20ac\U0010ffff":
    ...
    def test_split(self):
        ...
        # test mixed kinds
        dla left, right w ('ba', '\u0101\u0100', '\U00010301\U00010300'):
            left *= 9
            right *= 9
            dla delim w ('c', '\u0102', '\U00010302'):
    ...
    def test_join(self):
        ...
        klasa MyWrapper:
            ...
            def __str__(self): zwróć self.sval
    ...
    def test_replace(self):
        ...
        # method call forwarded z str implementation because of unicode argument
    ...
    def test_bytes_comparison(self):
        przy support.check_warnings():
            ...
            self.assertEqual('abc' == b'abc', Nieprawda)
            self.assertEqual('abc' != b'abc', Prawda)
            self.assertEqual('abc' == bytearray(b'abc'), Nieprawda)
            self.assertEqual('abc' != bytearray(b'abc'), Prawda)

    def test_comparison(self):
        # Comparisons:
        self.assertEqual('abc', 'abc')
        self.assertPrawda('abcd' > 'abc')
        self.assertPrawda('abc' < 'abcd')
        ...
        jeżeli 0:
            ...
            self.assertPrawda('\u0061' < '\ud800\udc02')

            # Non surrogate above surrogate value, fixup required
            def test_lecmp(s, s2):
                self.assertPrawda(s < s2)
        ...
        self.assertPrawda('\ud800\udc02' < '\ud84d\udc56')

    def test_islower(self):
        string_tests.MixinStrUnicodeUserStringTest.test_islower(self)
        self.checkequalnofix(Nieprawda, '\u1FFc', 'islower')
        self.assertNieprawda('\u2167'.islower())
        self.assertPrawda('\u2177'.islower())
        # non-BMP, uppercase
        self.assertNieprawda('\U00010401'.islower())
        self.assertNieprawda('\U00010427'.islower())
        # non-BMP, lowercase
        self.assertPrawda('\U00010429'.islower())
        self.assertPrawda('\U0001044E'.islower())
        # non-BMP, non-cased
        self.assertNieprawda('\U0001F40D'.islower())
        self.assertNieprawda('\U0001F46F'.islower())

    def test_isupper(self):
        string_tests.MixinStrUnicodeUserStringTest.test_isupper(self)
        jeżeli nie sys.platform.startswith('java'):
            self.checkequalnofix(Nieprawda, '\u1FFc', 'isupper')
        self.assertPrawda('\u2167'.isupper())
        self.assertNieprawda('\u2177'.isupper())
        # non-BMP, uppercase
        self.assertPrawda('\U00010401'.isupper())
        self.assertPrawda('\U00010427'.isupper())
        # non-BMP, lowercase
        self.assertNieprawda('\U00010429'.isupper())
        self.assertNieprawda('\U0001044E'.isupper())
        # non-BMP, non-cased
        self.assertNieprawda('\U0001F40D'.isupper())
        self.assertNieprawda('\U0001F46F'.isupper())

    def test_istitle(self):
        string_tests.MixinStrUnicodeUserStringTest.test_istitle(self)
        self.checkequalnofix(Prawda, '\u1FFc', 'istitle')
        self.checkequalnofix(Prawda, 'Greek \u1FFcitlecases ...', 'istitle')

        # non-BMP, uppercase + lowercase
        self.assertPrawda('\U00010401\U00010429'.istitle())
        self.assertPrawda('\U00010427\U0001044E'.istitle())
        # apparently there are no titlecased (Lt) non-BMP chars w Unicode 6
        dla ch w ['\U00010429', '\U0001044E', '\U0001F40D', '\U0001F46F']:
            self.assertNieprawda(ch.istitle(), '{!a} jest nie title'.format(ch))

    def test_isspace(self):
        string_tests.MixinStrUnicodeUserStringTest.test_isspace(self)
        self.checkequalnofix(Prawda, '\u2000', 'isspace')
        self.checkequalnofix(Prawda, '\u200a', 'isspace')
        self.checkequalnofix(Nieprawda, '\u2014', 'isspace')
        # apparently there are no non-BMP spaces chars w Unicode 6
        dla ch w ['\U00010401', '\U00010427', '\U00010429', '\U0001044E',
                   '\U0001F40D', '\U0001F46F']:
            self.assertNieprawda(ch.isspace(), '{!a} jest nie space.'.format(ch))

    def test_isalnum(self):
        string_tests.MixinStrUnicodeUserStringTest.test_isalnum(self)
        dla ch w ['\U00010401', '\U00010427', '\U00010429', '\U0001044E',
                   '\U0001D7F6', '\U00011066', '\U000104A0', '\U0001F107']:
            self.assertPrawda(ch.isalnum(), '{!a} jest alnum.'.format(ch))

    def test_isalpha(self):
        string_tests.MixinStrUnicodeUserStringTest.test_isalpha(self)
        self.checkequalnofix(Prawda, '\u1FFc', 'isalpha')
        # non-BMP, cased
        self.assertPrawda('\U00010401'.isalpha())
        self.assertPrawda('\U00010427'.isalpha())
        self.assertPrawda('\U00010429'.isalpha())
        self.assertPrawda('\U0001044E'.isalpha())
        # non-BMP, non-cased
        self.assertNieprawda('\U0001F40D'.isalpha())
        self.assertNieprawda('\U0001F46F'.isalpha())

    def test_isdecimal(self):
        self.checkequalnofix(Nieprawda, '', 'isdecimal')
        self.checkequalnofix(Nieprawda, 'a', 'isdecimal')
        self.checkequalnofix(Prawda, '0', 'isdecimal')
        self.checkequalnofix(Nieprawda, '\u2460', 'isdecimal') # CIRCLED DIGIT ONE
        self.checkequalnofix(Nieprawda, '\xbc', 'isdecimal') # VULGAR FRACTION ONE QUARTER
        self.checkequalnofix(Prawda, '\u0660', 'isdecimal') # ARABIC-INDIC DIGIT ZERO
        self.checkequalnofix(Prawda, '0123456789', 'isdecimal')
        self.checkequalnofix(Nieprawda, '0123456789a', 'isdecimal')

        self.checkraises(TypeError, 'abc', 'isdecimal', 42)

        dla ch w ['\U00010401', '\U00010427', '\U00010429', '\U0001044E',
                   '\U0001F40D', '\U0001F46F', '\U00011065', '\U0001F107']:
            self.assertNieprawda(ch.isdecimal(), '{!a} jest nie decimal.'.format(ch))
        dla ch w ['\U0001D7F6', '\U00011066', '\U000104A0']:
            self.assertPrawda(ch.isdecimal(), '{!a} jest decimal.'.format(ch))

    def test_isdigit(self):
        string_tests.MixinStrUnicodeUserStringTest.test_isdigit(self)
        self.checkequalnofix(Prawda, '\u2460', 'isdigit')
        self.checkequalnofix(Nieprawda, '\xbc', 'isdigit')
        self.checkequalnofix(Prawda, '\u0660', 'isdigit')

        dla ch w ['\U00010401', '\U00010427', '\U00010429', '\U0001044E',
                   '\U0001F40D', '\U0001F46F', '\U00011065']:
            self.assertNieprawda(ch.isdigit(), '{!a} jest nie a digit.'.format(ch))
        dla ch w ['\U0001D7F6', '\U00011066', '\U000104A0', '\U0001F107']:
            self.assertPrawda(ch.isdigit(), '{!a} jest a digit.'.format(ch))

    def test_isnumeric(self):
        self.checkequalnofix(Nieprawda, '', 'isnumeric')
        self.checkequalnofix(Nieprawda, 'a', 'isnumeric')
        self.checkequalnofix(Prawda, '0', 'isnumeric')
        self.checkequalnofix(Prawda, '\u2460', 'isnumeric')
        self.checkequalnofix(Prawda, '\xbc', 'isnumeric')
        self.checkequalnofix(Prawda, '\u0660', 'isnumeric')
        self.checkequalnofix(Prawda, '0123456789', 'isnumeric')
        self.checkequalnofix(Nieprawda, '0123456789a', 'isnumeric')

        self.assertRaises(TypeError, "abc".isnumeric, 42)

        dla ch w ['\U00010401', '\U00010427', '\U00010429', '\U0001044E',
                   '\U0001F40D', '\U0001F46F']:
            self.assertNieprawda(ch.isnumeric(), '{!a} jest nie numeric.'.format(ch))
        dla ch w ['\U00011065', '\U0001D7F6', '\U00011066',
                   '\U000104A0', '\U0001F107']:
            self.assertPrawda(ch.isnumeric(), '{!a} jest numeric.'.format(ch))

    def test_isidentifier(self):
        self.assertPrawda("a".isidentifier())
        self.assertPrawda("Z".isidentifier())
        self.assertPrawda("_".isidentifier())
        self.assertPrawda("b0".isidentifier())
        self.assertPrawda("bc".isidentifier())
        self.assertPrawda("b_".isidentifier())
        self.assertPrawda("µ".isidentifier())
        self.assertPrawda("𝔘𝔫𝔦𝔠𝔬𝔡𝔢".isidentifier())

        self.assertNieprawda(" ".isidentifier())
        self.assertNieprawda("[".isidentifier())
        self.assertNieprawda("©".isidentifier())
        self.assertNieprawda("0".isidentifier())

    def test_isprintable(self):
        self.assertPrawda("".isprintable())
        self.assertPrawda(" ".isprintable())
        self.assertPrawda("abcdefg".isprintable())
        self.assertNieprawda("abcdefg\n".isprintable())
        # some defined Unicode character
        self.assertPrawda("\u0374".isprintable())
        # undefined character
        self.assertNieprawda("\u0378".isprintable())
        # single surrogate character
        self.assertNieprawda("\ud800".isprintable())

        self.assertPrawda('\U0001F46F'.isprintable())
        self.assertNieprawda('\U000E0020'.isprintable())
    ...
    def test_surrogates(self):
        ...
        dla meth_name w ('islower', 'isupper', 'istitle'):
            meth = getattr(str, meth_name)
            dla s w ('\uD800', '\uDFFF', '\uD800\uD800', '\uDFFF\uDFFF'):
                self.assertNieprawda(meth(s), '%a.%s() jest Nieprawda' % (s, meth_name))

        dla meth_name w ('isalpha', 'isalnum', 'isdigit', 'isspace',
                          'isdecimal', 'isnumeric',
    ...
    def test_lower(self):
        ...
        # Special case dla GREEK CAPITAL LETTER SIGMA U+03A3
    ...
    def test_contains(self):
        ...
        self.assertIn('a', 'bdba')
        self.assertIn('a', ('a',1,Nic))
        self.assertIn('a', (1,Nic,'a'))
        self.assertIn('a', ('a',1,Nic))
        self.assertIn('a', (1,Nic,'a'))
        self.assertNotIn('a', ('x',1,'y'))
        self.assertNotIn('a', ('x',1,Nic))
        self.assertNotIn('abcd', 'abcxxxx')
        self.assertIn('ab', 'abcd')
        self.assertIn('ab', 'abc')
        self.assertIn('ab', (1,Nic,'ab'))
        self.assertIn('', 'abc')
        self.assertIn('', '')
        self.assertIn('', 'abc')
    ...
    def test_format(self):
        ...
        # examples z the PEP:
        zaimportuj datetime
        self.assertEqual("My name jest {0}".format('Fred'), "My name jest Fred")
        self.assertEqual("My name jest {0[name]}".format(dict(name='Fred')),
                         "My name jest Fred")
        self.assertEqual("My name jest {0} :-{{}}".format('Fred'),
                         "My name jest Fred :-{}")

        d = datetime.date(2007, 8, 18)
        self.assertEqual("The year jest {0.year}".format(d),
                         "The year jest 2007")

        # classes we'll use dla testing
        klasa C:
        ...
        # klasa przy __str__, but no __format__
        klasa E:
            def __init__(self, x):
                self.x = x
            def __str__(self):
        ...
        # klasa przy __repr__, but no __format__ albo __str__
        klasa F:
            def __init__(self, x):
                self.x = x
            def __repr__(self):
        ...
        # klasa przy __format__ that forwards to string, dla some format_spec's
        klasa G:
            def __init__(self, x):
                self.x = x
            def __str__(self):
                zwróć "string jest " + self.x
            def __format__(self, format_spec):
                jeżeli format_spec == 'd':
                    zwróć 'G(' + self.x + ')'
                zwróć object.__format__(self, format_spec)

        klasa I(datetime.date):
            def __format__(self, format_spec):
                zwróć self.strftime(format_spec)

        klasa J(int):
            def __format__(self, format_spec):
                zwróć int.__format__(self * 2, format_spec)
...
#        self.assertEqual('{ 0 }'.format('abc'), 'abc')
        ...
        # issue 12546: use \x00 jako a fill character
        ...
        # format specifiers dla user defined type
        self.assertEqual('{0:abc}'.format(C()), 'abc')

        # !r, !s oraz !a coercions
        ...
        self.assertEqual('{0!s}'.format(G('data')), 'string jest data')
        ...
        # test deriving z a builtin type oraz overriding __format__
        ...
        # issue 6089
        self.assertRaises(ValueError, "{0[0]x}".format, [Nic])
        self.assertRaises(ValueError, "{0[0](10)}".format, [Nic])
        ...
        # Alternate formatting jest nie supported
        self.assertRaises(ValueError, format, '', '#')
    ...
    def test_format_map(self):
        ...
        klasa InternalMapping:
            def __init__(self):
                self.mapping = {'a': 'hello'}
            def __getitem__(self, key):
                zwróć self.mapping[key]
    ...
    def test_format_auto_numbering(self):
        ...
        # can't mix oraz match numbering oraz auto-numbering
        self.assertRaises(ValueError, '{}{1}'.format, 1, 2)
        self.assertRaises(ValueError, '{1}{}'.format, 1, 2)
        self.assertRaises(ValueError, '{:{1}}'.format, 1, 2)
        self.assertRaises(ValueError, '{0:{}}'.format, 1, 2)

        # can mix oraz match auto-numbering oraz named
    ...
    def test_formatting(self):
        ...
        # formatting jobs delegated z the string implementation:
        ...
        self.assertRaisesRegex(TypeError, '%x format: an integer jest required, nie float', operator.mod, '%x', 3.14),
        self.assertRaisesRegex(TypeError, '%X format: an integer jest required, nie float', operator.mod, '%X', 2.11),
        self.assertRaisesRegex(TypeError, '%o format: an integer jest required, nie float', operator.mod, '%o', 1.79),
        self.assertRaisesRegex(TypeError, '%x format: an integer jest required, nie PseudoFloat', operator.mod, '%x', pi),
        self.assertRaises(TypeError, operator.mod, '%c', pi),

    def test_formatting_with_enum(self):
        # issue18780
        zaimportuj enum
        klasa Float(float, enum.Enum):
            PI = 3.1415926
        klasa Int(enum.IntEnum):
            IDES = 15
        klasa Str(str, enum.Enum):
            ABC = 'abc'
        # Testing Unicode formatting strings...
    ...
    def test_formatting_huge_precision_c_limits(self):
        z _testcapi zaimportuj INT_MAX
    ...
    def test_startswith_endswith_errors(self):
        dla meth w ('foo'.startswith, 'foo'.endswith):
    ...
    def test_format_float(self):
        # should nie format przy a comma, but always przy C locale
    ...
    def test_constructor(self):
        ...
        klasa UnicodeSubclass(str):
            dalej

        dla text w ('ascii', '\xe9', '\u20ac', '\U0010FFFF'):
        ...
        # unicode(obj) jest compatible to str():

        o = StringCompat('unicode(obj) jest compatible to str()')
        self.assertEqual(str(o), 'unicode(obj) jest compatible to str()')
        self.assertEqual(str(o), 'unicode(obj) jest compatible to str()')

        dla obj w (123, 123.45, 123):
        ...
        jeżeli nie sys.platform.startswith('java'):
            self.assertRaises(
                TypeError,
                str,
                'decoding unicode jest nie supported',
                'utf-8',
                'strict'
            )
    ...
    def test_constructor_keyword_args(self):
        ...
        # The object argument can be dalejed jako a keyword.
        ...
        self.assertEqual(str(b'foo', errors='strict'), 'foo')  # nie "b'foo'"
        self.assertEqual(str(object=b'foo', errors='strict'), 'foo')

    def test_constructor_defaults(self):
        """Check the constructor argument defaults."""
        # The object argument defaults to '' albo b''.
    ...
    def test_codecs_utf7(self):
        utfTests = [
            ...
            ('Item 3 jest \u00a31.', b'Item 3 jest +AKM-1.'), # RFC2152 example
            ('+', b'+-'),
            ('+-', b'+--'),
            ('+?', b'+-?'),
            ('\?', b'+AFw?'),
        ...
        # Unpaired surrogates are dalejed through
    ...
    def test_codecs_utf8(self):
        ...
        # * strict decoding testing dla all of the
        #   UTF8_ERROR cases w PyUnicode_DecodeUTF8
    ...
    def test_utf8_decode_valid_sequences(self):
        ...
        dla seq, res w sequences:
            self.assertEqual(seq.decode('utf-8'), res)


    def test_utf8_decode_invalid_sequences(self):
        # continuation bytes w a sequence of 2, 3, albo 4 bytes
        continuation_bytes = [bytes([x]) dla x w range(0x80, 0xC0)]
        # start bytes of a 2-byte sequence equivalent to code points < 0x7F
        invalid_2B_seq_start_bytes = [bytes([x]) dla x w range(0xC0, 0xC2)]
        # start bytes of a 4-byte sequence equivalent to code points > 0x10FFFF
        invalid_4B_seq_start_bytes = [bytes([x]) dla x w range(0xF5, 0xF8)]
        invalid_start_bytes = (
            continuation_bytes + invalid_2B_seq_start_bytes +
            invalid_4B_seq_start_bytes + [bytes([x]) dla x w range(0xF7, 0x100)]
        )

        dla byte w invalid_start_bytes:
            self.assertRaises(UnicodeDecodeError, byte.decode, 'utf-8')

        dla sb w invalid_2B_seq_start_bytes:
            dla cb w continuation_bytes:
                self.assertRaises(UnicodeDecodeError, (sb+cb).decode, 'utf-8')

        dla sb w invalid_4B_seq_start_bytes:
            dla cb1 w continuation_bytes[:3]:
                dla cb3 w continuation_bytes[:3]:
                    self.assertRaises(UnicodeDecodeError,
                                      (sb+cb1+b'\x80'+cb3).decode, 'utf-8')

        dla cb w [bytes([x]) dla x w range(0x80, 0xA0)]:
            self.assertRaises(UnicodeDecodeError,
                              (b'\xE0'+cb+b'\x80').decode, 'utf-8')
            self.assertRaises(UnicodeDecodeError,
                              (b'\xE0'+cb+b'\xBF').decode, 'utf-8')
        # surrogates
        dla cb w [bytes([x]) dla x w range(0xA0, 0xC0)]:
            self.assertRaises(UnicodeDecodeError,
                              (b'\xED'+cb+b'\x80').decode, 'utf-8')
            self.assertRaises(UnicodeDecodeError,
                              (b'\xED'+cb+b'\xBF').decode, 'utf-8')
        dla cb w [bytes([x]) dla x w range(0x80, 0x90)]:
            self.assertRaises(UnicodeDecodeError,
                              (b'\xF0'+cb+b'\x80\x80').decode, 'utf-8')
            self.assertRaises(UnicodeDecodeError,
                              (b'\xF0'+cb+b'\xBF\xBF').decode, 'utf-8')
        dla cb w [bytes([x]) dla x w range(0x90, 0xC0)]:
            self.assertRaises(UnicodeDecodeError,
    ...
    def test_issue8271(self):
        ...
        # only the start byte oraz the continuation byte(s) are now considered
        # invalid, instead of the number of bytes specified by the start byte.
        # See http://www.unicode.org/versions/Unicode5.2.0/ch03.pdf (page 95,
        # table 3-8, Row 2) dla more information about the algorithm used.
        FFFD = '\ufffd'
        sequences = [
            # invalid start bytes
            ...
            (b'\xc1\xc0', FFFD*2),
            (b'\xc0\xc1', FFFD*2),
            # przy start byte of a 2-byte sequence
            (b'\xc2', FFFD), # only the start byte
            (b'\xc2\xc2', FFFD*2), # 2 start bytes
            (b'\xc2\xc2\xc2', FFFD*3), # 3 start bytes
            (b'\xc2\x41', FFFD+'A'), # invalid continuation byte
            # przy start byte of a 3-byte sequence
            ...
            (b'\xe1\x80\xe1\x41', FFFD*2+'A'), # 1 valid oraz the other invalid
            (b'\xe1\x41\xe1\x80', FFFD+'A'+FFFD), # 1 invalid oraz the other valid
            # przy start byte of a 4-byte sequence
            ...
            (b'\xf1\x80\x41', FFFD+'A'), # 1 valid cb oraz 1 invalid
            (b'\xf1\x80\x41\x41', FFFD+'AA'), # 1 valid cb oraz 1 invalid
            (b'\xf1\x80\x80\x41', FFFD+'A'), # 2 valid cb oraz 1 invalid
            (b'\xf1\x41\x80', FFFD+'A'+FFFD), # 1 invalid cv oraz 1 valid
            (b'\xf1\x41\x80\x80', FFFD+'A'+FFFD*2), # 1 invalid cb oraz 2 invalid
            (b'\xf1\x41\x80\x41', FFFD+'A'+FFFD+'A'), # 2 invalid cb oraz 1 invalid
            (b'\xf1\x41\x41\x80', FFFD+'AA'+FFFD), # 1 valid cb oraz 1 invalid
            (b'\xf1\x41\xf1\x80', FFFD+'A'+FFFD),
            (b'\xf1\x41\x80\xf1', FFFD+'A'+FFFD*2),
            (b'\xf1\xf1\x80\x41', FFFD*2+'A'),
            (b'\xf1\x41\xf1\xf1', FFFD+'A'+FFFD*2),
            # przy invalid start byte of a 4-byte sequence (rfc2279)
            ...
            (b'\xf5\x80\x41', FFFD*2+'A'), #  1 valid cb oraz 1 invalid
            (b'\xf5\x80\x41\xf5', FFFD*2+'A'+FFFD),
            (b'\xf5\x41\x80\x80\x41', FFFD+'A'+FFFD*2+'A'),
            # przy invalid start byte of a 5-byte sequence (rfc2279)
            (b'\xf8', FFFD), # only the start byte
            (b'\xf8\xf8', FFFD*2), # 2 start bytes
            (b'\xf8\x80', FFFD*2), # only one continuation byte
            (b'\xf8\x80\x41', FFFD*2 + 'A'), # 1 valid cb oraz 1 invalid
            (b'\xf8\x80\x80\x80\x80', FFFD*5), # invalid 5 bytes seq przy 5 bytes
            # przy invalid start byte of a 6-byte sequence (rfc2279)
        ...
        dla n, (seq, res) w enumerate(sequences):
    ...
    def to_bytestring(self, seq):
        zwróć bytes(int(c, 16) dla c w seq.split())

    def assertCorrectUTF8Decoding(self, seq, res, err):
        """
        Check that an invalid UTF-8 sequence podnieśs an UnicodeDecodeError when
        'strict' jest used, returns res when 'replace' jest used, oraz that doesn't
        zwróć anything when 'ignore' jest used.
        """
        przy self.assertRaises(UnicodeDecodeError) jako cm:
            seq.decode('utf-8')
        exc = cm.exception
    ...
    def test_invalid_start_byte(self):
        """
        Test that an 'invalid start byte' error jest podnieśd when the first byte
        jest nie w the ASCII range albo jest nie a valid start byte of a 2-, 3-, albo
        4-bytes sequence. The invalid start byte jest replaced przy a single
        U+FFFD when errors='replace'.
        E.g. <80> jest a continuation byte oraz can appear only after a start byte.
        """
        FFFD = '\ufffd'
        dla byte w b'\x80\xA0\x9F\xBF\xC0\xC1\xF5\xFF':
            self.assertCorrectUTF8Decoding(bytes([byte]), '\ufffd',
                                           'invalid start byte')

    def test_unexpected_end_of_data(self):
        """
        Test that an 'unexpected end of data' error jest podnieśd when the string
        ends after a start byte of a 2-, 3-, albo 4-bytes sequence without having
        enough continuation bytes.  The incomplete sequence jest replaced przy a
        single U+FFFD when errors='replace'.
        E.g. w the sequence <F3 80 80>, F3 jest the start byte of a 4-bytes
        sequence, but it's followed by only 2 valid continuation bytes oraz the
        last continuation bytes jest missing.
        Note: the continuation bytes must be all valid, jeżeli one of them jest
        invalid another error will be podnieśd.
        """
        sequences = [
            'C2', 'DF',
    ...
    def test_invalid_cb_for_2bytes_seq(self):
        """
        Test that an 'invalid continuation byte' error jest podnieśd when the
        continuation byte of a 2-bytes sequence jest invalid.  The start byte
        jest replaced by a single U+FFFD oraz the second byte jest handled
        separately when errors='replace'.
        E.g. w the sequence <C2 41>, C2 jest the start byte of a 2-bytes
        sequence, but 41 jest nie a valid continuation byte because it's the
        ASCII letter 'A'.
        """
        FFFD = '\ufffd'
        FFFDx2 = FFFD * 2
    ...
    def test_invalid_cb_for_3bytes_seq(self):
        ...
        errors='replace', jeżeli the first continuation byte jest valid, the first
        two bytes (start byte + 1st cb) are replaced by a single U+FFFD oraz the
        third byte jest handled separately, otherwise only the start byte jest
        replaced przy a U+FFFD oraz the other continuation bytes are handled
        separately.
        E.g. w the sequence <E1 80 41>, E1 jest the start byte of a 3-bytes
        sequence, 80 jest a valid continuation byte, but 41 jest nie a valid cb
        because it's the ASCII letter 'A'.
        Note: when the start byte jest E0 albo ED, the valid ranges dla the first
        continuation byte are limited to A0..BF oraz 80..9F respectively.
        Python 2 used to consider all the bytes w range 80..BF valid when the
        start byte was ED.  This jest fixed w Python 3.
        """
        FFFD = '\ufffd'
        FFFDx2 = FFFD * 2
        sequences = [
    ...
    def test_invalid_cb_for_4bytes_seq(self):
        ...
        errors='replace',the start byte oraz all the following valid
        continuation bytes are replaced przy a single U+FFFD, oraz all the bytes
        starting z the first invalid continuation bytes (included) are
        handled separately.
        E.g. w the sequence <E1 80 41>, E1 jest the start byte of a 3-bytes
        sequence, 80 jest a valid continuation byte, but 41 jest nie a valid cb
        because it's the ASCII letter 'A'.
        Note: when the start byte jest E0 albo ED, the valid ranges dla the first
        continuation byte are limited to A0..BF oraz 80..9F respectively.
        However, when the start byte jest ED, Python 2 considers all the bytes
        w range 80..BF valid.  This jest fixed w Python 3.
        """
        FFFD = '\ufffd'
        FFFDx2 = FFFD * 2
        sequences = [
    ...
    def test_codecs_idna(self):
        # Test whether trailing dot jest preserved
    ...
    def test_codecs_errors(self):
        ...
        # Error handling (lone surrogate w PyUnicode_TransformDecimalToASCII())
    ...
    def test_codecs(self):
        ...
        # Roundtrip safety dla BMP (just the first 1024 chars)
        dla c w range(1024):
            u = chr(c)
            dla encoding w ('utf-7', 'utf-8', 'utf-16', 'utf-16-le',
        ...
        # Roundtrip safety dla BMP (just the first 256 chars)
        dla c w range(256):
            u = chr(c)
            dla encoding w ('latin-1',):
                self.assertEqual(str(u.encode(encoding),encoding), u)

        # Roundtrip safety dla BMP (just the first 128 chars)
        dla c w range(128):
            u = chr(c)
            dla encoding w ('ascii',):
                self.assertEqual(str(u.encode(encoding),encoding), u)

        # Roundtrip safety dla non-BMP (just a few chars)
        przy warnings.catch_warnings():
            # unicode-internal has been deprecated
            warnings.simplefilter("ignore", DeprecationWarning)

            u = '\U00010001\U00020002\U00030003\U00040004\U00050005'
            dla encoding w ('utf-8', 'utf-16', 'utf-16-le', 'utf-16-be',
                             'raw_unicode_escape',
                             'unicode_escape', 'unicode_internal'):
                self.assertEqual(str(u.encode(encoding),encoding), u)

        # UTF-8 must be roundtrip safe dla all code points
        # (wyjąwszy surrogates, which are forbidden).
        u = ''.join(map(chr, list(range(0, 0xd800)) +
                             list(range(0xe000, 0x110000))))
        dla encoding w ('utf-8',):
            self.assertEqual(str(u.encode(encoding),encoding), u)
    ...
    def test_ucs4(self):
        ...
        inaczej:
            self.fail("Should have podnieśd UnicodeDecodeError")
    ...
    def test_conversion(self):
        ...
        klasa StrSubclassToStr(str):
            def __str__(self):
                zwróć "foo"

        klasa StrSubclassToStrSubclass(str):
            def __new__(cls, content=""):
                zwróć str.__new__(cls, 2*content)
            def __str__(self):
                zwróć self
    ...
    # an int jako the max value, nie a 64-bit C long.  If expandtabs jest changed
    ...
    def test_raiseMemError(self):
        ...
        dla char w ('a', '\xe9', '\u20ac', '\U0010ffff'):
            ...
            inaczej:
                ...
                struct_size = compact_struct_size
            # Note: sys.maxsize jest half of the actual max allocation because of
            # the signedness of Py_ssize_t. Strings of maxlen-1 should w principle
    ...
    def test_from_format(self):
        ...
        def PyUnicode_FromFormat(format, *args):
            cargs = tuple(
                py_object(arg) jeżeli isinstance(arg, str) inaczej arg
                dla arg w args)
            zwróć _PyUnicode_FromFormat(format, *cargs)
        ...
        check_format('\u20acAB',
                     b'%.3V', '\u20acABCDEF', Nic)
        check_format('abc[\ufffd',
                     b'%.5V', Nic, 'abc[\u20ac]'.encode('utf8'))

        # following tests comes z #7330
        # test width modifier oraz precision modifier przy %S
        ...
        # test width modifier oraz precision modifier przy %R
        check_format("repr=   'abc'",
                     b'repr=%8R', 'abc')
        check_format("repr='ab",
                     b'repr=%.3R', 'abc')
        ...
        # test width modifier oraz precision modifier przy %A
        check_format("repr=   'abc'",
                     b'repr=%8A', 'abc')
        check_format("repr='ab",
                     b'repr=%.3A', 'abc')
        ...
        # test width modifier oraz precision modifier przy %U
        check_format("repr=  abc",
                     b'repr=%5U', 'abc')
        check_format("repr=ab",
                     b'repr=%.2U', 'abc')
        ...
        # test width modifier oraz precision modifier przy %V
        check_format("repr=  abc",
                     b'repr=%5V', 'abc', b'123')
        check_format("repr=ab",
                     b'repr=%.2V', 'abc', b'123')
        ...
        # Test string decode z parameter of %s using utf-8.
        # b'\xe4\xba\xba\xe6\xb0\x91' jest utf-8 encoded byte sequence of
        # '\u4eba\u6c11'
        check_format('repr=\u4eba\u6c11',
                     b'repr=%V', Nic, b'\xe4\xba\xba\xe6\xb0\x91')

        #Test replace error handler.
        check_format('repr=abc\ufffd',
                     b'repr=%V', Nic, b'abc\xff')

        # nie supported: copy the raw format string. these tests are just here
        # to check dla crashs oraz should nie be considered jako specifications
        check_format('%s',
                     b'%1%s', b'abc')
        check_format('%1abc',
                     b'%1abc')
    ...
    def test_aswidechar(self):
        z _testcapi zaimportuj unicode_aswidechar
        support.import_module('ctypes')
        z ctypes zaimportuj c_wchar, sizeof
        ...
        nonbmp = chr(0x10ffff)
        jeżeli sizeof(c_wchar) == 2:
            buflen = 3
            nchar = 2
        inaczej: # sizeof(c_wchar) == 4
            buflen = 2
            nchar = 1
    ...
    def test_aswidecharstring(self):
        z _testcapi zaimportuj unicode_aswidecharstring
        support.import_module('ctypes')
        z ctypes zaimportuj c_wchar, sizeof
    ...
    def test_encode_decimal(self):
        z _testcapi zaimportuj unicode_encodedecimal
    ...
    def test_transform_decimal(self):
        z _testcapi zaimportuj unicode_transformdecimaltoascii jako transform_decimal
    ...
    def test_resize(self):
        dla length w range(1, 100, 7):
            # generate a fresh string (refcount=1)
            text = 'a' * length + 'b'

            przy support.check_warnings(('unicode_internal codec has been '
                                         'deprecated', DeprecationWarning)):
                # fill wstr internal field
                abc = text.encode('unicode_internal')
                self.assertEqual(abc.decode('unicode_internal'), text)

                # resize text: wstr field must be cleared oraz then recomputed
                text += 'c'
    ...
    def test_compare(self):
        ...
        strings = (
            ...
            bmp, bmp2,
            astral, astral2)
        dla text1, text2 w itertools.combinations(strings, 2):
            equal = (text1 jest text2)
            self.assertEqual(text1 == text2, equal)
            self.assertEqual(text1 != text2, nie equal)

            jeżeli equal:
                self.assertPrawda(text1 <= text2)
                self.assertPrawda(text1 >= text2)

                # text1 jest text2: duplicate strings to skip the "str1 == str2"
                # optimization w unicode_compare_eq() oraz really compare
                # character per character
                copy1 = duplicate_string(text1)
                copy2 = duplicate_string(text2)
                self.assertIsNot(copy1, copy2)

                self.assertPrawda(copy1 == copy2)
                self.assertNieprawda(copy1 != copy2)

                self.assertPrawda(copy1 <= copy2)
                self.assertPrawda(copy2 >= copy2)

        self.assertPrawda(ascii < ascii2)
        self.assertPrawda(ascii < latin)
        self.assertPrawda(ascii < bmp)
        self.assertPrawda(ascii < astral)
        self.assertNieprawda(ascii >= ascii2)
        self.assertNieprawda(ascii >= latin)
        self.assertNieprawda(ascii >= bmp)
        self.assertNieprawda(ascii >= astral)

        self.assertNieprawda(latin < ascii)
        self.assertPrawda(latin < latin2)
        self.assertPrawda(latin < bmp)
        self.assertPrawda(latin < astral)
        self.assertPrawda(latin >= ascii)
        self.assertNieprawda(latin >= latin2)
        self.assertNieprawda(latin >= bmp)
        self.assertNieprawda(latin >= astral)

        self.assertNieprawda(bmp < ascii)
        self.assertNieprawda(bmp < latin)
        self.assertPrawda(bmp < bmp2)
        self.assertPrawda(bmp < astral)
        self.assertPrawda(bmp >= ascii)
        self.assertPrawda(bmp >= latin)
        self.assertNieprawda(bmp >= bmp2)
        self.assertNieprawda(bmp >= astral)

        self.assertNieprawda(astral < ascii)
        self.assertNieprawda(astral < latin)
        self.assertNieprawda(astral < bmp2)
        self.assertPrawda(astral < astral2)
        self.assertPrawda(astral >= ascii)
        self.assertPrawda(astral >= latin)
        self.assertPrawda(astral >= bmp2)
        self.assertNieprawda(astral >= astral2)


klasa StringModuleTest(unittest.TestCase):
    def test_formatter_parser(self):
        def parse(format):
            zwróć list(_string.formatter_parser(format))
    ...
    def test_formatter_field_name_split(self):
        ...
        self.assertEqual(split("obj.arg"), ["obj", [(Prawda, 'arg')]])
        self.assertEqual(split("obj[key]"), ["obj", [(Nieprawda, 'key')]])
