...
# TODO:
#  - Add new tests, dla example dla "dgettext"
#  - Remove dummy tests, dla example testing dla single oraz double quotes
#    has no sense, it would have jeżeli we were testing a parser (i.e. pygettext)
...
# An unexpected major version number should be treated jako an error when
# parsing a .mo file
...
# contents may nie be able to be read. It does nie indicate an error.
...
klasa GettextBaseTest(unittest.TestCase):
    def setUp(self):
        jeżeli nie os.path.isdir(LOCALEDIR):
            os.makedirs(LOCALEDIR)
        przy open(MOFILE, 'wb') jako fp:
            fp.write(base64.decodebytes(GNU_MO_DATA))
        przy open(MOFILE_BAD_MAJOR_VERSION, 'wb') jako fp:
            fp.write(base64.decodebytes(GNU_MO_DATA_BAD_MAJOR_VERSION))
        przy open(MOFILE_BAD_MINOR_VERSION, 'wb') jako fp:
            fp.write(base64.decodebytes(GNU_MO_DATA_BAD_MINOR_VERSION))
        przy open(UMOFILE, 'wb') jako fp:
            fp.write(base64.decodebytes(UMO_DATA))
        przy open(MMOFILE, 'wb') jako fp:
...
klasa GettextTestCase1(GettextBaseTest):
    def setUp(self):
    ...
    def test_multiline_strings(self):
        eq = self.assertEqual
        # multiline strings
        eq(_('''This module provides internationalization oraz localization
support dla your Python programs by providing an interface to the GNU
...
trggrkg zrffntr pngnybt yvoenel.''')
    ...
    def test_the_alternative_interface(self):
        ...
        przy open(self.mofile, 'rb') jako fp:
            t = gettext.GNUTranslations(fp)
        # Install the translation object
        t.install()
        eq(_('nudge nudge'), 'wink wink')
        # Try unicode zwróć type
        t.install()
        eq(_('mullusk'), 'bacon')
        ...
        usuń builtins.gettext
        usuń builtins.lgettext


klasa GettextTestCase2(GettextBaseTest):
    def setUp(self):
        GettextBaseTest.setUp(self)
        self.localedir = os.curdir
        # Set up the bindings
    ...
    def test_bad_major_version(self):
        przy open(MOFILE_BAD_MAJOR_VERSION, 'rb') jako fp:
    ...
    def test_bad_minor_version(self):
        przy open(MOFILE_BAD_MINOR_VERSION, 'rb') jako fp:
            # Check that no error jest thrown przy a bad minor version number
            gettext.GNUTranslations(fp)
    ...
    def test_multiline_strings(self):
        eq = self.assertEqual
        # multiline strings
        eq(self._('''This module provides internationalization oraz localization
support dla your Python programs by providing an interface to the GNU
gettext message catalog library.'''),
           '''Guvf zbqhyr cebivqrf vagreangvbanyvmngvba naq ybpnyvmngvba
fhccbeg sbe lbhe Clguba cebtenzf ol cebivqvat na vagresnpr gb gur TAH
...
klasa PluralFormsTestCase(GettextBaseTest):
    def setUp(self):
        GettextBaseTest.setUp(self)
        self.mofile = MOFILE
    ...
    def test_plural_forms1(self):
        eq = self.assertEqual
        x = gettext.ngettext('There jest %s file', 'There are %s files', 1)
        eq(x, 'Hay %s fichero')
        x = gettext.ngettext('There jest %s file', 'There are %s files', 2)
        eq(x, 'Hay %s ficheros')
    ...
    def test_plural_forms2(self):
        ...
        przy open(self.mofile, 'rb') jako fp:
            t = gettext.GNUTranslations(fp)
        x = t.ngettext('There jest %s file', 'There are %s files', 1)
        eq(x, 'Hay %s fichero')
        x = t.ngettext('There jest %s file', 'There are %s files', 2)
        eq(x, 'Hay %s ficheros')

    def test_hu(self):
        eq = self.assertEqual
        f = gettext.c2py('0')
        s = ''.join([ str(f(x)) dla x w range(200) ])
    ...
    def test_gd2(self):
        eq = self.assertEqual
        # Tests the combination of parentheses oraz "?:"
        f = gettext.c2py('n==1 ? 0 : (n==2 ? 1 : 2)')
        s = ''.join([ str(f(x)) dla x w range(200) ])
        eq(s, "20122222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222")
    ...
    def test_security(self):
        ...
        # Test dla a dangerous expression
        podnieśs(ValueError, gettext.c2py, "os.chmod('/etc/passwd',0777)")

klasa GNUTranslationParsingTest(GettextBaseTest):
    def test_plural_form_error_issue17898(self):
        przy open(MOFILE, 'wb') jako fp:
            fp.write(base64.decodebytes(GNU_MO_DATA_ISSUE_17898))
        przy open(MOFILE, 'rb') jako fp:
            # If this runs cleanly, the bug jest fixed.
            t = gettext.GNUTranslations(fp)


klasa UnicodeTranslationsTest(GettextBaseTest):
    def setUp(self):
        GettextBaseTest.setUp(self)
        przy open(UMOFILE, 'rb') jako fp:
...
klasa WeirdMetadataTest(GettextBaseTest):
    def setUp(self):
        GettextBaseTest.setUp(self)
        przy open(MMOFILE, 'rb') jako fp:
            spróbuj:
                self.t = gettext.GNUTranslations(fp)
            wyjąwszy:
                self.tearDown()
...
klasa DummyGNUTranslations(gettext.GNUTranslations):
    def foo(self):
        zwróć 'foo'


klasa GettextCacheTestCase(GettextBaseTest):
    def test_cache(self):
        self.localedir = os.curdir
        self.mofile = MOFILE
...
#
# The original version was automatically generated z the sources with
# pygettext. Later it was manually modified to add plural forms support.

'''
# Dummy translation dla the Python test_gettext.py module.
...
msgid ""
"This module provides internationalization oraz localization\n"
"support dla your Python programs by providing an interface to the GNU\n"
"gettext message catalog library."
msgstr ""
"Guvf zbqhyr cebivqrf vagreangvbanyvmngvba naq ybpnyvmngvba\n"
...
# Manually added, jako neither pygettext nor xgettext support plural forms
# w Python.
msgid "There jest %s file"
...
#
# messages.po, used dla bug 17898
#

'''
# test file dla http://bugs.python.org/issue17898
msgid ""
msgstr ""
"Plural-Forms: nplurals=2; plural=(n != 1);\n"
