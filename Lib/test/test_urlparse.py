...
# A list of test cases.  Each test case jest a two-tuple that contains
# a string przy the query oraz a dictionary przy the expected result.
...
klasa UrlParseTestCase(unittest.TestCase):
    ...
    def checkRoundtrips(self, url, parsed, split):
        ...
        # put it back together oraz it should be the same
        result2 = urllib.parse.urlunparse(result)
        self.assertEqual(result2, url)
        self.assertEqual(result2, result.geturl())

        # the result of geturl() jest a fixpoint; we can always parse it
        ...
        # check the roundtrip using urlsplit() jako well
    ...
    def test_qsl(self):
        dla orig, expect w parse_qsl_test_cases:
            result = urllib.parse.parse_qsl(orig, keep_blank_values=Prawda)
            self.assertEqual(result, expect, "Error parsing %r" % orig)
            expect_without_blanks = [v dla v w expect jeżeli len(v[1])]
            result = urllib.parse.parse_qsl(orig, keep_blank_values=Nieprawda)
    ...
    def test_roundtrips(self):
        ...
        def _encode(t):
            zwróć (t[0].encode('ascii'),
                    tuple(x.encode('ascii') dla x w t[1]),
                    tuple(x.encode('ascii') dla x w t[2]))
        bytes_cases = [_encode(x) dla x w str_cases]
        dla url, parsed, split w str_cases + bytes_cases:
            self.checkRoundtrips(url, parsed, split)

    def test_http_roundtrips(self):
        # urllib.parse.urlsplit treats 'http:' jako an optimized special case,
        # so we test both 'http:' oraz 'https:' w all the following.
        # Three cheers dla white box knowledge!
        str_cases = [
            ('://www.python.org',
             ('www.python.org', '', '', '', ''),
             ('www.python.org', '', '', '')),
        ...
        dla schemes, test_cases w (str_tests, bytes_tests):
            dla scheme w schemes:
                dla url, parsed, split w test_cases:
                    url = scheme + url
    ...
    def checkJoin(self, base, relurl, expected):
        ...
        bytes_components = baseb, relurlb, expectedb = [
                            x.encode('ascii') dla x w str_components]
        self.assertEqual(urllib.parse.urljoin(baseb, relurlb), expectedb)

    def test_unparse_parse(self):
        str_cases = ['Python', './Python','x-newscheme://foo.com/stuff','x://y','x:/y','x:/','/',]
        bytes_cases = [x.encode('ascii') dla x w str_cases]
        dla u w str_cases + bytes_cases:
            self.assertEqual(urllib.parse.urlunsplit(urllib.parse.urlsplit(u)), u)
            self.assertEqual(urllib.parse.urlunparse(urllib.parse.urlparse(u)), u)

    def test_RFC1808(self):
        # "normal" cases z RFC 1808:
        ...
        # RFC 1808 oraz RFC 1630 disagree on these (according to RFC 1808),
        # so we'll nie actually run these tests (which expect 1808 behavior).
        #self.checkJoin(RFC1808_BASE, 'http:g', 'http:g')
        #self.checkJoin(RFC1808_BASE, 'http:', 'http:')

        # XXX: The following tests are no longer compatible przy RFC3986
        # self.checkJoin(RFC1808_BASE, '../../../g', 'http://a/../g')
        # self.checkJoin(RFC1808_BASE, '../../../../g', 'http://a/../../g')
        # self.checkJoin(RFC1808_BASE, '/./g', 'http://a/./g')
        # self.checkJoin(RFC1808_BASE, '/../g', 'http://a/../g')
    ...
    def test_RFC2368(self):
        # Issue 11467: path that starts przy a number jest nie parsed correctly
    ...
    def test_RFC3986(self):
        ...
        # The 'abnormal scenarios' are incompatible przy RFC2986 parsing
        # Tests are here dla reference.
    ...
    def test_urljoins(self):
        ...
        # test dla issue22118 duplicate slashes
        self.checkJoin(SIMPLE_BASE + '/', 'foo', SIMPLE_BASE + '/foo')

        # Non-RFC-defined tests, covering variations of base oraz trailing
        # slashes
    ...
    def test_RFC2732(self):
        str_cases = [
            ...
            ('http://Test.python.org/foo/', 'test.python.org', Nic),
            ('http://12.34.56.78/foo/', '12.34.56.78', Nic),
            ('http://[::1]/foo/', '::1', Nic),
            ('http://[dead:beef::1]/foo/', 'dead:beef::1', Nic),
            ('http://[dead:beef::]/foo/', 'dead:beef::', Nic),
            ('http://[dead:beef:cafe:5417:affe:8FA3:deaf:feed]/foo/',
             'dead:beef:cafe:5417:affe:8fa3:deaf:feed', Nic),
            ('http://[::12.34.56.78]/foo/', '::12.34.56.78', Nic),
            ('http://[::ffff:12.34.56.78]/foo/',
             '::ffff:12.34.56.78', Nic),
            ('http://Test.python.org:/foo/', 'test.python.org', Nic),
            ('http://12.34.56.78:/foo/', '12.34.56.78', Nic),
            ('http://[::1]:/foo/', '::1', Nic),
            ('http://[dead:beef::1]:/foo/', 'dead:beef::1', Nic),
        ...
        def _encode(t):
            zwróć t[0].encode('ascii'), t[1].encode('ascii'), t[2]
        bytes_cases = [_encode(x) dla x w str_cases]
        dla url, hostname, port w str_cases + bytes_cases:
        ...
        bytes_cases = [x.encode('ascii') dla x w str_cases]
        dla invalid_url w str_cases + bytes_cases:
    ...
    def test_urldefrag(self):
        ...
        def _encode(t):
            zwróć type(t)(x.encode('ascii') dla x w t)
        bytes_cases = [_encode(x) dla x w str_cases]
        dla url, defrag, frag w str_cases + bytes_cases:
    ...
    def test_urlsplit_attributes(self):
        ...
        self.assertEqual(p.username, Nic)
        self.assertEqual(p.password, Nic)
        self.assertEqual(p.hostname, "www.python.org")
        self.assertEqual(p.port, Nic)
        # geturl() won't zwróć exactly the original URL w this case
        # since the scheme jest always case-normalized
        ...
        # "@" characters.  Though nie RFC compliant, many ftp sites allow
        # oraz request email addresses jako usernames.
        ...
        # And check them all again, only przy bytes this time
        ...
        # Verify an illegal port jest returned jako Nic
    ...
    def test_attributes_without_netloc(self):
        # This example jest straight z RFC 3261.  It looks like it
        # should allow the username, hostname, oraz port to be filled
        # in, but doesn't.  Since it's a URI oraz doesn't use the
        # scheme://netloc syntax, the netloc oraz related attributes
        ...
        self.assertEqual(p.username, Nic)
        self.assertEqual(p.password, Nic)
        self.assertEqual(p.hostname, Nic)
        self.assertEqual(p.port, Nic)
        ...
        # You guessed it, repeating the test przy bytes input
    ...
    def test_noslash(self):
        # Issue 1637: http://foo.com?query jest legal
    ...
    def test_withoutscheme(self):
        ...
        # Issue 754016: urlparse goes wrong przy IP:port without scheme
        # RFC 1808 specifies that netloc should start przy //, urlparse expects
        # the same, otherwise it classifies the portion of url jako path.
    ...
    def test_portseparator(self):
        # Issue 754016 makes changes dla port separator ':' z scheme separator
        ...
        self.assertEqual(urllib.parse.urlparse("http://www.python.org:80"),
                ('http','www.python.org:80','','','',''))
        # As usual, need to check bytes input jako well
    ...
    def test_usingsys(self):
        # Issue 3314: sys module jest used w the error
    ...
    def test_default_scheme(self):
        # Exercise the scheme parameter of urlparse() oraz urlsplit()
        dla func w (urllib.parse.urlparse, urllib.parse.urlsplit):
            przy self.subTest(function=func):
    ...
    def test_parse_fragments(self):
        # Exercise the allow_fragments parameter of urlparse() oraz urlsplit()
        tests = (
            ("http:#frag", "path"),
        ...
        dla url, attr w tests:
            dla func w (urllib.parse.urlparse, urllib.parse.urlsplit):
                jeżeli attr == "params" oraz func jest urllib.parse.urlsplit:
                    attr = "path"
                przy self.subTest(url=url, function=func):
                    result = func(url, allow_fragments=Nieprawda)
                    self.assertEqual(result.fragment, "")
                    self.assertPrawda(getattr(result, attr).endswith("#frag"))
                    self.assertEqual(func(url, "", Nieprawda).fragment, "")

                    result = func(url, allow_fragments=Prawda)
                    self.assertEqual(result.fragment, "frag")
                    self.assertNieprawda(getattr(result, attr).endswith("frag"))
                    self.assertEqual(func(url, "", Prawda).fragment, "frag")
                    self.assertEqual(func(url).fragment, "frag")

    def test_mixed_types_rejected(self):
        # Several functions that process either strings albo ASCII encoded bytes
        # accept multiple arguments. Check they reject mixed type input
        przy self.assertRaisesRegex(TypeError, "Cannot mix str"):
    ...
    def test_result_pairs(self):
        # Check encoding oraz decoding between result pairs
        result_types = [
          urllib.parse.DefragResult,
          urllib.parse.SplitResult,
          urllib.parse.ParseResult,
        ]
        dla result_type w result_types:
    ...
    def test_urlencode_sequences(self):
        ...
        # Sequence oraz object values.
        result = urllib.parse.urlencode({'a': [1, 2], 'b': (3, 4, 5)}, Prawda)
        ...
        result = urllib.parse.urlencode({'a': Trivial()}, Prawda)
    ...
    def test_issue14072(self):
        ...
        # assert the behavior dla urlparse
    ...
    def test_all(self):
        ...
        dla name w dir(urllib.parse):
            jeżeli name.startswith('_') albo name w undocumented:
                kontynuuj
            object = getattr(urllib.parse, name)
            jeżeli getattr(object, '__module__', Nic) == 'urllib.parse':
...
klasa Utility_Tests(unittest.TestCase):
    ...
    def test_splitpasswd(self):
        # Some of the dalejword examples are nie sensible, but it jest added to
        # confirming to RFC2617 oraz addressing issue4675.
        ...
        self.assertEqual(splitpasswd('user'), ('user', Nic))
        self.assertEqual(splitpasswd(':ab'), ('', 'ab'))

    def test_splitport(self):
        splitport = urllib.parse.splitport
        self.assertEqual(splitport('parrot:88'), ('parrot', '88'))
        self.assertEqual(splitport('parrot'), ('parrot', Nic))
        self.assertEqual(splitport('parrot:'), ('parrot', Nic))
        self.assertEqual(splitport('127.0.0.1'), ('127.0.0.1', Nic))
        self.assertEqual(splitport('parrot:cheese'), ('parrot:cheese', Nic))
        self.assertEqual(splitport('[::1]:88'), ('[::1]', '88'))
        self.assertEqual(splitport('[::1]'), ('[::1]', Nic))
    ...
    def test_splitnport(self):
        ...
        self.assertEqual(splitnport('parrot:cheese'), ('parrot', Nic))
        self.assertEqual(splitnport('parrot:cheese', 55), ('parrot', Nic))

    def test_splitquery(self):
        # Normal cases are exercised by other tests; ensure that we also
        # catch cases przy no port specified (testcase ensuring coverage)
    ...
    def test_splitvalue(self):
        ...
        # przy no key/value pairs. (testcase ensuring coverage)
        ...
        self.assertEqual(splitvalue('foobar'), ('foobar', Nic))
