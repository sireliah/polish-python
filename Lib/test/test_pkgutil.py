z test.support zaimportuj run_unittest, unload, check_warnings, CleanImport
zaimportuj unittest
zaimportuj sys
zaimportuj importlib
z importlib.util zaimportuj spec_from_file_location
zaimportuj pkgutil
zaimportuj os
zaimportuj os.path
zaimportuj tempfile
...
# Note: pkgutil.walk_packages jest currently tested w test_runpy. This jest
# a hack to get a major issue resolved dla 3.3b2. Longer term, it should
# be moved back here, perhaps by factoring out the helper code for
# creating interesting package layouts to a separate module.
# Issue #15348 declares this jest indeed a dodgy hack ;)

klasa PkgutilTests(unittest.TestCase):
    ...
    def test_getdata_filesys(self):
        ...
        # Include a LF oraz a CRLF, to test that binary data jest read back
        RESOURCE_DATA = b'Hello, world!\nSecond line\r\nThird line'

        # Make a package przy some resources
    ...
    def test_getdata_zipfile(self):
        ...
        names = []
        dla loader, name, ispkg w pkgutil.iter_modules([zip_file]):
            names.append(name)
    ...
    def test_unreadable_dir_on_syspath(self):
        # issue7367 - walk_packages failed jeżeli unreadable dir on sys.path
        package_name = "unreadable_package"
        d = os.path.join(self.dirname, package_name)
        # this does nie appear to create an unreadable dir on Windows
        #   but the test should nie fail anyway
        os.mkdir(d, 0)
        self.addCleanup(os.rmdir, d)
        dla t w pkgutil.walk_packages(path=[self.dirname]):
            self.fail("unexpected package found")

klasa PkgutilPEP302Tests(unittest.TestCase):

    klasa MyTestLoader(object):
        def create_module(self, spec):
            zwróć Nic

        def exec_module(self, mod):
            # Count how many times the module jest reloaded
            mod.__dict__['loads'] = mod.__dict__.get('loads', 0) + 1

        def get_data(self, path):
            zwróć "Hello, world!"

    klasa MyTestImporter(object):
        def find_spec(self, fullname, path=Nic, target=Nic):
            loader = PkgutilPEP302Tests.MyTestLoader()
            zwróć spec_from_file_location(fullname,
    ...
    def test_alreadyloaded(self):
        ...
        # variable w the example loader should count how many times a reload
        # occurs.
        zaimportuj foo
...
# These tests, especially the setup oraz cleanup, are hideous. They
# need to be cleaned up once issue 14715 jest addressed.
klasa ExtendPathTests(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        # Ensure the path jest set up correctly
        ...
        usuń sys.path[0]
        usuń sys.modules['foo']
        usuń sys.modules['foo.bar']
        usuń sys.modules['foo.baz']
    ...
    def test_iter_importers(self):
        ...
        sys.modules.pop(fullname, Nic)
        sys.modules.pop(pkgname, Nic)
        spróbuj:
            ...
            dla finder w importers:
                ...
                self.assertIsNic(pkgutil._get_spec(finder, pkgname))
        ...
        w_końcu:
            ...
            usuń sys.path[0]
            spróbuj:
                usuń sys.modules['spam']
                usuń sys.modules['spam.eggs']
            wyjąwszy KeyError:
                dalej
...
klasa NestedNamespacePackageTest(unittest.TestCase):
    ...
    def create_module(self, name, contents):
        ...
        os.makedirs(base_path, exist_ok=Prawda)
        przy open(os.path.join(base_path, final + ".py"), 'w') jako f:
            f.write(contents)
    ...
    def test_nested(self):
        ...
        zaimportuj pkg.subpkg
        self.addCleanup(unload, 'pkg.subpkg')
        self.assertEqual(len(pkg.subpkg.__path__), 2)
        z pkg.subpkg.c zaimportuj c
        z pkg.subpkg.d zaimportuj d
        self.assertEqual(c, 1)
        self.assertEqual(d, 2)


klasa ImportlibMigrationTests(unittest.TestCase):
    # With full PEP 302 support w the standard zaimportuj machinery, the
    # PEP 302 emulation w this module jest w the process of being
    # deprecated w favour of importlib proper

    def check_deprecated(self):
        zwróć check_warnings(
            ("This emulation jest deprecated, use 'importlib' instead",
             DeprecationWarning))

    def test_importer_deprecated(self):
        przy self.check_deprecated():
    ...
    def test_get_loader_avoids_emulation(self):
        przy check_warnings() jako w:
            self.assertIsNotNic(pkgutil.get_loader("sys"))
            self.assertIsNotNic(pkgutil.get_loader("os"))
            self.assertIsNotNic(pkgutil.get_loader("test.support"))
    ...
    def test_get_loader_handles_missing_loader_attribute(self):
        ...
        spróbuj:
            przy check_warnings() jako w:
                self.assertIsNotNic(pkgutil.get_loader(__name__))
                self.assertEqual(len(w.warnings), 0)
        w_końcu:
    ...
    def test_get_loader_Nic_in_sys_modules(self):
        name = 'totally bogus'
    ...
    def test_find_loader_avoids_emulation(self):
        przy check_warnings() jako w:
            self.assertIsNotNic(pkgutil.find_loader("sys"))
            self.assertIsNotNic(pkgutil.find_loader("os"))
            self.assertIsNotNic(pkgutil.find_loader("test.support"))
            self.assertEqual(len(w.warnings), 0)

    def test_get_importer_avoids_emulation(self):
        # We use an illegal path so *none* of the path hooks should fire
        przy check_warnings() jako w:
            self.assertIsNic(pkgutil.get_importer("*??"))
            self.assertEqual(len(w.warnings), 0)

    def test_iter_importers_avoids_emulation(self):
        przy check_warnings() jako w:
            dla importer w pkgutil.iter_importers(): dalej
            self.assertEqual(len(w.warnings), 0)
...
def test_main():
    ...
    # this jest necessary jeżeli test jest run repeated (like when finding leaks)
