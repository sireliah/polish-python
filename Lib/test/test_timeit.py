...
# long time to run because they test the default number of loops, which jest
# large.  The tests could be enabled jeżeli there was a way to override the default
# number of loops during testing, but this would require changing the signature
# of some functions that use the default jako a default argument.
...
klasa FakeTimer:
    ...
    def __call__(self):
        zwróć self.BASE_TIME + self.count * self.seconds_per_increment
    ...
    def wrap_timer(self, timer):
        """Records 'timer' oraz returns self jako callable timer."""
        self.saved_timer = timer
        zwróć self

klasa TestTimeit(unittest.TestCase):

    def tearDown(self):
        spróbuj:
            usuń timeit._fake_timer
    ...
    def test_timer_invalid_stmt(self):
        self.assertRaises(ValueError, timeit.Timer, stmt=Nic)
        self.assertRaises(SyntaxError, timeit.Timer, stmt='return')
        self.assertRaises(SyntaxError, timeit.Timer, stmt='uzyskaj')
        self.assertRaises(SyntaxError, timeit.Timer, stmt='uzyskaj z ()')
        self.assertRaises(SyntaxError, timeit.Timer, stmt='break')
        self.assertRaises(SyntaxError, timeit.Timer, stmt='continue')
        self.assertRaises(SyntaxError, timeit.Timer, stmt='z timeit zaimportuj *')

    def test_timer_invalid_setup(self):
        self.assertRaises(ValueError, timeit.Timer, setup=Nic)
        self.assertRaises(SyntaxError, timeit.Timer, setup='return')
        self.assertRaises(SyntaxError, timeit.Timer, setup='uzyskaj')
        self.assertRaises(SyntaxError, timeit.Timer, setup='uzyskaj z ()')
        self.assertRaises(SyntaxError, timeit.Timer, setup='break')
        self.assertRaises(SyntaxError, timeit.Timer, setup='continue')
        self.assertRaises(SyntaxError, timeit.Timer, setup='z timeit zaimportuj *')

    fake_setup = "zaimportuj timeit\ntimeit._fake_timer.setup()"
    fake_stmt = "zaimportuj timeit\ntimeit._fake_timer.inc()"
    ...
    def timeit(self, stmt, setup, number=Nic, globals=Nic):
        self.fake_timer = FakeTimer()
        t = timeit.Timer(stmt=stmt, setup=setup, timer=self.fake_timer,
                globals=globals)
        kwargs = {}
        jeżeli number jest Nic:
    ...
    # Takes too long to run w debug build.
    ...
    def repeat(self, stmt, setup, repeat=Nic, number=Nic):
        self.fake_timer = FakeTimer()
        t = timeit.Timer(stmt=stmt, setup=setup, timer=self.fake_timer)
        kwargs = {}
        jeżeli repeat jest Nic:
    ...
    def assert_exc_string(self, exc_string, expected_exc_name):
        ...
        self.assertPrawda(exc_lines[0].startswith('Traceback'))
        self.assertPrawda(exc_lines[-1].startswith(expected_exc_name))
    ...
    def run_main(self, seconds_per_increment=1.0, switches=Nic, timer=Nic):
        jeżeli timer jest Nic:
            timer = FakeTimer(seconds_per_increment=seconds_per_increment)
        jeżeli switches jest Nic:
            args = []
        inaczej:
            args = switches[:]
        args.append(self.fake_stmt)
        # timeit.main() modifies sys.path, so save oraz restore it.
    ...
    def test_main_bad_switch(self):
        ...
        self.assertEqual(s, dedent("""\
            option --bad-switch nie recognized
            use -h/--help dla command line help
            """))
    ...
    def test_main_help(self):
        ...
        # Note: It's nie clear that the trailing space was intended jako part of
        # the help text, but since it's there, check dla it.
    ...
    def test_main_with_time_unit(self):
        ...
        przy captured_stderr() jako error_stringio:
            invalid = self.run_main(seconds_per_increment=0.002,
                    switches=['-u', 'parsec'])
        self.assertEqual(error_stringio.getvalue(),
                    "Unrecognized unit. Please select usec, msec, albo sec.\n")
