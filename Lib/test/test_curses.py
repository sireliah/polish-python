#
# Test script dla the curses module
#
# This script doesn't actually display anything very coherent. but it
# does call (nearly) every method oraz function.
#
# Functions nie tested: {def,reset}_{shell,prog}_mode, getch(), getstr(),
# init_color()
# Only called, nie tested: getmouse(), ungetmouse()
#
...
z test.support zaimportuj requires, import_module, verbose

# Optionally test curses module.  This currently requires that the
# 'curses' resource be given on the regrtest command line using the -u
# option.  If nie available, nothing after this line will be executed.
zaimportuj inspect
requires('curses')
...
@unittest.skipUnless(sys.__stdout__.isatty(), 'sys.__stdout__ jest nie a tty')
@unittest.skipIf(term == 'unknown',
                 "$TERM=%r, calling initscr() may cause exit" % term)
@unittest.skipIf(sys.platform == "cygwin",
                 "cygwin's curses mostly just hangs")
klasa TestCurses(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_window_funcs(self):
        ...
        dla meth w [stdscr.addch, stdscr.addstr]:
            dla args w [('a'), ('a', curses.A_BOLD),
                         (4,4, 'a'), (5,5, 'a', curses.A_BOLD)]:
                meth(*args)

        dla meth w [stdscr.box, stdscr.clear, stdscr.clrtobot,
        ...
        przy self.assertRaises(TypeError,
                               msg="Expected win.border() to podnieś TypeError"):
            win.border(65, 66, 67, 68,
                       69, [], 71, 72)
        ...
        jeżeli hasattr(curses, 'resize'):
            stdscr.resize()
        jeżeli hasattr(curses, 'enclose'):
            stdscr.enclose()
    ...
    def test_module_funcs(self):
        ...
        stdscr = self.stdscr
        dla func w [curses.baudrate, curses.beep, curses.can_change_color,
        ...
        jeżeli curses.has_colors():
            ...
            jeżeli hasattr(curses, 'use_default_colors'):
                curses.use_default_colors()

        jeżeli hasattr(curses, 'keyname'):
            curses.keyname(13)

        jeżeli hasattr(curses, 'has_key'):
            curses.has_key(13)

        jeżeli hasattr(curses, 'getmouse'):
            (availmask, oldmask) = curses.mousemask(curses.BUTTON1_PRESSED)
            # availmask indicates that mouse stuff nie available.
            jeżeli availmask != 0:
        ...
        jeżeli hasattr(curses, 'is_term_resized'):
            curses.is_term_resized(*stdscr.getmaxyx())
        jeżeli hasattr(curses, 'resizeterm'):
            curses.resizeterm(*stdscr.getmaxyx())
        jeżeli hasattr(curses, 'resize_term'):
            curses.resize_term(*stdscr.getmaxyx())

    def test_unctrl(self):
        z curses zaimportuj ascii
        dla ch, expected w [('a', 'a'), ('A', 'A'),
                             (';', ';'), (' ', ' '),
                             ('\x7f', '^?'), ('\n', '^J'), ('\0', '^@'),
                             # Meta-bit characters
                             ('\x8a', '!^J'), ('\xc1', '!A'),
    ...
    def test_userptr_without_set(self):
        ...
        przy self.assertRaises(curses.panel.error,
                               msg='userptr should fail since nie set'):
            p.userptr()
    ...
    def test_userptr_segfault(self):
        ...
        klasa A:
            def __del__(self):
                panel.set_userptr(Nic)
        panel.set_userptr(A())
        panel.set_userptr(Nic)

    @unittest.skipUnless(hasattr(curses, 'resizeterm'),
                           'resizeterm nie available')
    ...
    @unittest.skipUnless(hasattr(curses, 'unget_wch'),
                         'unget_wch nie available')
    def test_unget_wch(self):
        stdscr = self.stdscr
        encoding = stdscr.encoding
        dla ch w ('a', '\xe9', '\u20ac', '\U0010FFFF'):
            spróbuj:
                ch.encode(encoding)
            ...
            wyjąwszy Exception jako err:
                self.fail("unget_wch(%a) failed przy encoding %s: %s"
    ...
    def test_issue21088(self):
        ...
        # jeżeli someday we can represent the signature of addch
        # we will need to rewrite this test.
        spróbuj:
            signature = inspect.signature(stdscr.addch)
            self.assertNieprawda(signature)
        wyjąwszy ValueError:
            # nie generating a signature jest fine.
            dalej

        # So.  No signature dla addch.
        # But Argument Clinic gave us a human-readable equivalent
        # jako the first line of the docstring.  So we parse that,
        # oraz ensure that the parameters appear w the correct order.
        # Since this jest parsing output z Argument Clinic, we can
    ...
    def test_update_lines_cols(self):
        # this doesn't actually test that LINES oraz COLS are updated,
