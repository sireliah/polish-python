"""
Test suite dla _osx_support: shared OS X support functions.
"""
...
klasa Test_OSXSupport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dla cv w ('CFLAGS', 'LDFLAGS', 'CPPFLAGS',
                            'BASECFLAGS', 'BLDSHARED', 'LDSHARED', 'CC',
                            'CXX', 'PY_CFLAGS', 'PY_LDFLAGS', 'PY_CPPFLAGS',
                            'PY_CORE_CFLAGS'):
            jeżeli cv w self.env:
                self.env.unset(cv)

    def add_expected_saved_initial_values(self, config_vars, expected_vars):
        # Ensure that the initial values dla all modified config vars
        # are also saved przy modified keys.
        expected_vars.update(('_OSX_SUPPORT_INITIAL_'+ k,
                config_vars[k]) dla k w config_vars
                    jeżeli config_vars[k] != expected_vars[k])

    def test__find_executable(self):
        jeżeli self.env['PATH']:
            self.env['PATH'] = self.env['PATH'] + ':'
        self.env['PATH'] = self.env['PATH'] + os.path.abspath(self.temp_path_dir)
        test.support.unlink(self.prog_name)
        self.assertIsNic(_osx_support._find_executable(self.prog_name))
        self.addCleanup(test.support.unlink, self.prog_name)
        przy open(self.prog_name, 'w') jako f:
            f.write("#!/bin/sh\n/bin/echo OK\n")
    ...
    def test__find_build_tool(self):
        ...
        self.assertPrawda(os.path.isfile(out),
                            'cc nie found - check xcode-select')

    def test__get_system_version(self):
        self.assertPrawda(platform.mac_ver()[0].startswith(
    ...
    def test__supports_universal_builds(self):
        zaimportuj platform
        mac_ver_tuple = tuple(int(i) dla i w
    ...
    def test__remove_unsupported_archs(self):
        ...
        test.support.unlink(c_name)
        self.addCleanup(test.support.unlink, c_name)
        # exit status 255 means no PPC support w this compiler chain
        przy open(c_name, 'w') jako f:
            f.write("#!/bin/sh\nexit 255")
        os.chmod(c_name, stat.S_IRWXU)
        self.assertEqual(expected_vars,
    ...
    def test_get_platform_osx(self):
        # Note, get_platform_osx jest currently tested more extensively
        # indirectly by test_sysconfig oraz test_distutils
        config_vars = {
        'CFLAGS': '-fno-strict-aliasing  -g -O3 -arch ppc -arch i386  '
                        '-isysroot /Developer/SDKs/MacOSX10.1.sdk',
