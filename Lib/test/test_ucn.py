...
Modified dla Python 2.0 by Fredrik Lundh (fredrik@pythonware.com)
...
z test.test_normalization zaimportuj check_version

spróbuj:
    z _testcapi zaimportuj INT_MAX, PY_SSIZE_T_MAX, UINT_MAX
wyjąwszy ImportError:
    INT_MAX = PY_SSIZE_T_MAX = UINT_MAX = 2**64 - 1

klasa UnicodeNamesTest(unittest.TestCase):

    def checkletter(self, name, code):
        # Helper that put all \N escapes inside eval'd raw strings,
        # to make sure this script runs even jeżeli the compiler
        # chokes on \N escapes
        res = eval(r'"\N{%s}"' % name)
    ...
    def test_general(self):
        # General oraz case insensitivity test:
        chars = [
        ...
        self.assertEqual(
            "".join([self.checkletter(*args) dla args w zip(chars, string)]),
            string
        )

    def test_ascii_letters(self):
        dla char w "".join(map(chr, range(ord("a"), ord("z")))):
    ...
    def test_bmp_characters(self):
        dla code w range(0x10000):
            char = chr(code)
            name = unicodedata.name(char, Nic)
    ...
    def test_aliases(self):
        # Check that the aliases defined w the NameAliases.txt file work.
        # This should be updated when new aliases are added albo the file
        # should be downloaded oraz parsed instead.  See #12753.
        aliases = [
    ...
    def test_aliases_names_in_pua_range(self):
        # We are storing aliases w the PUA 15, but their names shouldn't leak
        dla cp w range(0xf0000, 0xf0100):
    ...
    def test_named_sequences_names_in_pua_range(self):
        # We are storing named seq w the PUA 15, but their names shouldn't leak
        dla cp w range(0xf0100, 0xf0fff):
            przy self.assertRaises(ValueError) jako cm:
                unicodedata.name(chr(cp))
            self.assertEqual(str(cm.exception), 'no such name')
    ...
    def test_named_sequences_sample(self):
        ...
        dla seqname, codepoints w sequences:
            self.assertEqual(unicodedata.lookup(seqname), codepoints)
            przy self.assertRaises(SyntaxError):
                self.checkletter(seqname, Nic)
            przy self.assertRaises(KeyError):
    ...
    def test_named_sequences_full(self):
        ...
        wyjąwszy (OSError, HTTPException):
            self.skipTest("Could nie retrieve " + url)
        ...
        dla line w testdata:
            ...
            codepoints = ''.join(chr(int(cp, 16)) dla cp w codepoints.split())
            self.assertEqual(unicodedata.lookup(seqname), codepoints)
            przy self.assertRaises(SyntaxError):
                self.checkletter(seqname, Nic)
            przy self.assertRaises(KeyError):
    ...
    @support.bigmemtest(size=UINT_MAX + 1, memuse=2 + 1, dry_run=Nieprawda)
