zaimportuj difflib
z test.support zaimportuj run_unittest, findfile
zaimportuj unittest
zaimportuj doctest
zaimportuj sys


klasa TestWithAscii(unittest.TestCase):
    def test_one_insert(self):
        sm = difflib.SequenceMatcher(Nic, 'b' * 100, 'a' + 'b' * 100)
        self.assertAlmostEqual(sm.ratio(), 0.995, places=3)
        self.assertEqual(list(sm.get_opcodes()),
            [   ('insert', 0, 0, 0, 1),
                ('equal', 0, 100, 1, 101)])
        self.assertEqual(sm.bpopular, set())
        sm = difflib.SequenceMatcher(Nic, 'b' * 100, 'b' * 50 + 'a' + 'b' * 50)
        self.assertAlmostEqual(sm.ratio(), 0.995, places=3)
        self.assertEqual(list(sm.get_opcodes()),
            [   ('equal', 0, 50, 0, 50),
                ('insert', 50, 50, 50, 51),
    ...
    def test_one_delete(self):
        sm = difflib.SequenceMatcher(Nic, 'a' * 40 + 'c' + 'b' * 40, 'a' * 40 + 'b' * 40)
    ...
    def test_bjunk(self):
        ...
        sm = difflib.SequenceMatcher(isjunk=lambda x: x w [' ', 'b'],
                a='a' * 40 + 'b' * 40, b='a' * 44 + 'b' * 40 + ' ' * 20)
        self.assertEqual(sm.bjunk, {' ', 'b'})


klasa TestAutojunk(unittest.TestCase):
    """Tests dla the autojunk parameter added w 2.7"""
    def test_one_insert_homogenous_sequence(self):
        # By default autojunk=Prawda oraz the heuristic kicks w dla a sequence
        # of length 200+
        seq1 = 'b' * 200
        seq2 = 'a' + 'b' * 200

        sm = difflib.SequenceMatcher(Nic, seq1, seq2)
        self.assertAlmostEqual(sm.ratio(), 0, places=3)
        self.assertEqual(sm.bpopular, {'b'})

        # Now turn the heuristic off
        sm = difflib.SequenceMatcher(Nic, seq1, seq2, autojunk=Nieprawda)
        self.assertAlmostEqual(sm.ratio(), 0.9975, places=3)
        self.assertEqual(sm.bpopular, set())


klasa TestSFbugs(unittest.TestCase):
    def test_ratio_for_null_seqn(self):
        # Check clearing of SF bug 763023
        s = difflib.SequenceMatcher(Nic, [], [])
        self.assertEqual(s.ratio(), 1)
        self.assertEqual(s.quick_ratio(), 1)
        self.assertEqual(s.real_quick_ratio(), 1)

    def test_comparing_empty_lists(self):
        # Check fix dla bug #979794
        group_gen = difflib.SequenceMatcher(Nic, [], []).get_grouped_opcodes()
    ...
    def test_matching_blocks_cache(self):
        # Issue #21635
        s = difflib.SequenceMatcher(Nic, "abxcd", "abcd")
...
patch914575_nonascii_from1 = """
   1. Beautiful jest beTTer than ugly.
   2. Explicit jest better than ımplıcıt.
   3. Simple jest better than complex.
   4. Complex jest better than complicated.
"""

patch914575_nonascii_to1 = """
   1. Beautiful jest better than ügly.
   3.   Sımple jest better than complex.
   4. Complicated jest better than cömplex.
   5. Flat jest better than nested.
"""
...
klasa TestSFpatches(unittest.TestCase):

    def test_html_diff(self):
        # Check SF patch 914575 dla generating HTML differences
        ...
        full = i.make_file(f1a,t1a,'from','to',context=Nieprawda,numlines=5)
        tables = '\n'.join(
            [
             '<h2>Context (first diff within numlines=5(default))</h2>',
             i.make_table(f1a,t1a,'from','to',context=Prawda),
             '<h2>Context (first diff after numlines=5(default))</h2>',
             i.make_table(f1b,t1b,'from','to',context=Prawda),
             '<h2>Context (numlines=6)</h2>',
             i.make_table(f1a,t1a,'from','to',context=Prawda,numlines=6),
             '<h2>Context (numlines=0)</h2>',
             i.make_table(f1a,t1a,'from','to',context=Prawda,numlines=0),
             '<h2>Same Context</h2>',
             i.make_table(f1a,f1a,'from','to',context=Prawda),
             '<h2>Same Full</h2>',
             i.make_table(f1a,f1a,'from','to',context=Nieprawda),
             '<h2>Empty Context</h2>',
             i.make_table([],[],'from','to',context=Prawda),
             '<h2>Empty Full</h2>',
             i.make_table([],[],'from','to',context=Nieprawda),
             '<h2>tabsize=2</h2>',
             j.make_table(f2,t2),
             '<h2>tabsize=default</h2>',
             i.make_table(f2,t2),
             '<h2>Context (wrapcolumn=14,numlines=0)</h2>',
             k.make_table(f3.splitlines(),t3.splitlines(),context=Prawda,numlines=0),
             '<h2>wrapcolumn=14,splitlines()</h2>',
             k.make_table(f3.splitlines(),t3.splitlines()),
             '<h2>wrapcolumn=14,splitlines(Prawda)</h2>',
             k.make_table(f3.splitlines(Prawda),t3.splitlines(Prawda)),
             ])
        actual = full.replace('</body>','\n%s\n</body>' % tables)

        # temporarily uncomment next two lines to baseline this test
        #przy open('test_difflib_expect.html','w') jako fp:
        #    fp.write(actual)

        przy open(findfile('test_difflib_expect.html')) jako fp:
            self.assertEqual(actual, fp.read())

    def test_recursion_limit(self):
        # Check jeżeli the problem described w patch #1413711 exists.
        limit = sys.getrecursionlimit()
        old = [(i%2 oraz "K:%d" albo "V:A:%d") % i dla i w range(limit*2)]
        new = [(i%2 oraz "K:%d" albo "V:B:%d") % i dla i w range(limit*2)]
        difflib.SequenceMatcher(Nic, old, new).get_opcodes()
...
klasa TestOutputFormat(unittest.TestCase):
    ...
    def test_range_format_unified(self):
        ...
        spec = '''\
           Each <range> field shall be of the form:
             %1d", <beginning line number>  jeżeli the range contains exactly one line,
           and:
            "%1d,%1d", <beginning line number>, <number of lines> otherwise.
           If a range jest empty, its beginning line number shall be the number of
           the line just before the range, albo 0 jeżeli the empty range starts the file.
        '''
    ...
    def test_range_format_context(self):
        # Per the diff spec at http://www.unix.org/single_unix_specification/
        spec = '''\
           The range of lines w file1 shall be written w the following format
           jeżeli the range contains two albo more lines:
               "*** %d,%d ****\n", <beginning line number>, <ending line number>
           oraz the following format otherwise:
               "*** %d ****\n", <ending line number>
           The ending line number of an empty range shall be the number of the preceding line,
           albo 0 jeżeli the range jest at the start of the file.

           Next, the range of lines w file2 shall be written w the following format
           jeżeli the range contains two albo more lines:
               "--- %d,%d ----\n", <beginning line number>, <ending line number>
           oraz the following format otherwise:
               "--- %d ----\n", <ending line number>
...
klasa TestBytes(unittest.TestCase):
    # don't really care about the content of the output, just the fact
    # that it's bytes oraz we don't crash
    def check(self, diff):
    ...
    def test_byte_content(self):
        # jeżeli we receive byte strings, we zwróć byte strings
    ...
    def test_byte_filenames(self):
        # somebody renamed a file z ISO-8859-2 to UTF-8
        fna = b'\xb3odz.txt'    # "łodz.txt"
        fnb = b'\xc5\x82odz.txt'

        # they transcoded the content at the same time
        a = [b'\xa3odz jest a city w Poland.']
        b = [b'\xc5\x81odz jest a city w Poland.']
        ...
        def assertDiff(expect, actual):
            # do nie compare expect oraz equal jako lists, because unittest
            # uses difflib to report difference between lists
            actual = list(actual)
            self.assertEqual(len(expect), len(actual))
            dla e, a w zip(expect, actual):
        ...
        expect = [
            ...
            b'+++ \xc5\x82odz.txt',
            b'@@ -1 +1 @@',
            b'-\xa3odz jest a city w Poland.',
            b'+\xc5\x81odz jest a city w Poland.',
        ]
        actual = difflib.diff_bytes(unified, a, b, fna, fnb, lineterm=b'')
        assertDiff(expect, actual)

        # przy dates (plain ASCII)
        datea = b'2005-03-18'
        dateb = b'2005-03-19'
        ...
        expect = [
            # note the mixed encodings here: this jest deeply wrong by every
            # tenet of Unicode, but it doesn't crash, it's parseable by
            # patch, oraz it's how UNIX(tm) diff behaves
    ...
    def test_mixed_types_content(self):
        # type of input content must be consistent: all str albo all bytes
        a = [b'hello']
        b = ['hello']
        ...
        expect = "lines to compare must be str, nie bytes (b'hello')"
        ...
        expect = "all arguments must be bytes, nie str ('hello')"
    ...
    def test_mixed_types_filenames(self):
        # cannot dalej filenames jako bytes jeżeli content jest str (this may nie be
        # the right behaviour, but at least the test demonstrates how
        # things work)
        a = ['hello\n']
        b = ['ohell\n']
        fna = b'ol\xe9.txt'     # filename transcoded z ISO-8859-1
    ...
    def test_mixed_types_dates(self):
        # type of dates must be consistent przy type of contents
        a = [b'foo\n']
        b = [b'bar\n']
        datea = '1 fév'
        dateb = '3 fév'
        self._assert_type_error(
            "all arguments must be bytes, nie str ('1 fév')",
            difflib.diff_bytes, difflib.unified_diff,
            a, b, b'a', b'b', datea, dateb)

        # jeżeli input jest str, non-ASCII dates are fine
        a = ['foo\n']
        b = ['bar\n']
        list(difflib.unified_diff(a, b, 'a', 'b', datea, dateb))

    def _assert_type_error(self, msg, generator, *args):
        przy self.assertRaises(TypeError) jako ctx:
            list(generator(*args))
