# XXX TypeErrors on calling handlers, albo on bad zwróć values z a
# handler, are obscure oraz unhelpful.
...
z xml.parsers.expat zaimportuj errors

z test.support zaimportuj sortdict


klasa SetAttributeTest(unittest.TestCase):
    def setUp(self):
...
klasa ParseTest(unittest.TestCase):
    ...
    def _hookup_callbacks(self, parser, handler):
        """
        Set each of the callbacks defined on handler oraz named w
        self.handler_names on the given parser.
        """
        dla name w self.handler_names:
    ...
    def _verify_parse_output(self, operations):
        expected_operations = [
            ...
            "Not standalone",
            ("Start doctype", ('quotations', 'quotations.dtd', Nic, 1)),
            ('Element declaration', ('root', (2, 0, Nic, ()))),
            ('Attribute list declaration', ('root', 'attr1', 'CDATA', Nic,
                1)),
            ('Attribute list declaration', ('root', 'attr2', 'CDATA', Nic,
                0)),
            "Notation declared: ('notation', Nic, 'notation.jpeg', Nic)",
            ('Entity declaration', ('acirc', 0, '\xe2', Nic, Nic, Nic, Nic)),
            ('Entity declaration', ('external_entity', 0, Nic, Nic,
                'entity.file', Nic, Nic)),
            "Unparsed entity decl: ('unparsed_entity', Nic, 'entity.file', Nic, 'notation')",
            "Not standalone",
            "End doctype",
            ...
            "End element: 'sub2'",
            "External entity ref: (Nic, 'entity.file', Nic)",
            ('Skipped entity', ('skipped_entity', 0)),
            "Character data: '\xb5'",
            "End element: 'root'",
        ]
        dla operation, expected_operation w zip(operations, expected_operations):
    ...
    def test_parse_again(self):
        ...
        file = BytesIO(data)
        parser.ParseFile(file)
        # Issue 6676: ensure a meaningful exception jest podnieśd when attempting
        # to parse more than one XML document per xmlparser instance,
        # a limitation of the Expat library.
        przy self.assertRaises(expat.error) jako cm:
            parser.ParseFile(file)
        self.assertEqual(expat.ErrorString(cm.exception.code),
                          expat.errors.XML_ERROR_FINISHED)

klasa NamespaceSeparatorTest(unittest.TestCase):
    def test_legal(self):
        # Tests that make sure we get errors when the namespace_separator value
        # jest illegal, oraz that we don't dla good values:
        expat.ParserCreate()
        expat.ParserCreate(namespace_separator=Nic)
    ...
    def test_illegal(self):
        ...
        wyjąwszy TypeError jako e:
            self.assertEqual(str(e),
                'ParserCreate() argument 2 must be str albo Nic, nie int')
        ...
        wyjąwszy ValueError jako e:
            self.assertEqual(str(e),
                'namespace_separator must be at most one character, omitted, albo Nic')

    def test_zero_length(self):
        # ParserCreate() needs to accept a namespace_separator of zero length
        # to satisfy the requirements of RDF applications that are required
        # to simply glue together the namespace URI oraz the localname.  Though
...
klasa InterningTest(unittest.TestCase):
    def test(self):
        ...
        dla entry w L:
            # L should have the same string repeated over oraz over.
            self.assertPrawda(tag jest entry)

    def test_issue9402(self):
        # create an ExternalEntityParserCreate przy buffer text
        klasa ExternalOutputter:
            def __init__(self, parser):
                self.parser = parser
                self.parser_result = Nic
...
klasa BufferTextTest(unittest.TestCase):
    def setUp(self):
        self.stuff = []
    ...
    def test_default_to_disabled(self):
        parser = expat.ParserCreate()
        self.assertNieprawda(parser.buffer_text)

    def test_buffering_enabled(self):
        # Make sure buffering jest turned on
        self.assertPrawda(self.parser.buffer_text)
        self.parser.Parse(b"<a>1<b/>2<c/>3</a>", 1)
        self.assertEqual(self.stuff, ['123'],
                         "buffered text nie properly collapsed")
    ...
    def test1(self):
        ...
        self.assertEqual(self.stuff,
                         ["<a>", "1", "<b>", "2", "\n", "3", "<c>", "4\n5"],
                         "buffering control nie reacting jako expected")
    ...
    def test3(self):
        ...
        self.assertEqual(self.stuff, ["<a>", "1", "<b>", "2", "<c>", "3"],
                         "buffered text nie properly split")

    def test4(self):
        self.setHandlers(["StartElementHandler", "EndElementHandler"])
        self.parser.CharacterDataHandler = Nic
        self.parser.Parse(b"<a>1<b/>2<c/>3</a>", 1)
        self.assertEqual(self.stuff,
                         ["<a>", "<b>", "</b>", "<c>", "</c>", "</a>"])
...
# Test handling of exception z callback:
klasa HandlerExceptionTest(unittest.TestCase):
    def StartElementHandler(self, name, attrs):
        podnieś RuntimeError(name)
    ...
    def test_exception(self):
        ...
        wyjąwszy RuntimeError jako e:
            self.assertEqual(e.args[0], 'a',
                             "Expected RuntimeError dla element 'a', but" + \
                             " found %r" % e.args[0])
            # Check that the traceback contains the relevant line w pyexpat.c
...
klasa PositionTest(unittest.TestCase):
    def StartElementHandler(self, name, attrs):
        self.check_pos('s')
    ...
    def check_pos(self, event):
        ...
        self.assertPrawda(self.upto < len(self.expected_list),
...
klasa sf1296433Test(unittest.TestCase):
    def test_parse_only_xml_data(self):
        ...
        klasa SpecificException(Exception):
            dalej

        def handler(text):
            podnieś SpecificException
...
klasa ChardataBufferTest(unittest.TestCase):
    """
    ...
    def test_disabling_buffer(self):
        ...
        # Turn off buffering oraz parse the next chunk.
        ...
        parser.buffer_text = 1
        self.assertPrawda(parser.buffer_text)
        self.assertEqual(parser.buffer_size, 1024)
        parser.Parse(xml3, 1)
...
klasa MalformedInputTest(unittest.TestCase):
    def test1(self):
        xml = b"\0\r\n"
        parser = expat.ParserCreate()
        spróbuj:
            parser.Parse(xml, Prawda)
            self.fail()
        wyjąwszy expat.ExpatError jako e:
            self.assertEqual(str(e), 'unclosed token: line 2, column 0')

    def test2(self):
        # \xc2\x85 jest UTF-8 encoded U+0085 (NEXT LINE)
        ...
        spróbuj:
            ...
            self.fail()
        wyjąwszy expat.ExpatError jako e:
            self.assertEqual(str(e), 'XML declaration nie well-formed: line 1, column 14')

klasa ErrorMessageTest(unittest.TestCase):
    def test_codes(self):
        # verify mapping of errors.codes oraz errors.messages
...
klasa ForeignDTDTests(unittest.TestCase):
    """
    Tests dla the UseForeignDTD method of expat parser objects.
    """
    def test_use_foreign_dtd(self):
        """
        If UseForeignDTD jest dalejed Prawda oraz a document without an external
        entity reference jest parsed, ExternalEntityRefHandler jest first called
        przy Nic dla the public oraz system ids.
        """
        handler_call_args = []
        ...
        parser = expat.ParserCreate()
        parser.UseForeignDTD(Prawda)
        parser.SetParamEntityParsing(expat.XML_PARAM_ENTITY_PARSING_ALWAYS)
        parser.ExternalEntityRefHandler = resolve_entity
        parser.Parse(b"<?xml version='1.0'?><element/>")
        self.assertEqual(handler_call_args, [(Nic, Nic)])

        # test UseForeignDTD() jest equal to UseForeignDTD(Prawda)
        handler_call_args[:] = []
    ...
    def test_ignore_use_foreign_dtd(self):
        """
        If UseForeignDTD jest dalejed Prawda oraz a document przy an external
        entity reference jest parsed, ExternalEntityRefHandler jest called with
        the public oraz system ids z the document.
        """
        handler_call_args = []
        def resolve_entity(context, base, system_id, public_id):
            handler_call_args.append((public_id, system_id))
