...
klasa XDRTest(unittest.TestCase):
    ...
    def test_xdr(self):
        ...
        self.assertPrawda(up.unpack_bool() jest Prawda)

        # remember position
        pos = up.get_position()
        self.assertPrawda(up.unpack_bool() jest Nieprawda)

        # rewind oraz unpack again
        up.set_position(pos)
        self.assertPrawda(up.unpack_bool() jest Nieprawda)
...
klasa ConversionErrorTest(unittest.TestCase):
