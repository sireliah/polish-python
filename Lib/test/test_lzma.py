...
z io zaimportuj BytesIO, UnsupportedOperation, DEFAULT_BUFFER_SIZE
zaimportuj os
zaimportuj pickle
zaimportuj random
zaimportuj unittest
...
z lzma zaimportuj LZMACompressor, LZMADecompressor, LZMAError, LZMAFile


klasa CompressorDecompressorTestCase(unittest.TestCase):
    ...
    def test_simple_bad_args(self):
        ...
        # Can't specify a preset oraz a custom filter chain at the same time.
        ...
        # Cannot specify a memory limit przy FILTER_RAW.
        przy self.assertRaises(ValueError):
            LZMADecompressor(lzma.FORMAT_RAW, memlimit=0x1000000)
        # Can only specify a custom filter chain przy FILTER_RAW.
    ...
    def test_decompressor_chunks(self):
        lzd = LZMADecompressor()
        out = []
        dla i w range(0, len(COMPRESSED_XZ), 10):
            self.assertNieprawda(lzd.eof)
    ...
    def test_decompressor_chunks_maxsize(self):
        ...
        self.assertNieprawda(lzd.needs_input)
    ...
    # LZMADecompressor intentionally does nie handle concatenated streams.
    ...
    # Test przy inputs larger than 4GiB.
    ...
    def test_decompressor_bigmem(self, size):
        ...
        w_końcu:
            input = cdata = ddata = Nic

    # Pickling podnieśs an exception; there's no way to serialize an lzma_stream.
...
klasa CompressDecompressFunctionTestCase(unittest.TestCase):
    ...
    def test_decompress_memlimit(self):
        przy self.assertRaises(LZMAError):
...
klasa TempFile:
    """Context manager - creates a file, oraz deletes it on __exit__."""
...
klasa FileTestCase(unittest.TestCase):

    def test_init(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            dalej
        przy LZMAFile(BytesIO(), "w") jako f:
            dalej
        przy LZMAFile(BytesIO(), "x") jako f:
            dalej
        przy LZMAFile(BytesIO(), "a") jako f:
            dalej

    def test_init_with_filename(self):
        przy TempFile(TESTFN, COMPRESSED_XZ):
            przy LZMAFile(TESTFN) jako f:
                dalej
            przy LZMAFile(TESTFN, "w") jako f:
                dalej
            przy LZMAFile(TESTFN, "a") jako f:
                dalej

    def test_init_mode(self):
        przy TempFile(TESTFN):
            przy LZMAFile(TESTFN, "r"):
                dalej
            przy LZMAFile(TESTFN, "rb"):
                dalej
            przy LZMAFile(TESTFN, "w"):
                dalej
            przy LZMAFile(TESTFN, "wb"):
                dalej
            przy LZMAFile(TESTFN, "a"):
                dalej
            przy LZMAFile(TESTFN, "ab"):
                dalej
    ...
    def test_init_bad_check(self):
        ...
        # CHECK_UNKNOWN oraz anything above CHECK_ID_MAX should be invalid.
        przy self.assertRaises(LZMAError):
            LZMAFile(BytesIO(), "w", check=lzma.CHECK_UNKNOWN)
        przy self.assertRaises(LZMAError):
            LZMAFile(BytesIO(), "w", check=lzma.CHECK_ID_MAX + 3)
        # Cannot specify a check przy mode="r".
        przy self.assertRaises(ValueError):
    ...
    def test_init_bad_preset(self):
        ...
        # Cannot specify a preset przy mode="r".
        przy self.assertRaises(ValueError):
    ...
    def test_close(self):
        przy BytesIO(COMPRESSED_XZ) jako src:
            f = LZMAFile(src)
            f.close()
            # LZMAFile.close() should nie close the underlying file object.
            self.assertNieprawda(src.closed)
            # Try closing an already-closed LZMAFile.
            f.close()
            self.assertNieprawda(src.closed)

        # Test przy a real file on disk, opened directly by LZMAFile.
        przy TempFile(TESTFN, COMPRESSED_XZ):
            f = LZMAFile(TESTFN)
            fp = f._fp
            f.close()
            # Here, LZMAFile.close() *should* close the underlying file object.
            self.assertPrawda(fp.closed)
            # Try closing an already-closed LZMAFile.
            f.close()
    ...
    def test_seekable(self):
        ...
        f = LZMAFile(BytesIO(), "w")
        spróbuj:
            self.assertNieprawda(f.seekable())
        w_końcu:
            f.close()
        self.assertRaises(ValueError, f.seekable)
    ...
    def test_readable(self):
        ...
        f = LZMAFile(BytesIO(), "w")
        spróbuj:
            self.assertNieprawda(f.readable())
        w_końcu:
            f.close()
        self.assertRaises(ValueError, f.readable)
    ...
    def test_writable(self):
        f = LZMAFile(BytesIO(COMPRESSED_XZ))
        spróbuj:
            self.assertNieprawda(f.writable())
            f.read()
            self.assertNieprawda(f.writable())
        w_końcu:
            f.close()
    ...
    def test_read(self):
        ...
        przy LZMAFile(BytesIO(COMPRESSED_ALONE)) jako f:
            self.assertEqual(f.read(), INPUT)
        przy LZMAFile(BytesIO(COMPRESSED_XZ), format=lzma.FORMAT_XZ) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
        przy LZMAFile(BytesIO(COMPRESSED_ALONE), format=lzma.FORMAT_ALONE) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
        przy LZMAFile(BytesIO(COMPRESSED_RAW_1),
                      format=lzma.FORMAT_RAW, filters=FILTERS_RAW_1) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
        przy LZMAFile(BytesIO(COMPRESSED_RAW_2),
                      format=lzma.FORMAT_RAW, filters=FILTERS_RAW_2) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
        przy LZMAFile(BytesIO(COMPRESSED_RAW_3),
                      format=lzma.FORMAT_RAW, filters=FILTERS_RAW_3) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
        przy LZMAFile(BytesIO(COMPRESSED_RAW_4),
                      format=lzma.FORMAT_RAW, filters=FILTERS_RAW_4) jako f:
            self.assertEqual(f.read(), INPUT)
            self.assertEqual(f.read(), b"")
    ...
    def test_read_multistream(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ * 5)) jako f:
            self.assertEqual(f.read(), INPUT * 5)
        przy LZMAFile(BytesIO(COMPRESSED_XZ + COMPRESSED_ALONE)) jako f:
            self.assertEqual(f.read(), INPUT * 2)
        przy LZMAFile(BytesIO(COMPRESSED_RAW_3 * 4),
                      format=lzma.FORMAT_RAW, filters=FILTERS_RAW_3) jako f:
            self.assertEqual(f.read(), INPUT * 4)

    def test_read_multistream_buffer_size_aligned(self):
        # Test the case where a stream boundary coincides przy the end
    ...
    def test_read_trailing_junk(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ + COMPRESSED_BOGUS)) jako f:
            self.assertEqual(f.read(), INPUT)

    def test_read_multistream_trailing_junk(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ * 5 + COMPRESSED_BOGUS)) jako f:
            self.assertEqual(f.read(), INPUT * 5)
    ...
    def test_read_from_file_with_bytes_filename(self):
        ...
        przy TempFile(TESTFN, COMPRESSED_XZ):
            przy LZMAFile(bytes_filename) jako f:
                self.assertEqual(f.read(), INPUT)
                self.assertEqual(f.read(), b"")

    def test_read_incomplete(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ[:128])) jako f:
            self.assertRaises(EOFError, f.read)

    def test_read_truncated(self):
        # Drop stream footer: CRC (4 bytes), index size (4 bytes),
        # flags (2 bytes) oraz magic number (2 bytes).
        truncated = COMPRESSED_XZ[:-12]
        przy LZMAFile(BytesIO(truncated)) jako f:
            self.assertRaises(EOFError, f.read)
        przy LZMAFile(BytesIO(truncated)) jako f:
        ...
        dla i w range(12):
            przy LZMAFile(BytesIO(truncated[:i])) jako f:
                self.assertRaises(EOFError, f.read, 1)
    ...
    def test_read_bad_data(self):
        przy LZMAFile(BytesIO(COMPRESSED_BOGUS)) jako f:
    ...
    def test_read1_bad_args(self):
        ...
        przy LZMAFile(BytesIO(), "w") jako f:
            self.assertRaises(ValueError, f.read1)
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            self.assertRaises(TypeError, f.read1, Nic)

    def test_peek(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            result = f.peek()
            self.assertGreater(len(result), 0)
            self.assertPrawda(INPUT.startswith(result))
            self.assertEqual(f.read(), INPUT)
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            result = f.peek(10)
            self.assertGreater(len(result), 0)
    ...
    def test_write(self):
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w") jako f:
                f.write(INPUT)
            expected = lzma.compress(INPUT)
            self.assertEqual(dst.getvalue(), expected)
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w", format=lzma.FORMAT_XZ) jako f:
                f.write(INPUT)
            expected = lzma.compress(INPUT, format=lzma.FORMAT_XZ)
            self.assertEqual(dst.getvalue(), expected)
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w", format=lzma.FORMAT_ALONE) jako f:
                f.write(INPUT)
            expected = lzma.compress(INPUT, format=lzma.FORMAT_ALONE)
            self.assertEqual(dst.getvalue(), expected)
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w", format=lzma.FORMAT_RAW,
                          filters=FILTERS_RAW_2) jako f:
                f.write(INPUT)
    ...
    def test_write_10(self):
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w") jako f:
                dla start w range(0, len(INPUT), 10):
    ...
    def test_write_append(self):
        ...
        part3 = INPUT[1536:]
        expected = b"".join(lzma.compress(x) dla x w (part1, part2, part3))
        przy BytesIO() jako dst:
            przy LZMAFile(dst, "w") jako f:
                f.write(part1)
            przy LZMAFile(dst, "a") jako f:
                f.write(part2)
            przy LZMAFile(dst, "a") jako f:
                f.write(part3)
            self.assertEqual(dst.getvalue(), expected)
    ...
    def test_write_to_file_with_bytes_filename(self):
        ...
        wyjąwszy UnicodeEncodeError:
            self.skipTest("Temporary file name needs to be ASCII")
        spróbuj:
            przy LZMAFile(bytes_filename, "w") jako f:
                f.write(INPUT)
            expected = lzma.compress(INPUT)
            przy open(TESTFN, "rb") jako f:
                self.assertEqual(f.read(), expected)
    ...
    def test_write_bad_args(self):
        ...
        przy LZMAFile(BytesIO(COMPRESSED_XZ), "r") jako f:
            self.assertRaises(ValueError, f.write, b"bar")
        przy LZMAFile(BytesIO(), "w") jako f:
            self.assertRaises(TypeError, f.write, Nic)
    ...
    def test_seek_forward_across_streams(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ * 2)) jako f:
    ...
    def test_seek_bad_args(self):
        ...
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            ...
            # io.BufferedReader podnieśs TypeError instead of ValueError
            self.assertRaises((TypeError, ValueError), f.seek, 9, ())
            self.assertRaises(TypeError, f.seek, Nic)
    ...
    def test_tell(self):
        przy LZMAFile(BytesIO(COMPRESSED_XZ)) jako f:
            ...
            self.assertEqual(f.tell(), len(INPUT))
        przy LZMAFile(BytesIO(), "w") jako f:
            dla pos w range(0, len(INPUT), 144):
                self.assertEqual(f.tell(), pos)
...
klasa OpenTestCase(unittest.TestCase):

    def test_binary_modes(self):
        przy lzma.open(BytesIO(COMPRESSED_XZ), "rb") jako f:
            self.assertEqual(f.read(), INPUT)
        przy BytesIO() jako bio:
            przy lzma.open(bio, "wb") jako f:
                f.write(INPUT)
            file_data = lzma.decompress(bio.getvalue())
            self.assertEqual(file_data, INPUT)
            przy lzma.open(bio, "ab") jako f:
                f.write(INPUT)
            file_data = lzma.decompress(bio.getvalue())
    ...
    def test_text_modes(self):
        ...
        przy lzma.open(BytesIO(COMPRESSED_XZ), "rt") jako f:
            self.assertEqual(f.read(), uncompressed)
        przy BytesIO() jako bio:
            przy lzma.open(bio, "wt") jako f:
                f.write(uncompressed)
            file_data = lzma.decompress(bio.getvalue()).decode("ascii")
            self.assertEqual(file_data, uncompressed_raw)
            przy lzma.open(bio, "at") jako f:
                f.write(uncompressed)
            file_data = lzma.decompress(bio.getvalue()).decode("ascii")
    ...
    def test_filename(self):
        przy TempFile(TESTFN):
            przy lzma.open(TESTFN, "wb") jako f:
                f.write(INPUT)
            przy open(TESTFN, "rb") jako f:
                file_data = lzma.decompress(f.read())
                self.assertEqual(file_data, INPUT)
            przy lzma.open(TESTFN, "rb") jako f:
                self.assertEqual(f.read(), INPUT)
            przy lzma.open(TESTFN, "ab") jako f:
                f.write(INPUT)
            przy lzma.open(TESTFN, "rb") jako f:
                self.assertEqual(f.read(), INPUT * 2)
    ...
    def test_format_and_filters(self):
        # Test non-default format oraz filter chain.
        options = {"format": lzma.FORMAT_RAW, "filters": FILTERS_RAW_1}
        przy lzma.open(BytesIO(COMPRESSED_RAW_1), "rb", **options) jako f:
            self.assertEqual(f.read(), INPUT)
        przy BytesIO() jako bio:
            przy lzma.open(bio, "wb", **options) jako f:
                f.write(INPUT)
    ...
    def test_encoding(self):
        ...
        uncompressed_raw = uncompressed.replace("\n", os.linesep)
        przy BytesIO() jako bio:
            przy lzma.open(bio, "wt", encoding="utf-16-le") jako f:
                f.write(uncompressed)
            file_data = lzma.decompress(bio.getvalue()).decode("utf-16-le")
            self.assertEqual(file_data, uncompressed_raw)
            bio.seek(0)
            przy lzma.open(bio, "rt", encoding="utf-16-le") jako f:
                self.assertEqual(f.read(), uncompressed)

    def test_encoding_error_handler(self):
        # Test wih non-default encoding error handler.
        przy BytesIO(lzma.compress(b"foo\xffbar")) jako bio:
            przy lzma.open(bio, "rt", encoding="ascii", errors="ignore") jako f:
    ...
    def test_newline(self):
        ...
        przy BytesIO() jako bio:
            przy lzma.open(bio, "wt", newline="\n") jako f:
                f.write(text)
            bio.seek(0)
            przy lzma.open(bio, "rt", newline="\r") jako f:
...
klasa MiscellaneousTestCase(unittest.TestCase):

    def test_is_check_supported(self):
        # CHECK_NONE oraz CHECK_CRC32 should always be supported,
        # regardless of the options liblzma was compiled with.
        self.assertPrawda(lzma.is_check_supported(lzma.CHECK_NONE))
        self.assertPrawda(lzma.is_check_supported(lzma.CHECK_CRC32))

        # The .xz format spec cannot store check IDs above this value.
        self.assertNieprawda(lzma.is_check_supported(lzma.CHECK_ID_MAX + 1))

        # This value should nie be a valid check ID.
        self.assertNieprawda(lzma.is_check_supported(lzma.CHECK_UNKNOWN))
    ...
    def test__encode_filter_properties(self):
        ...
        przy self.assertRaises(lzma.LZMAError):
            lzma._encode_filter_properties({"id": lzma.FILTER_DELTA,
                                           "dist": 9001})

        # Test przy parameters used by zipfile module.
...
LAERTES
       ...
       A double blessing jest a double grace,
       Occasion smiles upon a second leave.

LORD POLONIUS

       Yet here, Laertes! aboard, aboard, dla shame!
       The wind sits w the shoulder of your sail,
       And you are stay'd for. There; my blessing przy thee!
       And these few precepts w thy memory
       See thou character. Give thy thoughts no tongue,
       Nor any unproportioned thought his act.
       Be thou familiar, but by no means vulgar.
       Those friends thou hast, oraz their adoption tried,
       Grapple them to thy soul przy hoops of steel;
       But do nie dull thy palm przy entertainment
       Of each new-hatch'd, unfledged comrade. Beware
       Of entrance to a quarrel, but being in,
       Bear't that the opposed may beware of thee.
       Give every man thy ear, but few thy voice;
       Take each man's censure, but reserve thy judgment.
       Costly thy habit jako thy purse can buy,
       But nie express'd w fancy; rich, nie gaudy;
       For the apparel oft proclaims the man,
       And they w France of the best rank oraz station
       Are of a most select oraz generous chief w that.
       Neither a borrower nor a lender be;
       For loan oft loses both itself oraz friend,
       And borrowing dulls the edge of husbandry.
       This above all: to thine ownself be true,
       And it must follow, jako the night the day,
       Thou canst nie then be false to any man.
       Farewell: my blessing season this w thee!
...
LAERTES
       ...
       Farewell, Ophelia; oraz remember well
       What I have said to you.

OPHELIA

       'Tis w my memory lock'd,
...
COMPRESSED_BOGUS = b"this jest nie a valid lzma stream"
