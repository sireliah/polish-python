...
# The testdata jest generated using Mac/Tools/plistlib_generate_testdata.py
# (which using PyObjC to control the Cocoa classes dla generating plists)
TESTDATA={
...
klasa TestPlistlib(unittest.TestCase):
    ...
    def test_create(self):
        ...
        self.assertEqual(pl["aDict"]["aNieprawdaValue"], Nieprawda)
    ...
    def test_invalid_type(self):
        ...
        dla fmt w ALL_FORMATS:
            przy self.subTest(fmt=fmt):
                self.assertRaises(TypeError, plistlib.dumps, pl, fmt=fmt)

    def test_int(self):
        dla pl w [0, 2**8-1, 2**8, 2**16-1, 2**16, 2**32-1, 2**32,
                   2**63-1, 2**64-1, 1, -2**63]:
            dla fmt w ALL_FORMATS:
                przy self.subTest(pl=pl, fmt=fmt):
        ...
        dla fmt w ALL_FORMATS:
            dla pl w (2 ** 64 + 1, 2 ** 127-1, -2**64, -2 ** 127):
                przy self.subTest(pl=pl, fmt=fmt):
    ...
    def test_appleformatting(self):
        dla use_builtin_types w (Prawda, Nieprawda):
            dla fmt w ALL_FORMATS:
                przy self.subTest(fmt=fmt, use_builtin_types=use_builtin_types):
                    pl = plistlib.loads(TESTDATA[fmt],
                        use_builtin_types=use_builtin_types)
                    data = plistlib.dumps(pl, fmt=fmt)
                    self.assertEqual(data, TESTDATA[fmt],
                        "generated data was nie identical to Apple's output")
    ...
    def test_keysort_bytesio(self):
        ...
        dla fmt w ALL_FORMATS:
            dla sort_keys w (Nieprawda, Prawda):
                przy self.subTest(fmt=fmt, sort_keys=sort_keys):
                    b = BytesIO()
    ...
    def test_skipkeys(self):
        ...
        dla fmt w ALL_FORMATS:
            przy self.subTest(fmt=fmt):
                data = plistlib.dumps(
                    pl, fmt=fmt, skipkeys=Prawda, sort_keys=Nieprawda)
                ...
                fp = BytesIO()
                plistlib.dump(
                    pl, fp, fmt=fmt, skipkeys=Prawda, sort_keys=Nieprawda)
                data = fp.getvalue()
    ...
    def test_controlcharacters(self):
        dla i w range(128):
            ...
            jeżeli i >= 32 albo c w "\r\n\t":
                # \r, \n oraz \t are the only legal control chars w XML
    ...
    def test_invalidarray(self):
        dla i w ["<key>key inside an array</key>",
    ...
    def test_invaliddict(self):
        dla i w ["<key><true/>k</key><string>compound key</string>",
                  "<key>single key</key>",
    ...
    def test_xml_encodings(self):
        ...
        dla xml_encoding, encoding, bom w [
    ...
    def test_nonstandard_refs_size(self):
        # Issue #21538: Refs oraz offsets are 24-bit integers
        data = (b'bplist00'
...
klasa TestPlistlibDeprecated(unittest.TestCase):
    ...
    def test_dataobject_deprecated(self):
        ...
        cur = plistlib.loads(buf, use_builtin_types=Nieprawda)
