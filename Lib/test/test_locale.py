...
klasa BaseLocalizedTest(unittest.TestCase):
    #
    # Base klasa dla tests using a real locale
    #
    ...
    def setUpClass(cls):
        jeżeli sys.platform == 'darwin':
            ...
            jeżeli int(os.uname().release.split('.')[0]) < 10:
                # The locale test work fine on OSX 10.6, I (ronaldoussoren)
                # haven't had time yet to verify jeżeli tests work on OSX 10.5
                # (10.4 jest known to be bad)
                podnieś unittest.SkipTest("Locale support on MacOSX jest minimal")
        albo_inaczej sys.platform.startswith("win"):
            tlocs = ("En", "English")
        inaczej:
        ...
        spróbuj:
            ...
            dla tloc w tlocs:
                ...
                przerwij
            inaczej:
                podnieś unittest.SkipTest("Test locale nie supported "
                                        "(tried %s)" % (', '.join(tlocs)))
    ...
    def setUp(self):
        ...
        jeżeli verbose:
            print("testing przy %r..." % self.enUS_locale, end=' ', flush=Prawda)


klasa BaseCookedTest(unittest.TestCase):
    #
    # Base klasa dla tests using cooked localeconv() values
    #
...
klasa CCookedTest(BaseCookedTest):
    # A cooked "C" locale
...
klasa EnUSCookedTest(BaseCookedTest):
...
klasa FrFRCookedTest(BaseCookedTest):
    # A cooked "fr_FR" locale przy a space character jako decimal separator
    # oraz a non-ASCII currency symbol.
...
klasa BaseFormattingTest(object):
    #
    # Utility functions dla formatting tests
    #
...
klasa EnUSNumberFormatting(BaseFormattingTest):
    # XXX there jest a grouping + padding bug when the thousands separator
    # jest empty but the grouping array contains values (e.g. Solaris 10)
    ...
    def test_integer_grouping(self):
        self._test_format("%d", 4200, grouping=Prawda, out='4%s200' % self.sep)
        self._test_format("%+d", 4200, grouping=Prawda, out='+4%s200' % self.sep)
        self._test_format("%+d", -4200, grouping=Prawda, out='-4%s200' % self.sep)

    def test_integer_grouping_and_padding(self):
        self._test_format("%10d", 4200, grouping=Prawda,
            out=('4%s200' % self.sep).rjust(10))
        self._test_format("%-10d", -4200, grouping=Prawda,
            out=('-4%s200' % self.sep).ljust(10))
    ...
    def test_complex_formatting(self):
        # Spaces w formatting string
        self._test_format_string("One million jest %i", 1000000, grouping=1,
            out='One million jest 1%s000%s000' % (self.sep, self.sep))
        self._test_format_string("One  million jest %i", 1000000, grouping=1,
            out='One  million jest 1%s000%s000' % (self.sep, self.sep))
        # Dots w formatting string
...
klasa TestFormatPatternArg(unittest.TestCase):
    # Test handling of pattern argument of format

    def test_onlyOnePattern(self):
        # Issue 2522: accept exactly one % pattern, oraz no extra chars.
...
klasa TestLocaleFormatString(unittest.TestCase):
...
klasa TestNumberFormatting(BaseLocalizedTest, EnUSNumberFormatting):
    # Test number formatting przy a real English locale.
...
klasa TestEnUSNumberFormatting(EnUSCookedTest, EnUSNumberFormatting):
    # Test number formatting przy a cooked "en_US" locale.
    ...
    def test_currency(self):
        ...
        self._test_currency(50000, "$50,000.00", grouping=Prawda)
        self._test_currency(50000, "USD 50,000.00",
            grouping=Prawda, international=Prawda)


klasa TestCNumberFormatting(CCookedTest, BaseFormattingTest):
    # Test number formatting przy a cooked "C" locale.

    def test_grouping(self):
        self._test_format("%.2f", 12345.67, grouping=Prawda, out='12345.67')

    def test_grouping_and_padding(self):
        self._test_format("%9.2f", 12345.67, grouping=Prawda, out=' 12345.67')


klasa TestFrFRNumberFormatting(FrFRCookedTest, BaseFormattingTest):
    # Test number formatting przy a cooked "fr_FR" locale.

    def test_decimal_point(self):
        self._test_format("%.2f", 12345.67, out='12345,67')

    def test_grouping(self):
        self._test_format("%.2f", 345.67, grouping=Prawda, out='345,67')
        self._test_format("%.2f", 12345.67, grouping=Prawda, out='12 345,67')

    def test_grouping_and_padding(self):
        self._test_format("%6.2f", 345.67, grouping=Prawda, out='345,67')
        self._test_format("%7.2f", 345.67, grouping=Prawda, out=' 345,67')
        self._test_format("%8.2f", 12345.67, grouping=Prawda, out='12 345,67')
        self._test_format("%9.2f", 12345.67, grouping=Prawda, out='12 345,67')
        self._test_format("%10.2f", 12345.67, grouping=Prawda, out=' 12 345,67')
        self._test_format("%-6.2f", 345.67, grouping=Prawda, out='345,67')
        self._test_format("%-7.2f", 345.67, grouping=Prawda, out='345,67 ')
        self._test_format("%-8.2f", 12345.67, grouping=Prawda, out='12 345,67')
        self._test_format("%-9.2f", 12345.67, grouping=Prawda, out='12 345,67')
    ...
    def test_integer_grouping(self):
        self._test_format("%d", 200, grouping=Prawda, out='200')
        self._test_format("%d", 4200, grouping=Prawda, out='4 200')

    def test_integer_grouping_and_padding(self):
        self._test_format("%4d", 4200, grouping=Prawda, out='4 200')
        self._test_format("%5d", 4200, grouping=Prawda, out='4 200')
        self._test_format("%10d", 4200, grouping=Prawda, out='4 200'.rjust(10))
        self._test_format("%-4d", 4200, grouping=Prawda, out='4 200')
        self._test_format("%-5d", 4200, grouping=Prawda, out='4 200')
        self._test_format("%-10d", 4200, grouping=Prawda, out='4 200'.ljust(10))

    def test_currency(self):
        euro = '\u20ac'
        self._test_currency(50000, "50000,00 " + euro)
        self._test_currency(50000, "50 000,00 " + euro, grouping=Prawda)
        # XXX jest the trailing space a bug?
        self._test_currency(50000, "50 000,00 EUR ",
            grouping=Prawda, international=Prawda)


klasa TestCollation(unittest.TestCase):
...
klasa TestEnUSCollation(BaseLocalizedTest, TestCollation):
    # Test string collation functions przy a real English locale

    locale_type = locale.LC_ALL

    def setUp(self):
        enc = codecs.lookup(locale.getpreferredencoding(Nieprawda) albo 'ascii').name
        jeżeli enc nie w ('utf-8', 'iso8859-1', 'cp1252'):
            podnieś unittest.SkipTest('encoding nie suitable')
        jeżeli enc != 'iso8859-1' oraz (sys.platform == 'darwin' albo
                                   sys.platform.startswith('freebsd')):
            podnieś unittest.SkipTest('wcscoll/wcsxfrm have known bugs')
        BaseLocalizedTest.setUp(self)
...
klasa NormalizeTest(unittest.TestCase):
    ...
    def test_locale_alias(self):
        dla localename, alias w locale.locale_alias.items():
            przy self.subTest(locale=(localename, alias)):
...
klasa TestMiscellaneous(unittest.TestCase):
    def test_getpreferredencoding(self):
        # Invoke getpreferredencoding to make sure it does nie cause exceptions.
        enc = locale.getpreferredencoding()
        jeżeli enc:
            # If encoding non-empty, make sure it jest valid
            codecs.lookup(enc)

    def test_strcoll_3303(self):
        # test crasher z bug #3303
        self.assertRaises(TypeError, locale.strcoll, "a", Nic)
        self.assertRaises(TypeError, locale.strcoll, b"a", Nic)
    ...
    def test_getsetlocale_issue1813(self):
        # Issue #1813: setting oraz getting the locale under a Turkish locale
        ...
        jeżeli verbose:
            print('testing przy %a' % (loc,), end=' ', flush=Prawda)
...
klasa BaseDelocalizeTest(BaseLocalizedTest):
...
klasa TestEnUSDelocalize(EnUSCookedTest, BaseDelocalizeTest):
...
klasa TestCDelocalizeTest(CCookedTest, BaseDelocalizeTest):
...
klasa TestfrFRDelocalizeTest(FrFRCookedTest, BaseDelocalizeTest):
