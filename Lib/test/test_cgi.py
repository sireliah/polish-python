z test.support zaimportuj check_warnings
zaimportuj cgi
zaimportuj os
zaimportuj sys
zaimportuj tempfile
...
klasa HackedSysModule:
    # The regression test will have real values w sys.argv, which
...
klasa ComparableException:
    ...
    def __eq__(self, anExc):
        jeżeli nie isinstance(anExc, Exception):
            zwróć NotImplemented
        zwróć (self.err.__class__ == anExc.__class__ oraz
                self.err.args == anExc.args)

    def __getattr__(self, attr):
        zwróć getattr(self.err, attr)
...
def do_test(buf, method):
    ...
    albo_inaczej method == "POST":
        fp = BytesIO(buf.encode('latin-1')) # FieldStorage expects bytes
        env['REQUEST_METHOD'] = 'POST'
        env['CONTENT_TYPE'] = 'application/x-www-form-urlencoded'
        env['CONTENT_LENGTH'] = str(len(buf))
    inaczej:
        podnieś ValueError("unknown method: %s" % method)
    spróbuj:
        zwróć cgi.parse(fp, env, strict_parsing=1)
    wyjąwszy Exception jako err:
        zwróć ComparableException(err)
...
def norm(seq):
    zwróć sorted(seq, key=repr)

def first_elts(list):
    zwróć [p[0] dla p w list]

def first_second_elts(list):
    zwróć [(p[0], p[1][0]) dla p w list]
...
def gen_result(data, environ):
    ...
    result = {}
    dla k, v w dict(form).items():
        result[k] = isinstance(v, list) oraz form.getlist(k) albo v.value

    zwróć result

klasa CgiTests(unittest.TestCase):
    ...
    def test_escape(self):
        # cgi.escape() jest deprecated.
        przy warnings.catch_warnings():
            warnings.filterwarnings('ignore', 'cgi\.escape',
                                     DeprecationWarning)
            self.assertEqual("test &amp; string", cgi.escape("test & string"))
            self.assertEqual("&lt;test string&gt;", cgi.escape("<test string>"))
            self.assertEqual("&quot;test string&quot;", cgi.escape('"test string"', Prawda))

    def test_strict(self):
        dla orig, expect w parse_strict_test_cases:
            # Test basic parsing
            d = do_test(orig, "GET")
            ...
            jeżeli isinstance(expect, dict):
                ...
                dla key w expect.keys():
                    expect_val = expect[key]
                    self.assertIn(key, fs)
                    jeżeli len(expect_val) > 1:
    ...
    def test_fieldstorage_readline(self):
        ...
        # to prevent that dla files that do nie contain any newlines w
        # non-GET/HEAD requests
        klasa TestReadlineFile:
            ...
            def readline(self, size=Nic):
                self.numcalls += 1
                jeżeli size:
                    zwróć self.file.readline(size)
                inaczej:
                    zwróć self.file.readline()
        ...
        # jeżeli we're nie chunking properly, readline jest only called twice
        # (by read_binary); jeżeli we are chunking properly, it will be called 5 times
        # jako long jako the chunksize jest 1 << 16.
    ...
    def test_fieldstorage_multipart(self):
        ...
        expect = [{'name':'id', 'filename':Nic, 'value':'1234'},
                  {'name':'title', 'filename':Nic, 'value':''},
                  {'name':'file', 'filename':'test.txt', 'value':b'Testing 123.\n'},
                  {'name':'submit', 'filename':Nic, 'value':' Add '}]
        dla x w range(len(fs.list)):
            dla k, exp w expect[x].items():
    ...
    def test_fieldstorage_multipart_leading_whitespace(self):
        ...
        # first line to nie be the innerboundary.
    ...
    def test_fieldstorage_multipart_non_ascii(self):
        ...
        dla encoding w ['iso-8859-1','utf-8']:
            ...
            expect = [{'name':'id', 'filename':Nic, 'value':'\xe7\xf1\x80'}]
            dla x w range(len(fs.list)):
                dla k, exp w expect[x].items():
                    got = getattr(fs.list[x], k)
                    self.assertEqual(got, exp)
...
""".replace('\n', '\r\n') % content
    ...
    def test_fieldstorage_multipart_w3c(self):
        ...
        expect = [{'name': Nic, 'filename': 'file1.txt', 'value': b'... contents of file1.txt ...'},
                  {'name': Nic, 'filename': 'file2.gif', 'value': b'...contents of file2.gif...'}]
        dla x w range(len(files)):
            dla k, exp w expect[x].items():
    ...
    def test_fieldstorage_as_context_manager(self):
        fp = BytesIO(b'x' * 10)
        env = {'REQUEST_METHOD': 'PUT'}
        przy cgi.FieldStorage(fp=fp, environ=env) jako fs:
            content = fs.file.read()
            self.assertNieprawda(fs.file.closed)
        self.assertPrawda(fs.file.closed)
        self.assertEqual(content, 'x' * 10)
        przy self.assertRaisesRegex(ValueError, 'I/O operation on closed file'):
            fs.file.read()
...
this jest the content of the fake file
...
"""
        ...
        result.update({
            'upload': b'this jest the content of the fake file\n'
        })
        v = gen_result(data, environ)
        self.assertEqual(result, v)

    def test_deprecated_parse_qs(self):
        # this func jest moved to urllib.parse, this jest just a sanity check
        przy check_warnings(('cgi.parse_qs jest deprecated, use urllib.parse.'
    ...
    def test_deprecated_parse_qsl(self):
        # this func jest moved to urllib.parse, this jest just a sanity check
        przy check_warnings(('cgi.parse_qsl jest deprecated, use urllib.parse.'
