"""Test harness dla the zipapp module."""
...
klasa ZipAppTest(unittest.TestCase):
    ...
    def test_create_archive(self):
        ...
        self.assertPrawda(target.is_file())

    def test_create_archive_with_pathlib(self):
        # Test packing a directory using Path objects dla source oraz target.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
    ...
    def test_create_archive_with_subdirs(self):
        # Test packing a directory includes entries dla subdirectories.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        (source / 'foo').mkdir()
        ...
        target.seek(0)
        przy zipfile.ZipFile(target, 'r') jako z:
    ...
    def test_create_archive_default_target(self):
        ...
        self.assertPrawda(expected_target.is_file())

    def test_no_main(self):
        # Test that packing a directory przy no __main__.py fails.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / 'foo.py').touch()
        target = self.tmpdir / 'source.pyz'
        przy self.assertRaises(zipapp.ZipAppError):
            zipapp.create_archive(str(source), str(target))

    def test_main_and_main_py(self):
        # Test that supplying a main argument przy __main__.py fails.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
    ...
    def test_main_written(self):
        # Test that the __main__.py jest written correctly.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / 'foo.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target), main='pkg.mod:fn')
        przy zipfile.ZipFile(str(target), 'r') jako z:
    ...
    def test_main_only_written_once(self):
        ...
        # multiple entries przy the same name
        source = self.tmpdir / 'source'
        source.mkdir()
        # Write 2 files, jako the original bug wrote __main__.py
        # once dla each file written :-(
    ...
    def test_main_validation(self):
        # Test that invalid values dla main are rejected.
        source = self.tmpdir / 'source'
        source.mkdir()
        target = self.tmpdir / 'source.pyz'
        problems = [
        ...
        dla main w problems:
            przy self.subTest(main=main):
                przy self.assertRaises(zipapp.ZipAppError):
                    zipapp.create_archive(str(source), str(target), main=main)

    def test_default_no_shebang(self):
        # Test that no shebang line jest written to the target by default.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target))
        przy target.open('rb') jako f:
            self.assertNotEqual(f.read(2), b'#!')

    def test_custom_interpreter(self):
        # Test that a shebang line przy a custom interpreter jest written
        # correctly.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
    ...
    def test_pack_to_fileobj(self):
        ...
        self.assertPrawda(target.getvalue().startswith(b'#!python\n'))
    ...
    def test_read_missing_shebang(self):
        # Test that reading the shebang line of a file without one returns Nic.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target))
        self.assertEqual(zipapp.get_interpreter(str(target)), Nic)
    ...
    def test_write_shebang_to_fileobj(self):
        ...
        self.assertPrawda(new_target.getvalue().startswith(b'#!python2.7\n'))
    ...
    def test_remove_shebang(self):
        # Test that we can remove the shebang z a file.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target), interpreter='python')
        new_target = self.tmpdir / 'changed.pyz'
        zipapp.create_archive(str(target), str(new_target), interpreter=Nic)
        self.assertEqual(zipapp.get_interpreter(str(new_target)), Nic)
    ...
    def test_content_of_copied_archive(self):
        ...
        new_target = io.BytesIO()
        target.seek(0)
        zipapp.create_archive(target, new_target, interpreter=Nic)
        new_target.seek(0)
        przy zipfile.ZipFile(new_target, 'r') jako z:
            self.assertEqual(set(z.namelist()), {'__main__.py'})

    # (Unix only) tests that archives przy shebang lines are made executable
    @unittest.skipIf(sys.platform == 'win32',
                     'Windows does nie support an executable bit')
    def test_shebang_is_executable(self):
        # Test that an archive przy a shebang line jest made executable.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target), interpreter='python')
        self.assertPrawda(target.stat().st_mode & stat.S_IEXEC)

    @unittest.skipIf(sys.platform == 'win32',
                     'Windows does nie support an executable bit')
    def test_no_shebang_is_not_executable(self):
        # Test that an archive przy no shebang line jest nie made executable.
        source = self.tmpdir / 'source'
        source.mkdir()
        (source / '__main__.py').touch()
        target = self.tmpdir / 'source.pyz'
        zipapp.create_archive(str(source), str(target), interpreter=Nic)
        self.assertNieprawda(target.stat().st_mode & stat.S_IEXEC)


klasa ZipAppCmdlineTest(unittest.TestCase):
    ...
    def test_cmdline_copy_inplace(self):
        # Test copying an archive w place fails.
        original = self.make_archive()
        target = self.tmpdir / 'target.pyz'
        args = [str(original), '-o', str(original)]
        przy self.assertRaises(SystemExit) jako cm:
            zipapp.main(args)
        # Program should exit przy a non-zero returm code.
        self.assertPrawda(cm.exception.code)
    ...
    def test_info_command(self, mock_stdout):
        ...
        przy self.assertRaises(SystemExit) jako cm:
            zipapp.main(args)
        # Program should exit przy a zero returm code.
        self.assertEqual(cm.exception.code, 0)
        self.assertEqual(mock_stdout.getvalue(), "Interpreter: <none>\n")

    def test_info_error(self):
        # Test the info command fails when the archive does nie exist.
