...
jeżeli sys.platform == 'win32':
    podnieś unittest.SkipTest('UNIX only')


zaimportuj asyncio
z asyncio zaimportuj log
...
def close_pipe_transport(transport):
    ...
    # are mocked
    jeżeli transport._pipe jest Nic:
        zwróć
    transport._pipe.close()
    transport._pipe = Nic


@unittest.skipUnless(signal, 'Signals are nie supported')
klasa SelectorEventLoopSignalTests(test_utils.TestCase):
    ...
    def test_add_signal_handler_coroutine_error(self, m_signal):
        ...
        # callback must nie be a coroutine function
        coro_func = simple_coroutine
        coro_obj = coro_func()
        self.addCleanup(coro_obj.close)
        dla func w (coro_func, coro_obj):
            self.assertRaisesRegex(
                TypeError, 'coroutines cannot be used przy add_signal_handler',
                self.loop.add_signal_handler,
                signal.SIGINT, func)
    ...
    def test_add_signal_handler_install_error2(self, m_logging, m_signal):
        ...
        self.loop._signal_handlers[signal.SIGHUP] = lambda: Prawda
        self.assertRaises(
            RuntimeError,
            self.loop.add_signal_handler,
            signal.SIGINT, lambda: Prawda)
        self.assertNieprawda(m_logging.info.called)
    ...
    def test_remove_signal_handler(self, m_signal):
        ...
        self.loop.add_signal_handler(signal.SIGHUP, lambda: Prawda)

        self.assertPrawda(
            self.loop.remove_signal_handler(signal.SIGHUP))
        self.assertPrawda(m_signal.set_wakeup_fd.called)
        self.assertPrawda(m_signal.signal.called)
        self.assertEqual(
    ...
    def test_remove_signal_handler_2(self, m_signal):
        ...
        self.loop.add_signal_handler(signal.SIGINT, lambda: Prawda)
        self.loop._signal_handlers[signal.SIGHUP] = object()
        m_signal.set_wakeup_fd.reset_mock()

        self.assertPrawda(
            self.loop.remove_signal_handler(signal.SIGINT))
        self.assertNieprawda(m_signal.set_wakeup_fd.called)
        self.assertPrawda(m_signal.signal.called)
        self.assertEqual(
    ...
    def test_remove_signal_handler_cleanup_error(self, m_logging, m_signal):
        ...
        self.loop.remove_signal_handler(signal.SIGHUP)
        self.assertPrawda(m_logging.info)
    ...
    def test_close(self, m_signal):
        ...
        self.loop.add_signal_handler(signal.SIGHUP, lambda: Prawda)
        self.loop.add_signal_handler(signal.SIGCHLD, lambda: Prawda)
...
@unittest.skipUnless(hasattr(socket, 'AF_UNIX'),
                     'UNIX Sockets are nie supported')
klasa SelectorEventLoopUnixSocketTests(test_utils.TestCase):
    ...
    def test_create_unix_server_existing_path_sock(self):
        przy test_utils.unix_socket_path() jako path:
            sock = socket.socket(socket.AF_UNIX)
            sock.bind(path)
            przy sock:
                coro = self.loop.create_unix_server(lambda: Nic, path)
                przy self.assertRaisesRegex(OSError,
                                            'Address.*is already w use'):
                    self.loop.run_until_complete(coro)

    def test_create_unix_server_existing_path_nonsock(self):
        przy tempfile.NamedTemporaryFile() jako file:
            coro = self.loop.create_unix_server(lambda: Nic, file.name)
            przy self.assertRaisesRegex(OSError,
                                        'Address.*is already w use'):
                self.loop.run_until_complete(coro)

    def test_create_unix_server_ssl_bool(self):
        coro = self.loop.create_unix_server(lambda: Nic, path='spam',
                                            ssl=Prawda)
    ...
    def test_create_unix_server_nopath_nosock(self):
        coro = self.loop.create_unix_server(lambda: Nic, path=Nic)
        przy self.assertRaisesRegex(ValueError,
                                    'path was nie specified, oraz no sock'):
            self.loop.run_until_complete(coro)
    ...
    def test_create_unix_server_bind_error(self, m_socket):
        # Ensure that the socket jest closed on any bind error
        sock = mock.Mock()
        m_socket.socket.return_value = sock

        sock.bind.side_effect = OSError
        coro = self.loop.create_unix_server(lambda: Nic, path="/test")
        ...
        coro = self.loop.create_unix_server(lambda: Nic, path="/test")
        przy self.assertRaises(MemoryError):
            self.loop.run_until_complete(coro)
        self.assertPrawda(sock.close.called)

    def test_create_unix_connection_path_sock(self):
        coro = self.loop.create_unix_connection(
            lambda: Nic, os.devnull, sock=object())
        przy self.assertRaisesRegex(ValueError, 'path oraz sock can nie be'):
            self.loop.run_until_complete(coro)
    ...
    def test_create_unix_connection_nopath_nosock(self):
        coro = self.loop.create_unix_connection(
            lambda: Nic, Nic)
        przy self.assertRaisesRegex(ValueError,
                                    'no path oraz sock were specified'):
            self.loop.run_until_complete(coro)

    def test_create_unix_connection_nossl_serverhost(self):
        coro = self.loop.create_unix_connection(
            lambda: Nic, os.devnull, server_hostname='spam')
        przy self.assertRaisesRegex(ValueError,
                                    'server_hostname jest only meaningful'):
            self.loop.run_until_complete(coro)

    def test_create_unix_connection_ssl_noserverhost(self):
        coro = self.loop.create_unix_connection(
            lambda: Nic, os.devnull, ssl=Prawda)

        przy self.assertRaisesRegex(
            ValueError, 'you have to dalej server_hostname when using ssl'):

            self.loop.run_until_complete(coro)


klasa UnixReadPipeTransportTests(test_utils.TestCase):
    ...
    def read_pipe_transport(self, waiter=Nic):
    ...
    def test_close(self, m_read):
        ...
        tr._close = mock.Mock()
        tr.close()
        tr._close.assert_called_with(Nic)
    ...
    def test_close_already_closing(self, m_read):
        ...
        tr._close = mock.Mock()
        tr.close()
        self.assertNieprawda(tr._close.called)
...
klasa UnixWritePipeTransportTests(test_utils.TestCase):
    ...
    def write_pipe_transport(self, waiter=Nic):
    ...
    def test_ctor(self):
        ...
        self.protocol.connection_made.assert_called_with(tr)
        self.loop.assert_reader(5, tr._read_ready)
        self.assertEqual(Nic, waiter.result())
    ...
    def test_write_no_data(self, m_write):
        tr = self.write_pipe_transport()
        tr.write(b'')
        self.assertNieprawda(m_write.called)
        self.assertNieprawda(self.loop.writers)
        self.assertEqual([], tr._buffer)
    ...
    def test_write_err(self, m_write, m_log):
        ...
        tr.write(b'data')
        tr.write(b'data')
        # This jest a bit overspecified. :-(
        m_log.warning.assert_called_with(
            'pipe closed by peer albo os.write(pipe, data) podnieśd exception.')
        tr.close()
    ...
    def test_close(self):
        ...
        # closing the transport twice must nie fail
        tr.close()
    ...
    def test_close_closing(self):
        ...
        tr._closing = Prawda
        tr.close()
        self.assertNieprawda(tr.write_eof.called)
...
klasa AbstractChildWatcherTests(unittest.TestCase):
...
klasa BaseChildWatcherTests(unittest.TestCase):
...
klasa ChildWatcherTestsMixin:
    ...
    def setUp(self):
        ...
        przy mock.patch.object(
                self.loop, "add_signal_handler") jako self.m_add_signal_handler:
            self.watcher = self.create_watcher()
            self.watcher.attach_loop(self.loop)

    def waitpid(self, pid, flags):
        jeżeli isinstance(self.watcher, asyncio.SafeChildWatcher) albo pid != -1:
            self.assertGreater(pid, 0)
        spróbuj:
            jeżeli pid < 0:
                zwróć self.zombies.popitem()
            inaczej:
                zwróć pid, self.zombies.pop(pid)
        wyjąwszy KeyError:
            dalej
        jeżeli self.running:
            zwróć 0, 0
        inaczej:
            podnieś ChildProcessError()
    ...
    def WIFSIGNALED(self, status):
        zwróć 32700 < status < 32768

    def WEXITSTATUS(self, status):
        self.assertPrawda(self.WIFEXITED(status))
        zwróć status - 32768

    def WTERMSIG(self, status):
        self.assertPrawda(self.WIFSIGNALED(status))
        zwróć 32768 - status
    ...
    def waitpid_mocks(func):
        def wrapped_func(self):
            def patch(target, wrapper):
                zwróć mock.patch(target, wraps=wrapper,
                                  new_callable=mock.Mock)

            przy patch('os.WTERMSIG', self.WTERMSIG) jako m_WTERMSIG, \
                 patch('os.WEXITSTATUS', self.WEXITSTATUS) jako m_WEXITSTATUS, \
                 patch('os.WIFSIGNALED', self.WIFSIGNALED) jako m_WIFSIGNALED, \
                 patch('os.WIFEXITED', self.WIFEXITED) jako m_WIFEXITED, \
                 patch('os.waitpid', self.waitpid) jako m_waitpid:
    ...
    def test_sigchld(self, m):
        ...
        self.assertNieprawda(callback.called)
        self.assertNieprawda(m.WIFEXITED.called)
        self.assertNieprawda(m.WIFSIGNALED.called)
        self.assertNieprawda(m.WEXITSTATUS.called)
        self.assertNieprawda(m.WTERMSIG.called)
        ...
        self.assertPrawda(m.WIFEXITED.called)
        self.assertPrawda(m.WEXITSTATUS.called)
        self.assertNieprawda(m.WTERMSIG.called)
        ...
        # ensure that the child jest effectively reaped
        self.add_zombie(42, 13)
        przy self.ignore_warnings:
            self.watcher._sig_chld()
    ...
    def test_sigchld_two_children(self, m):
        ...
        self.assertNieprawda(callback1.called)
        self.assertNieprawda(callback2.called)
        self.assertNieprawda(m.WIFEXITED.called)
        self.assertNieprawda(m.WIFSIGNALED.called)
        self.assertNieprawda(m.WEXITSTATUS.called)
        self.assertNieprawda(m.WTERMSIG.called)
        ...
        self.assertNieprawda(callback2.called)
        self.assertPrawda(m.WIFSIGNALED.called)
        self.assertNieprawda(m.WEXITSTATUS.called)
        self.assertPrawda(m.WTERMSIG.called)
    ...
    def test_remove_child_handler(self, m):
        ...
        self.assertPrawda(self.watcher.remove_child_handler(54))

        # remove child handler 2 multiple times
        self.assertPrawda(self.watcher.remove_child_handler(55))
        self.assertNieprawda(self.watcher.remove_child_handler(55))
        self.assertNieprawda(self.watcher.remove_child_handler(55))
    ...
    def test_sigchld_unhandled_exception(self, m):
        ...
        przy mock.patch.object(log.logger,
                               'error') jako m_error:

            self.assertEqual(self.watcher._sig_chld(), Nic)
            self.assertPrawda(m_error.called)

    @waitpid_mocks
    def test_sigchld_child_reaped_inaczejwhere(self, m):
        # register a child
        callback = mock.Mock()
        ...
        # waitpid jest called inaczejwhere
        ...
        jeżeli isinstance(self.watcher, asyncio.FastChildWatcher):
            # here the FastChildWatche enters a deadlock
            # (there jest no way to prevent it)
            self.assertNieprawda(callback.called)
        inaczej:
    ...
    def test_sigchld_unknown_pid_during_registration(self, m):
        ...
        przy self.ignore_warnings, self.watcher:
            self.running = Prawda
            # child 1 terminates
    ...
    def test_set_loop(self, m):
        ...
        przy patch(old_loop, "remove_signal_handler") jako m_old_remove, \
             patch(self.loop, "add_signal_handler") jako m_new_add:
    ...
    def test_set_loop_race_condition(self, m):
        ...
        przy mock.patch.object(
                old_loop, "remove_signal_handler") jako m_remove_signal_handler:

            self.watcher.attach_loop(Nic)
        ...
        # SIGCHLD was nie caught
        self.assertNieprawda(callback1.called)
        self.assertNieprawda(callback2.called)
        self.assertNieprawda(callback3.called)

        # attach a new loop
        self.loop = self.new_test_loop()

        przy mock.patch.object(
                self.loop, "add_signal_handler") jako m_add_signal_handler:
    ...
    def test_close(self, m):
        ...
        przy self.watcher:
            ...
            przy mock.patch.object(
                    self.loop,
                    "remove_signal_handler") jako m_remove_signal_handler:

                self.watcher.close()

                m_remove_signal_handler.assert_called_once_with(
                    signal.SIGCHLD)
                self.assertNieprawda(self.watcher._callbacks)
                jeżeli isinstance(self.watcher, asyncio.FastChildWatcher):
                    self.assertNieprawda(self.watcher._zombies)


klasa SafeChildWatcherTests (ChildWatcherTestsMixin, test_utils.TestCase):
    def create_watcher(self):
        zwróć asyncio.SafeChildWatcher()


klasa FastChildWatcherTests (ChildWatcherTestsMixin, test_utils.TestCase):
    def create_watcher(self):
        zwróć asyncio.FastChildWatcher()


klasa PolicyTests(unittest.TestCase):

    def create_policy(self):
        zwróć asyncio.DefaultEventLoopPolicy()

    def test_get_child_watcher(self):
        policy = self.create_policy()
        self.assertIsNic(policy._watcher)
        ...
        self.assertIsNic(watcher._loop)
    ...
    def test_child_watcher_replace_mainloop_existing(self):
        ...
        self.assertIs(watcher._loop, Nic)
