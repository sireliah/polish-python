...
zaimportuj asyncio
z asyncio.proactor_events zaimportuj BaseProactorEventLoop
z asyncio.proactor_events zaimportuj _ProactorSocketTransport
z asyncio.proactor_events zaimportuj _ProactorWritePipeTransport
z asyncio.proactor_events zaimportuj _ProactorDuplexPipeTransport
z asyncio zaimportuj test_utils


def close_transport(transport):
    # Don't call transport.close() because the event loop oraz the IOCP proactor
    # are mocked
...
klasa ProactorSocketTransportTests(test_utils.TestCase):
    ...
    def test_loop_reading(self):
        ...
        self.assertNieprawda(self.protocol.eof_received.called)
    ...
    def test_loop_reading_no_data(self):
        ...
        tr._read_fut = res
        tr._loop_reading(res)
        self.assertNieprawda(self.loop._proactor.recv.called)
        self.assertPrawda(self.protocol.eof_received.called)
        self.assertPrawda(tr.close.called)
    ...
    def test_loop_reading_aborted_closing(self):
        ...
        tr._fatal_error = mock.Mock()
        tr._loop_reading()
        self.assertNieprawda(tr._fatal_error.called)

    def test_loop_reading_aborted_is_fatal(self):
        self.loop._proactor.recv.side_effect = ConnectionAbortedError()
        tr = self.socket_transport()
        tr._closing = Nieprawda
        tr._fatal_error = mock.Mock()
        tr._loop_reading()
        self.assertPrawda(tr._fatal_error.called)
    ...
    def test_write(self):
        ...
        tr.write(b'data')
        self.assertEqual(tr._buffer, Nic)
        tr._loop_writing.assert_called_with(data=b'data')

    def test_write_no_data(self):
        tr = self.socket_transport()
        tr.write(b'')
        self.assertNieprawda(tr._buffer)
    ...
    def test_write_more(self):
        ...
        self.assertNieprawda(tr._loop_writing.called)
    ...
    def test_loop_writing_stop(self):
        ...
        self.assertIsNic(tr._write_fut)
    ...
    def test_fatal_error(self, m_logging):
        ...
        tr._force_close = mock.Mock()
        tr._fatal_error(Nic)
        self.assertPrawda(tr._force_close.called)
        self.assertPrawda(m_logging.error.called)
    ...
    def test_force_close(self):
        ...
        test_utils.run_briefly(self.loop)
        self.protocol.connection_lost.assert_called_with(Nic)
        self.assertEqual(Nic, tr._buffer)
        self.assertEqual(tr._conn_lost, 1)
    ...
    def test_call_connection_lost(self):
        tr = self.socket_transport()
        tr._call_connection_lost(Nic)
        self.assertPrawda(self.protocol.connection_lost.called)
        self.assertPrawda(self.sock.close.called)
    ...
    def test_write_eof_buffer(self):
        ...
        tr.write(b'data')
        tr.write_eof()
        self.assertPrawda(tr._eof_written)
    ...
    def test_pause_resume_reading(self):
        tr = self.socket_transport()
        futures = []
        dla msg w [b'data1', b'data2', b'data3', b'data4', b'']:
            f = asyncio.Future(loop=self.loop)
            f.set_result(msg)
            futures.append(f)
    ...
    def pause_writing_transport(self, high):
        ...
        self.assertNieprawda(self.protocol.pause_writing.called)
        self.assertNieprawda(self.protocol.resume_writing.called)
        zwróć tr
    ...
    def test_pause_resume_writing(self):
        ...
        self.loop._run_once()
        self.assertPrawda(self.protocol.pause_writing.called)

        # flush the buffer
        fut.set_result(Nic)
        self.loop._run_once()
        self.assertEqual(tr.get_write_buffer_size(), 0)
        self.assertPrawda(self.protocol.resume_writing.called)

    def test_pause_writing_2write(self):
        tr = self.pause_writing_transport(high=4)

        # first short write, the buffer jest nie full (3 <= 4)
    ...
    def test_pause_writing_3write(self):
        ...
        # first short write, the buffer jest nie full (1 <= 4)
        fut = asyncio.Future(loop=self.loop)
        self.loop._proactor.send.return_value = fut
        tr.write(b'1')
        self.loop._run_once()
        ...
        # second short write, the buffer jest nie full (3 <= 4)
        tr.write(b'23')
        self.loop._run_once()
        self.assertEqual(tr.get_write_buffer_size(), 3)
        self.assertNieprawda(self.protocol.pause_writing.called)
...
klasa BaseProactorEventLoopTests(test_utils.TestCase):
    ...
    def setUp(self):
        ...
        klasa EventLoop(BaseProactorEventLoop):
            def _socketpair(s):
                zwróć (self.ssock, self.csock)
    ...
    def test_close_self_pipe(self):
        ...
        self.assertPrawda(self.ssock.close.called)
        self.assertPrawda(self.csock.close.called)
        ...
        self.loop._closed = Prawda

    def test_close(self):
        self.loop._close_self_pipe = mock.Mock()
        self.loop.close()
        self.assertPrawda(self.loop._close_self_pipe.called)
        self.assertPrawda(self.proactor.close.called)
        self.assertIsNic(self.loop._proactor)

        self.loop._close_self_pipe.reset_mock()
        self.loop.close()
        self.assertNieprawda(self.loop._close_self_pipe.called)
    ...
    def test_socketpair(self):
        klasa EventLoop(BaseProactorEventLoop):
            # override the destructor to nie log a ResourceWarning
            def __del__(self):
                dalej
        self.assertRaises(
    ...
    def test_loop_self_reading_fut(self):
        ...
        self.assertPrawda(fut.result.called)
        self.proactor.recv.assert_called_with(self.ssock, 4096)
        self.proactor.recv.return_value.add_done_callback.assert_called_with(
            self.loop._loop_self_reading)
    ...
    def test_loop_self_reading_exception(self):
        ...
        self.loop._loop_self_reading()
        self.assertPrawda(self.loop.call_exception_handler.called)
    ...
    def test_create_server(self, m_log):
        ...
        self.assertPrawda(call_soon.called)
        ...
        loop(fut)
        self.assertPrawda(fut.result.called)
        self.assertPrawda(make_tr.called)
