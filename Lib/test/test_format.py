z test.support zaimportuj verbose, TestFailed
zaimportuj locale
zaimportuj sys
...
# test string formatting operator (I am nie sure jeżeli this jest being tested
# inaczejwhere but, surely, some of the given cases are *not* tested because
# they crash python)
# test on bytes object jako well

def testformat(formatstr, args, output=Nic, limit=Nic, overflowok=Nieprawda):
    jeżeli verbose:
        jeżeli output:
    ...
    wyjąwszy OverflowError:
        jeżeli nie overflowok:
            podnieś
        jeżeli verbose:
            print('overflow (this jest fine)')
    inaczej:
        jeżeli output oraz limit jest Nic oraz result != output:
            jeżeli verbose:
                print('no')
            podnieś AssertionError("%r %% %r == %r != %r" %
                                (formatstr, args, result, output))
        # when 'limit' jest specified, it determines how many characters
        # must match exactly; lengths must always match.
        # ex: limit=5, '12345678' matches '12345___'
        # (mainly dla floating point format tests dla which an exact match
        # can't be guaranteed due to rounding oraz representation errors)
        albo_inaczej output oraz limit jest nie Nic oraz (
                len(result)!=len(output) albo result[:limit]!=output[:limit]):
            jeżeli verbose:
                print('no')
            print("%s %% %s == %s != %s" % \
...
def testcommon(formatstr, args, output=Nic, limit=Nic, overflowok=Nieprawda):
    # jeżeli formatstr jest a str, test str, bytes, oraz bytearray;
    # otherwise, test bytes oraz bytearry
    jeżeli isinstance(formatstr, str):
...
klasa FormatTest(unittest.TestCase):
    ...
    def test_common_format(self):
        ...
        # str, bytes, oraz bytearrays (integer, float, oct, hex)
        testcommon("%.1d", (1,), "1")
        testcommon("%.*d", (sys.maxsize,1), overflowok=Prawda)  # expect overflow
        testcommon("%.100d", (1,), '00000000000000000000000000000000000000'
                 '000000000000000000000000000000000000000000000000000000'
                 '00000001', overflowok=Prawda)
        ...
        # formatting buffer, jeżeli that number changes then these tests are less
        # effective
        ...
        # check dla internal overflow validation on length of precision
        # these tests should no longer cause overflow w Python
        # 2.7/3.1 oraz later.
        testcommon("%#.*g", (110, -1.e+100/3.))
        testcommon("%#.*G", (110, -1.e+100/3.))
        testcommon("%#.*f", (110, -1.e+100/3.))
        testcommon("%#.*F", (110, -1.e+100/3.))
        # Formatting of integers. Overflow jest nie ok
        testcommon("%x", 10, "a")
        ...
        testcommon("%#+27.23X", big, " +0X001234567890ABCDEF12345")
        # next one gets two leading zeroes z precision, oraz another z the
        # 0 flag oraz the width
        testcommon("%#+027.23X", big, "+0X0001234567890ABCDEF12345")
        # same, wyjąwszy no 0 flag
        testcommon("%#+27.23X", big, " +0X001234567890ABCDEF12345")
        ...
        testcommon("%#+37.34o", big, "+0o0012345670123456701234567012345670")
        # next one gets one leading zero z precision
        testcommon("%.33o", big, "012345670123456701234567012345670")
        # base marker shouldn't change that, since "0" jest redundant
        testcommon("%#.33o", big, "0o012345670123456701234567012345670")
        # but reduce precision, oraz base marker should add a zero
        testcommon("%#.32o", big, "0o12345670123456701234567012345670")
        # one leading zero z precision, oraz another z "0" flag & width
        ...
        # Some small ints, w both Python int oraz flavors).
        testcommon("%d", 42, "42")
        testcommon("%d", -42, "-42")
        testcommon("%d", 42, "42")
        testcommon("%d", -42, "-42")
    ...
    def test_str_format(self):
        ...
        # Test exception dla unknown format characters, etc.
        jeżeli verbose:
        ...
        def test_exc(formatstr, args, exception, excmsg):
            ...
            wyjąwszy exception jako exc:
                jeżeli str(exc) == excmsg:
                    jeżeli verbose:
                        print("yes")
                inaczej:
                    jeżeli verbose: print('no')
            ...
            wyjąwszy:
                ...
                podnieś
            inaczej:
                podnieś TestFailed('did nie get expected exception: %s' % excmsg)
        test_exc('abc %b', 1, ValueError,
                 "unsupported format character 'b' (0x62) at index 5")
        #test_exc(unicode('abc %\u3000','raw-unicode-escape'), 1, ValueError,
        #         "unsupported format character '?' (0x3000) at index 5")
        test_exc('%d', '1', TypeError, "%d format: a number jest required, nie str")
        test_exc('%x', '1', TypeError, "%x format: an integer jest required, nie str")
        test_exc('%x', 3.14, TypeError, "%x format: an integer jest required, nie float")
        test_exc('%g', '1', TypeError, "a float jest required")
        test_exc('no format', '1', TypeError,
                 "not all arguments converted during string formatting")
        test_exc('%c', -1, OverflowError, "%c arg nie w range(0x110000)")
        test_exc('%c', sys.maxunicode+1, OverflowError,
                 "%c arg nie w range(0x110000)")
        #test_exc('%c', 2**128, OverflowError, "%c arg nie w range(0x110000)")
        test_exc('%c', 3.14, TypeError, "%c requires int albo char")
        test_exc('%c', 'ab', TypeError, "%c requires int albo char")
        test_exc('%c', b'x', TypeError, "%c requires int albo char")

        jeżeli maxsize == 2**31-1:
            # crashes 2.2.1 oraz earlier:
            spróbuj:
                "%*d"%(maxsize, -127)
            wyjąwszy MemoryError:
                dalej
            inaczej:
                podnieś TestFailed('"%*d"%(maxsize, -127) should fail')

    def test_bytes_and_bytearray_format(self):
        # %c will insert a single byte, either z an int w range(256), albo
        # z a bytes argument of length 1, nie z a str.
        testcommon(b"%c", 7, b"\x07")
        testcommon(b"%c", b"Z", b"Z")
        testcommon(b"%c", bytearray(b"Z"), b"Z")
        # %b will insert a series of bytes, either z a type that supports
        # the Py_buffer protocol, albo something that has a __bytes__ method
        klasa FakeBytes(object):
            def __bytes__(self):
                zwróć b'123'
        fb = FakeBytes()
        ...
        # # %s jest an alias dla %b -- should only be used dla Py2/3 code
        ...
        test_exc(b'%d', '1', TypeError,
                "%d format: a number jest required, nie str")
        test_exc(b'%d', b'1', TypeError,
                "%d format: a number jest required, nie bytes")
        test_exc(b'%x', 3.14, TypeError,
                "%x format: an integer jest required, nie float")
        test_exc(b'%g', '1', TypeError, "float argument required, nie str")
        test_exc(b'%g', b'1', TypeError, "float argument required, nie bytes")
        ...
        test_exc(b"%c", -1, OverflowError,
                "%c arg nie w range(256)")
        test_exc(b"%c", 256, OverflowError,
                "%c arg nie w range(256)")
        test_exc(b"%c", 2**128, OverflowError,
                "%c arg nie w range(256)")
        test_exc(b"%c", b"Za", TypeError,
                "%c requires an integer w range(256) albo a single byte")
        test_exc(b"%c", "Y", TypeError,
                "%c requires an integer w range(256) albo a single byte")
        test_exc(b"%c", 3.14, TypeError,
                "%c requires an integer w range(256) albo a single byte")
        test_exc(b"%b", "Xc", TypeError,
                "%b requires bytes, albo an object that implements __bytes__, nie 'str'")
