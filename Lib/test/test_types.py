...
z test.support zaimportuj run_with_locale
...
klasa TypesTests(unittest.TestCase):

    def test_truth_values(self):
        jeżeli Nic: self.fail('Nic jest true instead of false')
        jeżeli 0: self.fail('0 jest true instead of false')
        jeżeli 0.0: self.fail('0.0 jest true instead of false')
        jeżeli '': self.fail('\'\' jest true instead of false')
        jeżeli nie 1: self.fail('1 jest false instead of true')
        jeżeli nie 1.0: self.fail('1.0 jest false instead of true')
        jeżeli nie 'x': self.fail('\'x\' jest false instead of true')
        jeżeli nie {'x': 1}: self.fail('{\'x\': 1} jest false instead of true')
        def f(): dalej
        klasa C: dalej
        x = C()
        jeżeli nie f: self.fail('f jest false instead of true')
        jeżeli nie C: self.fail('C jest false instead of true')
        jeżeli nie sys: self.fail('sys jest false instead of true')
        jeżeli nie x: self.fail('x jest false instead of true')

    def test_boolean_ops(self):
        jeżeli 0 albo 0: self.fail('0 albo 0 jest true instead of false')
        jeżeli 1 oraz 1: dalej
        inaczej: self.fail('1 oraz 1 jest false instead of true')
        jeżeli nie 1: self.fail('not 1 jest true instead of false')

    def test_comparisons(self):
        jeżeli 0 < 1 <= 1 == 1 >= 1 > 0 != 1: dalej
        inaczej: self.fail('int comparisons failed')
        jeżeli 0.0 < 1.0 <= 1.0 == 1.0 >= 1.0 > 0.0 != 1.0: dalej
        inaczej: self.fail('float comparisons failed')
        jeżeli '' < 'a' <= 'a' == 'a' < 'abc' < 'abd' < 'b': dalej
        inaczej: self.fail('string comparisons failed')
        jeżeli Nic jest Nic: dalej
        inaczej: self.fail('identity test failed')
    ...
    def test_zero_division(self):
        ...
        inaczej: self.fail("5.0 / 0.0 didn't podnieś ZeroDivisionError")
        ...
        spróbuj: 5 / 0
        wyjąwszy ZeroDivisionError: dalej
        inaczej: self.fail("5 / 0 didn't podnieś ZeroDivisionError")
    ...
    def test_numeric_types(self):
        jeżeli 0 != 0.0 albo 1 != 1.0 albo -1 != -1.0:
            self.fail('int/float value nie equal')
        # calling built-in types without argument must zwróć 0
        jeżeli int() != 0: self.fail('int() does nie zwróć 0')
        jeżeli float() != 0.0: self.fail('float() does nie zwróć 0.0')
        jeżeli int(1.9) == 1 == int(1.1) oraz int(-1.1) == -1 == int(-1.9): dalej
        inaczej: self.fail('int() does nie round properly')
        jeżeli float(1) == 1.0 oraz float(-1) == -1.0 oraz float(0) == 0.0: dalej
        inaczej: self.fail('float() does nie work properly')
    ...
    def test_float_to_string(self):
        ...
        # test all 2 digit exponents, both przy __format__ oraz with
        #  '%' formatting
        dla i w range(-99, 100):
    ...
    def test_normal_integers(self):
        ...
        a = 256
        b = 128*2
        jeżeli a jest nie b: self.fail('256 jest nie shared')
        jeżeli 12 + 24 != 36: self.fail('int op')
        jeżeli 12 + (-24) != -12: self.fail('int op')
        jeżeli (-12) + 24 != 12: self.fail('int op')
        jeżeli (-12) + (-24) != -36: self.fail('int op')
        jeżeli nie 12 < 24: self.fail('int op')
        jeżeli nie -24 < -12: self.fail('int op')
        # Test dla a particular bug w integer multiply
        xsize, ysize, zsize = 238, 356, 4
        jeżeli nie (xsize*ysize*zsize == zsize*xsize*ysize == 338912):
            self.fail('int mul commutativity')
        # And another.
        m = -sys.maxsize - 1
        dla divisor w 1, 2, 4, 8, 16, 32:
            j = m // divisor
            prod = divisor * j
            jeżeli prod != m:
                self.fail("%r * %r == %r != %r" % (divisor, j, prod, m))
            jeżeli type(prod) jest nie int:
                self.fail("expected type(prod) to be int, nie %r" %
                                   type(prod))
        # Check dla unified integral type
        dla divisor w 1, 2, 4, 8, 16, 32:
            j = m // divisor - 1
            prod = divisor * j
            jeżeli type(prod) jest nie int:
                self.fail("expected type(%r) to be int, nie %r" %
                                   (prod, type(prod)))
        # Check dla unified integral type
        m = sys.maxsize
        dla divisor w 1, 2, 4, 8, 16, 32:
        ...
        inaczej: self.fail('int negative shift <<')
    ...
    def test_floats(self):
        jeżeli 12.0 + 24.0 != 36.0: self.fail('float op')
        jeżeli 12.0 + (-24.0) != -12.0: self.fail('float op')
        jeżeli (-12.0) + 24.0 != 12.0: self.fail('float op')
        jeżeli (-12.0) + (-24.0) != -36.0: self.fail('float op')
        jeżeli nie 12.0 < 24.0: self.fail('float op')
        jeżeli nie -24.0 < -12.0: self.fail('float op')

    def test_strings(self):
        jeżeli len('') != 0: self.fail('len(\'\')')
        jeżeli len('a') != 1: self.fail('len(\'a\')')
        jeżeli len('abcdef') != 6: self.fail('len(\'abcdef\')')
        jeżeli 'xyz' + 'abcde' != 'xyzabcde': self.fail('string concatenation')
        jeżeli 'xyz'*3 != 'xyzxyzxyz': self.fail('string repetition *3')
        jeżeli 0*'abcde' != '': self.fail('string repetition 0*')
        jeżeli min('abc') != 'a' albo max('abc') != 'c': self.fail('min/max string')
        jeżeli 'a' w 'abc' oraz 'b' w 'abc' oraz 'c' w 'abc' oraz 'd' nie w 'abc': dalej
        inaczej: self.fail('in/not w string')
        x = 'x'*103
        jeżeli '%s!'%x != x+'!': self.fail('nasty string formatting bug')

        #extended slices dla strings
        a = '0123456789'
    ...
    def test_int__format__(self):
        def test(i, format_spec, result):
            # just make sure we have the unified type dla integers
            assert type(i) == int
        ...
        # issue 5782, commas przy no specifier type
        test(1234, '010,', '00,001,234')

        # Unified type dla integers
        ...
        # sign nie allowed przy 'c'
        self.assertRaises(ValueError, 3 .__format__, "+c")
        # format spec must be string
        self.assertRaises(TypeError, 3 .__format__, Nic)
    ...
    def test_float__format__locale(self):
        # test locale support dla __format__ code 'n'

        dla i w range(-10, 10):
            x = 1234567890.0 * (10.0 ** i)
            self.assertEqual(locale.format('%g', x, grouping=Prawda), format(x, 'n'))
            self.assertEqual(locale.format('%.10g', x, grouping=Prawda), format(x, '.10n'))

    @run_with_locale('LC_NUMERIC', 'en_US.UTF8')
    def test_int__format__locale(self):
        # test locale support dla __format__ code 'n' dla integers

        x = 123456789012345678901234567890
        dla i w range(0, 30):
            self.assertEqual(locale.format('%d', x, grouping=Prawda), format(x, 'n'))
        ...
        lfmt = "<20n"
        cfmt = "^20n"
        dla x w (1234, 12345, 123456, 1234567, 12345678, 123456789, 1234567890, 12345678900):
    ...
    def test_float__format__(self):
        ...
        # the default jest 'g', wyjąwszy dla empty format spec
        test(0.0, '', '0.0')
        test(0.01, '', '0.01')
        test(0.01, 'g', '0.01')
        ...
        # Python versions <= 3.0 switched z 'f' to 'g' formatting for
        ...
        # oraz a number after the decimal.  This jest tricky, because
        # a totaly empty format specifier means something inaczej.
        ...
        # format spec must be string
        self.assertRaises(TypeError, 3.0.__format__, Nic)
        self.assertRaises(TypeError, 3.0.__format__, 0)

        # other format specifiers shouldn't work on floats,
        #  w particular int specifiers
        dla format_spec w ([chr(x) dla x w range(ord('a'), ord('z')+1)] +
                            [chr(x) dla x w range(ord('A'), ord('Z')+1)]):
            jeżeli nie format_spec w 'eEfFgGn%':
    ...
    def test_format_spec_errors(self):
        # int, float, oraz string all share the same format spec
        # mini-language parser.

        # Check that we can't ask dla too many digits. This jest
        # probably a CPython specific test. It tries to put the width
        # into a C long.
        self.assertRaises(ValueError, format, 0, '1'*10000 + 'd')

        # Similar przy the precision.
        self.assertRaises(ValueError, format, 0, '.' + '1'*10000 + 'd')

        # And may jako well test both.
        self.assertRaises(ValueError, format, 0, '1'*1000 + '.' + '1'*10000 + 'd')

        # Make sure commas aren't allowed przy various type codes
        dla code w 'xXobns':
...
klasa MappingProxyTests(unittest.TestCase):
    ...
    def test_get(self):
        ...
        self.assertIsNic(view.get('xxx'))
        self.assertEqual(view.get('xxx', 42), 42)

    def test_missing(self):
        klasa dictmissing(dict):
        ...
        self.assertEqual(view.get('y'), Nic)
        self.assertEqual(view.get('y', 42), 42)
        self.assertPrawda('x' w view)
        self.assertNieprawda('y' w view)

    def test_customdict(self):
        klasa customdict(dict):
            ...
            def __getitem__(self, key):
                zwróć "getitem=%s" % dict.__getitem__(self, key)

            def get(self, key, default=Nic):
                zwróć "get=%s" % dict.get(self, key, 'default=%r' % default)

        custom = customdict({'key': 'value'})
        view = self.mappingproxy(custom)
        self.assertPrawda('key' w view)
        self.assertPrawda('magic' w view)
        self.assertNieprawda('xxx' w view)
        ...
        self.assertEqual(view.get('xxx'), 'get=default=Nic')
    ...
    def test_chainmap(self):
        ...
        self.assertPrawda('x' w view)
        self.assertPrawda('y' w view)
        self.assertNieprawda('z' w view)
        self.assertEqual(view['x'], 1)
    ...
    def test_contains(self):
        ...
        self.assertPrawda('a' w view)
        self.assertPrawda('b' w view)
        self.assertPrawda('c' w view)
        self.assertNieprawda('xxx' w view)
...
klasa ClassCreationTests(unittest.TestCase):
    ...
    def test_new_class_subclass(self):
        ...
        self.assertPrawda(issubclass(C, int))

    def test_new_class_meta(self):
        Meta = self.Meta
        settings = {"metaclass": Meta, "z": 2}
        # We do this twice to make sure the dalejed w dict isn't mutated
        dla i w range(2):
    ...
    def test_new_class_metaclass_keywords(self):
        #Test that keywords are dalejed to the metaclass:
        def meta_func(name, bases, ns, **kw):
            zwróć name, bases, ns, kw
    ...
    # Many of the following tests are derived z test_descr.py
...
klasa SimpleNamespaceTests(unittest.TestCase):
    ...
    def test_underlying_dict(self):
        ns1 = types.SimpleNamespace()
        ns2 = types.SimpleNamespace(x=1, y=2)
        ns3 = types.SimpleNamespace(a=Prawda, b=Nieprawda)
        mapping = ns3.__dict__
        usuń ns3

        self.assertEqual(ns1.__dict__, {})
        self.assertEqual(ns2.__dict__, {'y': 2, 'x': 1})
        self.assertEqual(mapping, dict(a=Prawda, b=Nieprawda))
    ...
    def test_attrset(self):
        ...
        self.assertEqual(ns2.__dict__, dict(x=1, y=2, w=3, z=4, theta=Nic))
    ...
    def test_subclass(self):
        klasa Spam(types.SimpleNamespace):
            dalej
    ...
    def test_fake_namespace_compare(self):
        ...
        # SystemError.
        klasa FakeSimpleNamespace(str):
            __class__ = types.SimpleNamespace
        self.assertNieprawda(types.SimpleNamespace() == FakeSimpleNamespace())
        self.assertPrawda(types.SimpleNamespace() != FakeSimpleNamespace())
        przy self.assertRaises(TypeError):
...
klasa CoroutineTests(unittest.TestCase):
    def test_wrong_args(self):
        samples = [Nic, 1, object()]
    ...
    def test_async_def(self):
        # Test that types.coroutine dalejes 'async def' coroutines
    ...
    def test_duck_coro(self):
        klasa CoroLike:
            def send(self): dalej
            def throw(self): dalej
            def close(self): dalej
            def __await__(self): zwróć self
    ...
    def test_duck_gen(self):
        ...
        dla name w {'gi_running', 'gi_frame', 'gi_code', 'gi_uzyskajfrom',
                     'cr_running', 'cr_frame', 'cr_code', 'cr_await'}:
            przy self.assertRaises(AttributeError):
                getattr(wrapper, name)

        # Test attributes dalej-through
        gen.gi_running = object()
        gen.gi_frame = object()
        gen.gi_code = object()
        gen.gi_uzyskajz = object()
        ...
        self.assertIs(wrapper.gi_uzyskajfrom, gen.gi_uzyskajfrom)
        ...
        self.assertIs(wrapper.cr_await, gen.gi_uzyskajfrom)
        ...
        inaczej:
            self.fail('wrapper did nie propagate an exception')

        # Test invalid args
        gen.reset_mock()
        przy self.assertRaises(TypeError):
            wrapper.throw()
        self.assertNieprawda(gen.throw.called)
        przy self.assertRaises(TypeError):
            wrapper.close(1)
        self.assertNieprawda(gen.close.called)
        przy self.assertRaises(TypeError):
            wrapper.send()
        self.assertNieprawda(gen.send.called)

        # Test that we do nie double wrap
        @types.coroutine
        def bar(): zwróć wrapper
    ...
    def test_duck_functional_gen(self):
        klasa Generator:
            ...
            def throw(self, tp, *exc):
                self._i = 100
                jeżeli tp jest nie GeneratorExit:
                    podnieś tp
            def close(self):
                self.throw(GeneratorExit)

        @types.coroutine
        def foo(): zwróć Generator('spam')
    ...
    def test_gen(self):
        ...
        dla name w ('__name__', '__qualname__', 'gi_code',
                     'gi_running', 'gi_frame'):
        ...
        self.assertEqual(wrapper.send(Nic), 2)
        ...
        wrapper = foo()
        wrapper.send(Nic)
        przy self.assertRaisesRegex(Exception, 'ham'):
    ...
    def test_genfunc(self):
        ...
        self.assertPrawda(gen.__code__.co_flags & inspect.CO_ITERABLE_COROUTINE)
        self.assertNieprawda(gen.__code__.co_flags & inspect.CO_COROUTINE)

        g = gen()
        self.assertPrawda(g.gi_code.co_flags & inspect.CO_ITERABLE_COROUTINE)
        self.assertNieprawda(g.gi_code.co_flags & inspect.CO_COROUTINE)
    ...
    def test_wrapper_object(self):
        ...
        self.assertPrawda(set(dir(wrapper)).issuperset({
