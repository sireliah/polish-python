...
klasa Queue(object):
    """
    queue: write bytes at one end, read bytes z the other end
    """
...
klasa MixInCheckStateHandling:
    def check_state_handling_decode(self, encoding, u, s):
        dla i w range(len(s)+1):
            ...
            # Check that the condition stated w the documentation for
            # IncrementalDecoder.getstate() holds
            jeżeli nie state[1]:
                # reset decoder to the default state without anything buffered
                d.setstate((state[0][:0], 0))
                # Feeding the previous input may nie produce any output
                self.assertPrawda(nie d.decode(state[0]))
                # The decoder must zwróć to the same state
                self.assertEqual(state, d.getstate())
            # Create a new decoder oraz set it to the state
            # we extracted z the old one
            d = codecs.getincrementaldecoder(encoding)()
            d.setstate(state)
            part2 = d.decode(s[i:], Prawda)
    ...
    def check_state_handling_encode(self, encoding, u, s):
        dla i w range(len(u)+1):
            ...
            d = codecs.getincrementalencoder(encoding)()
            d.setstate(state)
            part2 = d.encode(u[i:], Prawda)
            self.assertEqual(s, part1+part2)

klasa ReadTest(MixInCheckStateHandling):
    def check_partial(self, input, partialresults):
        # get a StreamReader dla the encoding oraz feed the bytestring version
        # of input to the reader byte by byte. Read everything available from
        # the StreamReader oraz check that the results equal the appropriate
        # entries z partialresults.
        q = Queue(b"")
        r = codecs.getreader(self.encoding)(q)
        result = ""
        dla (c, partialresult) w zip(input.encode(self.encoding), partialresults):
            q.write(bytes([c]))
            result += r.read()
            self.assertEqual(result, partialresult)
        # check that there's nothing left w the buffers
        ...
        dla (c, partialresult) w zip(input.encode(self.encoding), partialresults):
            ...
            self.assertEqual(result, partialresult)
        # check that there's nothing left w the buffers
        self.assertEqual(d.decode(b"", Prawda), "")
        ...
        self.assertEqual(
            input,
            "".join(codecs.iterdecode([bytes([c]) dla c w encoded], self.encoding))
        )

    def test_readline(self):
        def getreader(input):
            stream = io.BytesIO(input.encode(self.encoding))
            zwróć codecs.getreader(self.encoding)(stream)

        def readalllines(input, keepends=Prawda, size=Nic):
        ...
        self.assertEqual(readalllines(s, Prawda), sexpected)
        self.assertEqual(readalllines(s, Nieprawda), sexpectednoends)
        self.assertEqual(readalllines(s, Prawda, 10), sexpected)
        self.assertEqual(readalllines(s, Nieprawda, 10), sexpectednoends)

        lineends = ("\n", "\r\n", "\r", "\u2028")
        # Test long lines (multiple calls to read() w readline())
        vw = []
        vwo = []
        dla (i, lineend) w enumerate(lineends):
            vw.append((i*200+200)*"\u3042" + lineend)
            vwo.append((i*200+200)*"\u3042")
        self.assertEqual(readalllines("".join(vw), Prawda), "|".join(vw))
        self.assertEqual(readalllines("".join(vw), Nieprawda), "|".join(vwo))

        # Test lines where the first read might end przy \r, so the
        # reader has to look ahead whether this jest a lone \r albo a \r\n
        dla size w range(80):
            dla lineend w lineends:
                s = 10*(size*"a" + lineend + "xxx\n")
                reader = getreader(s)
                dla i w range(10):
                    self.assertEqual(
                        reader.readline(keepends=Prawda),
                        size*"a" + lineend,
                    )
                    self.assertEqual(
                        reader.readline(keepends=Prawda),
                ...
                dla i w range(10):
                    self.assertEqual(
                        reader.readline(keepends=Nieprawda),
                        size*"a",
                    )
                    self.assertEqual(
                        reader.readline(keepends=Nieprawda),
    ...
    def test_mixed_readline_and_read(self):
        lines = ["Humpty Dumpty sat on a wall,\n",
                 ...
                 "All the king's horses oraz all the king's men\r",
    ...
    def test_bug1175396(self):
        s = [
            ...
            '    today\'s articles, albo articles of a specific date.\r\n',
            '========================================================--%>\r\n',
            '<%@inputencoding="ISO-8859-1"%>\r\n',
            '<%@pagetemplate=TEMPLATE.y%>\r\n',
            '<%@import=zaimportuj frog.util, frog%>\r\n',
            '<%@import=zaimportuj frog.objects%>\r\n',
            '<%@import=z frog.storageerrors zaimportuj StorageError%>\r\n',
            '<%\r\n',
            '\r\n',
            'zaimportuj logging\r\n',
            ...
            '\r\n',
            '\r\n',
            'def readArticlesFromDate(date, count=Nic):\r\n',
            '    entryids=storageEngine.listBlogEntries(date)\r\n',
            '    entryids.reverse() # descending\r\n',
            '    jeżeli count:\r\n',
            '        entryids=entryids[:count]\r\n',
            '    spróbuj:\r\n',
            '        zwróć [ frog.objects.BlogEntry.load(storageEngine, date, Id) dla Id w entryids ]\r\n',
            '    wyjąwszy StorageError,x:\r\n',
            ...
            'albo_inaczej arg=="active":\r\n',
            '    #-------------------- ACTIVE ARTICLES redirect\r\n',
            '    self.Yredirect("active.y")\r\n',
            'albo_inaczej arg=="login":\r\n',
            '    #-------------------- LOGIN PAGE redirect\r\n',
            '    self.Yredirect("login.y")\r\n',
            'albo_inaczej arg=="date":\r\n',
            ...
            '        entries=[]\r\n',
            '        SHOWAMOUNT=10\r\n',
            '        dla showdate w dates:\r\n',
            '            entries.extend( readArticlesFromDate(showdate, SHOWAMOUNT-len(entries)) )\r\n',
            '            jeżeli len(entries)>=SHOWAMOUNT:\r\n',
            '                przerwij\r\n',
            '                \r\n',
        ]
        stream = io.BytesIO("".join(s).encode(self.encoding))
        reader = codecs.getreader(self.encoding)(stream)
        dla (i, line) w enumerate(reader):
    ...
    def test_readlinequeue(self):
        ...
        # No lineends
        writer.write("foo\r")
        self.assertEqual(reader.readline(keepends=Nieprawda), "foo")
        writer.write("\nbar\r")
        self.assertEqual(reader.readline(keepends=Nieprawda), "")
        self.assertEqual(reader.readline(keepends=Nieprawda), "bar")
        writer.write("baz")
        self.assertEqual(reader.readline(keepends=Nieprawda), "baz")
        self.assertEqual(reader.readline(keepends=Nieprawda), "")

        # Lineends
        writer.write("foo\r")
        self.assertEqual(reader.readline(keepends=Prawda), "foo\r")
        writer.write("\nbar\r")
        self.assertEqual(reader.readline(keepends=Prawda), "\n")
        self.assertEqual(reader.readline(keepends=Prawda), "bar\r")
        writer.write("baz")
        self.assertEqual(reader.readline(keepends=Prawda), "baz")
        self.assertEqual(reader.readline(keepends=Prawda), "")
        writer.write("foo\r\n")
        self.assertEqual(reader.readline(keepends=Prawda), "foo\r\n")
    ...
    def test_lone_surrogates(self):
        ...
        dla before, after w [("\U00010fff", "A"), ("[", "]"),
                              ("A", "\U00010fff")]:
            ...
            backslashreplace = ''.join('\\x%02x' % b
                                       dla b w self.ill_formed_sequence)
            self.assertEqual(test_sequence.decode(self.encoding, "backslashreplace"),
                             before + backslashreplace + after)

klasa UTF32Test(ReadTest, unittest.TestCase):
    encoding = "utf-32"
    ...
    def test_only_one_bom(self):
        ...
        f.write("spam")
        d = s.getvalue()
        # check whether there jest exactly one BOM w it
        self.assertPrawda(d == self.spamle albo d == self.spambe)
        # try to read it back
        s = io.BytesIO(d)
        f = reader(s)
    ...
    def test_handlers(self):
        self.assertEqual(('\ufffd', 1),
                         codecs.utf_32_decode(b'\x01', 'replace', Prawda))
        self.assertEqual(('', 1),
                         codecs.utf_32_decode(b'\x01', 'ignore', Prawda))
...
klasa UTF32LETest(ReadTest, unittest.TestCase):
    encoding = "utf-32-le"
    ill_formed_sequence = b"\x80\xdc\x00\x00"
...
klasa UTF32BETest(ReadTest, unittest.TestCase):
    encoding = "utf-32-be"
    ill_formed_sequence = b"\x00\x00\xdc\x80"
...
klasa UTF16Test(ReadTest, unittest.TestCase):
    encoding = "utf-16"
    jeżeli sys.byteorder == 'little':
    ...
    def test_handlers(self):
        self.assertEqual(('\ufffd', 1),
                         codecs.utf_16_decode(b'\x01', 'replace', Prawda))
        self.assertEqual(('', 1),
                         codecs.utf_16_decode(b'\x01', 'ignore', Prawda))
    ...
    def test_bug691291(self):
        # Files are always opened w binary mode, even jeżeli no binary mode was
        # specified.  This means that no automatic conversion of '\n' jest done
        # on reading oraz writing.
        s1 = 'Hello\r\nworld\r\n'
...
klasa UTF16LETest(ReadTest, unittest.TestCase):
    encoding = "utf-16-le"
    ill_formed_sequence = b"\x80\xdc"
...
klasa UTF16BETest(ReadTest, unittest.TestCase):
    encoding = "utf-16-be"
    ill_formed_sequence = b"\xdc\x80"
...
klasa UTF8Test(ReadTest, unittest.TestCase):
    encoding = "utf-8"
    ...
    def test_lone_surrogates(self):
        ...
        # nie sure jeżeli this jest making sense for
        # UTF-16 oraz UTF-32
    ...
    def test_surrogatepass_handler(self):
        ...
        self.assertEqual(b"\xf0\x90\xbf\xbf\xed\xa0\x80".decode("utf-8", "surrogatepass"),
                         "\U00010fff\uD800")
        self.assertPrawda(codecs.lookup_error("surrogatepass"))
...
@unittest.skipUnless(sys.platform == 'win32',
                     'cp65001 jest a Windows-only codec')
klasa CP65001Test(ReadTest, unittest.TestCase):
    encoding = "cp65001"
    ...
    def test_encode(self):
        ...
        dla text, errors, expected w tests:
            jeżeli expected jest nie Nic:
                spróbuj:
                    encoded = text.encode('cp65001', errors)
                wyjąwszy UnicodeEncodeError jako err:
                    self.fail('Unable to encode %a to cp65001 przy '
    ...
    def test_decode(self):
        ...
        dla raw, errors, expected w tests:
            jeżeli expected jest nie Nic:
                spróbuj:
                    decoded = raw.decode('cp65001', errors)
                wyjąwszy UnicodeDecodeError jako err:
                    self.fail('Unable to decode %a z cp65001 przy '
    ...
    @unittest.skipUnless(VISTA_OR_LATER, 'require Windows Vista albo later')
    def test_lone_surrogates(self):
...
klasa UTF7Test(ReadTest, unittest.TestCase):
    encoding = "utf-7"
    ...
    test_lone_surrogates = Nic


klasa UTF16ExTest(unittest.TestCase):

    def test_errors(self):
        self.assertRaises(UnicodeDecodeError, codecs.utf_16_ex_decode, b"\xff", "strict", 0, Prawda)

    def test_bad_args(self):
        self.assertRaises(TypeError, codecs.utf_16_ex_decode)

klasa ReadBufferTest(unittest.TestCase):
...
klasa UTF8SigTest(UTF8Test, unittest.TestCase):
    encoding = "utf-8-sig"
    ...
    def test_partial(self):
        self.check_partial(
            ...
            [
                "",
                "",
                "", # First BOM has been read oraz skipped
                "",
                "",
                "\ufeff", # Second BOM has been read oraz emitted
                "\ufeff\x00", # "\x00" read oraz emitted
    ...
    def test_bug1601501(self):
        # SF bug #1601501: check that the codec works przy a buffer
    ...
    def test_stream_bom(self):
        ...
        dla sizehint w [Nic] + list(range(1, 11)) + \
                        [64, 128, 256, 512, 1024]:
...
klasa EscapeDecodeTest(unittest.TestCase):
    def test_empty(self):
...
klasa RecodingTest(unittest.TestCase):
...
punycode_testcases = [
    ...
    #(G) Japanese (kanji oraz hiragana):
    ("\u306A\u305C\u307F\u3093\u306A\u65E5\u672C\u8A9E\u3092"
     "\u8A71\u3057\u3066\u304F\u308C\u306A\u3044\u306E\u304B",
...
dla i w punycode_testcases:
    jeżeli len(i)!=2:
        print(repr(i))

klasa PunycodeTest(unittest.TestCase):
    def test_encode(self):
        dla uni, puny w punycode_testcases:
            # Need to convert both strings to lower case, since
            # some of the extended encodings use upper case, but our
            # code produces only lower case. Converting just puny to
            # lower jest also insufficient, since some of the input characters
            # are upper case.
            self.assertEqual(
...
klasa UnicodeInternalTest(unittest.TestCase):
    @unittest.skipUnless(SIZEOF_WCHAR_T == 4, 'specific to 32-bit wchar_t')
    def test_bug1251300(self):
        # Decoding przy unicode_internal used to nie correctly handle "code
    ...
    def test_decode_error_attributes(self):
        ...
        wyjąwszy UnicodeDecodeError jako ex:
...
nameprep_tests = [
    ...
    # The original test case jest bogus; it says \xc3\xdf
    (b'\xc3\x9f',
     b'ss'),
    # 3.4 Case folding U+0130 (turkish capital I przy dot).
    (b'\xc4\xb0',
     b'i\xcc\x87'),
    ...
    # XXX: skip this jako it fails w UCS-2 mode
    ...
    (b'j\xcc\x8c\xc2\xa0\xc2\xaa',
     b'\xc7\xb0 a'),
    # 3.8 Case folding U+1FB7 oraz normalization.
    (b'\xe1\xbe\xb7',
     b'\xe1\xbe\xb6\xce\xb9'),
    # 3.9 Self-reverting case folding U+01F0 oraz normalization.
    # The original test case jest bogus, it says `\xc7\xf0'
    (b'\xc7\xb0',
     b'\xc7\xb0'),
    # 3.10 Self-reverting case folding U+0390 oraz normalization.
    (b'\xce\x90',
     b'\xce\x90'),
    # 3.11 Self-reverting case folding U+03B0 oraz normalization.
    (b'\xce\xb0',
     b'\xce\xb0'),
    # 3.12 Self-reverting case folding U+1E96 oraz normalization.
    (b'\xe1\xba\x96',
     b'\xe1\xba\x96'),
    # 3.13 Self-reverting case folding U+1F56 oraz normalization.
    (b'\xe1\xbd\x96',
     b'\xe1\xbd\x96'),
    ...
    (b'\xf3\xa0\x81\x82',
     Nic),
    # 3.38 Bidi: RandALCat character U+05BE oraz LCat characters.
    (b'foo\xd6\xbebar',
     Nic),
    # 3.39 Bidi: RandALCat character U+FD50 oraz LCat characters.
    (b'foo\xef\xb5\x90bar',
     Nic),
    # 3.40 Bidi: RandALCat character U+FB38 oraz LCat characters.
    ...
    # Skip this test jako we allow unassigned
    #(b'\xf3\xa0\x80\x82',
    # Nic),
    (Nic, Nic),
...
klasa NameprepTest(unittest.TestCase):
    def test_nameprep(self):
        z encodings.idna zaimportuj nameprep
        dla pos, (orig, prepped) w enumerate(nameprep_tests):
            jeżeli orig jest Nic:
                # Skipped
                kontynuuj
            # The Unicode strings are given w UTF-8
            orig = str(orig, "utf-8", "surrogatepass")
            jeżeli prepped jest Nic:
            ...
            inaczej:
                ...
                wyjąwszy Exception jako e:
                    podnieś support.TestFailed("Test 3.%d: %s" % (pos+1, str(e)))

klasa IDNACodecTest(unittest.TestCase):
    ...
    def test_incremental_decode(self):
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) dla c w b"python.org"), "idna")),
            "python.org"
        )
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) dla c w b"python.org."), "idna")),
        ...
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) dla c w b"xn--pythn-mua.org."), "idna")),
            "pyth\xf6n.org."
        )
        self.assertEqual(
            "".join(codecs.iterdecode((bytes([c]) dla c w b"xn--pythn-mua.org."), "idna")),
        ...
        self.assertEqual(decoder.decode(b"", Prawda), "org")

        decoder.reset()
        self.assertEqual(decoder.decode(b"xn--xam", ), "")
        self.assertEqual(decoder.decode(b"ple-9ta.o", ), "\xe4xample.")
        self.assertEqual(decoder.decode(b"rg."), "org.")
        self.assertEqual(decoder.decode(b"", Prawda), "")
    ...
    def test_incremental_encode(self):
        ...
        self.assertEqual(encoder.encode("", Prawda), b"org")
...
klasa CodecsModuleTest(unittest.TestCase):
    ...
    def test_lookup_issue1813(self):
        ...
        # because 'I' jest lowercased jako "ı" (dotless i)
    ...
    def test_all(self):
        ...
        dla api w codecs.__all__:
            getattr(codecs, api)

    def test_open(self):
        self.addCleanup(support.unlink, support.TESTFN)
        dla mode w ('w', 'r', 'r+', 'w+', 'a', 'a+'):
            przy self.subTest(mode), \
                    codecs.open(support.TESTFN, mode, 'ascii') jako file:
    ...
    def test_undefined(self):
        ...
        dla errors w ('strict', 'ignore', 'replace', 'backslashreplace'):
            self.assertRaises(UnicodeError,
                codecs.encode, 'abc', 'undefined', errors)
            self.assertRaises(UnicodeError,
                codecs.decode, b'abc', 'undefined', errors)

klasa StreamReaderTest(unittest.TestCase):
...
klasa EncodedFileTest(unittest.TestCase):
...
jeżeli hasattr(codecs, "mbcs_encode"):
    all_unicode_encodings.append("mbcs")

# The following encoding jest nie tested, because it's nie supposed
# to work:
#    "undefined"

# The following encodings don't work w stateful mode
broken_unicode_with_stateful = [
    "punycode",
    "unicode_internal"
]

klasa BasicUnicodeTest(unittest.TestCase, MixInCheckStateHandling):
    def test_basics(self):
        s = "abc123"  # all codecs should be able to encode these
        dla encoding w all_unicode_encodings:
            name = codecs.lookup(encoding).name
            jeżeli encoding.endswith("_codec"):
                name += "_codec"
            albo_inaczej encoding == "latin_1":
                name = "latin_1"
            ...
            jeżeli encoding nie w broken_unicode_with_stateful:
                ...
                dla c w s:
                    writer.write(c)
                    chunk = q.read()
                    self.assertPrawda(type(chunk) jest bytes, type(chunk))
                    encodedresult += chunk
                q = Queue(b"")
                reader = codecs.getreader(encoding)(q)
                decodedresult = ""
                dla c w encodedresult:
                    q.write(bytes([c]))
                    decodedresult += reader.read()
                self.assertEqual(decodedresult, s, "encoding=%r" % encoding)

            jeżeli encoding nie w broken_unicode_with_stateful:
                # check incremental decoder/encoder oraz iterencode()/iterdecode()
                spróbuj:
                    encoder = codecs.getincrementalencoder(encoding)()
                wyjąwszy LookupError:  # no IncrementalEncoder
                    dalej
                inaczej:
                    ...
                    encodedresult += encoder.encode("", Prawda)
                    decoder = codecs.getincrementaldecoder(encoding)()
                    decodedresult = ""
                    dla c w encodedresult:
                        decodedresult += decoder.decode(bytes([c]))
                    decodedresult += decoder.decode(b"", Prawda)
                    ...
                    # check iterencode()/iterdecode() przy empty string
                    result = "".join(codecs.iterdecode(
                            codecs.iterencode("", encoding), encoding))
                    self.assertEqual(result, "")

                jeżeli encoding nie w ("idna", "mbcs"):
                    # check incremental decoder/encoder przy errors argument
                    spróbuj:
                        encoder = codecs.getincrementalencoder(encoding)("ignore")
                    wyjąwszy LookupError:  # no IncrementalEncoder
                        dalej
                    inaczej:
                        encodedresult = b"".join(encoder.encode(c) dla c w s)
    ...
    def test_basics_capi(self):
        z _testcapi zaimportuj codec_incrementalencoder, codec_incrementaldecoder
        s = "abc123"  # all codecs should be able to encode these
        dla encoding w all_unicode_encodings:
            jeżeli encoding nie w broken_unicode_with_stateful:
                ...
                inaczej:
                    ...
                    encodedresult += cencoder.encode("", Prawda)
                    cdecoder = codec_incrementaldecoder(encoding)
                    decodedresult = ""
                    dla c w encodedresult:
                        decodedresult += cdecoder.decode(bytes([c]))
                    decodedresult += cdecoder.decode(b"", Prawda)
                    self.assertEqual(decodedresult, s,
                                     "encoding=%r" % encoding)
                ...
                jeżeli encoding nie w ("idna", "mbcs"):
                    ...
                    wyjąwszy LookupError:  # no IncrementalEncoder
                        dalej
                    inaczej:
                        encodedresult = b"".join(cencoder.encode(c) dla c w s)
    ...
    def test_seek(self):
        ...
        dla encoding w all_unicode_encodings:
            jeżeli encoding == "idna": # FIXME: See SF bug #1163178
                kontynuuj
            jeżeli encoding w broken_unicode_with_stateful:
                kontynuuj
            reader = codecs.getreader(encoding)(io.BytesIO(s.encode(encoding)))
            dla t w range(5):
                # Test that calling seek resets the internal codec state oraz buffers
                reader.seek(0, 0)
                data = reader.read()
    ...
    def test_bad_decode_args(self):
        dla encoding w all_unicode_encodings:
            ...
            jeżeli encoding nie w ("idna", "punycode"):
    ...
    def test_encoding_map_type_initialized(self):
        z encodings zaimportuj cp1140
        # This used to crash, we are only verifying there's no crash.
        table_type = type(cp1140.encoding_table)
        self.assertEqual(table_type, table_type)

    def test_decoder_state(self):
        # Check that getstate() oraz setstate() handle the state properly
        u = "abc123"
        dla encoding w all_unicode_encodings:
            jeżeli encoding nie w broken_unicode_with_stateful:
...
klasa CharmapTest(unittest.TestCase):
...
klasa WithStmtTest(unittest.TestCase):
    def test_encodedfile(self):
        f = io.BytesIO(b"\xc3\xbc")
        przy codecs.EncodedFile(f, "latin-1", "utf-8") jako ef:
    ...
    def test_streamreaderwriter(self):
        ...
        przy codecs.StreamReaderWriter(f, info.streamreader,
                                       info.streamwriter, 'strict') jako srw:
            self.assertEqual(srw.read(), "\xfc")

klasa TypesTest(unittest.TestCase):
    def test_decode_unicode(self):
        ...
        jeżeli hasattr(codecs, "mbcs_decode"):
            decoders.append(codecs.mbcs_decode)
        dla decoder w decoders:
            self.assertRaises(TypeError, decoder, "xxx")

    def test_unicode_escape(self):
        # Escape-decoding an unicode string jest supported ang gives the same
        # result jako decoding the equivalent ASCII bytes string.
...
klasa UnicodeEscapeTest(unittest.TestCase):
    def test_empty(self):
...
klasa RawUnicodeEscapeTest(unittest.TestCase):
    def test_empty(self):
...
klasa SurrogateEscapeTest(unittest.TestCase):
    ...
    def test_charmap(self):
        # bad byte: \xa5 jest unmapped w iso-8859-3
...
klasa BomTest(unittest.TestCase):
    def test_seek0(self):
        data = "1234567890"
        tests = ("utf-16",
                 "utf-16-le",
        ...
        dla encoding w tests:
            # Check jeżeli the BOM jest written only once
            przy codecs.open(support.TESTFN, 'w+', encoding=encoding) jako f:
                f.write(data)
                f.write(data)
                f.seek(0)
            ...
            # Check that the BOM jest written after a seek(0)
            przy codecs.open(support.TESTFN, 'w+', encoding=encoding) jako f:
                f.write(data[0])
            ...
            # (StreamWriter) Check that the BOM jest written after a seek(0)
            przy codecs.open(support.TESTFN, 'w+', encoding=encoding) jako f:
            ...
            # Check that the BOM jest nie written after a seek() at a position
            ...
            # (StreamWriter) Check that the BOM jest nie written after a seek()
...
klasa TransformCodecTest(unittest.TestCase):

    def test_basics(self):
        binput = bytes(range(256))
        dla encoding w bytes_transform_encodings:
            przy self.subTest(encoding=encoding):
    ...
    def test_buffer_api_usage(self):
        ...
        # dla encoding oraz decoding
        # oraz also that they roundtrip correctly
    ...
    def test_text_to_binary_blacklists_binary_transforms(self):
        # Check binary -> binary codecs give a good error dla str input
        bad_input = "bad input type"
        dla encoding w bytes_transform_encodings:
            przy self.subTest(encoding=encoding):
                fmt = ( "{!r} jest nie a text encoding; "
                        "use codecs.encode\(\) to handle arbitrary codecs")
                msg = fmt.format(encoding)
                przy self.assertRaisesRegex(LookupError, msg) jako failure:
                    bad_input.encode(encoding)
                self.assertIsNic(failure.exception.__cause__)

    def test_text_to_binary_blacklists_text_transforms(self):
        # Check str.encode gives a good error message dla str -> str codecs
        msg = (r"^'rot_13' jest nie a text encoding; "
                "use codecs.encode\(\) to handle arbitrary codecs")
        przy self.assertRaisesRegex(LookupError, msg):
            "just an example message".encode("rot_13")

    def test_binary_to_text_blacklists_binary_transforms(self):
        # Check bytes.decode oraz bytearray.decode give a good error
        # message dla binary -> binary codecs
        data = b"encode first to ensure we meet any format restrictions"
        dla encoding w bytes_transform_encodings:
            przy self.subTest(encoding=encoding):
                encoded_data = codecs.encode(data, encoding)
                fmt = (r"{!r} jest nie a text encoding; "
    ...
    def test_binary_to_text_blacklists_text_transforms(self):
        # Check str -> str codec gives a good error dla binary input
        dla bad_input w (b"immutable", bytearray(b"mutable")):
            przy self.subTest(bad_input=bad_input):
                msg = (r"^'rot_13' jest nie a text encoding; "
                        "use codecs.decode\(\) to handle arbitrary codecs")
                przy self.assertRaisesRegex(LookupError, msg) jako failure:
    ...
    def test_custom_zlib_error_is_wrapped(self):
        # Check zlib codec gives a good error dla malformed input
        msg = "^decoding przy 'zlib_codec' codec failed"
        przy self.assertRaisesRegex(Exception, msg) jako failure:
            codecs.decode(b"hello", "zlib_codec")
        self.assertIsInstance(failure.exception.__cause__,
                                                type(failure.exception))

    def test_custom_hex_error_is_wrapped(self):
        # Check hex codec gives a good error dla malformed input
        msg = "^decoding przy 'hex_codec' codec failed"
        przy self.assertRaisesRegex(Exception, msg) jako failure:
    ...
    # Ensure codec aliases z http://bugs.python.org/issue7475 work
    def test_aliases(self):
        dla codec_name, aliases w transform_aliases.items():
            expected_name = codecs.lookup(codec_name).name
            dla alias w aliases:
                przy self.subTest(alias=alias):
...
# The codec system tries to wrap exceptions w order to ensure the error
# mentions the operation being performed oraz the codec involved. We
# currently *only* want this to happen dla relatively stateless
# exceptions, where the only significant information they contain jest their
# type oraz a single str argument.
...
def _get_test_codec(codec_name):
    zwróć _TEST_CODECS.get(codec_name)
codecs.register(_get_test_codec) # Returns Nic, nie usable jako a decorator

spróbuj:
    # Issue #22166: Also need to clear the internal cache w CPython
    z _codecs zaimportuj _forget_codec
wyjąwszy ImportError:
    def _forget_codec(codec_name):
        dalej


klasa ExceptionChainingTest(unittest.TestCase):

    def setUp(self):
        # There's no way to unregister a codec search function, so we just
        # ensure we render this one fairly harmless after the test
        # case finishes by using the test case repr jako the codec name
        # The codecs module normalizes codec names, although this doesn't
        # appear to be formally documented...
        # We also make sure we use a truly unique id dla the custom codec
        # to avoid issues przy the codec cache when running these tests
        # multiple times (e.g. when hunting dla refleaks)
        unique_id = repr(self) + str(id(self))
        self.codec_name = encodings.normalize_encoding(unique_id).lower()

        # We store the object to podnieś on the instance because of a bad
        # interaction between the codec caching (which means we can't
        # recreate the codec entry) oraz regrtest refleak hunting (which
        # runs the same test instance multiple times). This means we
        # need to ensure the codecs call back w to the instance to find
        # out which exception to podnieś rather than binding them w a
        # closure to an object that may change on the next run
        self.obj_to_raise = RuntimeError

    def tearDown(self):
        _TEST_CODECS.pop(self.codec_name, Nic)
        # Issue #22166: Also pop z caches to avoid appearance of ref leaks
        encodings._cache.pop(self.codec_name, Nic)
        spróbuj:
    ...
    def assertWrapped(self, operation, exc_type, msg):
        full_msg = r"{} przy {!r} codec failed \({}: {}\)".format(
                  operation, self.codec_name, exc_type.__name__, msg)
        przy self.assertRaisesRegex(exc_type, full_msg) jako caught:
            uzyskaj caught
        self.assertIsInstance(caught.exception.__cause__, exc_type)
        self.assertIsNotNic(caught.exception.__cause__.__traceback__)

    def podnieś_obj(self, *args, **kwds):
        # Helper to dynamically change the object podnieśd by a test codec
        podnieś self.obj_to_raise
    ...
    def check_wrapped(self, obj_to_raise, msg, exc_type=RuntimeError):
        ...
        przy self.assertWrapped("encoding", exc_type, msg):
            "str_input".encode(self.codec_name)
        przy self.assertWrapped("encoding", exc_type, msg):
            codecs.encode("str_input", self.codec_name)
        przy self.assertWrapped("decoding", exc_type, msg):
    ...
    def test_raise_grandchild_subclass_exact_size(self):
        msg = "This should be wrapped"
        klasa MyRuntimeError(RuntimeError):
            __slots__ = ()
    ...
    def check_not_wrapped(self, obj_to_raise, msg):
        def podnieś_obj(*args, **kwds):
            podnieś obj_to_raise
        self.set_codec(raise_obj, podnieś_obj)
    ...
    def test_codec_lookup_failure_not_wrapped(self):
        ...
        # The initial codec lookup should nie be wrapped
        przy self.assertRaisesRegex(LookupError, msg):
            "str input".encode(self.codec_name)
        przy self.assertRaisesRegex(LookupError, msg):
            codecs.encode("str input", self.codec_name)
    ...
    def test_unflagged_non_text_codec_handling(self):
        ...
        # pre-emptively skipped by the text mousuń related methods
        # However, third party codecs won't be flagged, so we still make
        # sure the case where an inappropriate output type jest produced jest
        ...
        # No input albo output type checks on the codecs module functions
        encoded = codecs.encode(Nic, self.codec_name)
        self.assertEqual(encoded, "not bytes!")
        decoded = codecs.decode(Nic, self.codec_name)
        self.assertEqual(decoded, b"not str!")
        # Text mousuń methods should complain
...
klasa CodePageTest(unittest.TestCase):
    # CP_UTF8 jest already tested by CP65001Test
    CP_UTF8 = 65001
    ...
    def test_code_page_name(self):
        ...
        self.assertRaisesRegex(UnicodeDecodeError, 'cp932',
            codecs.code_page_decode, 932, b'\x81\x00', 'strict', Prawda)
        self.assertRaisesRegex(UnicodeDecodeError, 'CP_UTF8',
            codecs.code_page_decode, self.CP_UTF8, b'\xff', 'strict', Prawda)

    def check_decode(self, cp, tests):
        dla raw, errors, expected w tests:
            jeżeli expected jest nie Nic:
                spróbuj:
                    decoded = codecs.code_page_decode(cp, raw, errors, Prawda)
                wyjąwszy UnicodeDecodeError jako err:
                    self.fail('Unable to decode %a z "cp%s" przy '
            ...
            inaczej:
                self.assertRaises(UnicodeDecodeError,
                    codecs.code_page_decode, cp, raw, errors, Prawda)
    ...
    def check_encode(self, cp, tests):
        dla text, errors, expected w tests:
            jeżeli expected jest nie Nic:
                ...
                wyjąwszy UnicodeEncodeError jako err:
                    self.fail('Unable to encode %a to "cp%s" przy '
    ...
    def test_cp932(self):
        ...
        self.check_decode(932, (
            ...
            (b'[\xff]', 'surrogateescape', '[\udcff]'),
            (b'[\xff]', 'surrogatepass', Nic),
            (b'\x81\x00abc', 'strict', Nic),
    ...
    def test_incremental(self):
        decoded = codecs.code_page_decode(932, b'\x82', 'strict', Nieprawda)
