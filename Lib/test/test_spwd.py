...
klasa TestSpwdRoot(unittest.TestCase):
    ...
    def test_getspnam(self):
        entries = spwd.getspall()
        jeżeli nie entries:
            self.skipTest('empty shadow dalejword database')
        ...
        przy self.assertRaises(KeyError) jako cx:
            spwd.getspnam('invalid user name')
        self.assertEqual(str(cx.exception), "'getspnam(): name nie found'")
