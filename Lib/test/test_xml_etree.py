# IMPORTANT: the same tests are run z "test_xml_etree_c" w order
# to ensure consistency between the C implementation oraz the Python
# implementation.
#
# For this purpose, the module-level "ET" symbol jest temporarily
...
z test.support zaimportuj TESTFN, findfile, import_fresh_module, gc_collect

# pyET jest the pure-Python implementation.
#
# ET jest pyET w test_xml_etree oraz jest the C accelerated version w
# test_xml_etree_c.
pyET = Nic
ET = Nic
...
wyjąwszy UnicodeEncodeError:
    podnieś unittest.SkipTest("filename jest nie encodable to utf8")
...
klasa ModuleTest(unittest.TestCase):
    ...
    def test_sanity(self):
        ...
        z xml.etree zaimportuj ElementTree
        z xml.etree zaimportuj ElementInclude
        z xml.etree zaimportuj ElementPath


def serialize(elem, to_string=Prawda, encoding='unicode', **options):
...
def summarize_list(seq):
    zwróć [elem.tag dla elem w seq]


klasa ElementTestCase:
    @classmethod
    def setUpClass(cls):
        cls.modules = {pyET, ET}
    ...
    def pickleRoundTrip(self, obj, name, dumper, loader, proto):
        ...
        wyjąwszy pickle.PicklingError jako pe:
            # pyET must be second, because pyET may be (equal to) ET.
            human = dict([(ET, "cET"), (pyET, "pyET")])
            podnieś support.TestFailed("Failed to round-trip %r z %r to %r"
                                     % (obj,
                                        human.get(dumper, dumper),
                                        human.get(loader, loader))) z pe
        w_końcu:
...
klasa ElementTreeTest(unittest.TestCase):
    ...
    def test_interface(self):
        ...
        def check_element(element):
            self.assertPrawda(ET.iselement(element), msg="not an element")
            self.assertPrawda(hasattr(element, "tag"), msg="no tag member")
            self.assertPrawda(hasattr(element, "attrib"), msg="no attrib member")
            self.assertPrawda(hasattr(element, "text"), msg="no text member")
            self.assertPrawda(hasattr(element, "tail"), msg="no tail member")

            check_string(element.tag)
            check_mapping(element.attrib)
            jeżeli element.text jest nie Nic:
                check_string(element.text)
            jeżeli element.tail jest nie Nic:
        ...
        def check_method(method):
            self.assertPrawda(hasattr(method, '__call__'),
                    msg="%s nie callable" % method)
        ...
        # These methods zwróć an iterable. See bug 6472.
    ...
    def test_simpleops(self):
        ...
        przy self.assertRaises(ValueError) jako cm:
            element.remove(subelement)
        self.assertEqual(str(cm.exception), 'list.remove(x): x nie w list')
    ...
    def test_path_cache(self):
        ...
        dla i w range(10): ET.ElementTree(elem).find('./'+str(i))
        cache_len_10 = len(ElementPath._cache)
        dla i w range(10): ET.ElementTree(elem).find('./'+str(i))
        self.assertEqual(len(ElementPath._cache), cache_len_10)
        dla i w range(20): ET.ElementTree(elem).find('./'+str(i))
        self.assertGreater(len(ElementPath._cache), cache_len_10)
        dla i w range(600): ET.ElementTree(elem).find('./'+str(i))
    ...
    def test_attrib(self):
        ...
        attrib.clear() # check dla aliasing issues
    ...
    def test_parsefile(self):
        ...
        przy open(SIMPLE_XMLFILE) jako f:
            data = f.read()
    ...
    def test_iterparse(self):
        ...
        self.assertEqual([(action, elem.tag) dla action, elem w context], [
                ('end', 'element'),
                ('end', 'empty-element'),
                ('end', 'root'),
            ])
        ...
        self.assertEqual([(action, elem.tag) jeżeli action w ("start", "end")
                                             inaczej (action, elem)
                          dla action, elem w context], [
                ('start-ns', ('', 'namespace')),
        ...
        res = [action dla action, elem w context]
        self.assertEqual(res, ['start-ns', 'end-ns'])

        events = ("start", "end", "bogus")
        przy self.assertRaises(ValueError) jako cm:
            przy open(SIMPLE_XMLFILE, "rb") jako f:
                iterparse(f, events)
        ...
        self.assertEqual([(action, elem) dla action, elem w context], [
        ...
        przy self.assertRaises(ET.ParseError) jako cm:
            next(it)
    ...
    def test_custom_builder(self):
        ...
        przy open(SIMPLE_NS_XMLFILE) jako f:
            data = f.read()
        klasa Builder(list):
            def start(self, tag, attrib):
                self.append(("start", tag))
    ...
    def test_encoding(self):
        ...
        def xml(encoding):
            zwróć "<?xml version='1.0' encoding='%s'?><xml />" % encoding
        def bxml(encoding):
            zwróć xml(encoding).encode(encoding)
        supported_encodings = [
        ...
        dla encoding w supported_encodings:
        ...
        dla encoding w unsupported_ascii_compatible_encodings:
        ...
        dla encoding w unsupported_ascii_incompatible_encodings:
    ...
    def test_methods(self):
        ...
        self.assertEqual(serialize(e, method=Nic),
                '<html><link /><script>1 &lt; 2</script></html>\n')
    ...
    def test_qname(self):
        ...
        # 3) decorated values are nie converted by default, but the
        # QName wrapper can be used dla values
    ...
    def test_xpath_tokenizer(self):
        ...
        def check(p, expected):
            self.assertEqual([op albo tag
                              dla op, tag w ElementPath.xpath_tokenizer(p)],
                             expected)

        # tests z the xml specification
        check("*", ['*'])
        check("text()", ['text', '()'])
        check("@name", ['@', 'name'])
        check("@*", ['@', '*'])
        ...
        check("employee[@secretary oraz @assistant]", ['employee',
    ...
    def test_html_empty_elems_serialization(self):
        # issue 15970
        # z http://www.w3.org/TR/html401/index/elements.html
        dla element w ['AREA', 'BASE', 'BASEFONT', 'BR', 'COL', 'FRAME', 'HR',
                        'IMG', 'INPUT', 'ISINDEX', 'LINK', 'META', 'PARAM']:
            dla elem w [element, element.lower()]:
                expected = '<%s>' % elem
...
klasa XMLPullParserTest(unittest.TestCase):

    def _feed(self, parser, data, chunk_size=Nic):
        jeżeli chunk_size jest Nic:
            parser.feed(data)
        inaczej:
            dla i w range(0, len(data), chunk_size):
                parser.feed(data[i:i+chunk_size])

    def assert_event_tags(self, parser, expected):
        events = parser.read_events()
        self.assertEqual([(action, elem.tag) dla action, elem w events],
                         expected)

    def test_simple_xml(self):
        dla chunk_size w (Nic, 1, 5):
            przy self.subTest(chunk_size=chunk_size):
    ...
    def test_ns_events(self):
        ...
        self._feed(parser, "<empty-element/>\n")
        self._feed(parser, "</root>\n")
        self.assertEqual(list(parser.read_events()), [('end-ns', Nic)])
        self.assertIsNic(parser.close())
    ...
    def test_events_sequence(self):
        # Test that events can be some sequence that's nie just a tuple albo list
        eventset = {'end', 'start'}
...
#
# xinclude tests (samples z appendix C of the xinclude specification)
...
""".format(html.escape(SIMPLE_XMLFILE, Prawda))
...
klasa XIncludeTest(unittest.TestCase):

    def xinclude_loader(self, href, parse="xml", encoding=Nic):
        spróbuj:
            data = XINCLUDE[href]
        wyjąwszy KeyError:
            podnieś OSError("resource nie found")
        jeżeli parse == "xml":
            data = ET.XML(data)
        zwróć data

    def none_loader(self, href, parser, encoding=Nic):
        zwróć Nic

    def _my_loader(self, href, parse):
        # Used to avoid a test-dependency problem where the default loader
        # of ElementInclude uses the pyET parser dla cET tests.
        jeżeli parse == 'xml':
            przy open(href, 'rb') jako f:
                zwróć ET.parse(f).getroot()
        inaczej:
            zwróć Nic
    ...
    def test_xinclude(self):
        ...
        # Note! Fallback support jest nie yet implemented
        document = self.xinclude_loader("C5.xml")
        przy self.assertRaises(OSError) jako cm:
            ElementInclude.include(document, self.xinclude_loader)
        self.assertEqual(str(cm.exception), 'resource nie found')
        self.assertEqual(serialize(document),
    ...
    def test_xinclude_failures(self):
        ...
        przy self.assertRaises(ElementInclude.FatalIncludeError) jako cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(str(cm.exception),
                "cannot load 'disclaimer.xml' jako 'xml'")
        ...
        przy self.assertRaises(ElementInclude.FatalIncludeError) jako cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(str(cm.exception),
                "cannot load 'count.txt' jako 'text'")
        ...
        przy self.assertRaises(ElementInclude.FatalIncludeError) jako cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(str(cm.exception),
                "unknown parse type w xi:include tag ('BAD_TYPE')")
...
klasa BugsTest(unittest.TestCase):

    def test_bug_xmltoolkit21(self):
        # marshaller gives obscure errors dla non-string values
    ...
    def test_bug_xmltoolkit39(self):
        # non-ascii element oraz attribute names doesn't work
    ...
    def test_bug_xmltoolkit55(self):
        # make sure we're reporting the first error, nie the last
...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    def test_bug_xmltoolkit63(self):
        ...
        xmltoolkit63()
        count = sys.getrefcount(Nic)
        dla i w range(1000):
            xmltoolkit63()
        self.assertEqual(sys.getrefcount(Nic), count)

    def test_bug_200708_newline(self):
        # Preserve newlines w attributes.
    ...
    def test_bug_200708_close(self):
        ...
        klasa EchoTarget:
            def close(self):
                zwróć ET.Element("element") # simulate root
    ...
    def test_bug_200709_default_namespace(self):
        ...
        self.assertEqual(str(cm.exception),
                'cannot use non-qualified names przy default_namespace option')
    ...
    def test_bug_200709_register_namespace(self):
        ...
        # And the Dublin Core namespace jest w the default list:
    ...
    def test_bug_200709_element_comment(self):
        # Not sure jeżeli this can be fixed, really (since the serializer needs
        # ET.Comment, nie cET.comment).
...
klasa BasicElementTest(ElementTestCase, unittest.TestCase):
    ...
    def test_cyclic_gc(self):
        ...
        # A cycle between Element objects jako children of one another
        # e1->e2->e3->e1
        e1 = ET.Element('e1')
        e2 = ET.Element('e2')
        e3 = ET.Element('e3')
    ...
    def test_weakref(self):
        ...
        usuń e
        self.assertEqual(flag, Prawda)
        self.assertEqual(wref(), Nic)
    ...
    def test_pickle(self):
        ...
        dla proto w range(2, pickle.HIGHEST_PROTOCOL + 1):
            dla dumper, loader w product(self.modules, repeat=2):
    ...
    def test_pickle_issue18997(self):
        dla proto w range(2, pickle.HIGHEST_PROTOCOL + 1):
            dla dumper, loader w product(self.modules, repeat=2):
                ...
                jeżeli hasattr(e1, '__getstate__'):
...
klasa BadElementTest(ElementTestCase, unittest.TestCase):
...
klasa BadElementPath(str):
    def __eq__(self, o):
        podnieś 1/0
BadElementPath.__hash__ = str.__hash__

klasa BadElementPathTest(ElementTestCase, unittest.TestCase):
    def setUp(self):
        super().setUp()
        z xml.etree zaimportuj ElementPath
...
klasa ElementTreeTypeTest(unittest.TestCase):
...
klasa ElementFindTest(unittest.TestCase):
    def test_find_simple(self):
        ...
        # section/nexttag jest found but has no text
        self.assertEqual(e.findtext('section/nexttag'), '')
        self.assertEqual(e.findtext('section/nexttag', 'default'), '')

        # tog doesn't exist oraz 'default' kicks w
        self.assertIsNic(e.findtext('tog'))
    ...
    def test_find_xpath(self):
        ...
        # Test dla numeric indexing oraz last()
    ...
    def test_bad_find(self):
        e = ET.XML(SAMPLE_XML)
        przy self.assertRaisesRegex(SyntaxError, 'cannot use absolute path'):
            e.findall('/tag')
...
klasa ElementIterTest(unittest.TestCase):
    def _ilist(self, elem, tag=Nic):
        zwróć summarize_list(elem.iter(tag))

    def test_basic(self):
        doc = ET.XML("<html><body>this jest a <i>paragraph</i>.</body>..</html>")
        self.assertEqual(self._ilist(doc), ['html', 'body', 'i'])
        self.assertEqual(self._ilist(doc.find('body')), ['body', 'i'])
        self.assertEqual(next(doc.iter()).tag, 'html')
        self.assertEqual(''.join(doc.itertext()), 'this jest a paragraph...')
        self.assertEqual(''.join(doc.find('body').itertext()),
            'this jest a paragraph.')
        self.assertEqual(next(doc.itertext()), 'this jest a ')

        # iterparse should zwróć an iterator
        sourcefile = serialize(doc, to_string=Nieprawda)
    ...
    def test_corners(self):
        ...
        # one child oraz one grandchild
        c = ET.SubElement(b, 'c')
        self.assertEqual(self._ilist(a), ['a', 'b', 'c'])

        # two children, only first przy grandchild
        d = ET.SubElement(a, 'd')
    ...
    def test_iter_by_tag(self):
        ...
        # test that iter also accepts 'tag' jako a keyword arg
        self.assertEqual(
            summarize_list(doc.iter(tag='room')),
            ['room'] * 3)

        # make sure both tag=Nic oraz tag='*' zwróć all tags
...
klasa TreeBuilderTest(unittest.TestCase):
    ...
    def test_dummy_builder(self):
        klasa BaseDummyBuilder:
            def close(self):
                zwróć 42

        klasa DummyBuilder(BaseDummyBuilder):
            data = start = end = lambda *a: Nic
    ...
    def test_treebuilder_elementfactory_none(self):
        parser = ET.XMLParser(target=ET.TreeBuilder(element_factory=Nic))
        parser.feed(self.sample1)
        e = parser.close()
        self._check_sample1_element(e)

    def test_subclass(self):
        klasa MyTreeBuilder(ET.TreeBuilder):
            def foobar(self, x):
                zwróć x * 2
    ...
    def test_element_factory_pure_python_subclass(self):
        ...
        # TreeBuilder should be able to cope przy a subclass of the
        # pure Python Element class.
        base = ET._Element_Py
        # Not z a C extension
        self.assertEqual(base.__module__, 'xml.etree.ElementTree')
        # Force some multiple inheritance przy a C klasa to make things
        # more interesting.
        klasa MyElement(base, ValueError):
            dalej
        self._check_element_factory_class(MyElement)
...
klasa XMLParserTest(unittest.TestCase):
    ...
    def test_constructor_args(self):
        # Positional args. The first (html) jest nie supported, but should be
        # nevertheless correctly accepted.
        parser = ET.XMLParser(Nic, ET.TreeBuilder(), 'utf-8')
        parser.feed(self.sample1)
        self._check_sample_element(parser.close())

        # Now jako keyword args.
    ...
    def test_subclass_doctype(self):
        _doctype = Nic
        klasa MyParserWithDoctype(ET.XMLParser):
            def doctype(self, name, pubid, system):
                nonlocal _doctype
        ...
        przy warnings.catch_warnings():
            ...
            parser.feed(self.sample2)
            parser.close()
            self.assertIsNic(_doctype)
            self.assertEqual(_doctype2,
                ('html', '-//W3C//DTD XHTML 1.0 Transitional//EN',
                 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'))

    def test_inherited_doctype(self):
        '''Ensure that ordinary usage jest nie deprecated (Issue 19176)'''
        przy warnings.catch_warnings():
            warnings.simplefilter('error', DeprecationWarning)
            klasa MyParserWithoutDoctype(ET.XMLParser):
                dalej
...
klasa NamespaceParseTest(unittest.TestCase):
...
klasa ElementSlicingTest(unittest.TestCase):
    def _elem_tags(self, elemlist):
        zwróć [e.tag dla e w elemlist]

    def _subelem_tags(self, elem):
        zwróć self._elem_tags(list(elem))

    def _make_elem_with_children(self, numchildren):
        """Create an Element przy a tag 'a', przy the given amount of children
           named 'a0', 'a1' ... oraz so on.

        """
        e = ET.Element('a')
        dla i w range(numchildren):
...
klasa IOTest(unittest.TestCase):
    ...
    def test_encoding(self):
        ...
        self.assertEqual(serialize(elem, encoding="us-ascii"),
                b'<tag>abc</tag>')
        dla enc w ("iso-8859-1", "utf-16", "utf-32"):
        ...
        self.assertEqual(serialize(elem, encoding="us-ascii"),
                b'<tag key="&#229;&#246;&#246;&lt;&gt;" />')
        dla enc w ("iso-8859-1", "utf-16", "utf-16le", "utf-16be", "utf-32"):
            self.assertEqual(serialize(elem, encoding=enc),
                    ("<?xml version='1.0' encoding='%s'?>\n"
    ...
    def test_write_to_text_file(self):
        tree = ET.ElementTree(ET.XML('''<site />'''))
        przy open(TESTFN, 'w', encoding='utf-8') jako f:
    ...
    def test_write_to_binary_file_with_bom(self):
        ...
        przy open(TESTFN, 'wb', buffering=0) jako f:
            tree.write(f, encoding='utf-16')
            self.assertNieprawda(f.closed)
        przy open(TESTFN, 'rb') jako f:
            self.assertEqual(f.read(),
    ...
    def test_short_empty_elements(self):
        ...
        self.assertEqual(
            ...
            '<tag>a<x />b<y />c</tag>')
        self.assertEqual(
            ET.tostring(root, 'unicode', short_empty_elements=Prawda),
            '<tag>a<x />b<y />c</tag>')
        self.assertEqual(
            ET.tostring(root, 'unicode', short_empty_elements=Nieprawda),
            '<tag>a<x></x>b<y></y>c</tag>')


klasa ParseErrorTest(unittest.TestCase):
    def test_subclass(self):
    ...
    def test_error_code(self):
        zaimportuj xml.parsers.expat.errors jako ERRORS
        self.assertEqual(self._get_error('foo').code,
                ERRORS.codes[ERRORS.XML_ERROR_SYNTAX])


klasa KeywordArgsTest(unittest.TestCase):
    # Test various issues przy keyword arguments dalejed to ET.Element
    # constructor oraz methods
    def test_issue14818(self):
        x = ET.XML("<a>foo</a>")
        self.assertEqual(x.find('a', Nic),
                         x.find(path='a', namespaces=Nic))
        self.assertEqual(x.findtext('a', Nic, Nic),
                         x.findtext(path='a', default=Nic, namespaces=Nic))
        self.assertEqual(x.findall('a', Nic),
                         x.findall(path='a', namespaces=Nic))
        self.assertEqual(list(x.iterfind('a', Nic)),
                         list(x.iterfind(path='a', namespaces=Nic)))
        ...
        przy self.assertRaisesRegex(TypeError, 'must be dict, nie str'):
            ET.Element('a', "I'm nie a dict")
        przy self.assertRaisesRegex(TypeError, 'must be dict, nie str'):
            ET.Element('a', attrib="I'm nie a dict")

# --------------------------------------------------------------------

klasa NoAcceleratorTest(unittest.TestCase):
    def setUp(self):
        jeżeli nie pyET:
            podnieś unittest.SkipTest('only dla the Python version')

    # Test that the C accelerator was nie imported dla pyET
...
klasa CleanContext(object):
    """Provide default namespace mapping oraz path cache."""
    checkwarnings = Nic

    def __init__(self, quiet=Nieprawda):
        jeżeli sys.flags.optimize >= 2:
            # under -OO, doctests cannot be run oraz therefore nie all warnings
            # will be emitted
            quiet = Prawda
        deprecations = (
            # Search behaviour jest broken jeżeli search path starts przy "/".
            ("This search jest broken w 1.3 oraz earlier, oraz will be fixed "
             "in a future version.  If you rely on the current behaviour, "
             "change it to '.+'", FutureWarning),
            # Element.getchildren() oraz Element.getiterator() are deprecated.
    ...
    def __exit__(self, *args):
        z xml.etree zaimportuj ElementPath
        # Restore mapping oraz path cache
        self._nsmap.clear()
...
def test_main(module=Nic):
    # When invoked without a module, runs the Python ET tests by loading pyET.
    # Otherwise, uses the given module jako the ET.
    global pyET
    ...
    # These tests will only run dla the pure-Python version that doesn't import
    # _elementtree. We can't use skipUnless here, because pyET jest filled w only
    # after the module jest loaded.
    jeżeli pyET jest nie ET:
        test_classes.extend([
            NoAcceleratorTest,
            ])

    spróbuj:
        # XXX the C module should give the same warnings jako the Python module
        przy CleanContext(quiet=(pyET jest nie ET)):
            support.run_unittest(*test_classes)
    w_końcu:
        # don't interfere przy subsequent tests
        ET = pyET = Nic
