...
klasa GeneralTests(TestCase):
    ...
    def testTimeoutNic(self):
        # Nic, having other default
        self.assertPrawda(socket.getdefaulttimeout() jest Nic)
        socket.setdefaulttimeout(30)
        spróbuj:
            telnet = telnetlib.Telnet(HOST, self.port, timeout=Nic)
        w_końcu:
            socket.setdefaulttimeout(Nic)
        self.assertPrawda(telnet.sock.gettimeout() jest Nic)
        telnet.sock.close()
...
klasa TelnetAlike(telnetlib.Telnet):
    def fileno(self):
        podnieś NotImplementedError()
    def close(self): dalej
    def sock_avail(self):
        zwróć (nie self.sock.block)
...
klasa MockSelector(selectors.BaseSelector):
    ...
    def select(self, timeout=Nic):
        block = Nieprawda
        dla fileobj w self.keys:
            jeżeli isinstance(fileobj, TelnetAlike):
                block = fileobj.sock.block
                przerwij
        jeżeli block:
            zwróć []
        inaczej:
            zwróć [(key, key.events) dla key w self.keys.values()]
...
def test_telnet(reads=(), cls=TelnetAlike):
    ''' zwróć a telnetlib.Telnet object that uses a SocketStub with
        reads queued up to be read '''
    dla x w reads:
        assert type(x) jest bytes, x
    przy test_socket(reads):
        telnet = cls('dummy', 0)
        telnet._messages = '' # debuglevel output
    zwróć telnet

klasa ExpectAndReadTestCase(TestCase):
    def setUp(self):
...
klasa ReadTests(ExpectAndReadTestCase):
    def test_read_until(self):
        """
        read_until(expected, timeout=Nic)
    ...
    def _read_eager(self, func_name):
        ...
        telnet.sock.block = Prawda
        self.assertEqual(b'', func())
        telnet.sock.block = Nieprawda
        data = b''
        dopóki Prawda:
            spróbuj:
                data += func()
    ...
    def test_read_eager(self):
        # read_eager oraz read_very_eager make the same gaurantees
        # (they behave differently but we only test the gaurantees)
        self._read_eager('read_eager')
        self._read_eager('read_very_eager')
        # NB -- we need to test the IAC block which jest mentioned w the
        # docstring but nie w the module docs

    def read_very_lazy(self):
        want = b'x' * 100
        telnet = test_telnet([want])
        self.assertEqual(b'', telnet.read_very_lazy())
        dopóki telnet.sock.reads:
            telnet.fill_rawq()
    ...
    def test_read_lazy(self):
        ...
        dopóki Prawda:
            ...
            wyjąwszy EOFError:
                przerwij
            self.assertPrawda(want.startswith(data))
        self.assertEqual(data, want)

klasa nego_collector(object):
    def __init__(self, sb_getter=Nic):
        self.seen = b''
    ...
    def do_nego(self, sock, cmd, opt):
        self.seen += cmd + opt
        jeżeli cmd == tl.SE oraz self.sb_getter:
            sb_data = self.sb_getter()
            self.sb_seen += sb_data

tl = telnetlib

klasa WriteTests(TestCase):
    '''The only thing that write does jest replace each tl.IAC for
    tl.IAC+tl.IAC'''
...
klasa OptionTests(TestCase):
    ...
    def _test_command(self, data):
        ...
        txt = telnet.read_all()
        cmd = nego.seen
        self.assertPrawda(len(cmd) > 0) # we expect at least one command
        self.assertIn(cmd[:1], self.cmds)
        self.assertEqual(cmd[1:2], tl.NOOPT)
        self.assertEqual(data_len, len(txt + cmd))
        nego.sb_getter = Nic # przerwij the nego => telnet cycle
    ...
    def test_IAC_commands(self):
        ...
        # all at once
        self._test_command([tl.IAC + cmd dla (cmd) w self.cmds])
    ...
    def test_debuglevel_reads(self):
        # test all the various places that self.msg(...) jest called
        given_a_expect_b = [
            # Telnet.fill_rawq
            (b'a', ": recv b''\n"),
            # Telnet.process_rawq
            (tl.IAC + bytes([88]), ": IAC 88 nie recognized\n"),
            (tl.IAC + tl.DO + bytes([1]), ": IAC DO 1\n"),
            (tl.IAC + tl.DONT + bytes([1]), ": IAC DONT 1\n"),
            (tl.IAC + tl.WILL + bytes([1]), ": IAC WILL 1\n"),
            (tl.IAC + tl.WONT + bytes([1]), ": IAC WONT 1\n"),
           ]
        dla a, b w given_a_expect_b:
...
klasa ExpectTests(ExpectAndReadTestCase):
