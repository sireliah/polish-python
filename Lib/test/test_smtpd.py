...
klasa DummyServer(smtpd.SMTPServer):
    ...
    def process_message(self, peer, mailfrom, rcpttos, data, **kw):
        ...
        jeżeli data == self.return_status:
            zwróć '250 Okish'
        jeżeli 'mail_options' w kw oraz 'SMTPUTF8' w kw['mail_options']:
            zwróć '250 SMTPUTF8 message okish'


klasa DummyDispatcherBroken(Exception):
    dalej


klasa BrokenDummyServer(DummyServer):
    def listen(self, num):
        podnieś DummyDispatcherBroken()


klasa SMTPDServerTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_process_message_unimplemented(self):
        ...
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=Prawda)
...
klasa DebuggingServerTest(unittest.TestCase):

    def setUp(self):
        smtpd.socket = asyncore.socket = mock_socket

    def send_data(self, channel, data, enable_SMTPUTF8=Nieprawda):
        def write_line(line):
            channel.socket.queue_recv(line)
            channel.handle_read()
    ...
    def test_process_message_with_decode_data_true(self):
        ...
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=Prawda)
        przy support.captured_stdout() jako s:
    ...
    def test_process_message_with_decode_data_false(self):
        ...
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=Nieprawda)
        przy support.captured_stdout() jako s:
            self.send_data(channel, b'From: test\n\nh\xc3\xa9llo\xff\n')
        stdout = s.getvalue()
        self.assertEqual(stdout, textwrap.dedent("""\
    ...
    def test_process_message_with_enable_SMTPUTF8_true(self):
        server = smtpd.DebuggingServer((support.HOST, 0), ('b', 0),
                                       enable_SMTPUTF8=Prawda)
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, enable_SMTPUTF8=Prawda)
        przy support.captured_stdout() jako s:
            self.send_data(channel, b'From: test\n\nh\xc3\xa9llo\xff\n')
        stdout = s.getvalue()
        self.assertEqual(stdout, textwrap.dedent("""\
...
klasa TestFamilyDetection(unittest.TestCase):
    def setUp(self):
        smtpd.socket = asyncore.socket = mock_socket
    ...
    @unittest.skipUnless(support.IPV6_ENABLED, "IPv6 nie enabled")
...
klasa TestRcptOptionParsing(unittest.TestCase):
    error_response = (b'555 RCPT TO parameters nie recognized albo nie '
                      b'implemented\r\n')
    ...
    def test_params_rejected(self):
        server = DummyServer((support.HOST, 0), ('b', 0), decode_data=Nieprawda)
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=Nieprawda)
...
klasa TestMailOptionParsing(unittest.TestCase):
    error_response = (b'555 MAIL FROM parameters nie recognized albo nie '
                      b'implemented\r\n')
    ...
    def test_with_decode_data_true(self):
        server = DummyServer((support.HOST, 0), ('b', 0), decode_data=Prawda)
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=Prawda)
        self.write_line(channel, b'EHLO example')
        dla line w [
    ...
    def test_with_enable_smtputf8_true(self):
        server = DummyServer((support.HOST, 0), ('b', 0), enable_SMTPUTF8=Prawda)
        conn, addr = server.accept()
        channel = smtpd.SMTPChannel(server, conn, addr, enable_SMTPUTF8=Prawda)
        self.write_line(channel, b'EHLO example')
        self.write_line(
...
klasa SMTPDChannelTest(unittest.TestCase):
    def setUp(self):
        smtpd.socket = asyncore.socket = mock_socket
        self.old_debugstream = smtpd.DEBUGSTREAM
        self.debug = smtpd.DEBUGSTREAM = io.StringIO()
    ...
    def test_broken_connect(self):
        self.assertRaises(
            ...
            (support.HOST, 0), ('b', 0), decode_data=Prawda)
    ...
    def test_MAIL_RCPT_unknown_parameters(self):
        ...
        self.assertEqual(self.channel.socket.last,
            b'555 MAIL FROM parameters nie recognized albo nie implemented\r\n')

        self.write_line(b'MAIL FROM:<eggs@example>')
        self.write_line(b'RCPT TO:<eggs@example> ham=green')
        self.assertEqual(self.channel.socket.last,
            b'555 RCPT TO parameters nie recognized albo nie implemented\r\n')
    ...
    def test_MAIL_syntax_HELO(self):
        self.write_line(b'HELO example')
        self.write_line(b'MAIL z eggs@example')
        self.assertEqual(self.channel.socket.last,
            b'501 Syntax: MAIL FROM: <address>\r\n')
    ...
    def test_VRFY(self):
        ...
        self.assertEqual(self.channel.socket.last,
            b'252 Cannot VRFY user, but will accept message oraz attempt ' + \
            b'delivery\r\n')
    ...
    def test_data_dialog(self):
        ...
        self.write_line(b'DATA')
        self.assertEqual(self.channel.socket.last,
            b'354 End data przy <CR><LF>.<CR><LF>\r\n')
        self.write_line(b'data\r\nmore\r\n.')
        self.assertEqual(self.channel.socket.last, b'250 OK\r\n')
    ...
    def test_manual_status(self):
        # checks that the Channel jest able to zwróć a custom status message
        self.write_line(b'HELO example')
        self.write_line(b'MAIL From:eggs@example')
        self.write_line(b'RCPT To:spam@example')
        self.write_line(b'DATA')
    ...
    def test_unknown_command(self):
        ...
        self.assertEqual(self.channel.socket.last,
                         b'500 Error: command "UNKNOWN_CMD" nie ' + \
                         b'recognized\r\n')
    ...
    def test_attribute_deprecations(self):
        ...
        przy support.check_warnings(('', DeprecationWarning)):
            self.channel._SMTPChannel__mailz = 'spam'
        przy support.check_warnings(('', DeprecationWarning)):
...
@unittest.skipUnless(support.IPV6_ENABLED, "IPv6 nie enabled")
klasa SMTPDChannelIPv6Test(SMTPDChannelTest):
    def setUp(self):
        smtpd.socket = asyncore.socket = mock_socket
        self.old_debugstream = smtpd.DEBUGSTREAM
        self.debug = smtpd.DEBUGSTREAM = io.StringIO()
...
klasa SMTPDChannelWithDataSizeLimitTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = DummyServer((support.HOST, 0), ('b', 0),
                                  decode_data=Prawda)
        conn, addr = self.server.accept()
        # Set DATA size limit to 32 bytes dla easy testing
    ...
    def test_data_limit_dialog_too_much_data(self):
        ...
        self.assertEqual(self.channel.socket.last,
            b'354 End data przy <CR><LF>.<CR><LF>\r\n')
        self.write_line(b'This message jest longer than 32 bytes\r\n.')
        self.assertEqual(self.channel.socket.last,
                         b'552 Error: Too much mail data\r\n')


klasa SMTPDChannelWithDecodeDataNieprawda(unittest.TestCase):
...
klasa SMTPDChannelWithDecodeDataPrawda(unittest.TestCase):
...
klasa SMTPDChannelTestWithEnableSMTPUTF8Prawda(unittest.TestCase):
    def setUp(self):
        smtpd.socket = asyncore.socket = mock_socket
        self.old_debugstream = smtpd.DEBUGSTREAM
        self.debug = smtpd.DEBUGSTREAM = io.StringIO()
    ...
    def test_process_smtputf8_message(self):
        self.write_line(b'EHLO example')
        dla mail_parameters w [b'', b'BODY=8BITMIME SMTPUTF8']:
