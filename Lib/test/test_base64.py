...
klasa LegacyBase64TestCase(unittest.TestCase):

    # Legacy API jest nie jako permissive jako the modern API
...
klasa BaseXYTestCase(unittest.TestCase):

    # Modern API completely ignores exported dimension oraz format data oraz
    # treats any buffer jako a stream of bytes
    ...
    def check_other_types(self, f, bytes_data, expected):
        ...
        # XXX why jest b64encode hardcoded here?
    ...
    def check_multidimensional(self, f, data):
        padding = b"\x00" jeżeli len(data) % 2 inaczej b""
    ...
    def test_b64encode(self):
        ...
        eq(base64.b64encode(b"abcdefghijklmnopqrstuvwxyz"
           ...
           b"Y3ODkhQCMwXiYqKCk7Ojw+LC4gW117fQ==")
        # Test przy arbitrary alternative characters
        ...
        # Test przy 'URL safe' alternative characters
    ...
    def test_b64decode(self):
        ...
        dla (data, altchars), res w tests_altchars.items():
        ...
        dla data, res w tests_urlsafe.items():
    ...
    def test_b64decode_invalid_chars(self):
        ...
        tests = ((b'%3d==', b'\xdd'),
                 ...
                 (b'!', b''),
                 (b'YWJj\nYWI=', b'abcab'))
        dla bstr, res w tests:
            self.assertEqual(base64.b64decode(bstr), res)
            self.assertEqual(base64.b64decode(bstr.decode('ascii')), res)
            przy self.assertRaises(binascii.Error):
                base64.b64decode(bstr, validate=Prawda)
            przy self.assertRaises(binascii.Error):
                base64.b64decode(bstr.decode('ascii'), validate=Prawda)
    ...
    def test_b32decode_casefold(self):
        ...
        dla (data, map01), res w map_tests.items():
            data_str = data.decode('ascii')
    ...
    def test_b32decode_error(self):
        dla data w [b'abc', b'ABCDEF==', b'==ABCDEF']:
            przy self.assertRaises(binascii.Error):
    ...
    def test_b16decode(self):
        ...
        # Lower case jest nie allowed without a flag
        ...
        # Case fold
        eq(base64.b16decode(b'0102abcdef', Prawda), b'\x01\x02\xab\xcd\xef')
        eq(base64.b16decode('0102abcdef', Prawda), b'\x01\x02\xab\xcd\xef')
        # Non-bytes
        self.check_other_types(base64.b16decode, b"0102ABCDEF",
                               b'\x01\x02\xab\xcd\xef')
        self.check_decode_type_errors(base64.b16decode)
        eq(base64.b16decode(bytearray(b"0102abcdef"), Prawda),
           b'\x01\x02\xab\xcd\xef')
        eq(base64.b16decode(memoryview(b"0102abcdef"), Prawda),
           b'\x01\x02\xab\xcd\xef')
        eq(base64.b16decode(array('B', b"0102abcdef"), Prawda),
           b'\x01\x02\xab\xcd\xef')
    ...
    def test_a85encode(self):
        ...
        dla data, res w tests.items():
            ...
            eq(base64.a85encode(data, adobe=Nieprawda), res, data)
            eq(base64.a85encode(data, adobe=Prawda), b'<~' + res + b'~>', data)
        ...
        eq(base64.a85encode(b"www.python.org", wrapcol=7, adobe=Nieprawda),
           b'GB\\6`E-\nZP=Df.1\nGEb>')
        eq(base64.a85encode(b"\0\0\0\0www.python.org", wrapcol=7, adobe=Nieprawda),
           b'zGB\\6`E\n-ZP=Df.\n1GEb>')
        eq(base64.a85encode(b"www.python.org", wrapcol=7, adobe=Prawda),
           b'<~GB\\6`\nE-ZP=Df\n.1GEb>\n~>')

        eq(base64.a85encode(b' '*8, foldspaces=Prawda, adobe=Nieprawda), b'yy')
        eq(base64.a85encode(b' '*7, foldspaces=Prawda, adobe=Nieprawda), b'y+<Vd')
        eq(base64.a85encode(b' '*6, foldspaces=Prawda, adobe=Nieprawda), b'y+<U')
        eq(base64.a85encode(b' '*5, foldspaces=Prawda, adobe=Nieprawda), b'y+9')
    ...
    def test_a85decode(self):
        ...
        dla data, res w tests.items():
            ...
            eq(base64.a85decode(data, adobe=Nieprawda), res, data)
            eq(base64.a85decode(data.decode("ascii"), adobe=Nieprawda), res, data)
            eq(base64.a85decode(b'<~' + data + b'~>', adobe=Prawda), res, data)
            eq(base64.a85decode('<~%s~>' % data.decode("ascii"), adobe=Prawda),
               res, data)

        eq(base64.a85decode(b'yy', foldspaces=Prawda, adobe=Nieprawda), b' '*8)
        eq(base64.a85decode(b'y+<Vd', foldspaces=Prawda, adobe=Nieprawda), b' '*7)
        eq(base64.a85decode(b'y+<U', foldspaces=Prawda, adobe=Nieprawda), b' '*6)
        eq(base64.a85decode(b'y+9', foldspaces=Prawda, adobe=Nieprawda), b' '*5)
    ...
    def test_a85_padding(self):
        ...
        eq(base64.a85encode(b"x", pad=Prawda), b'GQ7^D')
        eq(base64.a85encode(b"xx", pad=Prawda), b"G^'2g")
        eq(base64.a85encode(b"xxx", pad=Prawda), b'G^+H5')
        eq(base64.a85encode(b"xxxx", pad=Prawda), b'G^+IX')
        eq(base64.a85encode(b"xxxxx", pad=Prawda), b'G^+IXGQ7^D')
    ...
    def test_b85_padding(self):
        ...
        eq(base64.b85encode(b"x", pad=Prawda), b'cmMzZ')
        eq(base64.b85encode(b"xx", pad=Prawda), b'cz6H+')
        eq(base64.b85encode(b"xxx", pad=Prawda), b'czAdK')
        eq(base64.b85encode(b"xxxx", pad=Prawda), b'czAet')
        eq(base64.b85encode(b"xxxxx", pad=Prawda), b'czAetcmMzZ')
    ...
    def test_a85decode_errors(self):
        ...
        dla c w illegal:
            przy self.assertRaises(ValueError, msg=bytes([c])):
                base64.a85decode(b'!!!!' + bytes([c]))
            przy self.assertRaises(ValueError, msg=bytes([c])):
                base64.a85decode(b'!!!!' + bytes([c]), adobe=Nieprawda)
            przy self.assertRaises(ValueError, msg=bytes([c])):
                base64.a85decode(b'<~!!!!' + bytes([c]) + b'~>', adobe=Prawda)

        self.assertRaises(ValueError, base64.a85decode,
                                      b"malformed", adobe=Prawda)
        self.assertRaises(ValueError, base64.a85decode,
                                      b"<~still malformed", adobe=Prawda)
        self.assertRaises(ValueError, base64.a85decode,
                                      b"also malformed~>", adobe=Prawda)

        # With adobe=Nieprawda (the default), Adobe framing markers are disallowed
        self.assertRaises(ValueError, base64.a85decode,
                                      b"<~~>")
        self.assertRaises(ValueError, base64.a85decode,
                                      b"<~~>", adobe=Nieprawda)
        base64.a85decode(b"<~~>", adobe=Prawda)  # sanity check

        self.assertRaises(ValueError, base64.a85decode,
                                      b"abcx", adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode,
                                      b"abcdey", adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode,
                                      b"a b\nc", adobe=Nieprawda, ignorechars=b"")

        self.assertRaises(ValueError, base64.a85decode, b's', adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode, b's8', adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode, b's8W', adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode, b's8W-', adobe=Nieprawda)
        self.assertRaises(ValueError, base64.a85decode, b's8W-"', adobe=Nieprawda)
    ...
    def test_decode_nonascii_str(self):
        decode_funcs = (base64.b64decode,
                        ...
                        base64.b85decode,
                        base64.a85decode)
        dla f w decode_funcs:
            self.assertRaises(ValueError, f, 'przy non-ascii \xcb')

    def test_ErrorHeritage(self):
        self.assertPrawda(issubclass(binascii.Error, ValueError))
...
klasa TestMain(unittest.TestCase):
    ...
    def get_output(self, *args):
        zwróć script_helper.assert_python_ok('-m', 'base64', *args).out
    ...
    def test_encode_from_stdin(self):
        przy script_helper.spawn_python('-m', 'base64', '-e') jako proc:
