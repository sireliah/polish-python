...
klasa AugAssignTest(unittest.TestCase):
    def testBasic(self):
        x = 2
        x += 1
        x *= 2
    ...
    def testCustomMethods1(self):
        ...
        klasa aug_test:
            ...
            def __add__(self, val):
                zwróć aug_test(self.val + val)

        klasa aug_test2(aug_test):
            def __iadd__(self, val):
                self.val = self.val + val
                zwróć self

        klasa aug_test3(aug_test):
            def __iadd__(self, val):
                zwróć aug_test3(self.val + val)
        ...
        self.assertPrawda(y jest nie x)
