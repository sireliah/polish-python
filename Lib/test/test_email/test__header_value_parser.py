...
klasa TestTokens(TestEmailBase):
...
klasa TestParser(TestParserMixin, TestEmailBase):
    ...
    def test_get_encoded_word_missing_start_raises(self):
        przy self.assertRaises(errors.HeaderParseError):
    ...
    def test_get_encoded_word_internal_spaces(self):
        self._test_get_x(parser.get_encoded_word,
                         '=?us-ascii?q?this jest a test?=  bird',
                         'this jest a test',
                         'this jest a test',
    ...
    # get_word (jeżeli this were black box we'd repeat all the qs/atom tests)

    def test_get_word_atom_uzyskajs_atom(self):
    ...
    def test_get_word_qs_uzyskajs_qs(self):
        word = self._test_get_x(parser.get_word,
    ...
    def test_get_phrase_simple(self):
        phrase = self._test_get_x(parser.get_phrase,
            '"Fred A. Johnson" jest his name, oh.',
            '"Fred A. Johnson" jest his name',
            'Fred A. Johnson jest his name',
            [],
            ', oh.')
    ...
    def test_get_phrase_complex(self):
        phrase = self._test_get_x(parser.get_phrase,
            ' (A) bird (in (my|your)) "hand  " jest messy\t<>\t',
            ' (A) bird (in (my|your)) "hand  " jest messy\t',
            ' bird hand   jest messy ',
            [],
            '<>\t')
    ...
    def test_get_phrase_ending_with_obsolete(self):
        phrase = self._test_get_x(parser.get_phrase,
            'simple phrase.(przy trailing comment):boo',
            'simple phrase.(przy trailing comment)',
            'simple phrase. ',
            [errors.ObsoleteHeaderDefect]*2,
            ':boo')
        self.assertEqual(len(phrase), 4)
        self.assertEqual(phrase[3].comments, ['przy trailing comment'])
    ...
    def test_get_local_part_complex_obsolete_invalid(self):
        local_part = self._test_get_x(parser.get_local_part,
            ' (foo )Fred (bar).(bird) A.(sheep)Johnson "and  dogs"@python.org',
            ' (foo )Fred (bar).(bird) A.(sheep)Johnson "and  dogs"',
            ' Fred . A. Johnson oraz  dogs',
            [errors.InvalidHeaderDefect]*2,
            '@python.org')
        self.assertEqual(local_part.local_part, 'Fred.A.Johnson oraz  dogs')
    ...
    def test_get_local_part_unicode_defect(self):
        # Currently this only happens when parsing unicode, nie when parsing
    ...
    def test_get_obs_route_complex(self):
        obs_route = self._test_get_x(parser.get_obs_route,
            ...
            '(foo),, (blue)@example.com (bar),@two.(foo) example.com (bird):',
            ' ,, @example.com ,@two. example.com :',
            [errors.ObsoleteHeaderDefect],  # This jest the obs-domain
            '')
        self.assertEqual(obs_route.token_type, 'obs-route')
        self.assertEqual(obs_route.domains, ['example.com', 'two.example.com'])
    ...
    def test_get_angle_addr_simple(self):
        ...
        self.assertIsNic(angle_addr.route)
    ...
    def test_get_angle_addr_empty(self):
        angle_addr = self._test_get_x(parser.get_angle_addr,
            ...
            '')
        self.assertEqual(angle_addr.token_type, 'angle-addr')
        self.assertIsNic(angle_addr.local_part)
        self.assertIsNic(angle_addr.domain)
        self.assertIsNic(angle_addr.route)
    ...
    # get_display_name  This jest phrase but przy a different value.
    ...
    def test_get_display_name_complex1(self):
        display_name = self._test_get_x(parser.get_display_name,
            ...
            ', oh.')
        self.assertEqual(display_name.token_type, 'display-name')
        self.assertEqual(display_name.display_name, 'Fred A. Johnson jest his name')
    ...
    def test_get_display_name_complex2(self):
        ...
        self.assertEqual(display_name.display_name, 'bird hand   jest messy')
    ...
    def test_get_display_name_ending_with_obsolete(self):
        ...
        self.assertEqual(display_name[3].comments, ['przy trailing comment'])
    ...
    def test_get_mailbox_list_unparseable_mailbox_null(self):
        mailbox_list = self._test_get_x(parser.get_mailbox_list,
            ...
            [errors.InvalidHeaderDefect,   # the 'extra' text after the local part
             errors.InvalidHeaderDefect,   # the local part przy no angle-addr
             errors.ObsoleteHeaderDefect,  # period w extra text (example.com)
             errors.ObsoleteHeaderDefect], # (bird) w valid address.
            '')
        self.assertEqual(len(mailbox_list.mailboxes), 1)
        self.assertEqual(len(mailbox_list.all_mailboxes), 2)
        self.assertEqual(mailbox_list.all_mailboxes[0].token_type,
                        'invalid-mailbox')
        self.assertIsNic(mailbox_list.all_mailboxes[0].display_name)
        self.assertEqual(mailbox_list.all_mailboxes[0].local_part,
                        'Roy A. Bear')
        self.assertIsNic(mailbox_list.all_mailboxes[0].domain)
    ...
    def test_get_group_one_invalid(self):
        group = self._test_get_x(parser.get_group,
            ...
            [errors.InvalidHeaderDefect,   # non-angle addr makes local part invalid
             errors.InvalidHeaderDefect],   # oraz its nie obs-local either: no dots.
            '')
        self.assertEqual(group.token_type, 'group')
        self.assertEqual(group.display_name, 'Monty Python')
        ...
        self.assertIsNic(group.all_mailboxes[1].display_name)
    ...
    def test_get_address_invalid_mailbox_invalid(self):
        address = self._test_get_x(parser.get_address,
            ...
            'ping example.com',
            'ping example.com',
            [errors.InvalidHeaderDefect,    # addr-spec przy no domain
             errors.InvalidHeaderDefect,    # invalid local-part
             errors.InvalidHeaderDefect,    # missing .s w local-part
            ],
            ', next')
        self.assertEqual(address.token_type, 'address')
        self.assertEqual(len(address.mailboxes), 0)
        self.assertEqual(len(address.all_mailboxes), 1)
        self.assertIsNic(address.all_mailboxes[0].domain)
    ...
    def test_get_address_list_mailboxes_simple(self):
        ...
        self.assertEqual([str(x) dla x w address_list.mailboxes],
                         [str(x) dla x w address_list.addresses])
        self.assertEqual(address_list.mailboxes[0].domain, 'example.com')
        self.assertEqual(address_list[0].token_type, 'address')
        self.assertIsNic(address_list[0].display_name)
    ...
    def test_get_address_list_mailboxes_complex(self):
        address_list = self._test_get_x(parser.get_address_list,
            ...
            [errors.ObsoleteHeaderDefect, # period w Is.
            errors.ObsoleteHeaderDefect], # cfws w domain
            '')
        self.assertEqual(address_list.token_type, 'address-list')
    ...
    def test_get_address_list_mailboxes_invalid_addresses(self):
        address_list = self._test_get_x(parser.get_address_list,
            ...
            ('"Roy A. Bear" <dinsdale@example.com>, '
             ...
             [errors.InvalidHeaderDefect,   # invalid address w list
              errors.InvalidHeaderDefect,   # 'Foo x' local part invalid.
              errors.InvalidHeaderDefect,   # Missing . w 'Foo x' local part
              errors.ObsoleteHeaderDefect,  # period w 'Is.' disp-name phrase
              errors.InvalidHeaderDefect,   # no domain part w addr-spec
              errors.ObsoleteHeaderDefect], # addr-spec has comment w it
            '')
        self.assertEqual(address_list.token_type, 'address-list')
        self.assertEqual(len(address_list.mailboxes), 1)
        self.assertEqual(len(address_list.all_mailboxes), 3)
        self.assertEqual([str(x) dla x w address_list.all_mailboxes],
                         [str(x) dla x w address_list.addresses])
        self.assertEqual(address_list.mailboxes[0].domain, 'example.com')
        self.assertEqual(address_list.mailboxes[0].token_type, 'mailbox')
        self.assertEqual(address_list.addresses[0].token_type, 'address')
...
@parameterize
klasa Test_parse_mime_parameters(TestParserMixin, TestEmailBase):
    ...
    mime_parameters_params = {
        ...
        # Note that it jest undefined what we should do dla error recovery when
        # there are duplicate parameter names albo duplicate parts w a split
        # part.  We choose to ignore all duplicate parameters after the first
        # oraz to take duplicate albo missing rfc 2231 parts w apperance order.
        # This jest backward compatible przy get_param's behavior, but the
        ...
        # Here we depart z get_param oraz assume the *0* was missing.
        'duplicate_with_broken_split_value': (
            ...
            "filename=abc.gif;"
                " filename*2*=iso-8859-1''%32%30%31%2E; filename*3*=%74%69%66",
            [('filename', 'abc.gif201.tif')],
            # Defects are apparent missing *0*, oraz two 'out of sequence'.
            [errors.InvalidHeaderDefect]*3),

    }

@parameterize
klasa Test_parse_mime_version(TestParserMixin, TestEmailBase):
...
klasa TestFolding(TestEmailBase):
    ...
    def test_simple_unstructured_no_folds(self):
        self._test(parser.get_unstructured("This jest a test"),
                   "This jest a test\n")

    def test_simple_unstructured_folded(self):
        self._test(parser.get_unstructured("This jest also a test, but this "
                        "time there are enough words (and even some "
                        "symbols) to make it wrap; at least w theory."),
                   "This jest also a test, but this time there are enough "
                        "words (and even some\n"
                   " symbols) to make it wrap; at least w theory.\n")
    ...
    # XXX Need tests przy comments on various sides of a unicode token,
    # oraz przy unicode tokens w the comments.  Spaces inside the quotes
