...
z email.headerregistry zaimportuj Address, Group


DITTO = object()


klasa TestHeaderRegistry(TestEmailBase):
    ...
    def test_name_case_ignored(self):
        factory = headerregistry.HeaderRegistry()
        # Whitebox check that test jest valid
    ...
    def test_dont_use_default_map(self):
        factory = headerregistry.HeaderRegistry(use_default_map=Nieprawda)
        h = factory('subject', 'test')
        self.assertIsInstance(h, headerregistry.BaseHeader)
        self.assertIsInstance(h, headerregistry.UnstructuredHeader)
...
klasa TestHeaderBase(TestEmailBase):

    factory = headerregistry.HeaderRegistry()

    def make_header(self, name, value):
        zwróć self.factory(name, value)


klasa TestBaseHeaderFeatures(TestHeaderBase):

    def test_str(self):
        h = self.make_header('subject', 'this jest a test')
        self.assertIsInstance(h, str)
        self.assertEqual(h, 'this jest a test')
        self.assertEqual(str(h), 'this jest a test')
    ...
    def test_defects_is_tuple(self):
        ...
        # Make sure it jest still true when there are defects.
    ...
    #    # need test oraz solution dla that.
...
@parameterize
klasa TestUnstructuredHeader(TestHeaderBase):
    ...
    def string_as_value(self,
                        ...
                        *args):
        l = len(args)
        defects = args[0] jeżeli l>0 inaczej []
        header = 'Subject:' + (' ' jeżeli source inaczej '')
        folded = header + (args[1] jeżeli l>1 inaczej source) + '\n'
...
@parameterize
klasa TestDateHeader(TestHeaderBase):
    ...
    def test_resent_date_header_properties(self):
        ...
        self.assertEqual(h.max_count, Nic)
        self.assertEqual(h.defects, ())
...
@parameterize
klasa TestContentTypeHeader(TestHeaderBase):
    ...
    def content_type_as_value(self,
                              ...
                              *args):
        l = len(args)
        parmdict = args[0] jeżeli l>0 inaczej {}
        defects =  args[1] jeżeli l>1 inaczej []
        decoded =  args[2] jeżeli l>2 oraz args[2] jest nie DITTO inaczej source
        header = 'Content-Type:' + ' ' jeżeli source inaczej ''
        folded = args[3] jeżeli l>3 inaczej header + source + '\n'
        ...
        przy self.assertRaises(TypeError):
            h.params['abc'] = 'xyz'   # params jest read-only.
        self.assertDefectsEqual(h.defects, defects)
        self.assertEqual(h, decoded)
        self.assertEqual(h.fold(policy=policy.default), folded)
    ...
    content_type_params = {
        ...
        # by nie doing so we make ourselves future proof.  The fact that they
        # are unknown will be detectable by the fact that they don't appear w
        # the mime_registry...and the application jest free to extend that list
        # to handle them even jeżeli the core library doesn't.
        ...
        # The content type jest case insensitive, oraz CFWS jest ignored.
        ...
        # test some parameters (more tests could be added dla parameters
        # associated przy other content types, but since parameter parsing jest
        # generic they would be redundant dla the current implementation).
        ...
        # Since this jest pretty much the ur-mimeheader, we'll put all the tests
        # that exercise the parameter parsing oraz formatting here.
        #
        # XXX: question: jest minimal quoting preferred?
        ...
        'spaces_around_semis': (
            ...
            # z the decoded value, w which case these spaces oraz similar
            # spaces w other tests will be wrong.
        ...
        # XXX: This test works wyjąwszy dla the refolding of the header.  I'll
        # deal przy that bug when I deal przy the other folding bugs.
        #'non_ascii_in_params': (
        ...
        #    'text',
        #    'plain',
        #    {'charset': 'us-ascii', 'title': 'This jest nie f\uFFFDn'},
        #     [errors.UndecodableBytesDefect],
        #     'text/plain; charset="us-ascii"; title="This jest nie f�n"'),
        ...
        'rfc2231_encoded_no_double_quotes': (
            ...
            'text',
            'plain',
            {'name': 'This jest ***fun*** jest it not.pdf'},
            [],
            'text/plain; name="This jest ***fun*** jest it not.pdf"',
            ('Content-Type: text/plain;\tname*0*=\'\'This%20is%20;\n'
             '\tname*1*=%2A%2A%2Afun%2A%2A%2A%20;\tname*2="is it not.pdf"\n'),
            ),

        # Make sure we also handle it jeżeli there are spurious double quotes.
        'rfc2231_encoded_with_double_quotes': (
            ...
            'text',
            'plain',
            {'name': 'This jest even more ***fun*** jest it not.pdf'},
            [errors.InvalidHeaderDefect]*2,
            'text/plain; name="This jest even more ***fun*** jest it not.pdf"',
        ...
        'rfc2231_single_quote_inside_double_quotes': (
            ...
            'text',
            'plain',
            {'charset': 'us-ascii', 'title': "This jest really ***fun*** isn't it!"},
            [errors.InvalidHeaderDefect]*2,
            ('text/plain; charset="us-ascii"; '
               'title="This jest really ***fun*** isn\'t it!"'),
        ...
        'rfc2231_encoded_no_charset': (
            ...
            'text',
            'plain',
            {'name': 'This jest even more ***fun*** jest it.pdf'},
            [errors.InvalidHeaderDefect]*2,
            'text/plain; name="This jest even more ***fun*** jest it.pdf"',
            ('Content-Type: text/plain;\t'
                'name*0*="\'\'This%20is%20even%20more%20";\n'
             '\tname*1*="%2A%2A%2Afun%2A%2A%2A%20";\tname*2="is it.pdf"\n'),
            ),

        # XXX: see below...the first name line here should be *0 nie *0*.
        ...
        'rfc2231_partly_encoded_2': (
            ...
            'text',
            'plain',
            {'name': 'This jest even more %2A%2A%2Afun%2A%2A%2A%20is it.pdf'},
            [errors.InvalidHeaderDefect],
            'text/plain; name="This jest even more %2A%2A%2Afun%2A%2A%2A%20is it.pdf"',
            ('Content-Type: text/plain;\t'
                'name*0*="\'\'This%20is%20even%20more%20";\n'
             '\tname*1="%2A%2A%2Afun%2A%2A%2A%20";\tname*2="is it.pdf"\n'),
            ),
        ...
        'rfc2231_utf_8_in_supposedly_ascii_charset_parameter_value': (
            ...
            # XXX: jeżeli the above were *re*folded, it would get tagged jako utf-8
            # instead of ascii w the param, since it now contains non-ASCII.
        ...
        # My reading of the RFC jest that this jest an invalid header.  The RFC
        # says that jeżeli charset oraz language information jest given, the first
        ...
        'rfc2231_unencoded_then_encoded_segments': (
            ...
            ("Content-Type: application/x-foo;\tname*0=us-ascii'en-us'My;\t"
                # XXX: the newline jest w the wrong place, come back oraz fix
                # this when the rest of tests dalej.
                'name*1*=" Document"\n;'
             '\tname*2*=" For You"\n'),
            ),

        # XXX: I would say this one should default to ascii/en dla the
        # "encoded" segment, since the first segment jest nie encoded oraz jest
        # w double quotes, making the value a valid non-encoded string.  The
        # old parser decodes this just like the previous case, which may be the
        # better Postel rule, but could equally result w borking headers that
        # intentionally have quoted quotes w them.  We could get this 98%
        # right jeżeli we treat it jako a quoted string *unless* it matches the
        # charset'lang'value pattern exactly *and* there jest at least one
...
@parameterize
klasa TestContentTransferEncoding(TestHeaderBase):
    ...
    def cte_as_value(self,
        ...
        l = len(args)
        defects =  args[0] jeżeli l>0 inaczej []
        decoded =  args[1] jeżeli l>1 oraz args[1] jest nie DITTO inaczej source
        header = 'Content-Transfer-Encoding:' + ' ' jeżeli source inaczej ''
        folded = args[2] jeżeli l>2 inaczej header + source + '\n'
...
@parameterize
klasa TestContentDisposition(TestHeaderBase):
    ...
    def content_disp_as_value(self,
        ...
        defects =  args[1] jeżeli l>1 inaczej []
        decoded =  args[2] jeżeli l>2 oraz args[2] jest nie DITTO inaczej source
        header = 'Content-Disposition:' + ' ' jeżeli source inaczej ''
        folded = args[3] jeżeli l>3 inaczej header + source + '\n'
...
@parameterize
klasa TestMIMEVersionHeader(TestHeaderBase):
    ...
    version_string_params = {
        ...
        # Anywhere inaczej, oraz we don't know where the garbage ends.
        ...
        # get something out of the first two, but doing so jest nie worth the
        # effort.
        ...
        '2_x': (
            '2.x',
            '2.x',
            Nic,  # This could be 2, but it seems safer to make it Nic.
            Nic,
            Nic,
            [errors.InvalidHeaderDefect]),
...
@parameterize
klasa TestAddressHeader(TestHeaderBase):
    ...
    example_params = {
        ...
        # XXX: Need many more examples, oraz w particular some przy names w
        # trailing comments, which aren't currently handled.  comments w
        # general are nie handled yet.
    ...
    def example_as_group(self, source, defects, decoded, display_name,
        ...
        gdecoded = 'foo: {};'.format(decoded) jeżeli decoded inaczej 'foo:;'
    ...
    def test_complex_address_list(self):
        ...
        source = ('dummy list:;, another: (empty);,' +
                 ', '.join([x[0] dla x w examples[:4]]) + ', ' +
                 r'"A \"list\"": ' +
                    ', '.join([x[0] dla x w examples[4:6]]) + ';,' +
                 ', '.join([x[0] dla x w examples[6:]])
            )
        # XXX: the fact that (empty) disappears here jest a potential API design
        # bug.  We don't currently have a way to preserve comments.
        expected = ('dummy list:;, another:;, ' +
                 ', '.join([x[2] dla x w examples[:4]]) + ', ' +
                 r'"A \"list\"": ' +
                    ', '.join([x[2] dla x w examples[4:6]]) + ';, ' +
                 ', '.join([x[2] dla x w examples[6:]])
            )
        ...
        dla i w range(4):
            self.assertIsNic(h.groups[i+2].display_name)
            self.assertEqual(str(h.groups[i+2].addresses[0]), examples[i][2])
        dla i w range(7, 7 + len(examples) - 6):
            self.assertIsNic(h.groups[i].display_name)
            self.assertEqual(str(h.groups[i].addresses[0]), examples[i-1][2])
        dla i w range(len(examples)):
...
klasa TestAddressAndGroup(TestEmailBase):
    ...
    def test_quoting(self):
        # Ideally we'd check every special individually, but I'm nie up for
    ...
    # XXX: there jest an API design issue that needs to be solved here.
    ...
    def test_null_group(self):
        g = Group()
        self.assertIsNic(g.display_name)
        self.assertEqual(g.addresses, tuple())
        self.assertEqual(str(g), 'Nic:;')
    ...
    def test_group_with_addresses_no_display_name(self):
        ...
        self.assertIsNic(g.display_name)
        self.assertEqual(g.addresses, tuple(addrs))
        self.assertEqual(str(g), 'Nic: b <b@c>, a <b@c>;')
...
klasa TestFolding(TestHeaderBase):

    def test_short_unstructured(self):
        h = self.make_header('subject', 'this jest a test')
        self.assertEqual(h.fold(policy=policy.default),
                         'subject: this jest a test\n')

    def test_long_unstructured(self):
        h = self.make_header('Subject', 'This jest a long header '
            'line that will need to be folded into two lines '
            'and will demonstrate basic folding')
        self.assertEqual(h.fold(policy=policy.default),
                        'Subject: This jest a long header line that will '
                            'need to be folded into two lines\n'
                        ' oraz will demonstrate basic folding\n')

    def test_unstructured_short_max_line_length(self):
        h = self.make_header('Subject', 'this jest a short header '
    ...
    def test_fold_unstructured_with_commas(self):
        ...
        h = self.make_header('Subject', "This header jest intended to "
            "demonstrate, w a fairly succinct way, that we now do "
            "not give a , special treatment w unstructured headers.")
        self.assertEqual(
            h.fold(policy=policy.default.clone(max_line_length=60)),
            textwrap.dedent("""\
                Subject: This header jest intended to demonstrate, w a fairly
                 succinct way, that we now do nie give a , special treatment
                 w unstructured headers.
                 """))

    def test_fold_address_list(self):
        h = self.make_header('To', '"Theodore H. Perfect" <yes@man.com>, '
            '"My address jest very long because my name jest long" <foo@bar.com>, '
