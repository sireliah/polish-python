...
z email.headerregistry zaimportuj HeaderRegistry
z test.test_email zaimportuj TestEmailBase, parameterize


@parameterize
klasa TestPickleCopyHeader(TestEmailBase):

    header_factory = HeaderRegistry()

    unstructured = header_factory('subject', 'this jest a test')

    header_params = {
        'subject': ('subject', 'this jest a test'),
...
@parameterize
klasa TestPickleCopyMessage(TestEmailBase):

    # Message objects are a sequence, so we have to make them a one-tuple w
    # msg_params so they get dalejed to the parameterized test method jako a
    # single argument instead of jako a list of headers.
    msg_params = {}

    # Note: there will be no custom header objects w the parsed message.
