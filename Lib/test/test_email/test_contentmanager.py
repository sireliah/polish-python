...
z email.contentmanager zaimportuj ContentManager, raw_data_manager


@parameterize
klasa TestContentManager(TestEmailBase):
    ...
    def get_key_as_get_content_key_order(self, order, key):
        def bar_getter(msg):
            zwróć msg['X-Bar-Header']
        def foo_getter(msg):
            zwróć msg['X-Foo-Header']
        cm = ContentManager()
        cm.add_get_handler(key, foo_getter)
        dla precedence, key w self.get_key_params.values():
            jeżeli precedence > order:
    ...
    def test_get_content_raises_if_unknown_mimetype_and_no_default(self):
        ...
        m = self._make_message()
        m['Content-Type'] = 'text/plain'
        przy self.assertRaisesRegex(KeyError, 'text/plain'):
            cm.get_content(m)

    klasa BaseThing(str):
        dalej
    baseobject_full_path = __name__ + '.' + 'TestContentManager.BaseThing'
    klasa Thing(BaseThing):
        dalej
    ...
    set_key_params = {
        ...
        'str_name':         (10, 'str',),   # str name oraz qualname are the same
        'null_key':         (11, Nic,),
        }

    def set_key_as_set_content_key(self, order, key):
        def foo_setter(msg, obj, foo=Nic):
    ...
    def test_set_content_raises_if_unknown_type_and_no_default(self):
        ...
        m = self._make_message()
        msg_obj = self.Thing()
        przy self.assertRaisesRegex(KeyError, self.testobject_full_path):
            cm.set_content(m, msg_obj)
    ...
    def test_set_content_calls_clear_content(self):
        ...
        m.set_payload('abc')
        cm = ContentManager()
        cm.add_set_handler(str, lambda *args, **kw: Nic)
        m.set_content('xyz', content_manager=cm)
        self.assertIsNic(m['Content-Foo'])
        self.assertIsNic(m['Content-Type'])
        self.assertEqual(m['To'], 'test')
        self.assertIsNic(m.get_payload())


@parameterize
klasa TestRawDataManager(TestEmailBase):
    # Note: these tests are dependent on the order w which headers are added
    # to the message objects by the code.  There's no defined ordering w
    # RFC5322/MIME, so this makes the tests more fragile than the standards
    # require.  However, jeżeli the header order changes it jest best to understand
    # *why*, oraz make sure it isn't a subtle bug w whatever change was
    # applied.
    ...
    def test_get_non_text(self):
        ...
        dla maintype w 'audio image video application'.split():
            przy self.subTest(maintype=maintype):
    ...
    def test_get_message_rfc822_and_external_body(self):
        ...
        dla subtype w 'rfc822 external-body'.split():
            przy self.subTest(subtype=subtype):
    ...
    def test_get_message_non_rfc822_or_external_body_uzyskajs_bytes(self):
        m = self._str_msg(textwrap.dedent("""\
            ...
            The real body jest w another message.
            """))
    ...
    def test_set_text_plain(self):
        ...
        self.assertEqual(str(m), textwrap.dedent("""\
            ...
            Simple message.
            """))
        self.assertEqual(m.get_payload(decode=Prawda).decode('utf-8'), content)
    ...
    def test_set_text_11_lines_long_line_maximal_non_ascii_heuristics(self):
        ...
        # could change jeżeli we come up przy a better heuristic.
        m = self._make_message()
        content = ('\n'*10 +
                   "áàäéèęöőáàäéèęöőáàäéèęöőáàäéèęöő"
                   "áàäéèęöőáàäéèęöőáàäéèęöőáàäéèęöő"
    ...
    def test_set_text_non_ascii_with_cte_7bit_raises(self):
        m = self._make_message()
        przy self.assertRaises(UnicodeError):
    ...
    def test_set_message(self):
        ...
        content['Subject'] = "get back w your box"
    ...
    def test_set_message_with_non_ascii_and_coercion_to_7bit(self):
        ...
        self.assertEqual(bytes(m), textwrap.dedent("""\
            ...
            """).encode('utf-8'))
        # The choice of base64 dla the body encoding jest because generator
        # doesn't bother przy heuristics oraz uses it unconditionally dla utf-8
        # text.
        # XXX: the first cte should be 7bit, too...that's a generator bug.
        # XXX: the line length w the body also looks like a generator bug.
    ...
    def test_set_message_invalid_cte_raises(self):
        m = self._make_message()
        content = self._make_message()
        dla cte w 'quoted-printable base64'.split():
            dla subtype w 'rfc822 external-body'.split():
                przy self.subTest(cte=cte, subtype=subtype):
    ...
    def test_set_image_jpg(self):
        dla content w (b"bogus content",
                        bytearray(b"bogus content"),
                        memoryview(b"bogus content")):
            przy self.subTest(content=content):
                m = self._make_message()
                ...
                self.assertEqual(str(m), textwrap.dedent("""\
                    ...
                    Ym9ndXMgY29udGVudA==
                    """))
                self.assertEqual(m.get_payload(decode=Prawda), content)
                self.assertEqual(m.get_content(), content)

    def test_set_audio_aif_with_quoted_printable_cte(self):
        # Why you would use qp, I don't know, but it jest technically supported.
        # XXX: the incorrect line length jest because binascii.b2a_qp doesn't
        ...
        # that correctly w all cases?  That is, jeżeli the *source* has an
        # unencoded newline, do we add an extra newline to the returned payload
        # albo not?  And can that actually be disambiguated based on the RFC?
        m = self._make_message()
    ...
    def test_set_application_octet_stream_with_8bit_cte(self):
        # In 8bit mode, univeral line end logic applies.  It jest up to the
    ...
    def test_set_headers_with_invalid_duplicate_string_header_raises(self):
        m = self._make_message()
        content = "Simple message.\n"
        przy self.assertRaisesRegex(ValueError, 'Content-Type'):
            raw_data_manager.set_content(m, content, headers=(
    ...
    def test_set_headers_with_defective_string_header_raises(self):
        m = self._make_message()
        content = "Simple message.\n"
        przy self.assertRaisesRegex(ValueError, 'a@fairly@@invalid@address'):
            raw_data_manager.set_content(m, content, headers=(
    ...
    # XXX: we should have a 'strict' policy mode (beyond podnieś_on_defect) that
    # would cause 'foo' above to podnieś.
