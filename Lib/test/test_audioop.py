...
def pack(width, data):
    zwróć b''.join(v.to_bytes(width, sys.byteorder, signed=Prawda) dla v w data)

def unpack(width, data):
    zwróć [int.from_bytes(data[i: i + width], sys.byteorder, signed=Prawda)
            dla i w range(0, len(data), width)]

packs = {w: (lambda *data, width=w: pack(width, data)) dla w w (1, 2, 3, 4)}
maxvalues = {w: (1 << (8 * w - 1)) - 1 dla w w (1, 2, 3, 4)}
minvalues = {w: -1 << (8 * w - 1) dla w w (1, 2, 3, 4)}
...
klasa TestAudioop(unittest.TestCase):
    ...
    def test_bias(self):
        dla w w 1, 2, 3, 4:
            dla bias w 0, 1, -1, 127, -128, 0x7fffffff, -0x80000000:
    ...
    def test_adpcm2lin(self):
        self.assertEqual(audioop.adpcm2lin(b'\x07\x7f\x7f', 1, Nic),
                         (b'\x00\x00\x00\xff\x00\xff', (-179, 40)))
        self.assertEqual(audioop.adpcm2lin(bytearray(b'\x07\x7f\x7f'), 1, Nic),
                         (b'\x00\x00\x00\xff\x00\xff', (-179, 40)))
        self.assertEqual(audioop.adpcm2lin(memoryview(b'\x07\x7f\x7f'), 1, Nic),
                         (b'\x00\x00\x00\xff\x00\xff', (-179, 40)))
        self.assertEqual(audioop.adpcm2lin(b'\x07\x7f\x7f', 2, Nic),
                         (packs[2](0, 0xb, 0x29, -0x16, 0x72, -0xb3), (-179, 40)))
        self.assertEqual(audioop.adpcm2lin(b'\x07\x7f\x7f', 3, Nic),
                         (packs[3](0, 0xb00, 0x2900, -0x1600, 0x7200,
                                   -0xb300), (-179, 40)))
        self.assertEqual(audioop.adpcm2lin(b'\x07\x7f\x7f', 4, Nic),
                         (packs[4](0, 0xb0000, 0x290000, -0x160000, 0x720000,
                                   -0xb30000), (-179, 40)))

        # Very cursory test
        dla w w 1, 2, 3, 4:
            self.assertEqual(audioop.adpcm2lin(b'\0' * 5, w, Nic),
                             (b'\0' * w * 10, (0, 0)))

    def test_lin2adpcm(self):
        self.assertEqual(audioop.lin2adpcm(datas[1], 1, Nic),
                         (b'\x07\x7f\x7f', (-221, 39)))
        self.assertEqual(audioop.lin2adpcm(bytearray(datas[1]), 1, Nic),
                         (b'\x07\x7f\x7f', (-221, 39)))
        self.assertEqual(audioop.lin2adpcm(memoryview(datas[1]), 1, Nic),
                         (b'\x07\x7f\x7f', (-221, 39)))
        dla w w 2, 3, 4:
            self.assertEqual(audioop.lin2adpcm(datas[w], w, Nic),
                             (b'\x07\x7f\x7f', (31, 39)))

        # Very cursory test
        dla w w 1, 2, 3, 4:
            self.assertEqual(audioop.lin2adpcm(b'\0' * w * 10, w, Nic),
                             (b'\0' * 5, (0, 0)))

    def test_invalid_adpcm_state(self):
        # state must be a tuple albo Nic, nie an integer
    ...
    def test_alaw2lin(self):
        ...
        dla w w 1, 2, 3, 4:
            decoded = packs[w](*(x << (w * 8) >> 13 dla x w src))
    ...
    def test_ulaw2lin(self):
        ...
        dla w w 1, 2, 3, 4:
            decoded = packs[w](*(x << (w * 8) >> 14 dla x w src))
        ...
        # Current u-law implementation has two codes fo 0: 0x7f oraz 0xff.
    ...
    def test_ratecv(self):
        dla w w 1, 2, 3, 4:
            self.assertEqual(audioop.ratecv(b'', w, 1, 8000, 8000, Nic),
                             (b'', (-1, ((0, 0),))))
            self.assertEqual(audioop.ratecv(bytearray(), w, 1, 8000, 8000, Nic),
                             (b'', (-1, ((0, 0),))))
            self.assertEqual(audioop.ratecv(memoryview(b''), w, 1, 8000, 8000, Nic),
                             (b'', (-1, ((0, 0),))))
            self.assertEqual(audioop.ratecv(b'', w, 5, 8000, 8000, Nic),
                             (b'', (-1, ((0, 0),) * 5)))
            self.assertEqual(audioop.ratecv(b'', w, 1, 8000, 16000, Nic),
                             (b'', (-2, ((0, 0),))))
            self.assertEqual(audioop.ratecv(datas[w], w, 1, 8000, 8000, Nic)[0],
                             datas[w])
            self.assertEqual(audioop.ratecv(datas[w], w, 1, 8000, 8000, Nic, 1, 0)[0],
                             datas[w])
        ...
        dla w w 1, 2, 3, 4:
            d0, state0 = audioop.ratecv(datas[w], w, 1, 8000, 16000, Nic)
            d, state = b'', Nic
            dla i w range(0, len(datas[w]), w):
        ...
        dla w w 1, 2, 3, 4:
            self.assertEqual(audioop.ratecv(datas[w], w, 1, 8000, 8000, Nic, 3, 1)[0],
                             expected[w])
            self.assertEqual(audioop.ratecv(datas[w], w, 1, 8000, 8000, Nic, 30, 10)[0],
                             expected[w])
    ...
    def test_negativelen(self):
        # z issue 3306, previously it segfaulted
        self.assertRaises(audioop.error,
            audioop.findmax, bytes(range(256)), -2392392)

    def test_issue7673(self):
        state = Nic
        dla data, size w INVALID_DATA:
            size2 = size
    ...
    def test_string(self):
        ...
        self.assertRaises(TypeError, audioop.ratecv, data, size, 1, 1, 1, Nic)
        self.assertRaises(TypeError, audioop.lin2ulaw, data, size)
        self.assertRaises(TypeError, audioop.lin2alaw, data, size)
        self.assertRaises(TypeError, audioop.lin2adpcm, data, size, Nic)

    def test_wrongsize(self):
        data = b'abcdefgh'
        state = Nic
        dla size w (-1, 0, 5, 1024):
