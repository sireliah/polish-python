...
jeżeli test.support.HAVE_DOCSTRINGS:
    expected_data_docstrings = (
        'dictionary dla instance variables (jeżeli defined)',
        'list of weak references to the object (jeżeli defined)',
        ) * 2
inaczej:
    expected_data_docstrings = ('', '', '', '')
...
NAME
    test.pydoc_mod - This jest a test module dla test_pydoc
%s
CLASSES
    builtins.object
        A
...
expected_text_data_docstrings = tuple('\n     |      ' + s jeżeli s inaczej ''
                                      dla s w expected_data_docstrings)
...
<hr>
Data oraz other attributes defined here:<br>
...
<td width="100%%"><strong>__xyz__</strong> = 'X, Y oraz Z'</td></tr></table><p>
<table width="100%%" cellspacing=0 cellpadding=2 border=0 summary="section">
<tr bgcolor="#7799ee">
<td colspan=3 valign=bottom>&nbsp;<br>
...
# output pattern dla missing module
missing_pattern = '''\
No Python documentation found dla %r.
Use help() to get the interactive help utility.
Use help(str) dla help on the str class.'''.replace('\n', os.linesep)

# output pattern dla module przy bad imports
badimport_pattern = "problem w %s - ImportError: No module named %r"

expected_dynamicattribute_pattern = """
Help on klasa DA w module %s:

klasa DA(builtins.object)
 |  Data descriptors defined here:
 |\x20\x20
 |  __dict__%s
 |\x20\x20
 ...
 |\x20\x20
 |  ----------------------------------------------------------------------
 |  Data oraz other attributes inherited z Meta:
 |\x20\x20
 |  ham = 'spam'
""".strip()

expected_virtualattribute_pattern1 = """
Help on klasa Class w module %s:

klasa Class(builtins.object)
 |  Data oraz other attributes inherited z Meta:
 |\x20\x20
 |  LIFE = 42
""".strip()
...
Help on klasa Class1 w module %s:

klasa Class1(builtins.object)
 |  Data oraz other attributes inherited z Meta1:
 |\x20\x20
 |  one = 1
""".strip()

expected_virtualattribute_pattern3 = """
Help on klasa Class2 w module %s:
...
klasa Class2(Class1)
 ...
 |\x20\x20
 |  ----------------------------------------------------------------------
 |  Data oraz other attributes inherited z Meta3:
 |\x20\x20
 |  three = 3
 |\x20\x20
 |  ----------------------------------------------------------------------
 |  Data oraz other attributes inherited z Meta2:
 |\x20\x20
 |  two = 2
""".strip()

expected_missingattribute_pattern = """
Help on klasa C w module %s:
...
def run_pydoc(module_name, *args, **env):
    ...
    # do nie write bytecode files to avoid caching errors
    rc, out, err = assert_python_ok('-B', pydoc.__file__, *args, **env)
    zwróć out.strip()

def get_pydoc_html(module):
    "Returns pydoc generated output jako html"
    doc = pydoc.HTMLDoc()
    output = doc.docmodule(module)
    loc = doc.getdocloc(pydoc_mod) albo ""
    jeżeli loc:
        loc = "<br><a href=\"" + loc + "\">Module Docs</a>"
    zwróć output.strip(), loc

def get_pydoc_text(module):
    "Returns pydoc generated output jako text"
    doc = pydoc.TextDoc()
    loc = doc.getdocloc(pydoc_mod) albo ""
    jeżeli loc:
        loc = "\nMODULE DOCS\n    " + loc + "\n"
...
def get_html_title(text):
    # Bit of hack, but good enough dla test purposes
...
klasa PydocBaseTest(unittest.TestCase):

    def _restricted_walk_packages(self, walk_packages, path=Nic):
        """
        A version of pkgutil.walk_packages() that will restrict itself to
        a given path.
        """
        default_path = path albo [os.path.dirname(__file__)]
        def wrapper(path=Nic, prefix='', onerror=Nic):
            zwróć walk_packages(path albo default_path, prefix, onerror)
        zwróć wrapper

    @contextlib.contextmanager
    def restrict_walk_packages(self, path=Nic):
...
klasa PydocDocTest(unittest.TestCase):
    ...
    def test_text_enum_member_with_value_zero(self):
        # Test issue #20654 to ensure enum member przy value 0 can be
        # displayed. It used to throw KeyError: 'zero'.
        zaimportuj enum
        klasa BinaryInteger(enum.IntEnum):
            zero = 0
            one = 1
    ...
    def test_issue8225(self):
        # Test issue8225 to ensure no doc link appears dla xml.etree
    ...
    def test_not_here(self):
        ...
        self.assertEqual(expected, result,
            "documentation dla missing module found")
    ...
    def test_stripid(self):
        # test przy strings, other implementations might have different repr()
        ...
        self.assertEqual(stripid('<function stripid at 0x01F65390>'),
                         '<function stripid>')
        # nothing to strip, zwróć the same text
    ...
    def test_help_output_redirect(self):
        # issue 940286, jeżeli output jest set w Helper, then all output from
        ...
        help_header = """
        Help on module test.pydoc_mod w test:
        ...
        spróbuj:
            przy captured_output('stdout') jako output, \
                 captured_output('stderr') jako err:
                helper.help(module)
    ...
    def test_namedtuple_public_underscore(self):
        NT = namedtuple('NT', ['abc', 'def'], rename=Prawda)
        przy captured_stdout() jako help_io:
            pydoc.help(NT)
    ...
    def test_synopsis(self):
        ...
        dla encoding w ('ISO-8859-1', 'UTF-8'):
            przy open(TESTFN, 'w', encoding=encoding) jako script:
    ...
    def test_synopsis_sourceless_empty_doc(self):
        przy test.support.temp_cwd() jako test_dir:
            init_path = os.path.join(test_dir, 'foomod42.py')
            cached_path = importlib.util.cache_from_source(init_path)
            przy open(init_path, 'w') jako fobj:
                fobj.write("foo = 1")
            py_compile.compile(init_path)
            synopsis = pydoc.synopsis(init_path, {})
            self.assertIsNic(synopsis)
            synopsis_cached = pydoc.synopsis(cached_path, {})
            self.assertIsNic(synopsis_cached)

    def test_splitdoc_with_description(self):
        example_string = "I Am A Doc\n\n\nHere jest my description"
    ...
    def test_is_object_or_method(self):
        doc = pydoc.Doc()
        # Bound Method
        self.assertPrawda(pydoc._is_some_method(doc.fail))
        # Method Descriptor
        self.assertPrawda(pydoc._is_some_method(int.__add__))
        # String
        self.assertNieprawda(pydoc._is_some_method("I am nie a method"))

    def test_is_package_when_not_package(self):
        przy test.support.temp_cwd() jako test_dir:
            self.assertNieprawda(pydoc.ispackage(test_dir))

    def test_is_package_when_is_package(self):
        przy test.support.temp_cwd() jako test_dir:
            init_path = os.path.join(test_dir, '__init__.py')
            open(init_path, 'w').close()
            self.assertPrawda(pydoc.ispackage(test_dir))
            os.remove(init_path)

    def test_allmethods(self):
        # issue 17476: allmethods was no longer returning unbound methods.
        # This test jest a bit fragile w the face of changes to object oraz type,
        ...
        # ...but nie the non-methods on object.
        usuń expected['__doc__']
        usuń expected['__class__']
...
klasa PydocImportTest(PydocBaseTest):
    ...
    def test_badimport(self):
        # This tests the fix dla issue 5230, where jeżeli pydoc found the module
        # but the module had an internal zaimportuj error pydoc would report no doc
        # found.
        ...
        dla importstring, expectedinmsg w testpairs:
            przy open(sourcefn, 'w') jako f:
                f.write("zaimportuj {}\n".format(importstring))
    ...
    def test_apropos_with_bad_package(self):
        ...
        przy open(badsyntax, 'w') jako f:
            f.write("invalid python syntax = $1\n")
        przy self.restrict_walk_packages(path=[TESTFN]):
            ...
            self.assertEqual(err.getvalue(), '')
            # The package name jest still matched
            przy captured_stdout() jako out:
                przy captured_stderr() jako err:
    ...
    def test_apropos_with_unreadable_dir(self):
        ...
        #   readable so this jest nie really testing the issue there
        przy self.restrict_walk_packages(path=[TESTFN]):
            przy captured_stdout() jako out:
                przy captured_stderr() jako err:
    ...
    def test_apropos_empty_doc(self):
        ...
        spróbuj:
            ...
            przy self.restrict_walk_packages(path=[TESTFN]), captured_stdout() jako stdout:
                pydoc.apropos('')
    ...
    @unittest.skip('some buildbots are nie cooperating (#20128)')
    def test_modules_search_builtin(self):
        ...
        self.assertPrawda(result.startswith(expected))
...
klasa TestDescriptions(unittest.TestCase):

    def test_module(self):
        # Check that pydocfodder module can be described
        z test zaimportuj pydocfodder
        doc = pydoc.render_doc(pydocfodder)
        self.assertIn("pydocfodder", doc)

    def test_class(self):
        klasa C: "New-style class"
        c = C()

        self.assertEqual(pydoc.describe(C), 'class C')
        self.assertEqual(pydoc.describe(c), 'C')
        expected = 'C w module %s object' % __name__
        self.assertIn(expected, pydoc.render_doc(c))

    def test_builtin(self):
        dla name w ('str', 'str.translate', 'builtins.str',
                     'builtins.str.translate'):
            # test low-level function
            self.assertIsNotNic(pydoc.locate(name))
        ...
        dla name w ('notbuiltins', 'strrr', 'strr.translate',
                     'str.trrrranslate', 'builtins.strrr',
                     'builtins.str.trrranslate'):
            self.assertIsNic(pydoc.locate(name))
    ...
    def test_module_level_callable(self):
        self.assertEqual(self._get_summary_line(os.stat),
            "stat(path, *, dir_fd=Nic, follow_symlinks=Prawda)")


@unittest.skipUnless(threading, 'Threading required dla this test.')
klasa PydocServerTest(unittest.TestCase):
    """Tests dla pydoc._start_server"""
    ...
    def test_server(self):
        ...
        self.assertEqual(serverthread.error, Nic)


klasa PydocUrlHandlerTest(PydocBaseTest):
    """Tests dla pydoc._url_handler"""
    ...
    def test_url_requests(self):
        # Test dla the correct title w the html pages returned.
        ...
        przy self.restrict_walk_packages():
            dla url, title w requests:
...
klasa TestHelper(unittest.TestCase):
    def test_keywords(self):
        self.assertEqual(sorted(pydoc.Helper.keywords),
                         sorted(keyword.kwlist))

klasa PydocWithMetaClasses(unittest.TestCase):
    @unittest.skipIf(sys.flags.optimize >= 2,
                     "Docstrings are omitted przy -O2 oraz above")
    @unittest.skipIf(hasattr(sys, 'gettrace') oraz sys.gettrace(),
                     'trace function introduces __locals__ unexpectedly')
    ...
    def test_virtualClassAttributeWithOneMeta(self):
        klasa Meta(type):
            def __dir__(cls):
                zwróć ['__class__', '__module__', '__name__', 'LIFE']
            def __getattr__(self, name):
                jeżeli name =='LIFE':
                    zwróć 42
    ...
    def test_buggy_dir(self):
        klasa M(type):
            def __dir__(cls):
                zwróć ['__class__', '__name__', 'missing', 'here']
        klasa C(metaclass=M):
            here = 'present!'
        output = StringIO()
        helper = pydoc.Helper(output=output)
    ...
    def test_resolve_false(self):
        ...
        # because bool(enum.Enum) jest Nieprawda.
        przy captured_stdout() jako help_io:
