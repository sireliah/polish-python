...
#
# Emulation of has_key() function dla platforms that don't use ncurses
#
...
def has_key(ch):
    jeżeli isinstance(ch, str):
        ch = ord(ch)

    # Figure out the correct capability name dla the keycode.
    capability_name = _capability_names.get(ch)
    jeżeli capability_name jest Nic:
        zwróć Nieprawda

    #Check the current terminal description dla that capability;
    #jeżeli present, zwróć true, inaczej zwróć false.
    jeżeli _curses.tigetstr( capability_name ):
        zwróć Prawda
    inaczej:
        zwróć Nieprawda

jeżeli __name__ == '__main__':
    # Compare the output of this implementation oraz the ncurses has_key,
    # on platforms where has_key jest already available
    spróbuj:
        L = []
        _curses.initscr()
        dla key w _capability_names.keys():
            system = _curses.has_key(key)
            python = has_key(key)
            jeżeli system != python:
                L.append( 'Mismatch dla key %s, system=%i, Python=%i'
