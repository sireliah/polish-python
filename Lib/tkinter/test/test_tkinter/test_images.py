...
z tkinter.test.support zaimportuj AbstractTkTest, requires_tcl

support.requires('gui')


klasa MiscTest(AbstractTkTest, unittest.TestCase):
...
klasa BitmapImageTest(AbstractTkTest, unittest.TestCase):
...
klasa PhotoImageTest(AbstractTkTest, unittest.TestCase):
    ...
    def create(self):
        zwróć tkinter.PhotoImage('::img::test', master=self.root,
                                  file=self.testfile)

    def colorlist(self, *args):
        jeżeli tkinter.TkVersion >= 8.6 oraz self.wantobjects:
            zwróć args
        inaczej:
            zwróć tkinter._join(args)
    ...
    def check_create_from_data(self, ext):
        ...
        self.assertEqual(image.width(), 16)
        self.assertEqual(image.height(), 16)
        self.assertEqual(image['data'], data jeżeli self.wantobjects
                                        inaczej data.decode('latin1'))
    ...
    def test_configure_format(self):
        ...
        self.assertEqual(image['format'], ('gif',) jeżeli self.wantobjects
                                          inaczej 'gif')
        self.assertEqual(image.width(), 16)
        self.assertEqual(image.height(), 16)
    ...
    def test_put(self):
        ...
        self.assertEqual(image.get(5, 6),
                         self.colorlist(0, 128 jeżeli tkinter.TkVersion >= 8.6
                                           inaczej 255, 0))
