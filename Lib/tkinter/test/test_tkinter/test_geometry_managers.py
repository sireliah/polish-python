...
z tkinter.test.support zaimportuj pixels_conv, tcl_version, requires_tcl
z tkinter.test.widget_tests zaimportuj AbstractWidgetTest

requires('gui')


klasa PackTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_pack_configure_after(self):
        ...
        przy self.assertRaisesRegex(TclError, 'window "%s" isn\'t packed' % b):
            a.pack_configure(after=b)
        przy self.assertRaisesRegex(TclError, 'bad window path name ".foo"'):
    ...
    def test_pack_configure_expand(self):
        ...
        c.pack_configure(side='right', expand=Prawda)
        ...
        c.pack_configure(side='right', expand=Prawda, fill='both')
    ...
    def test_pack_configure_in(self):
        ...
        przy self.assertRaisesRegex(TclError,
    ...
    def test_pack_configure_padx_ipadx_fill(self):
        ...
        def check(geom1, geom2, **kwargs):
            ...
            b.pack_configure(expand=Prawda, fill='both')
            self.root.update()
    ...
    def test_pack_info(self):
        pack, a, b, c, d = self.create2()
        przy self.assertRaisesRegex(TclError, 'window "%s" isn\'t packed' % a):
            a.pack_info()
        a.pack_configure()
        b.pack_configure(side='right', in_=a, anchor='s', expand=Prawda, fill='x',
...
klasa PlaceTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_place_configure_in(self):
        ...
        przy self.assertRaisesRegex(TclError, "can't place %s relative to "
                                    "itself" % re.escape(str(f2))):
            f2.place_configure(in_=f2)
        jeżeli tcl_version >= (8, 5):
            self.assertEqual(f2.winfo_manager(), '')
        przy self.assertRaisesRegex(TclError, 'bad window path name'):
    ...
    def test_place_configure_x(self):
        ...
        przy self.assertRaisesRegex(TclError, 'bad screen distance "spam"'):
    ...
    def test_place_configure_relx(self):
        ...
        przy self.assertRaisesRegex(TclError, 'expected floating-point number '
                                    'but got "spam"'):
    ...
    def test_place_configure_anchor(self):
        ...
        przy self.assertRaisesRegex(TclError, 'bad anchor "j"'):
            f.place_configure(anchor='j')
        przy self.assertRaisesRegex(TclError, 'ambiguous anchor ""'):
            f.place_configure(anchor='')
        dla value w 'n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw', 'center':
    ...
    def test_place_configure_width(self):
        ...
        przy self.assertRaisesRegex(TclError, 'bad screen distance "abcd"'):
    ...
    def test_place_configure_bordermode(self):
        f = tkinter.Frame(self.root)
        przy self.assertRaisesRegex(TclError, 'bad bordermode "j"'):
            f.place_configure(bordermode='j')
        przy self.assertRaisesRegex(TclError, 'ambiguous bordermode ""'):
            f.place_configure(bordermode='')
        dla value w 'inside', 'outside', 'ignore':
    ...
    def test_place_forget(self):
        ...
        foo.place_forget()
        self.root.update()
        self.assertNieprawda(foo.winfo_ismapped())
...
klasa GridTest(AbstractWidgetTest, unittest.TestCase):

    def tearDown(self):
        cols, rows = self.root.grid_size()
        dla i w range(cols + 1):
            self.root.grid_columnconfigure(i, weight=0, minsize=0, pad=0, uniform='')
        dla i w range(rows + 1):
    ...
    def test_grid_configure_column(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad column value "-1": '
    ...
    def test_grid_configure_columnspan(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad columnspan value "0": '
    ...
    def test_grid_configure_ipadx(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad ipadx value "-1": '
    ...
    def test_grid_configure_ipady(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad ipady value "-1": '
                                    'must be positive screen distance'):
    ...
    def test_grid_configure_padx(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad pad value "-1": '
                                    'must be positive screen distance'):
    ...
    def test_grid_configure_row(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad (row|grid) value "-1": '
                                    'must be a non-negative integer'):
    ...
    def test_grid_configure_rownspan(self):
        b = tkinter.Button(self.root)
        przy self.assertRaisesRegex(TclError, 'bad rowspan value "0": '
                                    'must be a positive integer'):
    ...
    def test_grid_configure_sticky(self):
        ...
        przy self.assertRaisesRegex(TclError, 'bad stickyness value "glue"'):
    ...
    def test_grid_columnconfigure(self):
        ...
        self.assertEqual(self.root.grid_columnconfigure(0),
                         {'minsize': 0, 'pad': 0, 'uniform': Nic, 'weight': 0})
        przy self.assertRaisesRegex(TclError, 'bad option "-foo"'):
        ...
        jeżeli tcl_version >= (8, 5):
            ...
            przy self.assertRaisesRegex(TclError, 'expected integer but got "all"'):
    ...
    def test_grid_columnconfigure_minsize(self):
        przy self.assertRaisesRegex(TclError, 'bad screen distance "foo"'):
    ...
    def test_grid_columnconfigure_weight(self):
        przy self.assertRaisesRegex(TclError, 'expected integer but got "bad"'):
            self.root.grid_columnconfigure(0, weight='bad')
        przy self.assertRaisesRegex(TclError, 'invalid arg "-weight": '
    ...
    def test_grid_columnconfigure_pad(self):
        ...
        przy self.assertRaisesRegex(TclError, 'invalid arg "-pad": '
                                    'should be non-negative'):
    ...
    def test_grid_anchor(self):
        przy self.assertRaisesRegex(TclError, 'bad anchor "x"'):
    ...
    def test_grid_bbox(self):
        ...
        przy self.assertRaisesRegex(TclError, 'expected integer but got "x"'):
    ...
    def test_grid_location(self):
        ...
        przy self.assertRaisesRegex(TclError, 'bad screen distance "x"'):
            self.root.grid_location('x', 'y')
        przy self.assertRaisesRegex(TclError, 'bad screen distance "y"'):
    ...
    def test_grid_propagate(self):
        self.assertEqual(self.root.grid_propagate(), Prawda)
        przy self.assertRaises(TypeError):
            self.root.grid_propagate(Nieprawda, Nieprawda)
        self.root.grid_propagate(Nieprawda)
        self.assertNieprawda(self.root.grid_propagate())
