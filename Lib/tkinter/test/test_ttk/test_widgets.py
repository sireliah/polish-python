zaimportuj unittest
zaimportuj tkinter
z tkinter zaimportuj ttk, TclError
z test.support zaimportuj requires
zaimportuj sys

z tkinter.test.test_ttk.test_functions zaimportuj MockTclObj
z tkinter.test.support zaimportuj (AbstractTkTest, tcl_version, get_tk_patchlevel,
                                  simulate_mouse_click)
z tkinter.test.widget_tests zaimportuj (add_standard_options, noconv,
...
klasa StandardTtkOptionsTests(StandardOptionsTests):

    def test_class(self):
        widget = self.create()
        self.assertEqual(widget['class'], '')
        errmsg='attempt to change read-only option'
        jeżeli get_tk_patchlevel() < (8, 6, 0, 'beta', 3):
            errmsg='Attempt to change read-only option'
    ...
    def test_style(self):
        ...
        errmsg = 'Layout Foo nie found'
        jeżeli hasattr(self, 'default_orient'):
            errmsg = ('Layout %s.Foo nie found' %
...
klasa WidgetTest(AbstractTkTest, unittest.TestCase):
    """Tests methods available w every ttk widget."""
    ...
    def test_identify(self):
        ...
        self.assertRaises(tkinter.TclError, self.widget.identify, Nic, 5)
        self.assertRaises(tkinter.TclError, self.widget.identify, 5, Nic)
        self.assertRaises(tkinter.TclError, self.widget.identify, 5, '')


    def test_widget_state(self):
        # XXX nie sure about the portability of all these tests
        self.assertEqual(self.widget.state(), ())
        self.assertEqual(self.widget.instate(['!disabled']), Prawda)

        # changing z !disabled to disabled
        ...
        # ensuring that dalejing Nic jako state doesn't modify current state
        self.widget.state(['active', '!disabled'])
        self.assertEqual(self.widget.state(), ('active', ))


klasa AbstractToplevelTest(AbstractWidgetTest, PixelSizeTests):
    _conv_pixels = noconv


@add_standard_options(StandardTtkOptionsTests)
klasa FrameTest(AbstractToplevelTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Frame(self.root, **kwargs)


@add_standard_options(StandardTtkOptionsTests)
klasa LabelFrameTest(AbstractToplevelTest, unittest.TestCase):
    OPTIONS = (
        'borderwidth', 'class', 'cursor', 'height',
    ...
    def create(self, **kwargs):
        zwróć ttk.LabelFrame(self.root, **kwargs)
...
klasa AbstractLabelTest(AbstractWidgetTest):
...
@add_standard_options(StandardTtkOptionsTests)
klasa LabelTest(AbstractLabelTest, unittest.TestCase):
    OPTIONS = (
        'anchor', 'background',
    ...
    def create(self, **kwargs):
        zwróć ttk.Label(self.root, **kwargs)
...
@add_standard_options(StandardTtkOptionsTests)
klasa ButtonTest(AbstractLabelTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Button(self.root, **kwargs)
    ...
    def test_invoke(self):
        ...
        btn.invoke()
        self.assertPrawda(success)


@add_standard_options(StandardTtkOptionsTests)
klasa CheckbuttonTest(AbstractLabelTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Checkbutton(self.root, **kwargs)
    ...
    def test_invoke(self):
        ...
        def cb_test():
            success.append(1)
            zwróć "cb test called"

        cbtn = ttk.Checkbutton(self.root, command=cb_test)
        # the variable automatically created by ttk.Checkbutton jest actually
...
klasa ComboboxTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Combobox(self.root, **kwargs)
    ...
    def test_virtual_event(self):
        ...
        self.combo.bind('<<ComboboxSelected>>',
            lambda evt: success.append(Prawda))
        self.combo.pack()
    ...
    def test_postcommand(self):
        ...
        self.combo['postcommand'] = lambda: success.append(Prawda)
        self.combo.pack()
        self.combo.wait_visibility()
    ...
    def test_values(self):
        ...
        self.assertEqual(self.combo['values'],
                         () jeżeli tcl_version < (8, 5) inaczej '')
        check_get_current('', -1)
        ...
        self.checkParam(self.combo, 'values', '',
                        expected='' jeżeli get_tk_patchlevel() < (8, 5, 10) inaczej ())
        ...
        # testing values przy empty string
        self.combo.set('')
        self.combo['values'] = (1, 2, '', 3)
        check_get_current('', 2)

        # testing values przy empty string set through configure
        self.combo.configure(values=[1, '', 2])
        self.assertEqual(self.combo['values'],
                         ('1', '', '2') jeżeli self.wantobjects inaczej
                         '1 {} 2')

        # testing values przy spaces
        self.combo['values'] = ['a b', 'a\tb', 'a\nb']
        self.assertEqual(self.combo['values'],
                         ('a b', 'a\tb', 'a\nb') jeżeli self.wantobjects inaczej
                         '{a b} {a\tb} {a\nb}')

        # testing values przy special characters
        self.combo['values'] = [r'a\tb', '"a"', '} {']
        self.assertEqual(self.combo['values'],
                         (r'a\tb', '"a"', '} {') jeżeli self.wantobjects inaczej
                         r'a\\tb {"a"} \}\ \{')
        ...
        # testing creating combobox przy empty string w values
        combo2 = ttk.Combobox(self.root, values=[1, 2, ''])
        self.assertEqual(combo2['values'],
                         ('1', '2', '') jeżeli self.wantobjects inaczej '1 2 {}')
        combo2.destroy()


@add_standard_options(IntegerSizeTests, StandardTtkOptionsTests)
klasa EntryTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Entry(self.root, **kwargs)
    ...
    def test_bbox(self):
        ...
        self.assertRaises(tkinter.TclError, self.entry.bbox, Nic)
    ...
    def test_identify(self):
        ...
        self.assertRaises(tkinter.TclError, self.entry.identify, Nic, 5)
        self.assertRaises(tkinter.TclError, self.entry.identify, 5, Nic)
        self.assertRaises(tkinter.TclError, self.entry.identify, 5, '')


    def test_validation_options(self):
        success = []
        test_invalid = lambda: success.append(Prawda)

        self.entry['validate'] = 'none'
        self.entry['validatecommand'] = lambda: Nieprawda
        ...
        self.entry['invalidcommand'] = test_invalid
        self.entry['validatecommand'] = lambda: Prawda
        self.entry.validate()
        self.assertEqual(len(success), 1)
        ...
        self.entry['validatecommand'] = Prawda
    ...
    def test_validation(self):
        ...
        def validate(to_insert):
            jeżeli nie 'a' <= to_insert.lower() <= 'z':
                validation.append(Nieprawda)
                zwróć Nieprawda
            validation.append(Prawda)
            zwróć Prawda
        ...
        self.assertEqual(validation, [Nieprawda, Prawda])
        self.assertEqual(self.entry.get(), 'a')


    def test_revalidation(self):
        def validate(content):
            dla letter w content:
                jeżeli nie 'a' <= letter.lower() <= 'z':
                    zwróć Nieprawda
            zwróć Prawda
        ...
        self.assertEqual(self.entry.validate(), Prawda)
        ...
        self.assertEqual(self.entry.validate(), Nieprawda)
...
@add_standard_options(IntegerSizeTests, StandardTtkOptionsTests)
klasa PanedWindowTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
        'class', 'cursor', 'height',
    ...
    def create(self, **kwargs):
        zwróć ttk.PanedWindow(self.root, **kwargs)
    ...
    def test_add(self):
        # attempt to add a child that jest nie a direct child of the paned window
        ...
        # re-adding a child jest nie accepted
    ...
    def test_forget(self):
        self.assertRaises(tkinter.TclError, self.paned.forget, Nic)
    ...
    def test_insert(self):
        self.assertRaises(tkinter.TclError, self.paned.insert, Nic, 0)
        self.assertRaises(tkinter.TclError, self.paned.insert, 0, Nic)
        ...
        # moving child3 to child2 position should result w child2 ending up
        # w previous child position oraz child ending up w previous child3
        # position
    ...
    def test_pane(self):
        ...
        self.assertEqual(self.paned.pane(0, weight=Nic),
                         0 jeżeli self.wantobjects inaczej '0')
        # newer form dla querying a single option
    ...
    def test_sashpos(self):
        self.assertRaises(tkinter.TclError, self.paned.sashpos, Nic)
        ...
        self.paned.pack(expand=Prawda, fill='both')
...
@add_standard_options(StandardTtkOptionsTests)
klasa RadiobuttonTest(AbstractLabelTest, unittest.TestCase):
    OPTIONS = (
        'class', 'command', 'compound', 'cursor',
        'image',
        'state', 'style',
    ...
    def create(self, **kwargs):
        zwróć ttk.Radiobutton(self.root, **kwargs)
...
klasa MenubuttonTest(AbstractLabelTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Menubutton(self.root, **kwargs)
...
@add_standard_options(StandardTtkOptionsTests)
klasa ScaleTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Scale(self.root, **kwargs)

    def test_from(self):
        widget = self.create()
        self.checkFloatParam(widget, 'from', 100, 14.9, 15.1, conv=Nieprawda)
    ...
    def test_to(self):
        widget = self.create()
        self.checkFloatParam(widget, 'to', 300, 14.9, 15.1, -10, conv=Nieprawda)

    def test_value(self):
        widget = self.create()
        self.checkFloatParam(widget, 'value', 300, 14.9, 15.1, -10, conv=Nieprawda)
    ...
    def test_set(self):
        ...
        # the same happens przy the value option
        ...
        self.assertRaises(tkinter.TclError, self.scale.set, Nic)


@add_standard_options(StandardTtkOptionsTests)
klasa ProgressbarTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Progressbar(self.root, **kwargs)
    ...
    def test_maximum(self):
        widget = self.create()
        self.checkFloatParam(widget, 'maximum', 150.2, 77.7, 0, -10, conv=Nieprawda)
...
@unittest.skipIf(sys.platform == 'darwin',
                 'ttk.Scrollbar jest special on MacOSX')
@add_standard_options(StandardTtkOptionsTests)
klasa ScrollbarTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Scrollbar(self.root, **kwargs)


@add_standard_options(IntegerSizeTests, StandardTtkOptionsTests)
klasa NotebookTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Notebook(self.root, **kwargs)
    ...
    def test_tab_identifiers(self):
        ...
        self.assertPrawda(self.nb.tab('current'))
        self.nb.add(self.child1, text='a')
        ...
        dla i w range(5, 100, 5):
            spróbuj:
                jeżeli self.nb.tab('@%d, 5' % i, text=Nic) == 'a':
                    przerwij
            wyjąwszy tkinter.TclError:
                dalej

        inaczej:
            self.fail("Tab przy text 'a' nie found")


    def test_add_and_hidden(self):
        self.assertRaises(tkinter.TclError, self.nb.hide, -1)
        self.assertRaises(tkinter.TclError, self.nb.hide, 'hi')
        self.assertRaises(tkinter.TclError, self.nb.hide, Nic)
        self.assertRaises(tkinter.TclError, self.nb.add, Nic)
        ...
        # but the tab next to it (nie hidden) jest the one selected now
    ...
    def test_forget(self):
        ...
        self.assertRaises(tkinter.TclError, self.nb.forget, Nic)
    ...
    def test_index(self):
        ...
        self.assertRaises(tkinter.TclError, self.nb.index, Nic)
    ...
    def test_insert(self):
        ...
        # bad inserts
        self.assertRaises(tkinter.TclError, self.nb.insert, 'end', Nic)
        self.assertRaises(tkinter.TclError, self.nb.insert, Nic, 0)
        self.assertRaises(tkinter.TclError, self.nb.insert, Nic, Nic)
    ...
    def test_select(self):
        ...
        self.child1.bind('<Unmap>', lambda evt: success.append(Prawda))
        self.nb.bind('<<NotebookTabChanged>>',
            lambda evt: tab_changed.append(Prawda))
...
@add_standard_options(StandardTtkOptionsTests)
klasa TreeviewTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Treeview(self.root, **kwargs)
    ...
    def test_height(self):
        widget = self.create()
        self.checkPixelsParam(widget, 'height', 100, -100, 0, '3c', conv=Nieprawda)
    ...
    def test_bbox(self):
        ...
        self.assertPrawda(children)

        bbox = self.tv.bbox(children[0])
        self.assertIsBoundingBox(bbox)

        # compare width w bboxes
        self.tv['columns'] = ['test']
        self.tv.column('test', width=50)
        bbox_column0 = self.tv.bbox(children[0], 0)
        root_width = self.tv.column('#0', width=Nic)
        jeżeli nie self.wantobjects:
            root_width = int(root_width)
        self.assertEqual(bbox_column0[0], bbox[0] + root_width)

        # verify that bbox of a closed item jest the empty string
    ...
    def test_children(self):
        ...
        # add item_id oraz child3 jako children of child2
        ...
        # child3 has child2 jako parent, thus trying to set child2 jako a children
        # of child3 should result w an error
    ...
    def test_column(self):
        # zwróć a dict przy all options/values
        self.assertIsInstance(self.tv.column('#0'), dict)
        # zwróć a single value of the given option
        jeżeli self.wantobjects:
            self.assertIsInstance(self.tv.column('#0', width=Nic), int)
        # set a new value dla an option
        self.tv.column('#0', width=10)
        # testing new way to get option value
        self.assertEqual(self.tv.column('#0', 'width'),
                         10 jeżeli self.wantobjects inaczej '10')
        self.assertEqual(self.tv.column('#0', width=Nic),
                         10 jeżeli self.wantobjects inaczej '10')
    ...
    def test_delete(self):
        ...
        self.assertNieprawda(self.tv.get_children())
    ...
    def test_detach_reattach(self):
        ...
        # calling detach without items jest valid, although it does nothing
        ...
        # detach item przy children
        self.tv.detach(item_id)
        self.assertNieprawda(self.tv.get_children())

        # reattach item przy children
        self.tv.reattach(item_id, '', 'end')
        self.assertEqual(self.tv.get_children(), (item_id, ))
        self.assertEqual(self.tv.get_children(item_id), (item2, ))
    ...
    def test_exists(self):
        self.assertEqual(self.tv.exists('something'), Nieprawda)
        self.assertEqual(self.tv.exists(''), Prawda)
        self.assertEqual(self.tv.exists({}), Nieprawda)

        # the following will make a tk.call equivalent to
        # tk.call(treeview, "exists") which should result w an error
        # w the tcl interpreter since tk requires an item.
        self.assertRaises(tkinter.TclError, self.tv.exists, Nic)


    def test_focus(self):
        # nothing jest focused right now
    ...
    def test_heading_callback(self):
        ...
        success = [] # no success dla now

        self.tv.pack()
        self.tv.wait_visibility()
        self.tv.heading('#0', command=lambda: success.append(Prawda))
        ...
        self.tv.heading('#0', command=str(self.tv.heading('#0', command=Nic)))
        ...
        # XXX The following podnieśs an error w a tcl interpreter, but nie w
        # Python
    ...
    def test_index(self):
        ...
        # check that index still works even after its parent oraz siblings
        # have been detached
    ...
    def test_insert_item(self):
        ...
        self.assertRaises(tkinter.TclError, self.tv.insert, '', 'end',
            open='please')
        self.assertNieprawda(self.tv.delete(self.tv.insert('', 'end', open=Prawda)))
        self.assertNieprawda(self.tv.delete(self.tv.insert('', 'end', open=Nieprawda)))

        # invalid index
        self.assertRaises(tkinter.TclError, self.tv.insert, '', 'middle')

        # trying to duplicate item id jest invalid
        ...
        self.assertEqual(self.tv.item(item, 'values'),
                         (value,) jeżeli self.wantobjects inaczej value)
        self.assertEqual(self.tv.item(item, values=Nic),
                         (value,) jeżeli self.wantobjects inaczej value)

        self.tv.item(item, values=self.root.splitlist(self.tv.item(item, values=Nic)))
        self.assertEqual(self.tv.item(item, values=Nic),
                         (value,) jeżeli self.wantobjects inaczej value)
        ...
        self.assertNieprawda(self.tv.item(item, values=Nic))

        # item tags
        item = self.tv.insert('', 'end', tags=[1, 2, value])
        self.assertEqual(self.tv.item(item, tags=Nic),
                         ('1', '2', value) jeżeli self.wantobjects inaczej
                         '1 2 %s' % value)
        self.tv.item(item, tags=[])
        self.assertNieprawda(self.tv.item(item, tags=Nic))
        ...
        item = self.tv.insert('', 'end', values=('a b c',
            '%s %s' % (value, value)))
        self.assertEqual(self.tv.item(item, values=Nic),
            ('a b c', '%s %s' % (value, value)) jeżeli self.wantobjects inaczej
            '{a b c} {%s %s}' % (value, value))

        # text
        self.assertEqual(self.tv.item(
            self.tv.insert('', 'end', text="Label here"), text=Nic),
            "Label here")
        self.assertEqual(self.tv.item(
            self.tv.insert('', 'end', text=value), text=Nic),
            value)
    ...
    def test_set(self):
        ...
        self.assertEqual(self.tv.item(item, values=Nic),
                         ('a', 'a') jeżeli self.wantobjects inaczej 'a a')
        ...
        self.assertEqual(self.tv.item(item, values=Nic),
                         ('b', 'a') jeżeli self.wantobjects inaczej 'b a')

        self.tv.set(item, 'B', 123)
        self.assertEqual(self.tv.set(item, 'B'),
                         123 jeżeli self.wantobjects inaczej '123')
        self.assertEqual(self.tv.item(item, values=Nic),
                         (123, 'a') jeżeli self.wantobjects inaczej '123 a')
        self.assertEqual(self.tv.set(item),
                         {'B': 123} jeżeli self.wantobjects inaczej {'B': '123'})
    ...
    def test_tag_bind(self):
        ...
        pos_y = set()
        found = set()
        dla i w range(0, 100, 10):
            jeżeli len(found) == 2: # item1 oraz item2 already found
                przerwij
            item_id = self.tv.identify_row(i)
            jeżeli item_id oraz item_id nie w found:
                pos_y.add(i)
                found.add(item_id)

        self.assertEqual(len(pos_y), 2) # item1 oraz item2 y pos
        dla y w pos_y:
            simulate_mouse_click(self.tv, 0, y)

        # by now there should be 4 things w the events list, since each
        # item had a bind dla two events that were simulated above
        self.assertEqual(len(events), 4)
        dla evt w zip(events[::2], events[1::2]):
            self.assertEqual(evt, (1, 2))


    def test_tag_configure(self):
        # Just testing parameter dalejing dla now
        ...
        self.assertEqual(str(self.tv.tag_configure('test', 'foreground')),
            'blue')
        self.assertEqual(str(self.tv.tag_configure('test', foreground=Nic)),
            'blue')
    ...
    def test_tag_has(self):
        ...
        self.assertPrawda(self.tv.tag_has('tag1', item1))
        self.assertNieprawda(self.tv.tag_has('tag1', item2))
        self.assertNieprawda(self.tv.tag_has('tag2', item1))
        self.assertPrawda(self.tv.tag_has('tag2', item2))
        self.assertNieprawda(self.tv.tag_has('tag3', item1))
        self.assertNieprawda(self.tv.tag_has('tag3', item2))
...
@add_standard_options(StandardTtkOptionsTests)
klasa SeparatorTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Separator(self.root, **kwargs)


@add_standard_options(StandardTtkOptionsTests)
klasa SizegripTest(AbstractWidgetTest, unittest.TestCase):
    OPTIONS = (
    ...
    def create(self, **kwargs):
        zwróć ttk.Sizegrip(self.root, **kwargs)
