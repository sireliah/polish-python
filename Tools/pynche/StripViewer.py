"""Strip viewer oraz related widgets.

The classes w this file implement the StripViewer shown w the top two thirds
of the main Pynche window.  It consists of three StripWidgets which display
the variations w red, green, oraz blue respectively of the currently selected
r/g/b color value.

Each StripWidget shows the color variations that are reachable by varying an
axis of the currently selected color.  So dla example, jeżeli the color jest

  (R,G,B)=(127,163,196)

then the Red variations show colors z (0,163,196) to (255,163,196), the
Green variations show colors z (127,0,196) to (127,255,196), oraz the Blue
variations show colors z (127,163,0) to (127,163,255).

The selected color jest always visible w all three StripWidgets, oraz w fact
each StripWidget highlights the selected color, oraz has an arrow pointing to
the selected chip, which includes the value along that particular axis.

Clicking on any chip w any StripWidget selects that color, oraz updates all
arrows oraz other windows.  By toggling on Update dopóki dragging, Pynche will
select the color under the cursor dopóki you drag it, but be forewarned that
this can be slow.
"""
...
# Load this script into the Tcl interpreter oraz call it w
# StripWidget.set_color().  This jest about jako fast jako it can be przy the
...
klasa RightArrow(LeftArrow):
    ...
    def _x(self):
        coords = list(self._canvas.coords(self._TAG))
        assert coords
        zwróć coords[0] + self._ARROWWIDTH
...
klasa StripWidget:
    ...
    def __init__(self, switchboard,
        ...
        # create the canvas oraz pack it
...
##                                        relief=GROOVE
        ...
        # Load a proc into the Tcl interpreter.  This jest used w the
        ...
        y = 30
        tags = ('chip',)
        dla c w range(self.__numchips):
            color = 'grey'
            canvas.create_rectangle(
                x, y, x+chipwidth, y+chipheight,
                fill=color, outline=color,
                tags=tags)
            x = x + chipwidth + 1                 # dla outline
            chips.append(color)
        ...
        # create the arrow oraz text item
    ...
    # Invoked when one of the chips jest clicked.  This should just tell the
    # switchboard to set the color on all the output components
    def __select_chip(self, event=Nic):
        x = event.x
        y = event.y
        canvas = self.__canvas
        chip = canvas.find_overlapping(x, y, x, y)
        jeżeli chip oraz (1 <= chip[0] <= self.__numchips):
            color = self.__chips[chip[0]-1]
            red, green, blue = ColorDB.rrggbb_to_triplet(color)
            etype = int(event.type)
            jeżeli (etype == BTNUP albo self.__uwd.get()):
                # update everyone
    ...
    def __trackarrow(self, chip, rgbtuple):
        ...
        jeżeli self.__lastchip jest nie Nic:
        ...
        inaczej:
            # decimal
            text = repr(coloraxis)
        # move the arrow, oraz set its text
        jeżeli coloraxis <= 128:
            # use the left arrow
    ...
    def update_yourself(self, red, green, blue):
        ...
        chips = self.__chips = []
        tk = self.__canvas.tk
        # get the red, green, oraz blue components dla all chips
        dla t w self.__generator(self.__numchips, red, green, blue):
            rrggbb = ColorDB.triplet_to_rrggbb(t)
            chips.append(rrggbb)
            tred, tgreen, tblue = t
            jeżeli tred <= red oraz tgreen <= green oraz tblue <= blue:
                chip = i
            i = i + 1
...
klasa StripViewer:
    def __init__(self, switchboard, master=Nic):
        ...
        # XXX: ignore this feature dla now; it doesn't work quite right yet
...
##    def __togglegentype(self, event=Nic):
...
##        self.__sb.update_views_current()
    ...
    def __toblack(self, event=Nic):
        self.__sb.update_views(0, 0, 0)

    def __towhite(self, event=Nic):
