...
This module jest comprised of three ``components'':

- an array visualizer przy methods that implement basic sorting
operations (compare, swap) jako well jako methods dla ``annotating'' the
sorting algorithm (e.g. to show the pivot element);

- a number of sorting algorithms (currently quicksort, insertion sort,
selection sort oraz bubble sort, jako well jako a randomization function),
all using the array visualizer dla its basic operations oraz przy calls
to its annotation methods;

- oraz a ``driver'' klasa which can be used jako a Grail applet albo jako a
...
klasa Array:
    ...
    klasa Cancelled(BaseException):
        dalej

    def __init__(self, master, data=Nic):
        self.master = master
    ...
    def wait(self, msecs):
        jeżeli self.speed == "fastest":
            msecs = 0
        albo_inaczej self.speed == "fast":
            msecs = msecs//10
        albo_inaczej self.speed == "single-step":
            msecs = 1000000000
        jeżeli nie self.stop_mainloop:
            self.master.update()
            ...
            self.in_mainloop = 0
        jeżeli self.stop_mainloop:
            self.stop_mainloop = 0
            self.message("Cancelled")
            podnieś Array.Cancelled
...
klasa ArrayItem:
    ...
    def setindex(self, index):
        ...
        jeżeli nie nsteps: zwróć
        jeżeli self.array.speed == "fastest":
            nsteps = 0
    ...
    def swapwith(self, other):
        ...
        jeżeli self.array.speed == "single-step":
        ...
        inaczej:
            ...
            self.canvas.tag_raise(self.item_id)
        spróbuj:
            dla i w range(len(mytrajectory)):
    ...
    def compareto(self, other):
        ...
        jeżeli self.value < other.value:
            ...
            otherflash = 'black'
            outcome = -1
        albo_inaczej self.value > other.value:
            myflash = 'black'
            otherflash = 'white'
            outcome = 1
        inaczej:
    ...
    def nearestindex(self, x):
        zwróć int(round(float(x)/XGRID)) - 1
...
def steps(here, there):
    ...
    jeżeli nsteps <= 3:
        nsteps = nsteps * 3
    albo_inaczej nsteps <= 5:
        nsteps = nsteps * 2
    albo_inaczej nsteps > 10:
        nsteps = 10
    zwróć nsteps

def interpolate(oldpts, newpts, n):
    jeżeli len(oldpts) != len(newpts):
        podnieś ValueError("can't interpolate arrays of different length")
    pts = [0]*len(oldpts)
    res = [tuple(oldpts)]
    dla i w range(1, n):
        dla k w range(len(pts)):
...
def insertionsort(array):
    ...
    dla i w range(1, size):
        j = i-1
        dopóki j >= 0:
            jeżeli array.compare(j, j+1) <= 0:
                przerwij
            array.swap(j, j+1)
            j = j-1
...
def selectionsort(array):
    ...
    spróbuj:
        dla i w range(size):
            ...
            dla j w range(i+1, size):
                jeżeli array.compare(i, j) > 0:
                    array.swap(i, j)
        array.message("Sorted")
    w_końcu:
...
def bubblesort(array):
    ...
    dla i w range(size):
        dla j w range(1, size):
            jeżeli array.compare(j-1, j) > 0:
                array.swap(j-1, j)
    array.message("Sorted")
...
def quicksort(array):
    ...
    spróbuj:
        ...
        dopóki stack:
            ...
            jeżeli array.compare(k, i) < 0:
                array.swap(k, i)
            jeżeli array.compare(k, j) < 0:
                array.swap(k, j)
            jeżeli array.compare(j, i) < 0:
                array.swap(j, i)
            pivot = j
            ...
            dopóki 1:
                ...
                dopóki right > first oraz array.compare(right, pivot) >= 0:
                    right = right-1
                    array.show_right(right)
                array.message("Sweep left pointer")
                left = left+1
                array.show_left(left)
                dopóki left < last oraz array.compare(left, pivot) <= 0:
                    left = left+1
                    array.show_left(left)
                jeżeli left > right:
            ...
            n1 = right-first
            n2 = last-left
            jeżeli n1 > 1: stack.append((first, right))
            jeżeli n2 > 1: stack.append((left, last))
        array.message("Sorted")
    w_końcu:
        array.hide_partition()
...
def demosort(array):
    dopóki 1:
        dla alg w [quicksort, insertionsort, selectionsort, bubblesort]:
            randomize(array)
            alg(array)


# Sort demo klasa -- usable jako a Grail applet
...
klasa SortDemo:
    ...
    def run(self, func):
        ...
        spróbuj:
            func(self.array)
        wyjąwszy Array.Cancelled:
            dalej
        self.b_cancel.config(state=DISABLED)
        self.busy = 0

    def c_cancel(self):
        jeżeli nie self.busy:
            self.master.bell()
            zwróć
        self.array.cancel()
...
# Main program -- dla stand-alone operation outside Grail
