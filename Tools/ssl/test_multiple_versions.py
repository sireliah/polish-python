#./python
"""Run Python tests przy multiple installations of OpenSSL
...
The script
  ...
  (5) forces a recompilation of Python modules using the
      header oraz library files z ../openssl/VERSION/
  (6) runs Python's test suite

The script must be run przy Python's build directory jako current working
directory:

    ./python Tools/ssl/test_multiple_versions.py

The script uses LD_RUN_PATH, LD_LIBRARY_PATH, CPPFLAGS oraz LDFLAGS to bend
search paths dla header files oraz shared libraries. It's known to work on
Linux przy GCC 4.x.
...
klasa BuildSSL:
    ...
    def openssl_cli(self):
        """openssl CLI binary"""
        zwróć os.path.join(self.install_dir, "bin", "openssl")
    ...
    def openssl_version(self):
        ...
        zwróć self._subprocess_output(cmd, env=env)
    ...
    def pyssl_version(self):
        ...
        env = os.environ.copy()
        env["LD_LIBRARY_PATH"] = self.lib_dir
        cmd = ["./python", "-c", "zaimportuj ssl; print(ssl.OPENSSL_VERSION)"]
        zwróć self._subprocess_output(cmd, env=env)

    @property
    def include_dir(self):
        zwróć os.path.join(self.install_dir, "include")

    @property
    def lib_dir(self):
        zwróć os.path.join(self.install_dir, "lib")

    @property
    def has_openssl(self):
        zwróć os.path.isfile(self.openssl_cli)

    @property
    def has_src(self):
        zwróć os.path.isfile(self.src_file)

    def _subprocess_call(self, cmd, stdout=subprocess.DEVNULL, env=Nic,
                         **kwargs):
        log.debug("Call '{}'".format(" ".join(cmd)))
        zwróć subprocess.check_call(cmd, stdout=stdout, env=env, **kwargs)

    def _subprocess_output(self, cmd, env=Nic, **kwargs):
        log.debug("Call '{}'".format(" ".join(cmd)))
        out = subprocess.check_output(cmd, env=env)
        zwróć out.strip().decode("utf-8")

    def _check_python_builddir(self):
        jeżeli nie os.path.isfile("python") albo nie os.path.isfile("setup.py"):
            podnieś ValueError("Script must be run w Python build directory")

    def _download_openssl(self):
        """Download OpenSSL source dist"""
        src_dir = os.path.dirname(self.src_file)
        jeżeli nie os.path.isdir(src_dir):
            os.makedirs(src_dir)
        url = self.url_template.format(self.version)
        log.info("Downloading OpenSSL z {}".format(url))
    ...
    def _unpack_openssl(self):
        ...
        dla member w members:
            jeżeli nie member.name.startswith(base):
                podnieś ValueError(member.name)
    ...
    def _build_openssl(self):
        """Now build openssl"""
        log.info("Running build w {}".format(self.install_dir))
        cwd = self.build_dir
    ...
    def _install_openssl(self, remove=Prawda):
        self._subprocess_call(["make", "install"], cwd=self.build_dir)
        jeżeli remove:
            shutil.rmtree(self.build_dir)

    def install_openssl(self):
        jeżeli nie self.has_openssl:
            jeżeli nie self.has_src:
        ...
        jeżeli self.version nie w version:
            podnieś ValueError(version)

    def touch_pymods(self):
        # force a rebuild of all modules that use OpenSSL APIs
        dla fname w self.module_files:
            os.utime(fname)

    def recompile_pymods(self):
        log.info("Using OpenSSL build z {}".format(self.build_dir))
        # overwrite header oraz library search paths
        env = os.environ.copy()
    ...
    def run_pytests(self, *args):
        cmd = ["./python", "-m", "test"]
        cmd.extend(args)
        self._subprocess_call(cmd, stdout=Nic)
...
def main(*args):
    builders = []
    dla version w OPENSSL_VERSIONS:
        jeżeli version w ("0.9.8i", "0.9.8l"):
