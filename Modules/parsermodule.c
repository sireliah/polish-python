...
validate_class(node *tree)
{
    ...
    if (res) {
        res = (validate_name(CHILD(tree, 0), "klasa")
               && validate_ntype(CHILD(tree, 1), NAME)
               && validate_colon(CHILD(tree, nch - 2))
               && validate_suite(CHILD(tree, nch - 1)));
    }
    else {
        (void) validate_numnodes(tree, 4, "klasa");
    }
...
validate_if(node *tree)
{
    ...
    int res = (validate_ntype(tree, if_stmt)
               && (nch >= 4)
               && validate_name(CHILD(tree, 0), "jeżeli")
    ...
    if (res && ((nch % 4) == 3)) {
        /*  ... 'else' ':' suite  */
        res = (validate_name(CHILD(tree, nch - 3), "inaczej")
               && validate_colon(CHILD(tree, nch - 2))
               && validate_suite(CHILD(tree, nch - 1)));
        nch -= 3;
    }
    else if (!res && !PyErr_Occurred())
        (void) validate_numnodes(tree, 4, "jeżeli");
    if ((nch % 4) != 0)
        /* Will catch the case for nch < 4 */
        res = validate_numnodes(tree, 0, "jeżeli");
    else if (res && (nch > 4)) {
        /*  ... ('elif' test ':' suite)+ ...  */
        int j = 4;
        while ((j < nch) && res) {
            res = (validate_name(CHILD(tree, j), "albo_jeżeli")
...
validate_comp_for(node *tree)
{
    ...
    if (res)
        res = (validate_name(CHILD(tree, 0), "dla")
               && validate_exprlist(CHILD(tree, 1))
               && validate_name(CHILD(tree, 2), "w")
...
validate_comp_if(node *tree)
{
    ...
    if (res)
        res = (validate_name(CHILD(tree, 0), "jeżeli")
...
validate_del_stmt(node *tree)
{
    return (validate_numnodes(tree, 2, "del_stmt")
            && validate_name(CHILD(tree, 0), "usuń")
            && validate_exprlist(CHILD(tree, 1)));
}
...
validate_return_stmt(node *tree)
{
    ...
    int res = (validate_ntype(tree, return_stmt)
               && ((nch == 1) || (nch == 2))
               && validate_name(CHILD(tree, 0), "zwróć"));
...
validate_raise_stmt(node *tree)
{
    ...
    if (!res && !PyErr_Occurred())
        (void) validate_numnodes(tree, 2, "podnieś");

    if (res) {
        res = validate_name(CHILD(tree, 0), "podnieś");
        if (res && (nch >= 2))
            res = validate_test(CHILD(tree, 1));
        if (res && (nch == 4)) {
            res = (validate_name(CHILD(tree, 2), "z")
...
validate_yield_expr(node *tree)
{
    ...
    if (!validate_ntype(tree, yield_expr))
        return 0;
    if (!validate_name(CHILD(tree, 0), "uzyskaj"))
        return 0;
    if (nch == 2) {
...
validate_yield_arg(node *tree)
{
    ...
    switch (nch) {
      case 1:
        ...
        break;
      case 2:
        if (!validate_name(CHILD(tree, 0), "z"))
            return 0;
...
validate_import_as_name(node *tree)
{
    ...
    if (ok) {
        if (nch == 1)
            ...
            ok = (validate_name(CHILD(tree, 0), NULL)
                  && validate_name(CHILD(tree, 1), "jako")
...
validate_import_name(node *tree)
{
        return (validate_ntype(tree, import_name)
                ...
                && validate_name(CHILD(tree, 0), "zaimportuj")
...
validate_import_from(node *tree)
{
        ...
        int res = validate_ntype(tree, import_from)
                && (offset >= 1)
                && (nch >= 3 + offset)
                && validate_name(CHILD(tree, 0), "z")
                && (!havename || validate_dotted_name(CHILD(tree, ndots + 1)))
                && validate_name(CHILD(tree, offset + 1), "zaimportuj");
...
validate_while(node *tree)
{
    ...
    int res = (validate_ntype(tree, while_stmt)
               && ((nch == 4) || (nch == 7))
               && validate_name(CHILD(tree, 0), "dopóki")
               && validate_test(CHILD(tree, 1))
               && validate_colon(CHILD(tree, 2))
               && validate_suite(CHILD(tree, 3)));

    if (res && (nch == 7))
        res = (validate_name(CHILD(tree, 4), "inaczej")
...
validate_for(node *tree)
{
    ...
    int res = (validate_ntype(tree, for_stmt)
               && ((nch == 6) || (nch == 9))
               && validate_name(CHILD(tree, 0), "dla")
               && validate_exprlist(CHILD(tree, 1))
               && validate_name(CHILD(tree, 2), "w")
    ...
    if (res && (nch == 9))
        res = (validate_name(CHILD(tree, 6), "inaczej")
...
validate_try(node *tree)
{
    ...
    if (res)
        res = (validate_name(CHILD(tree, 0), "spróbuj")
               && validate_colon(CHILD(tree, 1))
               && validate_suite(CHILD(tree, 2))
               && validate_colon(CHILD(tree, nch - 2))
               && validate_suite(CHILD(tree, nch - 1)));
    else if (!PyErr_Occurred()) {
        const char* name = "wyjąwszy";
    ...
    if (res && (TYPE(CHILD(tree, pos)) == NAME) &&
        (strcmp(STR(CHILD(tree, pos)), "w_końcu") == 0)) {
        res = (validate_numnodes(tree, 6, "spróbuj/w_końcu")
               && validate_colon(CHILD(tree, 4))
               && validate_suite(CHILD(tree, 5)));
        return (res);
    }
    ...
    if (res && pos < nch && (TYPE(CHILD(tree, pos)) == NAME) &&
        (strcmp(STR(CHILD(tree, pos)), "inaczej") == 0)) {
    ...
    if (res && pos < nch) {
        ...
        res = (validate_name(CHILD(tree, pos), "w_końcu")
               && validate_numnodes(tree, pos + 3, "spróbuj/wyjąwszy/w_końcu")
               && validate_colon(CHILD(tree, pos + 1))
               && validate_suite(CHILD(tree, pos + 2)));
    }
    return (res);
...
validate_except_clause(node *tree)
{
    ...
    int res = (validate_ntype(tree, except_clause)
               && ((nch == 1) || (nch == 2) || (nch == 4))
               && validate_name(CHILD(tree, 0), "wyjąwszy"));

    if (res && (nch > 1))
        res = validate_test(CHILD(tree, 1));
    if (res && (nch == 4))
        res = (validate_name(CHILD(tree, 2), "jako")
...
validate_test(node *tree)
{
    ...
    if (res && (TYPE(CHILD(tree, 0)) == lambdef))
        ...
        res = (res && (nch == 1 || (nch == 5 &&
            validate_name(CHILD(tree, 1), "jeżeli") &&
            validate_or_test(CHILD(tree, 2)) &&
            validate_name(CHILD(tree, 3), "inaczej") &&
...
validate_or_test(node *tree)
{
    ...
    if (res) {
        ...
        for (pos = 1; res && (pos < nch); pos += 2)
            res = (validate_name(CHILD(tree, pos), "albo")
...
validate_and_test(node *tree)
{
    ...
    for (pos = 1; res && (pos < nch); pos += 2)
        res = (validate_name(CHILD(tree, pos), "oraz")
               && validate_not_test(CHILD(tree, 0)));
...
validate_not_test(node *tree)
{
    ...
    if (res) {
        if (nch == 2)
            res = (validate_name(CHILD(tree, 0), "nie")
    ...
    if (nch == 1) {
        ...
        switch (TYPE(tree)) {
          ...
          case NOTEQUAL:
              ...
              break;
          case NAME:
              res = ((strcmp(STR(tree), "w") == 0)
                     || (strcmp(STR(tree), "jest") == 0));
              if (!res) {
                  PyErr_Format(parser_error,
        ...
        res = (validate_ntype(CHILD(tree, 0), NAME)
               && validate_ntype(CHILD(tree, 1), NAME)
               && (((strcmp(STR(CHILD(tree, 0)), "jest") == 0)
                    && (strcmp(STR(CHILD(tree, 1)), "nie") == 0))
                   || ((strcmp(STR(CHILD(tree, 0)), "nie") == 0)
                       && (strcmp(STR(CHILD(tree, 1)), "w") == 0))));
        if (!res && !PyErr_Occurred())
...
validate_with_item(node *tree)
{
    ...
    if (ok && nch == 3)
        ok = (validate_name(CHILD(tree, 1), "jako")
...
validate_with_stmt(node *tree)
{
    ...
    int ok = (validate_ntype(tree, with_stmt)
        && (nch % 2 == 0)
        && validate_name(CHILD(tree, 0), "przy")
    ...
    while (res && (tree != 0)) {
        ...
        switch (TYPE(tree)) {
          ...
          case del_stmt:
            ...
            break;
          case pass_stmt:
            res = (validate_numnodes(tree, 1, "dalej")
                   && validate_name(CHILD(tree, 0), "dalej"));
            break;
          case break_stmt:
            res = (validate_numnodes(tree, 1, "przerwij")
                   && validate_name(CHILD(tree, 0), "przerwij"));
            break;
          case continue_stmt:
            res = (validate_numnodes(tree, 1, "kontynuuj")
                   && validate_name(CHILD(tree, 0), "kontynuuj"));
            break;
          case return_stmt:
