...
validate_stmt(stmt_ty stmt)
{
    ...
    case For_kind:
        return validate_expr(stmt->v.For.target, Store) &&
            ...
            validate_body(stmt->v.For.body, "Dla") &&
    ...
    case While_kind:
        return validate_expr(stmt->v.While.test, Load) &&
            validate_body(stmt->v.While.body, "Dopóki") &&
            validate_stmts(stmt->v.While.orelse);
    case If_kind:
        return validate_expr(stmt->v.If.test, Load) &&
            validate_body(stmt->v.If.body, "Jeżeli") &&
            validate_stmts(stmt->v.If.orelse);
    case With_kind:
        if (!validate_nonempty_seq(stmt->v.With.items, "items", "Przy"))
            return 0;
        ...
        return validate_body(stmt->v.With.body, "Przy");
    case AsyncWith_kind:
    ...
    case Raise_kind:
        ...
        return 1;
    case Try_kind:
        if (!validate_body(stmt->v.Try.body, "Spróbuj"))
            return 0;
    ...
    */
    ...
    if (NCH(n) == 1) {
        ...
        switch (TYPE(CHILD(n, 0))) {
            case NAME:
                if (strcmp(STR(CHILD(n, 1)), "w") == 0)
                    return NotIn;
                if (strcmp(STR(CHILD(n, 0)), "jest") == 0)
                    return IsNot;
            default:
 ...
 loop:
    switch (TYPE(n)) {
        ...
        case and_test:
            ...
            if (!strcmp(STR(CHILD(n, 1)), "oraz"))
                return BoolOp(And, seq, LINENO(n), n->n_col_offset,
                              c->c_arena);
            assert(!strcmp(STR(CHILD(n, 1)), "albo"));
...
ast_for_try_stmt(struct compiling *c, const node *n)
{
    ...
    if (TYPE(CHILD(n, nch - 3)) == NAME) {
        if (strcmp(STR(CHILD(n, nch - 3)), "w_końcu") == 0) {
